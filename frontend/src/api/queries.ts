...
import { type GatewayConnectionStatus, getCatalog, getOperation, type OperationEventStreamStatus, subscribeToGatewayConnectionStatus, subscribeToOperationEvents } from './client';
  ...
  useEffect(() => {
    ...
    return subscribeToOperationEvents(operationId, fromSequence, {
      onEvent: (operationEvent) => {
        ...
        queryClient.setQueryData<OperationState>(operationQueryKeys.state(operationId), (current) => updateOperationState(current, operationEvent));
      },
...
function updateOperationState(current: OperationState | undefined, operationEvent: OperationEvent): OperationState | undefined {
  if (!current) return current;
  const payload = operationEvent.payload;
  const status: OperationState['status'] = payload.kind === 'completed' ? payload.status : payload.kind === 'failed' ? 'failed' : payload.kind === 'accepted' ? 'accepted' : current.status === 'cancelling' ? 'cancelling' : 'running';
