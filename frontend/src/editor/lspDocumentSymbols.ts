...
export function sourceMethodFromDocumentSymbols(
  ...
  candidates.sort(
    (left, right) =>
      right.depth - left.depth || rangeSize(left.symbol.range) - rangeSize(right.symbol.range),
  )
  const selected = candidates[0]?.symbol
  return selected
    ? { name: selected.name, lineNumber: selected.selectionRange.start.line + 1 }
    : null
}
...
function collectContainingMethods(
  symbols: readonly LspDocumentSymbol[],
  position: LspPosition,
  languageId: string,
  depth: number,
  result: Array<{ symbol: LspDocumentSymbol; depth: number }>,
): void {
