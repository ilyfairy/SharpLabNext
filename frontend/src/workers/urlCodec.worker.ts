...
export const handleUrlCodecWorkerRequest = async (
  request: UrlCodecWorkerRequest,
): Promise<UrlCodecWorkerResponse> => {
  try {
    const value =
      request.operation === 'encode-v3'
        ? await encodeV3(request.state, request.options)
        : await decodeShareFragment(request.fragment, request.limits)
...
if (scope.document === undefined && typeof scope.postMessage === 'function') {
  scope.onmessage = (event) => {
    void handleUrlCodecWorkerRequest(event.data).then((response) => scope.postMessage(response))
  }
}
