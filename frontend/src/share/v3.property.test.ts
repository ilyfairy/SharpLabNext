...
const stateArbitrary = fc
  .record({
    ...
    texts: fc.array(fc.string({ maxLength: 300 }), { minLength: 1, maxLength: 5 }),
  })
        ...
        : languageId === 'visual-basic'
          ...
          : languageId === 'fsharp'
            ...
            : languageId === 'php'
              ? 'php'
              : 'il'
    const files = texts.map((text, index) => ({ path: `src/File${index}.${extension}`, text }))
    return {
      languageId,
...
describe('URL v3 property roundtrips', () => {
  it('roundtrips Unicode, multiple languages, files, and source order', async () => {
    await fc.assert(
      fc.asyncProperty(
        stateArbitrary,
        fc.constantFrom('live' as const, 'share' as const),
        async (state, profile) => {
  ...
  it('accepts valid raw DEFLATE streams across levels and workspace shapes', async () => {
    await fc.assert(
      fc.asyncProperty(stateArbitrary, fc.integer({ min: 0, max: 9 }), async (state, level) => {
        const payload = encodeCanonicalPayload(state, { ...defaultUrlCodecLimits })
        const envelope = await createV3Envelope(
          payload,
          1,
