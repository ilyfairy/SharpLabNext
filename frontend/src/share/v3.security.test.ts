...
const goldenEnvelope = async (codec: 0 | 1 = 0): Promise<Uint8Array> =>
  createV3Envelope(encodeCanonicalPayload(goldenState, { ...defaultUrlCodecLimits }), codec)
...
describe('URL v3 validation', () => {
  it('rejects invalid base64url', async () => {
    await expect(decodeV3('#v3:not+base64')).rejects.toMatchObject({ code: 'invalid-base64url' })
    await expect(decodeV3('#v3:a')).rejects.toMatchObject({ code: 'invalid-base64url' })
    await expect(decodeV3('#v3:AA')).rejects.toMatchObject({ code: 'envelope-truncated' })
  })
  ...
  it.each([
    ...
    await expect(decodeV3(await cloneWith(mutate))).rejects.toMatchObject({ code })
  })
  ...
  it('rejects unknown critical extensions and ignores non-critical extensions', async () => {
    ...
    await expect(decodeV3(toFragment(extended))).resolves.toMatchObject({ state: goldenState })
    extended[24] = 0x81
  ...
  it('rejects declared encoded and raw lengths that do not match', async () => {
    ...
    await expect(decodeV3(encodedMismatch)).rejects.toMatchObject({ code: 'length-mismatch' })

    const rawMismatch = await cloneWith((bytes) => {
      new DataView(bytes.buffer).setUint32(8, 1, true)
    })
    await expect(decodeV3(rawMismatch)).rejects.toMatchObject({ code: 'length-mismatch' })
  })
  ...
  it('rejects digest corruption', async () => {
    ...
    await expect(decodeV3(fragment)).rejects.toMatchObject({ code: 'digest-mismatch' })
  })
  ...
  it('bounds raw DEFLATE bombs using the declared output length', async () => {
    ...
    await expect(decodeV3(toFragment(envelope))).rejects.toMatchObject({ code: 'length-mismatch' })
  })
  ...
  it('rejects too many files and files over 256 KiB after decompression', async () => {
    ...
    await expect(decodeV3(toFragment(tooMany))).rejects.toMatchObject({ code: 'invalid-workspace' })
  ...
  it('rejects fragments over the hard URL limit', async () => {
    ...
    await expect(decodeV3(fragment)).rejects.toMatchObject({ code: 'url-too-long' })
  })
  ...
  it('rejects standard Base64 characters in v3', async () => {
    ...
    await expect(decodeV3(`#v3:${standard}`)).rejects.toMatchObject({ code: 'invalid-base64url' })
