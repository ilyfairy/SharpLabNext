...
export function sourceAssociationActivationKey(
  activation: SourceAssociationActivation | null,
  generationId: string | null,
): string | null {
  return activation?.generationId === generationId ? activation.associationKey : null
}

export function sourceAssociationLines(
  associations: readonly SourceAssociation[],
  activeAssociationKey: string | null | undefined,
): SourceAssociationLine[] {
  const lines = new Map<number, SourceAssociationLine>()
  const ordered = associations
    .filter(isLinkedLineSourceAssociation)
    .sort((left, right) => left.key.localeCompare(right.key))
  for (const association of ordered) {
    const active = association.key === activeAssociationKey
    const endLine =
      association.range.endLine > association.range.startLine && association.range.endColumn === 1
        ? association.range.endLine - 1
        : association.range.endLine
...
export function sourceAssociationForSelection(
  associations: readonly SourceAssociation[],
  documentPath: string,
  selection: ExecutionFlowSourceTarget['range'],
): SourceAssociation | null {
  const candidates = associations.filter(
    (association) =>
      association.presentation === 'active-range' && association.documentPath === documentPath,
  )
  ...
  const structuralCandidates = candidates.filter(
    (association) => association.astCategory !== 'trivia',
  )
  const selectionCandidates = structuralCandidates.length > 0 ? structuralCandidates : candidates

  const insideSelection = selectionCandidates.filter((association) =>
    rangeContains(selection, association.range),
  )
  if (insideSelection.length > 0) {
    ...
    const maximalInsideSelection = insideSelection.filter(
      (candidate) =>
        !insideSelection.some(
          (other) =>
            other !== candidate &&
    ...
    return insideSelection.reduce((largest, candidate) =>
      rangeContains(candidate.range, largest.range) ? candidate : largest,
    )
  }
...
function smallestContainingSelection(
  candidates: readonly SourceAssociation[],
  selection: ExecutionFlowSourceTarget['range'],
): SourceAssociation | null {
  const containingSelection = candidates.filter((association) =>
    rangeContains(association.range, selection),
  )
  if (containingSelection.length === 0) return null
  return containingSelection.reduce((smallest, candidate) =>
    rangeContains(smallest.range, candidate.range) ? candidate : smallest,
  )
}

export function createSourceAssociation(
...
export function sourceAssociationKey(target: ExecutionFlowSourceTarget): string {
  const { range } = target
  return [
    target.documentPath.replaceAll('\\', '/').toLowerCase(),
    range.startLine,
    range.startColumn,
    range.endLine,
    range.endColumn,
...
function rangesEqual(
  left: ExecutionFlowSourceTarget['range'],
  right: ExecutionFlowSourceTarget['range'],
): boolean {
  return (
...
function rangeContains(
  outer: ExecutionFlowSourceTarget['range'],
  inner: ExecutionFlowSourceTarget['range'],
): boolean {
  return (
