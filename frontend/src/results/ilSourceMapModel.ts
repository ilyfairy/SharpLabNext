...
export function createIlSourceLinks(
  linkedRanges: readonly LinkedRange[],
  sourceFiles: readonly IlSourceFile[],
  ilText?: string,
): IlSourceLink[] {
...
function expandIlSourceLinks(links: readonly IlSourceLink[], ilText: string): IlSourceLink[] {
  ...
  return links.map((link, index) => {
    ...
    for (
      let lineNumber = link.startLine;
      lineNumber < nextStartLine && lineNumber <= lines.length;
      lineNumber += 1
    ) {
    ...
    return firstInstructionLine === null || lastInstructionLine === null
      ? link
      : { ...link, startLine: firstInstructionLine, endLine: lastInstructionLine }
  })
}

function isIlSourceBoundary(line: string): boolean {
  return (
    /^\s*\/\/\s*sequence point:/i.test(line) ||
    /^\s*\.method\b/i.test(line) ||
    /^\s*}\s*\/\/\s*end of method\b/i.test(line)
  )
}
...
function resolveSourceFile(
  ...
  const exactMatches = sourceFiles.filter(
    (file) => normalizePath(file.path) === normalizedSourcePath,
  )
...
function validRange(range: TextRange, text: string): boolean {
  ...
  return (
    startText !== undefined &&
    endText !== undefined &&
    range.startCharacter <= startText.length &&
    range.endCharacter <= endText.length
  )
}
...
function toOneBasedLineRange(range: TextRange): Pick<IlSourceLink, 'startLine' | 'endLine'> | null {
  ...
  const inclusiveEndLine =
    range.endLine > range.startLine && range.endCharacter === 0 ? range.endLine - 1 : range.endLine
  return {
...
function sourceExcerpt(text: string, range: TextRange): string {
  const lines = text.replace(/\r\n?/g, '\n').split('\n')
  const endExclusive =
    range.endLine > range.startLine && range.endCharacter === 0 ? range.endLine : range.endLine + 1
