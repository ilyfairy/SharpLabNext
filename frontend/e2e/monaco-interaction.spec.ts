...
import { editorHost, openWorkbench, replaceSource, sourceEditor, switchEditor } from './helpers/workbench';
...
test.describe('Monaco interaction stability', () => {
  test('copies exactly and replaces a native drag selection on the first key', async ({ page, isMobile }) => {
    ...
    const points = await line.evaluate((element) => {
      const leaves = Array.from(element.querySelectorAll<HTMLElement>('span')).filter((candidate) => candidate.children.length === 0);
