...
test.describe('CodeMirror interaction stability', () => {
  ...
  test('keeps focused and inactive text selections visibly distinct from the active line', async ({
    page,
  }) => {
  ...
  test('keeps the previous semantic decorations mounted while the next token set is pending', async ({
    page,
  }) => {
    await openWorkbench(page, { waitForLsp: true })
    await switchEditor(page, 'codemirror')
