...
export function observeOperationWebSocket(page: Page): OperationWebSocketTrace {
  ...
  return {
    ...
    sent,
    received,
    findStart: (operation, requestText) => sent.find((frame) => frame.type === 'start' && frame.operation === operation && (requestText === undefined || JSON.stringify(frame.request).includes(requestText))),
    operationIdForStart: (start) => {
      if (!start.commandId) return null;
      const response = received.find((frame) => frame.type === 'response' && frame.commandId === start.commandId && frame.ok);
      return response ? operationIdFromPayload(response.payload) : null;
    },
    hasSubscription: (operationId, fromSequence = 0) => sent.some((frame) => frame.type === 'subscribe' && frame.operationId === operationId && frame.fromSequence === fromSequence),
    hasEvent: (operationId) => received.some((frame) => frame.type === 'event' && frame.operationId === operationId),
  };
}

function parseJsonFrame(payload: string | Buffer): OperationCommandFrame | null {
  try {
    const parsed = decodeWire<unknown>(JSON.parse(typeof payload === 'string' ? payload : payload.toString('utf8')))
    return parsed && typeof parsed === 'object' && typeof parsed.type === 'string' ? (parsed as OperationCommandFrame) : null
  } catch {
    return null;
  }
}
...
export async function waitForLanguageServiceReady(page: Page) {
  await expect(page.locator('[data-editor][data-language-service-status]')).toHaveAttribute('data-language-service-status', 'ready', { timeout: 30_000 });
}
...
export async function expectActiveEditor(page: Page, editor: EditorKind) {
  ...
  if (await control.isVisible()) {
    await expect(control.getByRole('button', { name: activeName, exact: true })).toHaveAttribute('aria-pressed', 'true')
    await expect(control.getByRole('button', { name: inactiveName, exact: true })).toHaveAttribute('aria-pressed', 'false')
  }
...
export async function expectEditorSwitchFits(page: Page) {
  await revealEditorSwitch(page)
  const control = editorSwitch(page)
  const monaco = await visibleBox(control.getByRole('button', { name: 'Monaco', exact: true }), 'Monaco editor switch')
  const codeMirror = await visibleBox(control.getByRole('button', { name: 'CodeMirror', exact: true }), 'CodeMirror editor switch')
...
export function expectHorizontalSplit(source: ElementBox, result: ElementBox) {
  ...
  expect(Math.min(source.y + source.height, result.y + result.height) - Math.max(source.y, result.y)).toBeGreaterThan(100)
}
...
export function expectVerticalSplit(source: ElementBox, result: ElementBox) {
  ...
  expect(Math.min(source.x + source.width, result.x + result.width) - Math.max(source.x, result.x)).toBeGreaterThan(100)
}
...
export async function expectInsideViewport(page: Page, box: ElementBox, label: string) {
  ...
  expect(box.x + box.width, `${label} overflows horizontally`).toBeLessThanOrEqual((viewport?.width ?? 0) + 0.5)
  expect(box.y + box.height, `${label} overflows vertically`).toBeLessThanOrEqual((viewport?.height ?? 0) + 0.5)
}
...
export async function expectResultContentFillsPane(page: Page) {
  ...
  expect(Math.abs(body.y - pane.y), 'result body should start at the pane top').toBeLessThanOrEqual(1)
  expect(Math.abs(body.y + body.height - (pane.y + pane.height)), 'result body should reach the pane bottom').toBeLessThanOrEqual(1)
  expect(Math.abs(shell.y + shell.height - (body.y + body.height)), 'result tabs should use the remaining result-body height').toBeLessThanOrEqual(1)
  expect(Math.abs(panel.height - (shell.height - toolbar.height)), 'the selected result should fill below the tabs and actions').toBeLessThanOrEqual(2)
  expect(Math.abs(content.y - panel.y), 'the selected result root should start at the panel top').toBeLessThanOrEqual(1)
  expect(Math.abs(content.y + content.height - (panel.y + panel.height)), 'the selected result root should reach the panel bottom').toBeLessThanOrEqual(1)

  const resultDocument = panelLocator.locator('.result-document')
  const resultDocumentCount = await resultDocument.count()
  expect(resultDocumentCount, 'a result tab should contain at most one text document').toBeLessThanOrEqual(1)
  if (resultDocumentCount === 1) {
    const document = await visibleBox(resultDocument, 'result text document')
    expect(Math.abs(document.y + document.height - (content.y + content.height)), 'the result text document should reach its result root bottom').toBeLessThanOrEqual(1)
  }

  const codeDocument = panelLocator.locator('.code-document-view')
  const codeDocumentCount = await codeDocument.count()
  expect(codeDocumentCount, 'a result tab should contain at most one code document').toBeLessThanOrEqual(1)
  if (codeDocumentCount === 1) {
    const host = await visibleBox(codeDocument, 'result code document')
    expect(Math.abs(host.y + host.height - (content.y + content.height)), 'the result code document should reach its result root bottom').toBeLessThanOrEqual(1)
    ...
    expect(codeMirrorCount + monacoCount, 'the result code document should contain exactly one editor').toBe(1)

    const editor = await visibleBox(codeMirrorCount === 1 ? codeMirrorEditor : monacoEditor, codeMirrorCount === 1 ? 'result CodeMirror editor' : 'result Monaco editor')
    expect(Math.abs(editor.y - host.y), 'the result editor should start at its host top').toBeLessThanOrEqual(1)
    expect(Math.abs(editor.y + editor.height - (host.y + host.height)), 'the result editor should reach its host bottom').toBeLessThanOrEqual(1)

    if (codeMirrorCount === 1) {
      const scroller = await visibleBox(codeDocument.locator('.cm-scroller'), 'result CodeMirror scroller')
      expect(Math.abs(scroller.y + scroller.height - (host.y + host.height)), 'the result editor scroller should reach its host bottom').toBeLessThanOrEqual(1)
    }
  }

  const copy = page.getByRole('button', { name: 'Copy output' })
  const copyBox = await visibleBox(copy, 'copy output action')
  expect(toolbar.height, 'result tabs and actions should use one compact row').toBeLessThanOrEqual(32)
  expect(Math.abs(tabs.y - toolbar.y), 'result tabs should share the action row').toBeLessThanOrEqual(1)
  expect(Math.abs(tabs.y + tabs.height / 2 - (copyBox.y + copyBox.height / 2)), 'result tabs and the copy action should be vertically aligned').toBeLessThanOrEqual(1)
...
export async function expectCodeDocumentHorizontalScrollAtPanelBottom(page: Page, ariaLabel: string) {
  ...
  const metrics = await document.evaluate((content) => {
    ...
    if (!(scroller instanceof HTMLElement)) throw new Error('Result CodeMirror scroller is missing.')
