...
import { editorHost, expectActiveEditor, openWorkbench, replaceSource, switchEditor, waitForCompletedOperation, waitForLanguageServiceReady } from './helpers/workbench';
...
function sourceAssociationLines(page: Page, editor: ResultEditorKind): Locator {
  return editor === 'monaco' ? editorHost(page, 'monaco').locator('.monaco-editor .monaco-source-association-line') : editorHost(page, 'codemirror').locator('.cm-line.cm-source-association-line')
}

function activeSourceAssociationLines(page: Page, editor: ResultEditorKind): Locator {
  return editor === 'monaco' ? editorHost(page, 'monaco').locator('.monaco-source-association-line-active') : editorHost(page, 'codemirror').locator('.cm-source-association-line-active')
}

function preciseSourceAssociationMarks(page: Page, editor: ResultEditorKind): Locator {
  return editor === 'monaco' ? editorHost(page, 'monaco').locator('.monaco-source-association-exact-active') : editorHost(page, 'codemirror').locator('.cm-source-association-exact-active')
}

function outputAssociationLines(page: Page, editor: ResultEditorKind): Locator {
  return editor === 'monaco' ? page.locator('.code-document-view.monaco-code-document .monaco-output-source-navigable.source-association') : page.locator('.code-document-view .cm-line.cm-source-navigable.source-association')
}

function activeOutputAssociationLines(page: Page, editor: ResultEditorKind): Locator {
  return editor === 'monaco' ? page.locator('.monaco-code-document .monaco-output-source-active') : page.locator('.code-document-view .cm-source-association-active')
}
...
async function expectWholeLineSourceAssociation(page: Page, editor: ResultEditorKind) {
  ...
  await expect(host.locator(editor === 'monaco' ? '.monaco-source-association' : '.cm-source-association')).toHaveCount(0)

  const metrics = await lines.first().evaluate((line, kind) => {
    const content = kind === 'monaco' ? line.closest('.monaco-editor')?.querySelector('.view-lines') : line.closest('.cm-content')
    ...
    return {
      ...
      hasInlineAssociation: line.querySelector(kind === 'monaco' ? '.monaco-source-association' : '.cm-source-association') !== null,
...
async function expectPreciseActiveSourceAssociation(page: Page, editor: ResultEditorKind): Promise<{ x: number; y: number }> {
  ...
  const sameLine = exactRects.filter((rect) => Math.abs(rect.top - first.top) <= 1).sort((left, right) => left.left - right.left)
  for (let index = 1; index < sameLine.length; index += 1) {
    const previous = sameLine[index - 1]
    const current = sameLine[index]
    if (!previous || !current) continue
    expect(current.left - previous.right, 'The precise source mark has a token gap').toBeLessThanOrEqual(1.5)
  }

  const lineMetrics = await activeSourceAssociationLines(page, editor).evaluateAll((lines, targetTop) => {
...
function pngPixelAtCssPoint(png: Buffer, point: { x: number; y: number }, viewport: { width: number; height: number }): RgbPixel {
  let offset = 8
  let width = 0
  let height = 0
  let bytesPerPixel = 0
  ...
  for (let row = 0; row < height; row += 1) {
    ...
    for (let column = 0; column < stride; column += 1) {
      ...
      const upperLeft = row > 0 && column >= bytesPerPixel ? (pixels[outputIndex - stride - bytesPerPixel] ?? 0) : 0
      const predictor = filter === 0 ? 0 : filter === 1 ? left : filter === 2 ? up : filter === 3 ? Math.floor((left + up) / 2) : filter === 4 ? paeth(left, up, upperLeft) : Number.NaN
...
async function expectSourceSelectionAboveAssociations(page: Page, editor: ResultEditorKind) {
  ...
  if (editor === 'monaco') {
    ...
    expect(activeElement.className?.includes('inputarea') || activeElement.className?.includes('native-edit-context'), `The source click did not give Monaco text focus: ${JSON.stringify(activeElement)}`).toBe(true)
  }
  const selectionLayers = editor === 'monaco' ? host.locator('.view-overlays .selected-text') : host.locator('.cm-selectionLayer .cm-selectionBackground')
  await expect(selectionLayers).toHaveCount(0)
  await expect(preciseSourceAssociationMarks(page, editor).first()).not.toHaveCSS('background-color', 'rgba(0, 0, 0, 0)')
  ...
  const selectionBackground = await selection.evaluate((element) => getComputedStyle(element).backgroundColor)
  expect(selectionBackground).not.toBe('rgba(0, 0, 0, 0)')
  const overlap = await page.evaluate(
    ({ editorKind, fallback }) => {
      const host = document.querySelector(editorKind === 'monaco' ? '[data-editor="monaco"]' : '[data-editor="codemirror"]')
      if (!(host instanceof HTMLElement)) return { point: fallback, found: false }
      const exact = [...host.querySelectorAll<HTMLElement>(editorKind === 'monaco' ? '.monaco-source-association-exact-active' : '.cm-source-association-exact-active')].map((element) => element.getBoundingClientRect())
      const selections = [...host.querySelectorAll<HTMLElement>(editorKind === 'monaco' ? '.view-overlays .selected-text' : '.cm-selectionLayer .cm-selectionBackground')].map((element) => element.getBoundingClientRect())
  ...
  expect(pixelDistance(beforeSelection, afterSelection), 'The real selection did not visibly replace the association color at the sampled source pixel.').toBeGreaterThan(12)
...
async function textPointForAssociation(association: Locator, editor: ResultEditorKind): Promise<{ x: number; y: number; color: string }> {
  return association.evaluate((element, kind) => {
    ...
    const candidates = [...root.querySelectorAll<HTMLElement>(kind === 'monaco' ? '.view-line span' : '.cm-line span')]
      .filter((candidate) => candidate.children.length === 0 && (candidate.textContent ?? '').trim().length > 0)
      .filter((candidate) => {
        const rect = candidate.getBoundingClientRect()
        return rect.top < associationRect.bottom && rect.bottom > associationRect.top && rect.width > 2
      })
...
async function activateOutputAssociation(page: Page, editor: ResultEditorKind, association: Locator) {
  const point = await textPointForAssociation(association, editor)
  await page.mouse.click(point.x, point.y)
}

async function hoverOutputAssociationForExactText(page: Page, editor: ResultEditorKind, expectedText: string): Promise<{ x: number; y: number }> {
  const scroller = page.locator(editor === 'monaco' ? '.monaco-code-document .monaco-scrollable-element' : '.code-document-view .cm-scroller')
  ...
  for (const fraction of fractions) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const activeText = await preciseSourceAssociationMarks(page, editor).evaluateAll((marks) => marks.map((mark) => mark.textContent ?? '').join(''))
...
async function visibleCodeMirrorAssociationPoint(page: Page): Promise<{ x: number; y: number }> {
  return outputAssociationLines(page, 'codemirror').evaluateAll((lines) => {
    ...
    const token = [...line.querySelectorAll<HTMLElement>('span')].find((candidate) => {
      const rect = candidate.getBoundingClientRect()
      return candidate.children.length === 0 && (candidate.textContent ?? '').trim().length > 0 && rect.width > 2
    })
...
async function expectCodeMirrorFoldDoesNotNavigate(page: Page, mobile: boolean) {
  ...
  const collapsedControlPoint = await gutter.locator('.cm-gutterElement').evaluateAll((controls) => {
    const control = controls.find((candidate) => {
      const rect = candidate.getBoundingClientRect()
      return (candidate.textContent ?? '').trim().length > 0 && rect.width > 0 && rect.height > 0
    })
    if (!(control instanceof HTMLElement)) throw new Error('The unfolded gutter control is missing.')
...
async function expectMonacoFoldDoesNotNavigate(page: Page) {
  ...
  await expect(page.locator('.monaco-code-document .codicon-folding-expanded').first()).toBeVisible()
}

async function tokenColors(page: Page, editor: ResultEditorKind, token: string): Promise<string[]> {
  const spans = editor === 'monaco' ? page.locator('.monaco-code-document .view-line span') : page.locator('.code-document-view .cm-line span')
  return spans.evaluateAll((candidates, expected) => {
    return candidates.filter((span) => span.children.length === 0 && (span.textContent ?? '').trim() === expected).map((span) => getComputedStyle(span).color)
  }, token)
}

async function sourceTokenColors(page: Page, editor: ResultEditorKind, token: string): Promise<string[]> {
  const spans = editor === 'monaco' ? editorHost(page, 'monaco').locator('.view-lines .view-line span') : editorHost(page, 'codemirror').locator('.cm-line span')
  return spans.evaluateAll((candidates, expected) => {
    return candidates.filter((span) => span.children.length === 0 && (span.textContent ?? '').trim() === expected).map((span) => getComputedStyle(span).color)
  }, token)
}
...
async function expectHeaderGeometry(page: Page, width: number) {
  ...
  const run = page.locator(compact ? '.mobile-command-bar .run-button' : '.selector-group--result > .run-button')
  const actions = page.locator('.app-bar-actions')
  await expect(run).toBeVisible()
  await expect(actions).toBeVisible()
  const [runRect, actionRect, brandRect] = await Promise.all([run.boundingBox(), actions.boundingBox(), page.locator('.brand').boundingBox()])
  if (!runRect || !actionRect || !brandRect) {
    throw new Error(`The ${width}px header is missing a geometry rectangle.`)
  }
  expect(runRect.x + runRect.width, `${width}px Render overlaps app actions`).toBeLessThanOrEqual(actionRect.x + 0.5)
  expect(brandRect.x + brandRect.width, `${width}px brand overlaps Render`).toBeLessThanOrEqual(runRect.x + 0.5)
  expect(actionRect.x + actionRect.width, `${width}px actions exceed the viewport`).toBeLessThanOrEqual(width)
  ...
  const [sourceRect, resultRect] = await Promise.all([page.locator('.source-pane').boundingBox(), page.locator('.result-pane').boundingBox()])
  if (!sourceRect || !resultRect) throw new Error(`The ${width}px workbench panes are missing.`)
  if (width > 860) {
    expect(Math.abs(sourceRect.y - resultRect.y), `${width}px panes should remain horizontal`).toBeLessThanOrEqual(1)
    expect(sourceRect.x + sourceRect.width).toBeLessThanOrEqual(resultRect.x + 1)
  } else {
    expect(resultRect.y, `${width}px result pane should be below source`).toBeGreaterThanOrEqual(sourceRect.y + sourceRect.height)
  }
}

test.describe('linked editor visual regressions', () => {
  test('keeps Render and app actions separate at responsive boundaries and disconnect', async ({ page, isMobile }, testInfo) => {
    ...
    await page.routeWebSocket('**/api/v1/operations/ws', (socket) =>
      socket.close({
        code: 1011,
        reason: 'responsive disconnected-state acceptance',
      }),
    )
    await page.reload()
    await expect(page.getByLabel('Language')).toBeEnabled()
    await expect(page.locator('.app-health[data-state="error"]')).toBeVisible({
      timeout: 10_000,
    })
  ...
  test('keeps new IL associations inactive until navigation and paints whole source lines', async ({ page, isMobile }, testInfo) => {
    ...
    expect(await preciseSourceAssociationMarks(page, editor).evaluateAll((marks) => marks.map((mark) => mark.textContent ?? '').join(''))).toBe('i++')
    ...
    expect(pixelDistance(idleWholeLinePixel, hoveredWholeLinePixel), 'Hover must remove the whole-line source fill so only the exact span remains.').toBeGreaterThan(12)
    expect(pixelDistance(idleExactPixel, hoveredExactPixel), 'Hover must visibly strengthen only the exact source span.').toBeGreaterThan(12)
    ...
    await capture(page, testInfo, `${isMobile ? 'mobile-codemirror' : 'desktop-monaco'}-il-linked.png`)
  })

  test('maps optimized JIT instructions to exact same-line source spans', async ({ page, isMobile }, testInfo) => {
    ...
    await capture(page, testInfo, `${isMobile ? 'mobile-codemirror' : 'desktop-monaco'}-jit-exact-i-plus-plus.png`)
  })

  test('does not snap CodeMirror JIT output back after a linked click and manual scroll', async ({ page, isMobile }, testInfo) => {
    ...
    await expect.poll(() => scroller.evaluate((element) => element.scrollTop)).toBe(immediateScrollTop)
    ...
    await page.mouse.move(immediateScrollerBox.x + Math.min(180, immediateScrollerBox.width / 2), immediateScrollerBox.y + immediateScrollerBox.height / 2)
    ...
    await expect.poll(() => scroller.evaluate((element) => element.scrollTop)).toBe(settledScrollTop)
  ...
  test('uses VS keyword and user-type colors in decompiled C# for the active result editor', async ({ page, isMobile }) => {
    const editor: ResultEditorKind = isMobile ? 'codemirror' : 'monaco'
    await openWorkbench(page)
    await expectActiveEditor(page, editor)
  ...
  test('gives CodeMirror keyword completion a readable face and strong selected contrast', async ({ page, isMobile }) => {
    ...
    const keywordIndex = await options.evaluateAll((rows) => rows.findIndex((row) => row.querySelector('.cm-completionIcon-keyword') !== null))
    expect(keywordIndex).toBeGreaterThanOrEqual(0)
    let selectedIndex = await options.evaluateAll((rows) => rows.findIndex((row) => row.getAttribute('aria-selected') === 'true'))
    const optionCount = await options.count()
    for (let step = 0; selectedIndex !== keywordIndex && step < optionCount; step += 1) {
      await page.keyboard.press('ArrowDown')
      selectedIndex = await options.evaluateAll((rows) => rows.findIndex((row) => row.getAttribute('aria-selected') === 'true'))
    }
    ...
    await expect(keyword.locator('.cm-completionIcon-keyword')).toHaveCSS('color', 'rgb(255, 255, 255)')
    ...
    if (optionCount > 1) {
      ...
      await expect(keyword.locator('.cm-completionIcon-keyword')).toHaveCSS('color', 'rgb(0, 0, 255)')
    }
  })
})
