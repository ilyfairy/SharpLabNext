...
async function splitPercent(page: Page): Promise<number> {
  const value = await page
    .getByRole('separator', { name: 'Resize source and result panes' })
...
test('resizes, persists, and resets the responsive source/result split', async ({
  page,
  isMobile,
}) => {
  await openWorkbench(page)
  const separator = page.getByRole('separator', { name: 'Resize source and result panes' })
  ...
  const targetX = isMobile
    ...
    : grid.x + grid.width * (targetPercent / 100)
  const targetY = isMobile
    ? grid.y + grid.height * (targetPercent / 100)
