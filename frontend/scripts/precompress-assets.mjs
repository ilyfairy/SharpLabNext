...
await Promise.all(
  files
    .filter((file) => file.endsWith('.br') || file.endsWith('.gz') || file.endsWith('.zst'))
...
const sourceFiles = files.filter(
  (file) =>
    ...
    !file.endsWith('.gz') &&
    !file.endsWith('.zst') &&
    compressibleExtensions.has(path.extname(file).toLowerCase()),
)
...
for (const file of sourceFiles) {
  ...
  const brotli = brotliCompressSync(source, {
    params: {
      [constants.BROTLI_PARAM_MODE]: isTextFile(file)
...
console.log(
  `  compressible assets: ${sourceFiles.length} (${eligibleFiles} eligible, ${smallFiles} below ${minimumBytes} B)`,
)
...
function isWorthWriting(sourceBytes, compressedBytes) {
  const requiredSavings = Math.max(
    minimumSavingsBytes,
    Math.ceil(sourceBytes * minimumSavingsRatio),
  )
...
function printSummary(label, variants, bytes, identityBytes) {
  ...
  console.log(
    `  ${label.padEnd(14)} ${String(variants).padStart(8)} ${formatBytes(bytes).padStart(20)} ${`${savings.toFixed(1)}%`.padStart(12)}`,
  )
}
