...
public sealed class MiniLanguageSessionService(
    MiniLanguageWorkerIdentity workerIdentity,
    LanguageWorkerCapabilityManifest manifest) : ILanguageWorkerSessionService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var session = new LanguageSession(
            ...
            $"{workerIdentity.ToolchainId}/{workerIdentity.CompilerVersion}",
        ...
        CancellationToken cancellationToken)
    {
        if (!_sessions.TryGetValue(sessionId, out var state))
        {
            throw new LanguageWorkerRequestException(
                "session-not-found",
                "The MiniLang language session does not exist.",
