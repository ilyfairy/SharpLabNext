...
public static partial class MiniLanguageCompiler
{
    ...
    public static MiniLanguageCompilation Compile(WorkspaceSnapshot workspace)
    {
        ...
        var sourceOrder = workspace.SourceOrder.Count > 0
            ...
            : workspace.Files.Select(static file => file.Path).ToArray();
        ...
        foreach (var path in sourceOrder)
        {
            if (!files.TryGetValue(path, out var file))
            {
                diagnostics.Add(CreateDiagnostic(
                    "MINI1003",
                    $"Source order refers to missing file '{path}'.",
                    path,
                    null,
                    workspace));
                continue;
        ...
        var cil = diagnostics.Any(static diagnostic => diagnostic.Severity == DiagnosticSeverity.Error)
            ? null
            : EmitCil(values, workspace.BuildOptions.OutputKind == BuildOutputKind.Console);
        ...
        long selectionRevision)
    {
        ...
        var workspace = new WorkspaceSnapshot(
            ...
            selectionRevision,
            LanguageId,
            [new WorkspaceFile(path, workspaceRevision, text)],
            path,
            [path],
            "net10-ref",
            options);
        ...
        List<Diagnostic> diagnostics)
    {
        ...
        for (var lineIndex = 0; lineIndex < lines.Length; lineIndex++)
        {
            ...
            if (!match.Success)
            {
                diagnostics.Add(CreateDiagnostic(
                    "MINI1001",
                    "Expected a statement in the form: print \"text\".",
                    file.Path,
                    new TextRange(lineIndex, 0, lineIndex, line.Length),
                    workspace));
                continue;
            }
            ...
            try
            {
                values.Add(JsonSerializer.Deserialize<string>(match.Groups["literal"].Value)
                    ?? string.Empty);
            }
            catch (JsonException)
            {
                diagnostics.Add(CreateDiagnostic(
                    "MINI1002",
                    "The print argument is not a valid quoted string.",
                    file.Path,
                    new TextRange(lineIndex, 0, lineIndex, line.Length),
                    workspace));
            }
    ...
    private static string EmitCil(IReadOnlyList<string> values, bool includeEntryPoint)
    {
        ...
        cil.AppendLine(".assembly extern System.Runtime {}")
            .AppendLine(".assembly extern System.Console {}")
            .AppendLine(".assembly MiniLanguageProgram {}")
            .AppendLine(".module MiniLanguageProgram.dll")
            .AppendLine()
            .AppendLine(".class public auto ansi abstract sealed Program extends [System.Runtime]System.Object")
            .AppendLine("{")
            .AppendLine("  .method public hidebysig static void Main() cil managed")
            .AppendLine("  {")
            .AppendLine("    .maxstack 1");
        if (includeEntryPoint)
            cil.AppendLine("    .entrypoint");
        foreach (var value in values)
        {
            cil.Append("    ldstr \"").Append(EscapeCilString(value)).AppendLine("\"")
                .AppendLine("    call void [System.Console]System.Console::WriteLine(string)");
        }
        cil.AppendLine("    ret")
            .AppendLine("  }")
            .AppendLine("}");
    ...
    private static string EscapeCilString(string value) => value
...
public sealed record MiniLanguageCompilation(
