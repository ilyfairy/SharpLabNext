...
public sealed class TargetRuntimeRunnerProjectTests
{
    ...
    private static readonly string ProjectDirectory = Path.Combine(
        ...
        "src",
        "RuntimeJobs",
        "SharpLabNext.TargetRuntimeRunner");
    ...
    public void ProjectKeepsPortableNet20AnyCpuContract()
    {
        var project = XDocument.Load(Path.Combine(
            ProjectDirectory,
            "SharpLabNext.TargetRuntimeRunner.csproj"));
        var properties = project.Root!
            .Elements("PropertyGroup")
            .Elements()
            .ToLookup(static element => element.Name.LocalName, StringComparer.Ordinal);
    ...
    public void PackageLockContainsNoRuntimeSpecificDependencyGraph()
    {
        using var document = JsonDocument.Parse(File.ReadAllBytes(Path.Combine(
            ProjectDirectory,
            "packages.lock.json")));
        var graphNames = document.RootElement
