...
public sealed class ProfileUpdaterTests
{
    ...
    public async Task CandidateWorkspaceInitializesSubmodulesAfterCreatingWorktree()
    {
        ...
        Assert.Collection(
            ...
            {
                Assert.Equal(workspace, command.WorkingDirectory);
                Assert.Equal(["submodule", "update", "--init", "--recursive", "--checkout", "--force"], command.Arguments);
            });
    }
    ...
    public async Task CandidateWorkspaceRefreshesSubmodulesWhenWorktreeAlreadyExists()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(workspace, ".git"), "gitdir: test", TestContext.Current.CancellationToken);
        try
        {
            await manager.PrepareAsync(root, workspace, TestContext.Current.CancellationToken);

            Assert.Collection(runner.Commands, command => Assert.Equal(["submodule", "sync", "--recursive"], command.Arguments), command => Assert.Equal(["submodule", "update", "--init", "--recursive", "--checkout", "--force"], command.Arguments));
        }
        finally
        {
    ...
    {
        var runner = new RecordingCommandRunner { FailurePredicate = static command => command.Arguments.SequenceEqual(["submodule", "update", "--init", "--recursive", "--checkout", "--force"]) };
        var manager = new GitProfileCandidateWorkspaceManager(runner);
        var root = Path.Combine(Path.GetTempPath(), $"SharpLabNext.Source.{Guid.NewGuid():N}");
        var workspace = Path.Combine(Path.GetTempPath(), $"SharpLabNext.Candidate.{Guid.NewGuid():N}");

        var exception = await Assert.ThrowsAsync<ProfileUpdateValidationException>(() => manager.PrepareAsync(root, workspace, TestContext.Current.CancellationToken));
    ...
    public async Task OfficialSourceRejectsRuntimeVersionFromArchiveWithWrongChecksum()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidDataException>(() => source.ResolveDotNetChannelAsync("10.0", TestContext.Current.CancellationToken));
    ...
    public async Task OfficialSourceResolvesGSharpVersionTagToExactCommitAndArchiveDigest()
    {
        ...
        var result = await source.ResolveGitCommitAsync("DavidObando", "gsharp", "v0.3.8", TestContext.Current.CancellationToken);
    ...
    public async Task ResolverRejectsInvalidDerivedToolchainContract(string kind, string versionTemplate)
    {
        using var channels = new TempChannelDirectory(
            toolchainsYaml: $$"""
                channels:
    ...
    public async Task CandidateIdentityClosureRejectsDerivedToolchainInputMismatch()
    {
        ...
        components["roslyn-stable-netfx48"] = components["roslyn-stable-netfx48"] with { SourceUri = "https://example.test/wrong-roslyn-package.nupkg" };
    ...
    public async Task CandidateIdentityClosureRejectsLegacyGSharpComponentWithWrongKind()
    {
        ...
        var catalog = await CatalogLoader.LoadCatalogAsync(Path.Combine(repositoryRoot, "profiles", "catalog", "catalog.json"), TestContext.Current.CancellationToken);
        var releaseLock = await CatalogLoader.LoadReleaseLockAsync(Path.Combine(repositoryRoot, "profiles", "lock.json"), TestContext.Current.CancellationToken);
        catalog = CandidateReleaseMaterializer.CreateCatalog(catalog, releaseLock, "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
        var components = releaseLock.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value, StringComparer.Ordinal);
        components["gsharp-legacy-0.3.8"] = components["gsharp-legacy-0.3.8"] with { Kind = "source" };
    ...
    public async Task CandidateIdentityClosureRejectsPayloadIdentityWhenMatrixRuntimeBecomesSelectable()
    {
        ...
        var candidateCatalog = catalog with { Runtimes = catalog.Runtimes.Select(runtime => runtime.Id == runtimeId ? runtime with { Availability = new ComponentAvailability { Installed = true, Health = "healthy" } } : runtime).ToArray() };
        var components = releaseLock.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value, StringComparer.Ordinal);
        components[runtimeId] = new LockedComponent { Kind = "runtime", ResolvedVersion = matrixRuntime.ResolvedVersion, Commit = $"payload-sha512:{new string('a', 128)}", JitCommit = $"payload-sha512:{new string('b', 128)}", Sha512 = new string('c', 128), SourceUri = "https://example.test/dotnet-5-runtime.tar.gz" };
    ...
    public async Task CandidateIdentityClosureAcceptsSelectableMonoWithOperatorDigest()
    {
        ...
        var candidateCatalog = catalog with { Runtimes = catalog.Runtimes.Select(runtime => runtime.Id == runtimeId ? runtime with { RuntimeCommit = "not-applicable", JitVersion = "not-applicable", JitCommit = "not-applicable", Availability = new ComponentAvailability { Installed = true, Health = "healthy" } } : runtime).ToArray() };
        var components = releaseLock.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value, StringComparer.Ordinal);
        components[runtimeId] = new LockedComponent { Kind = "runtime", ResolvedVersion = mono.ResolvedVersion, Digest = "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", SourceUri = "docker://example.test/mono@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" };
    ...
    public async Task CandidateIdentityClosureRejectsOperatorCoreClrCommitClaims()
    {
        ...
        catalog = CandidateReleaseMaterializer.CreateCatalog(catalog, releaseLock, "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
        const string runtimeId = "mono-6.12-linux-x64";
        var candidateCatalog = catalog with { Runtimes = catalog.Runtimes.Select(runtime => runtime.Id == runtimeId ? runtime with { RuntimeCommit = new string('a', 40) } : runtime).ToArray() };

        var exception = Assert.Throws<ProfileUpdateValidationException>(() => CandidateReleaseMaterializer.ValidateIdentityClosure(releaseLock, candidateCatalog));
    ...
    public async Task CandidateIdentityClosureRejectsSelectableMonoWithoutOperatorDigest()
    {
        ...
        const string runtimeId = "mono-6.12-linux-x64";
        var mono = catalog.Runtimes.Single(runtime => runtime.Id == runtimeId);
        var candidateCatalog = catalog with { Runtimes = catalog.Runtimes.Select(runtime => runtime.Id == runtimeId ? runtime with { RuntimeCommit = null, JitCommit = null, Availability = new ComponentAvailability { Installed = true, Health = "healthy" } } : runtime).ToArray() };
        var components = releaseLock.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value, StringComparer.Ordinal);
        components[runtimeId] = new LockedComponent { Kind = "runtime", ResolvedVersion = mono.ResolvedVersion, SourceUri = "docker://example.test/mono" };
    ...
    public async Task ResolverProducesAnExactCandidateWithoutMutatingTheInputLock()
    {
        ...
        var updater = new ReleaseLockUpdater(new FakeProfileSourceClient(), Path.Combine(FindRepositoryRoot(), "profiles", "channels"));

        var result = await updater.ResolveAsync(current, "candidate-1", TestContext.Current.CancellationToken);

        Assert.Equal("11.0.0-preview.5", result.Candidate.Components["dotnet-11-preview-linux-x64"].ResolvedVersion);
        Assert.Equal("f7b4c5716faaee8fb8a289aed29118cad955c45f", result.Candidate.Components["dotnet-11-preview-linux-x64"].Commit);
        Assert.Equal("f7b4c5716faaee8fb8a289aed29118cad955c45f", result.Candidate.Components["dotnet-11-preview-linux-x64"].JitCommit);
        Assert.Equal("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", result.Candidate.Components["roslyn-main"].Commit);
        Assert.Equal("5.10.0", result.Candidate.Components["roslyn-main"].ResolvedVersion);
        Assert.Equal("sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", result.Candidate.Components["roslyn-main"].Digest);
        Assert.Equal("https://github.com/dotnet/roslyn/archive/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.tar.gz", result.Candidate.Components["roslyn-main"].SourceUri);
        ...
        Assert.Equal("608bf30cf3f43f97e32825076a2cfdaa25043e50", result.Candidate.Components["peachpie-stable"].Commit);
        ...
        Assert.Equal("aaf35bb8d5e1e8704e982ad0ab95263451bd2d3d", result.Candidate.Components["gsharp-stable"].Commit);
        Assert.Equal("sha256:f52d21ef09b198bad69b7ac8dd5f6d2eaa91216b80bfc22e9610a1fef28f06d4", result.Candidate.Components["gsharp-source"].Digest);
        Assert.DoesNotContain("frontend-react", result.Candidate.Components);
        Assert.Equal("8.0.0-beta.23516.4", result.Candidate.Components["const-generics-versiontools"].ResolvedVersion);
        Assert.Equal($"sha256:{new string('d', 64)}", result.Candidate.Components["const-generics-versiontools"].Digest);
        ...
        Assert.Contains(result.Changes, static change => change.ComponentId == "frontend-react" && change.NewVersion is null);
    ...
    public async Task ResolveStoresCandidateByContentDigestAndWritesReceipt()
    {
        ...
        var result = await workflow.ResolveAsync("candidate-1", cancellationToken: TestContext.Current.CancellationToken);

        Assert.Equal(result.CandidateDigest, Digest(await File.ReadAllBytesAsync(result.CandidatePath, TestContext.Current.CancellationToken)));
        Assert.EndsWith(Path.Combine(result.CandidateDigest[7..], "lock.json"), result.CandidatePath, StringComparison.Ordinal);
        ...
        Assert.True(File.Exists(Path.Combine(Path.GetDirectoryName(result.CandidatePath)!, "workspace", "profiles", "versions.props")));
        var workspaceRoot = Path.Combine(Path.GetDirectoryName(result.CandidatePath)!, "workspace");
        var candidateReferenceSetConfigurationPath = Path.Combine(workspaceRoot, Path.GetRelativePath(repository.Root, repository.ReferenceSetConfigurationPath));
        var generatedJsonPaths = new List<string> { result.CandidatePath, Path.Combine(Path.GetDirectoryName(result.CandidatePath)!, "receipt.json"), Path.Combine(workspaceRoot, "profiles", "catalog", "catalog.json"), candidateReferenceSetConfigurationPath, Path.Combine(repository.StateRoot, "state.json"), repository.PublicStatusPath };
        generatedJsonPaths.AddRange(Directory.EnumerateFiles(Path.Combine(workspaceRoot, "profiles", "runtimes"), "*.json", SearchOption.TopDirectoryOnly));
        foreach (var generatedJsonPath in generatedJsonPaths)
            await AssertUtf8NoBomLfAsync(generatedJsonPath);
        var resolvedLock = await CatalogLoader.LoadReleaseLockAsync(result.CandidatePath, TestContext.Current.CancellationToken);
        using var referenceSetConfiguration = JsonDocument.Parse(await File.ReadAllBytesAsync(candidateReferenceSetConfigurationPath, TestContext.Current.CancellationToken));
        ...
        Assert.Equal(resolvedLock.Components["net10-ref"].ResolvedVersion, configuredNet10.GetProperty("FrameworkVersion").GetString());
        Assert.Equal(ReferenceSetIdentityResolver.ResolveLockedDigest(resolvedLock.Components["net10-ref"], "net10-ref"), configuredNet10.GetProperty("Digest").GetString());
        var configuredNet11 = configuredReferenceSets.GetProperty("net11-preview-ref");
        Assert.Equal(resolvedLock.Components["net11-preview-ref"].ResolvedVersion, configuredNet11.GetProperty("FrameworkVersion").GetString());
        ...
        var materializedCatalog = await CatalogLoader.LoadCatalogAsync(Path.Combine(workspaceRoot, "profiles", "catalog", "catalog.json"), TestContext.Current.CancellationToken);
        Assert.Equal(resolvedLock.Components["roslyn-stable-netfx48"].ResolvedVersion, materializedCatalog.Toolchains.Single(static item => item.Id == "roslyn-stable-netfx48").ResolvedVersion);
        Assert.Equal($"Roslyn Stable {resolvedLock.Components["roslyn-stable-netfx48"].ResolvedVersion} / .NET Framework", materializedCatalog.Toolchains.Single(static item => item.Id == "roslyn-stable-netfx48").DisplayName);
        Assert.Equal(resolvedLock.Components["gsharp-stable"].ResolvedVersion, materializedCatalog.Toolchains.Single(static item => item.Id == "gsharp-stable").DisplayName);
        Assert.Equal(resolvedLock.Components["roslyn-stable"] with { PatchDigest = null, ImageId = null }, resolvedLock.Components["roslyn-stable-netfx48"]);
        var frameworkReferenceSetIds = materializedCatalog.Toolchains.Single(static item => item.Id == "roslyn-stable-netfx48").AllowedReferenceSetIds;
        Assert.Equal(14, frameworkReferenceSetIds.Count);
        foreach (var referenceSetId in frameworkReferenceSetIds)
            Assert.Equal(ReferenceSetIdentityResolver.ResolveLockedDigest(resolvedLock.Components[referenceSetId], referenceSetId), materializedCatalog.ReferenceSets.Single(item => item.Id == referenceSetId).Digest);
        Assert.Equal(".NET 10", materializedCatalog.ReferenceSets.Single(static item => item.Id == "net10-ref").DisplayName);
        Assert.Equal(".NET Main", materializedCatalog.ReferenceSets.Single(static item => item.Id == "net11-preview-ref").DisplayName);
        Assert.Equal("Const Generics", materializedCatalog.ReferenceSets.Single(static item => item.Id == "const-generics-ref").DisplayName);
        Assert.Equal(".NET Framework 4.8", materializedCatalog.ReferenceSets.Single(static item => item.Id == "netfx48-ref").DisplayName);
        Assert.Equal(".NET 10", materializedCatalog.Runtimes.Single(static item => item.Id == "dotnet-10-linux-x64").DisplayName);
        Assert.Equal(".NET Main", materializedCatalog.Runtimes.Single(static item => item.Id == "dotnet-11-preview-linux-x64").DisplayName);
        var versions = XDocument.Load(Path.Combine(workspaceRoot, "profiles", "versions.props"));
        var properties = versions.Root!.Element("PropertyGroup")!.Elements().ToDictionary(element => element.Name.LocalName, element => element.Value, StringComparer.Ordinal);
        Assert.Equal(resolvedLock.Components["const-generics-ilspy-source"].Commit, properties["ConstGenericsIlSpyCommit"]);
        Assert.Equal(resolvedLock.Components["const-generics-runtime-source"].Commit, properties["ConstGenericsRuntimeCommit"]);
        Assert.Equal(resolvedLock.Components["const-generics-ref"].ResolvedVersion, properties["ConstGenericsReferenceVersion"]);
        ...
        Assert.Equal($"$(ConstGenericsRuntimeCommit)+{artifactProcessorVersion[processorVersionPrefix.Length..]}", properties["ConstGenericsVerificationProcessorVersion"]);
        await using var runtimeProfileStream = File.OpenRead(Path.Combine(workspaceRoot, "profiles", "runtimes", "const-generics-linux-x64.json"));
        ...
        {
            using var activeProfile = JsonDocument.Parse(await File.ReadAllBytesAsync(Path.Combine(workspaceRoot, "profiles", "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken));
            ...
            Assert.Equal(root.GetProperty("image").GetString(), root.GetProperty("runtimeImageId").GetString());
        ...
        var validationCompose = await File.ReadAllTextAsync(Path.Combine(Path.GetDirectoryName(result.CandidatePath)!, "workspace", "artifacts", "profile-candidate", "compose.validation.yaml"), TestContext.Current.CancellationToken);
        Assert.Equal(1, validationCompose.Split("  runtime-supervisor:", StringSplitOptions.None).Length - 1);
        Assert.Contains($"RuntimeSupervisor__ResourceScope: \"{result.CandidateDigest}\"", validationCompose, StringComparison.Ordinal);
        ...
        var validationEndpoints = JsonSerializer.Deserialize<CandidateValidationEndpoints>(await File.ReadAllTextAsync(Path.Combine(Path.GetDirectoryName(result.CandidatePath)!, "workspace", "artifacts", "profile-candidate", "endpoints.json"), TestContext.Current.CancellationToken), WebJsonOptions);
    ...
    public async Task ResolvePublishesSanitizedOperationalStatus()
    {
        ...
        var candidate = await workflow.ResolveAsync("candidate-public", cancellationToken: TestContext.Current.CancellationToken);

        var json = await File.ReadAllTextAsync(repository.PublicStatusPath, TestContext.Current.CancellationToken);
        var status = JsonSerializer.Deserialize<ProfileUpdateStatusDocument>(json, WebJsonOptions);
    ...
    public async Task BuildUsesRuntimeArgumentsFromCandidateLock()
    {
        ...
        var result = await workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken);

        Assert.Equal(9, runner.Commands.Count);
        Assert.Contains("eng/tools/verify-ilsense-inputs.cs", runner.Commands[0].Arguments);
        Assert.Contains("--verify-restore", runner.Commands[0].Arguments);
        Assert.Equal(["run", "eng/tools/verify-buildkit.cs"], runner.Commands[1].Arguments);
        ...
        var resolvedLock = await CatalogLoader.LoadReleaseLockAsync(candidate.CandidatePath, TestContext.Current.CancellationToken);
        ...
        Assert.Equal(resolvedLock.Components["jit-profiler-clr-samples"].Commit, bake.Environment["JIT_PROFILER_CLR_SAMPLES_COMMIT"]);
        Assert.Equal(resolvedLock.Components["jit-profiler-clr-samples"].SourceUri, bake.Environment["JIT_PROFILER_CLR_SAMPLES_SOURCE_URI"]);
        Assert.Equal(resolvedLock.Components["jit-profiler-runtime-headers"].Commit, bake.Environment["JIT_PROFILER_RUNTIME_HEADERS_COMMIT"]);
        Assert.Equal(resolvedLock.Components["jit-profiler-runtime-headers"].SourceUri, bake.Environment["JIT_PROFILER_RUNTIME_HEADERS_SOURCE_URI"]);
        ...
        Assert.Equal("aaf35bb8d5e1e8704e982ad0ab95263451bd2d3d", bake.Environment["GSHARP_COMMIT"]);
        Assert.Equal("f52d21ef09b198bad69b7ac8dd5f6d2eaa91216b80bfc22e9610a1fef28f06d4", bake.Environment["GSHARP_ARCHIVE_SHA256"]);
        Assert.Equal("0.1.0", bake.Environment["ILSENSE_VERSION"]);
        Assert.Equal("a2253dd77d052e02f654908e5ecb60b6602be782", bake.Environment["ILSENSE_COMMIT"]);
        Assert.Equal("5c42956f5a423f44ed1e22526ed11d7a30af127be5f90d0700c06bb13db9ca00", bake.Environment["ILSENSE_ARCHIVE_SHA256"]);
        ...
        Assert.Equal("608bf30cf3f43f97e32825076a2cfdaa25043e50", bake.Environment["PEACHPIE_COMMIT"]);
        Assert.EndsWith("/608bf30cf3f43f97e32825076a2cfdaa25043e50/LICENSE.txt", bake.Environment["PEACHPIE_LICENSE_URL"], StringComparison.Ordinal);
        Assert.Equal("sha512-Q1XzhqGM3cR1FW5hWh7JIfjCCNtmNM1u0HW1nM0UCyl4X5MM7cM9dxeBjmWETIumKpP/8yj19WXF0wRmfQgaew==", bake.Environment["PEACHPIE_CODEANALYSIS_PACKAGE_CONTENT_HASH"]);
        ...
        })
        {
            ...
            Assert.Equal(component.PackageContentHash, bake.Environment[$"{prefix}_REFERENCE_PACKAGE_CONTENT_HASH"]);
        }
        var netfx48ManagedReference = resolvedLock.Components["netfx48-managed-ref"];
        Assert.Equal(netfx48ManagedReference.ResolvedVersion, bake.Environment["NETFX48_MANAGED_REFERENCE_VERSION"]);
        Assert.Equal(netfx48ManagedReference.SourceUri, bake.Environment["NETFX48_MANAGED_REFERENCE_URL"]);
        Assert.Equal(netfx48ManagedReference.SourceUri, bake.Environment["NETFX48_MANAGED_REFERENCE_SOURCE_URI"]);
        Assert.Equal(netfx48ManagedReference.Sha512, bake.Environment["NETFX48_MANAGED_REFERENCE_SHA512"]);
        Assert.Equal(netfx48ManagedReference.PackageContentHash, bake.Environment["NETFX48_MANAGED_REFERENCE_PACKAGE_CONTENT_HASH"]);
        ...
        })
        {
            ...
            Assert.Equal(ReferenceSetIdentityResolver.ResolveLockedDigest(component, referenceSetId), bake.Environment[variable]);
        ...
        Assert.Equal(constGenericsReference.ResolvedVersion, bake.Environment["CONST_GENERICS_REFERENCE_VERSION"]);
        ...
        Assert.Equal(constGenericsRuntimeSource.Commit, bake.Environment["CONST_GENERICS_RUNTIME_COMMIT"]);
        Assert.Equal(constGenericsRuntimeSource.Digest![7..], bake.Environment["CONST_GENERICS_RUNTIME_ARCHIVE_SHA256"]);
        ...
        Assert.Equal(versionTools.Digest![7..], bake.Environment["CONST_GENERICS_VERSIONTOOLS_PACKAGE_SHA256"]);
        var constGenericsRoslyn = resolvedLock.Components["roslyn-const-generics"];
        Assert.Equal(constGenericsRoslyn.Commit, bake.Environment["CONST_GENERICS_ROSLYN_COMMIT"]);
        Assert.Equal(constGenericsRoslyn.ResolvedVersion.Split("-const-generics.", StringSplitOptions.None)[0], bake.Environment["CONST_GENERICS_ROSLYN_VERSION"]);
        Assert.Equal(constGenericsRoslyn.ResolvedVersion, bake.Environment["CONST_GENERICS_ROSLYN_COMPONENT_VERSION"]);
        Assert.Equal(resolvedLock.Components["artifacts-const-generics"].Commit, bake.Environment["CONST_GENERICS_ILSPY_COMMIT"]);
        Assert.Equal(resolvedLock.Components["artifacts-default"].ResolvedVersion, bake.Environment["ARTIFACTS_DEFAULT_VERSION"]);
        Assert.Equal(resolvedLock.Components["artifacts-const-generics"].ResolvedVersion, bake.Environment["ARTIFACTS_CONST_GENERICS_VERSION"]);
        Assert.Equal(resolvedLock.Components["il-assembler"].ResolvedVersion, bake.Environment["IL_ASSEMBLER_VERSION"]);
        Assert.Equal(resolvedLock.Components["minilang-stable"].ResolvedVersion, bake.Environment["MINILANG_VERSION"]);
    ...
    public async Task BakeEnvironmentCoversEveryDeclaredBakeVariable()
    {
        ...
        var bake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.hcl"), TestContext.Current.CancellationToken);
        var candidateBake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.runtime-candidates.hcl"), TestContext.Current.CancellationToken);
        var declaredVariables = Regex.Matches(bake, "(?m)^variable \\\"(?<name>[A-Z0-9_]+)\\\" \\{$", RegexOptions.CultureInvariant).Select(static match => match.Groups["name"].Value).ToHashSet(StringComparer.Ordinal);

        Assert.NotEmpty(declaredVariables);
        var candidateOnlyVariables = Regex.Matches(candidateBake, "(?m)^variable \\\"(?<name>RUNTIME_MATRIX_[A-Z0-9_]+)\\\" \\{$", RegexOptions.CultureInvariant).Select(static match => match.Groups["name"].Value).ToHashSet(StringComparer.Ordinal);
        Assert.NotEmpty(candidateOnlyVariables);
        Assert.DoesNotContain(declaredVariables, static name => name.StartsWith("RUNTIME_MATRIX_", StringComparison.Ordinal));
        ...
        Assert.Empty(declaredVariables.Except(environment.Keys.Concat(generatedImageInputs), StringComparer.Ordinal));
    ...
    public async Task BakeEnvironmentDerivesTheSourceControlledWineUserspaceIdentity()
    {
        ...
        Assert.Equal("wine-9.0~repack-4build3+xvfb-2:21.1.12-1ubuntu1.6", environment["WINE_CORECLR_USERSPACE_VERSION"]);
        Assert.Equal("sha256:4ecfff207a9b13eb6492aa3f9ca01d2d3dd1b713837ef2917524eaf23fa55981", environment["WINE_CORECLR_USERSPACE_DIGEST"]);
        Assert.Equal("https://snapshot.ubuntu.com/ubuntu/20260810T000000Z/", environment["WINE_CORECLR_USERSPACE_SOURCE_URI"]);

        var bake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.hcl"), TestContext.Current.CancellationToken);
        var wineOperator = ExtractNamedBlock(bake, "target", "operator-wine-coreclr");
        Assert.Contains("\"org.opencontainers.image.version\" = \"wine-9.0-noble-amd64\"", wineOperator, StringComparison.Ordinal);
    }
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var lockDocument = JsonSerializer.Deserialize<ReleaseLockDocument>(await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "profiles", "lock.json"), TestContext.Current.CancellationToken), WebJsonOptions) ?? throw new InvalidOperationException("Repository release lock is invalid.");
        var components = lockDocument.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value);
        components["wine-coreclr-userspace"] = components["wine-coreclr-userspace"] with { Kind = "operator-image" };
        var temporaryLock = Path.GetTempFileName();
        try
        {
            await File.WriteAllTextAsync(temporaryLock, JsonSerializer.Serialize(lockDocument with { Components = components }, WebJsonOptions), TestContext.Current.CancellationToken);

            var exception = await Assert.ThrowsAsync<BakeEnvironmentValidationException>(() => BakeEnvironmentResolver.CreateAsync(temporaryLock, Path.Combine(repositoryRoot, "profiles", "base-images.json"), "test-source-revision", "1700000000", cancellationToken: TestContext.Current.CancellationToken));
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var dockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.worker-roslyn-main"), TestContext.Current.CancellationToken);
    ...
    public async Task RoslynSourceVerifierRestoresAndRunsWithoutNetwork(string dockerfileName, string verifierFileName)
    {
        var repositoryRoot = FindRepositoryRoot();
        var dockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", dockerfileName), TestContext.Current.CancellationToken);
        var verifier = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "tools", verifierFileName), TestContext.Current.CancellationToken);
        var nugetConfig = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "NuGet.verifier.config"), TestContext.Current.CancellationToken);
        var verifierPath = $"/tools/{verifierFileName}";
        var copyMarker = $"COPY eng/tools/{verifierFileName} {verifierPath}";
        ...
        Assert.Contains("COPY eng/NuGet.verifier.config /tools/NuGet.verifier.config", verifierStage, StringComparison.Ordinal);
        ...
        Assert.True(verifierWorkdir >= 0 && verifierWorkdir < offlineRun, $"{dockerfileName} must select the pinned SDK outside the upstream Roslyn global.json scope.");
    ...
    public async Task ConstGenericsBuildsReplaceTheForkFeedWithPreparedLocalPackages()
    {
        var repositoryRoot = FindRepositoryRoot();
        var bake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.hcl"), TestContext.Current.CancellationToken);
        const string context = "\"const-generics-fork-packages\" = \"./artifacts/prerequisites/downloads/const-generics-fork-packages\"";

        foreach (var (target, dockerfileName, patchPath) in new[]
                 {
                     ("runtime-const-generics", "Dockerfile.runtime-const-generics", "eng/patches/const-generics-runtime/0001-use-local-fork-packages.patch"),
                     ("worker-roslyn-const-generics", "Dockerfile.worker-roslyn-const-generics", "eng/patches/roslyn-const-generics/0002-use-local-fork-packages.patch"),
                     ("worker-artifacts-const-generics", "Dockerfile.worker-artifacts-const-generics", "eng/patches/const-generics-runtime/0001-use-local-fork-packages.patch")
                 })
        {
            ...
            var dockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", dockerfileName), TestContext.Current.CancellationToken);
            Assert.Contains("COPY --from=const-generics-fork-packages / /const-generics-fork-packages/", dockerfile, StringComparison.Ordinal);
            ...
            Assert.Contains("value=\"/const-generics-fork-packages\"", await File.ReadAllTextAsync(Path.Combine(repositoryRoot, patchPath.Replace('/', Path.DirectorySeparatorChar)), TestContext.Current.CancellationToken), StringComparison.Ordinal);
            Assert.Contains("! grep --fixed-strings --quiet 'pkgs.dev.azure.com/hez2010' NuGet.config", dockerfile, StringComparison.Ordinal);
            Assert.Equal(1, Regex.Count(dockerfile, Regex.Escape("pkgs.dev.azure.com/hez2010")));
        }
    }
    ...
    public async Task RoslynCoreClrReferenceSetsAreBoundToEveryBuildArgumentAndUseNetStandardRuntimeApi()
    {
        ...
        var workerDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.worker"), TestContext.Current.CancellationToken);
        var mainDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.worker-roslyn-main"), TestContext.Current.CancellationToken);
                 ...
                 })
        {
            foreach (var target in new[] { stableTarget, mainTarget })
            {
                ...
                Assert.Contains($"{prefix}_REFERENCE_SHA512 = required({prefix}_REFERENCE_SHA512)", target, StringComparison.Ordinal);
                Assert.Contains($"{prefix}_REFERENCE_PACKAGE_CONTENT_HASH = required({prefix}_REFERENCE_PACKAGE_CONTENT_HASH)", target, StringComparison.Ordinal);
            }
    ...
    public async Task CppCliProductTargetsConsumeSourceBuiltBaseWithDirectInputProvenance()
    {
        ...
        Assert.Contains("\"cppcli-prepared-base-context\" = \"docker-image://${deferred_image(CPPCLI_PREPARED_BASE_IMAGE)}\"", runtimeTarget, StringComparison.Ordinal);
        Assert.Contains("\"cppcli-prepared-base\" = \"docker-image://${deferred_image(CPPCLI_PREPARED_BASE_IMAGE)}\"", workerTarget, StringComparison.Ordinal);
        ...
        var runtimeDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.runtime-wine-netfx48"), TestContext.Current.CancellationToken);
        var workerDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.worker-cppcli"), TestContext.Current.CancellationToken);
        ...
        var deployment = JsonNode.Parse(await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "images.json"), TestContext.Current.CancellationToken))!;
        var deploymentImages = deployment["images"]!.AsArray();
        foreach (var imageId in new[] { "worker-cppcli", "wine-netfx48-linux-x64" })
        {
            var image = deploymentImages.Single(node => string.Equals(node!["id"]!.GetValue<string>(), imageId, StringComparison.Ordinal))!;
            if (imageId == "worker-cppcli")
            {
                var componentIds = image["lockComponentIds"]!.AsArray().Select(static node => node!.GetValue<string>());
            ...
            {
                Assert.Equal("wine-netfx48-linux-x64", image["lockComponentId"]!.GetValue<string>());
            }
        }
    }
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var bake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.runtime-candidates.hcl"), TestContext.Current.CancellationToken);
        ...
        Assert.Contains("io.sharplabnext.component.${RUNTIME_MATRIX_PROFILE_ID}.version", bake, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.component.${RUNTIME_MATRIX_PROFILE_ID}.source-uri", bake, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.runtime.commit", bake, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.jit.commit", bake, StringComparison.Ordinal);

        var monoTarget = ExtractNamedBlock(bake, "target", "runtime-mono-matrix-candidate");
        Assert.Contains("RUNTIME_COMPONENT_DIGEST = RUNTIME_MATRIX_RUNTIME_DIGEST", monoTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.component.${RUNTIME_MATRIX_PROFILE_ID}.digest", monoTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.control-image", monoTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.operator-image.mono", monoTarget, StringComparison.Ordinal);
        Assert.DoesNotContain("io.sharplabnext.base-image.mono", monoTarget, StringComparison.Ordinal);

        var wineTarget = ExtractNamedBlock(bake, "target", "runtime-wine-dotnet-matrix-candidate");
        Assert.Contains("io.sharplabnext.component.${RUNTIME_MATRIX_PROFILE_ID}.commit", wineTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.control-image", wineTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.operator-image.wine", wineTarget, StringComparison.Ordinal);
        Assert.DoesNotContain("io.sharplabnext.base-image.wine", wineTarget, StringComparison.Ordinal);

        var combinedTarget = ExtractNamedBlock(bake, "target", "runtime-mono-wine-matrix-candidate");
        Assert.Contains("dockerfile = \"deploy/docker/Dockerfile.runtime-mono-wine-matrix\"", combinedTarget, StringComparison.Ordinal);
        Assert.Contains("MONO_WINE_IMAGE = RUNTIME_MATRIX_MONO_WINE_IMAGE", combinedTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.runtime.matrix.profile-group", combinedTarget, StringComparison.Ordinal);
        Assert.Contains("io.sharplabnext.operator-image.mono-wine", combinedTarget, StringComparison.Ordinal);
        Assert.DoesNotContain("runtime-${required(RUNTIME_MATRIX_PROFILE_ID)}:candidate", combinedTarget, StringComparison.Ordinal);

        var combinedDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.runtime-mono-wine-matrix"), TestContext.Current.CancellationToken);
        ...
        Assert.DoesNotContain("COPY --from=runtime-source /usr/ /usr/", combinedDockerfile, StringComparison.Ordinal);
        Assert.DoesNotContain("cmp --silent", combinedDockerfile, StringComparison.Ordinal);

        var wineDockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.runtime-wine-dotnet-matrix"), TestContext.Current.CancellationToken);
        Assert.Contains("--output /legacy-jit-helper", wineDockerfile, StringComparison.Ordinal);
        Assert.Contains("COPY --from=publish /legacy-jit-helper/ /opt/sharplabnext/", wineDockerfile, StringComparison.Ordinal);
        Assert.Contains("test -s /opt/sharplabnext/SharpLabNext.LegacyJitInspector.dll", wineDockerfile, StringComparison.Ordinal);
    }
    ...
    public async Task ImageBuildOwnsTheSharedBakeEnvironmentAndBundleIsPackagingOnly()
    {
        ...
        {
            var content = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", script), TestContext.Current.CancellationToken);
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var content = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, ".github", "workflows", "ci.yml"), TestContext.Current.CancellationToken);
        var yaml = new YamlStream();
        yaml.Load(new StringReader(content));
        Assert.Single(yaml.Documents);

        var buildStepStart = content.IndexOf("- name: Build every planned image and create deployment metadata", StringComparison.Ordinal);
        var buildStepEnd = content.IndexOf("- name: Validate immutable local deployment metadata", StringComparison.Ordinal);
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var content = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.hcl"), TestContext.Current.CancellationToken);

        Assert.Contains("output = [\"type=docker,rewrite-timestamp=true,unpack=false\"]", content, StringComparison.Ordinal);
        Assert.Contains("SOURCE_DATE_EPOCH = unix_seconds(required(SOURCE_DATE_EPOCH))", content, StringComparison.Ordinal);
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var content = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "Directory.Build.props"), TestContext.Current.CancellationToken);

        Assert.Contains("Condition=\"'$(DOTNET_RUNNING_IN_CONTAINER)' == 'true'\"", content, StringComparison.Ordinal);
    ...
    public async Task ServiceImagesOnlyCarryReferenceSetsWhenTheirRuntimeContractUsesThem()
    {
        var repositoryRoot = FindRepositoryRoot();
        var bake = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "eng", "bake.hcl"), TestContext.Current.CancellationToken);
        var dockerfile = await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "deploy", "docker", "Dockerfile.worker"), TestContext.Current.CancellationToken);
        ...
        Assert.Contains("target = \"final-with-framework-and-jsharp-reference-sets\"", bake, StringComparison.Ordinal);
        ...
        {
            Assert.Matches(Regex.Escape($"target \"{target}\" {{") + @"\s+inherits = \[""service-with-reference-sets""\]", bake);
        }
        Assert.Matches(Regex.Escape("target \"worker-roslyn-stable\" {") + @"\s+inherits = \[""service-with-roslyn-coreclr-reference-sets""\]", bake);
        Assert.Matches(Regex.Escape("target \"worker-artifacts-default\" {") + @"\s+inherits = \[""service-with-framework-reference-sets""\]", bake);
        Assert.Contains("\"jsharp-reference-source\" = \"target:worker-jsharp\"", bake, StringComparison.Ordinal);
        foreach (var target in new[] { "artifact-store", "runtime-supervisor", "worker-artifacts-il-assembler" }) Assert.Matches(Regex.Escape($"target \"{target}\" {{") + @"\s+inherits = \[""service""\]", bake);
        ...
        Assert.Contains("COPY eng/tools/materialize-framework-reference-sets.cs /tools/materialize-framework-reference-sets.cs", dockerfile, StringComparison.Ordinal);
        Assert.Contains("dotnet run materialize-framework-reference-sets.cs", dockerfile, StringComparison.Ordinal);
        ...
        Assert.Contains("FROM final-with-reference-sets AS final-with-framework-reference-sets", dockerfile, StringComparison.Ordinal);
        Assert.Contains("FROM final-with-framework-reference-sets AS final-with-framework-and-jsharp-reference-sets", dockerfile, StringComparison.Ordinal);
        Assert.Contains("COPY --from=reference-sets --chown=1654:1654 /reference-sets/ /reference-sets/", dockerfile, StringComparison.Ordinal);
        Assert.Contains("COPY --from=framework-reference-sets --chown=1654:1654", dockerfile, StringComparison.Ordinal);
        Assert.Contains("/reference-sets/ /reference-sets/", dockerfile, StringComparison.Ordinal);
        Assert.Contains("COPY --from=jsharp-reference-source --chown=1654:1654", dockerfile, StringComparison.Ordinal);
        Assert.Contains("/reference-sets/jsharp20-ref/ /reference-sets/jsharp20-ref/", dockerfile, StringComparison.Ordinal);
        Assert.Contains("test \"${reference_content_digest}\" = \"${JSHARP_REFERENCE_DIGEST}\"", dockerfile, StringComparison.Ordinal);
    }
    ...
    {
        var exception = Assert.Throws<BakeEnvironmentValidationException>(() => SourceDateEpochResolver.Validate(value));
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var manifest = JsonNode.Parse(await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "profiles", "base-images.json"), TestContext.Current.CancellationToken))!;
        manifest["images"]![0]!["reference"] = "node:24.18.0-bookworm-slim";
        var temporaryManifest = Path.GetTempFileName();
        try
        {
            await File.WriteAllTextAsync(temporaryManifest, manifest.ToJsonString(), TestContext.Current.CancellationToken);
            var exception = await Assert.ThrowsAsync<BakeEnvironmentValidationException>(() => BakeEnvironmentResolver.CreateAsync(Path.Combine(repositoryRoot, "profiles", "lock.json"), temporaryManifest, "test-source-revision", "1700000000", cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task BakeEnvironmentRejectsSourceInputsWithoutDigests(string componentId, string expectedError)
    {
        var repositoryRoot = FindRepositoryRoot();
        var lockDocument = JsonSerializer.Deserialize<ReleaseLockDocument>(await File.ReadAllTextAsync(Path.Combine(repositoryRoot, "profiles", "lock.json"), TestContext.Current.CancellationToken), WebJsonOptions) ?? throw new InvalidOperationException("Repository release lock is invalid.");
        var components = lockDocument.Components.ToDictionary(static pair => pair.Key, static pair => pair.Value);
        components[componentId] = components[componentId] with { Digest = null };
        var temporaryLock = Path.GetTempFileName();
        try
        {
            await File.WriteAllTextAsync(temporaryLock, JsonSerializer.Serialize(lockDocument with { Components = components }, WebJsonOptions), TestContext.Current.CancellationToken);
    ...
    public async Task BuildFailureIsRecordedWithoutChangingApprovedLock()
    {
        ...
        var workflow = repository.CreateWorkflow(runner);
        var candidate = await workflow.ResolveAsync(cancellationToken: TestContext.Current.CancellationToken);
        var approvedBefore = await File.ReadAllBytesAsync(repository.LockPath, TestContext.Current.CancellationToken);

        await Assert.ThrowsAsync<ProfileUpdateCommandFailedException>(() => workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken));

        Assert.Equal(approvedBefore, await File.ReadAllBytesAsync(repository.LockPath, TestContext.Current.CancellationToken));
        using var receipt = JsonDocument.Parse(await File.ReadAllTextAsync(Path.Combine(Path.GetDirectoryName(candidate.CandidatePath)!, "receipt.json"), TestContext.Current.CancellationToken));
        ...
        var publicJson = await File.ReadAllTextAsync(repository.PublicStatusPath, TestContext.Current.CancellationToken);
    ...
    public async Task TestRejectsCandidateWhenApprovedSourceDigestChanged()
    {
        ...
        var workflow = repository.CreateWorkflow(runner);
        var candidate = await workflow.ResolveAsync(cancellationToken: TestContext.Current.CancellationToken);
        await workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken);
        var commandCountBeforeTest = runner.Commands.Count;
        await File.AppendAllTextAsync(repository.LockPath, Environment.NewLine, TestContext.Current.CancellationToken);

        var exception = await Assert.ThrowsAsync<ProfileUpdateValidationException>(() => workflow.TestAsync(null, candidate.CandidateDigest, "Release", ProfileUpdateTestScope.Full, TestContext.Current.CancellationToken));
    ...
    public async Task FullTestRunsAllQualityGatesAgainstCandidateLock()
    {
        ...
        var result = await workflow.TestAsync(null, candidate.CandidateDigest, "Release", ProfileUpdateTestScope.Full, TestContext.Current.CancellationToken);
        ...
        var schema = Assert.Single(runner.Commands, static command => command.Arguments.Contains("eng/validation/validate-schemas.mjs"));
        Assert.Contains(Path.Combine(schema.WorkingDirectory, "profiles", "lock.json"), schema.Arguments);
        Assert.Contains(runner.Commands, static command => command.Arguments.Contains("eng/validation/validate-compose.mjs"));
        var compatibility = Assert.Single(runner.Commands, static command => command.Arguments.Contains("src/Tools/SharpLabNext.CompatibilityCli"));
        ...
        var verifier = Assert.Single(runner.Commands, static command => command.Arguments.Contains("eng/tools/verify-profile-candidate.cs"));
        Assert.Contains("--bundle", verifier.Arguments);
        Assert.Contains(runner.Commands, static command => command.Arguments.Contains("eng/smoke/gateway-compose.cs") && command.Arguments.Contains("--full"));
        var performance = Assert.Single(runner.Commands, static command => command.Arguments.Contains("eng/performance/gateway-performance.cs"));
        ...
        Assert.Contains(performance.Arguments, static argument => argument.EndsWith("performance-report-1.json", StringComparison.Ordinal));
        Assert.Contains(runner.Commands, static command => command.Arguments.Contains("eng/smoke/gateway-compose.cs") && command.Arguments.Contains("--security"));
    ...
    public async Task FullTestRunsComposeCleanupWhenIdentityVerificationFails()
    {
        ...
        await workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken);
        runner.Commands.Clear();
        runner.FailurePredicate = static command => command.Arguments.Contains("eng/tools/verify-profile-candidate.cs");

        await Assert.ThrowsAsync<ProfileUpdateCommandFailedException>(() => workflow.TestAsync(null, candidate.CandidateDigest, "Release", ProfileUpdateTestScope.Full, TestContext.Current.CancellationToken));
    ...
    public async Task FullTestRunsComposeCleanupWhenPerformanceGateFails()
    {
        ...
        Assert.Contains(runner.Commands, static command => command.Arguments.Contains("eng/performance/gateway-performance.cs"));
        Assert.DoesNotContain(runner.Commands, static command => command.Arguments.Contains("eng/smoke/gateway-compose.cs") && command.Arguments.Contains("--security"));
        var cleanup = Assert.Single(runner.Commands, static command => command.AlwaysRun);
        Assert.Contains("down", cleanup.Arguments);
        Assert.Equal(cleanup, runner.Commands[^1]);
    }
    ...
    {
        var workflowPath = Path.Combine(FindRepositoryRoot(), ".github", "workflows", "profile-update.yml");
        ...
        Assert.Contains("candidate_project=\"sln-profile-${digest_hex:0:12}-1\"", cleanupStep, StringComparison.Ordinal);
        Assert.Contains("com.docker.compose.project", cleanupStep, StringComparison.Ordinal);
        Assert.Contains("label=com.docker.compose.project=$candidate_project", cleanupStep, StringComparison.Ordinal);
        ...
        Assert.True(cleanupStep.Split("label=com.sharplabnext.release-id=$candidate_release_id", StringSplitOptions.None).Length >= 5, "Every runtime cleanup and verification query must include the candidate release label.");
        Assert.True(cleanupStep.Split("label=com.sharplabnext.resource-scope=$candidate_digest", StringSplitOptions.None).Length >= 5, "Every runtime cleanup and verification query must include the candidate resource scope.");
        Assert.DoesNotContain("grep '^sln-profile-'", cleanupStep, StringComparison.Ordinal);
        Assert.DoesNotContain("--filter label=com.sharplabnext.runtime-job=true\n          )", cleanupStep, StringComparison.Ordinal);
    ...
    public async Task TestRejectsCandidateWhenCatalogIdentityWasTampered()
    {
        ...
        var catalogPath = Path.Combine(repository.Root, candidate.Receipt.WorkspacePath, "profiles", "catalog", "catalog.json");
        var catalogText = await File.ReadAllTextAsync(catalogPath, TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(catalogPath, catalogText.Replace("Roslyn Stable 5.6.0", "Roslyn Stable 5.6.1", StringComparison.Ordinal).Replace("\"resolvedVersion\": \"5.6.0\"", "\"resolvedVersion\": \"5.6.1\"", StringComparison.Ordinal), TestContext.Current.CancellationToken);
    ...
    public async Task TestRejectsCandidateWhenBuiltMaterialWasTampered(string material)
    {
        ...
        var path = material switch
        {
            ...
            "runtime-profile" => Path.Combine(workspace, "profiles", "runtimes", "dotnet-10-linux-x64.json"),
            "reference-set-configuration" => Path.Combine(workspace, Path.GetRelativePath(repository.Root, repository.ReferenceSetConfigurationPath)),
            _ => Path.Combine(workspace, "packages.lock.json")
        };
        await File.AppendAllTextAsync(path, material is "runtime-profile" or "reference-set-configuration" ? " \n" : "\n<!-- tampered -->\n", TestContext.Current.CancellationToken);
    ...
    public async Task BuildRejectsCandidateWithAChangedActiveRuntimeProfileSet()
    {
        ...
        var workflow = repository.CreateWorkflow(runner);
        var candidate = await workflow.ResolveAsync(cancellationToken: TestContext.Current.CancellationToken);
        File.Delete(Path.Combine(repository.Root, candidate.Receipt.WorkspacePath, "profiles", "runtimes", "dotnet-10-linux-x64.json"));

        var exception = await Assert.ThrowsAsync<ProfileUpdateValidationException>(() => workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken));
    ...
    {
        using var repository = new TempRepository();
        var referenceSetConfigurationRelativePath = Path.GetRelativePath(repository.Root, repository.ReferenceSetConfigurationPath);
        var previousReferenceSetConfiguration = await File.ReadAllBytesAsync(repository.ReferenceSetConfigurationPath, TestContext.Current.CancellationToken);
        var previousRuntimeProfiles = new Dictionary<string, byte[]>(StringComparer.Ordinal);
        foreach (var runtimeProfileId in ActiveRuntimeProfileIds) previousRuntimeProfiles[runtimeProfileId] = await File.ReadAllBytesAsync(Path.Combine(repository.Root, "profiles", "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken);
        var runner = new RecordingCommandRunner();
        var workflow = repository.CreateWorkflow(runner);
        var candidate = await workflow.ResolveAsync("candidate-1", cancellationToken: TestContext.Current.CancellationToken);
        ...
        var candidateReferenceSetConfigurationPath = Path.Combine(candidateWorkspace, referenceSetConfigurationRelativePath);
        var candidateRuntimeProfilePaths = ActiveRuntimeProfileIds.ToDictionary(static id => id, id => Path.Combine(candidateWorkspace, "profiles", "runtimes", $"{id}.json"), StringComparer.Ordinal);
        var candidatePackageLockPath = Path.Combine(candidateWorkspace, "packages.lock.json");
        var candidateNamedPackageLockPath = Path.Combine(candidateWorkspace, "src", "Workers", "IL", "SharpLabNext.Worker.IL", "packages.EleCho.ILSense.lock.json");
        var candidateCatalog = await File.ReadAllTextAsync(candidateCatalogPath, TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(candidateCatalogPath, candidateCatalog.Replace("20260712.4-dev", "candidate-promotion", StringComparison.Ordinal), TestContext.Current.CancellationToken);
        ...
        await File.WriteAllTextAsync(candidateNamedPackageLockPath, "{\"namedCandidate\":true}\n", TestContext.Current.CancellationToken);
        await workflow.BuildAsync(null, candidate.CandidateDigest, "Release", TestContext.Current.CancellationToken);
        await workflow.TestAsync(null, candidate.CandidateDigest, "Release", ProfileUpdateTestScope.Affected, TestContext.Current.CancellationToken);

        await Assert.ThrowsAsync<ProfileUpdateValidationException>(() => workflow.PromoteAsync(null, candidate.CandidateDigest, TestContext.Current.CancellationToken));
        Assert.Equal(repository.ActiveDigest, Digest(await File.ReadAllBytesAsync(repository.LockPath, TestContext.Current.CancellationToken)));

        await workflow.TestAsync(null, candidate.CandidateDigest, "Release", ProfileUpdateTestScope.Full, TestContext.Current.CancellationToken);
        var promoted = await workflow.PromoteAsync(null, candidate.CandidateDigest, TestContext.Current.CancellationToken);

        Assert.Equal(candidate.CandidateDigest, Digest(await File.ReadAllBytesAsync(repository.LockPath, TestContext.Current.CancellationToken)));
        Assert.Equal(ProfileUpdateStage.Promote, promoted.Stage.Stage);
        var previousHistory = Path.Combine(repository.StateRoot, "history", repository.ActiveDigest[7..], "lock.json");
        Assert.True(File.Exists(previousHistory));
        Assert.Equal(repository.ActiveDigest, Digest(await File.ReadAllBytesAsync(previousHistory, TestContext.Current.CancellationToken)));
        Assert.True(File.Exists(Path.Combine(repository.StateRoot, "last-known-good", "previous.lock.json")));
        Assert.Equal(candidate.CandidateDigest, Digest(await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "last-known-good", "lock.json"), TestContext.Current.CancellationToken)));
        Assert.Equal(await File.ReadAllBytesAsync(candidateCatalogPath, TestContext.Current.CancellationToken), await File.ReadAllBytesAsync(Path.Combine(repository.Root, "profiles", "catalog", "catalog.json"), TestContext.Current.CancellationToken));
        Assert.Equal(await File.ReadAllBytesAsync(candidateVersionsPath, TestContext.Current.CancellationToken), await File.ReadAllBytesAsync(Path.Combine(repository.Root, "profiles", "versions.props"), TestContext.Current.CancellationToken));
        var candidateReferenceSetConfiguration = await File.ReadAllBytesAsync(candidateReferenceSetConfigurationPath, TestContext.Current.CancellationToken);
        Assert.Equal(candidateReferenceSetConfiguration, await File.ReadAllBytesAsync(repository.ReferenceSetConfigurationPath, TestContext.Current.CancellationToken));
        Assert.Equal(previousReferenceSetConfiguration, await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "history", repository.ActiveDigest[7..], "reference-set-configurations", referenceSetConfigurationRelativePath), TestContext.Current.CancellationToken));
        Assert.Equal(candidateReferenceSetConfiguration, await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "history", candidate.CandidateDigest[7..], "reference-set-configurations", referenceSetConfigurationRelativePath), TestContext.Current.CancellationToken));
        Assert.Equal(candidateReferenceSetConfiguration, await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "last-known-good", "material", "reference-set-configurations", referenceSetConfigurationRelativePath), TestContext.Current.CancellationToken));
        foreach (var runtimeProfileId in ActiveRuntimeProfileIds)
        {
            var candidateRuntimeProfile = await File.ReadAllBytesAsync(candidateRuntimeProfilePaths[runtimeProfileId], TestContext.Current.CancellationToken);
            Assert.Equal(candidateRuntimeProfile, await File.ReadAllBytesAsync(Path.Combine(repository.Root, "profiles", "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken));
            Assert.Equal(previousRuntimeProfiles[runtimeProfileId], await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "history", repository.ActiveDigest[7..], "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken));
            Assert.Equal(candidateRuntimeProfile, await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "history", candidate.CandidateDigest[7..], "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken));
            Assert.Equal(candidateRuntimeProfile, await File.ReadAllBytesAsync(Path.Combine(repository.StateRoot, "last-known-good", "material", "runtimes", $"{runtimeProfileId}.json"), TestContext.Current.CancellationToken));
        }
        Assert.Equal(await File.ReadAllBytesAsync(candidatePackageLockPath, TestContext.Current.CancellationToken), await File.ReadAllBytesAsync(Path.Combine(repository.Root, "packages.lock.json"), TestContext.Current.CancellationToken));
        Assert.Equal(await File.ReadAllBytesAsync(candidateNamedPackageLockPath, TestContext.Current.CancellationToken), await File.ReadAllBytesAsync(Path.Combine(repository.Root, "src", "Workers", "IL", "SharpLabNext.Worker.IL", "packages.EleCho.ILSense.lock.json"), TestContext.Current.CancellationToken));
        var publicStatus = JsonSerializer.Deserialize<ProfileUpdateStatusDocument>(await File.ReadAllTextAsync(repository.PublicStatusPath, TestContext.Current.CancellationToken), WebJsonOptions);
    ...
    {
        Assert.Equal(ProfileUpdaterCommandKind.Check, ProfileUpdaterCommand.Parse(["--check", "--fail-on-change"]).Kind);
        Assert.Equal(ProfileUpdaterCommandKind.Test, ProfileUpdaterCommand.Parse(["test", "--test-scope", "affected"]).Kind);
        Assert.Equal(ProfileUpdaterCommandKind.Resolve, ProfileUpdaterCommand.Parse(["--output", "candidate.json"]).Kind);
        Assert.Equal(ProfileUpdaterCommandKind.Pipeline, ProfileUpdaterCommand.Parse(["--apply"]).Kind);
    }
    ...
    private sealed class RecordingCommandRunner : IProfileUpdateCommandRunner
    {
        ...
        public Task<ProfileUpdateCommandResult> RunAsync(ProfileUpdateExternalCommand command, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            Commands.Add(command);
            return Task.FromResult(new ProfileUpdateCommandResult(string.Equals(command.FileName, FailureFileName, StringComparison.Ordinal) || FailurePredicate?.Invoke(command) == true ? 17 : 0));
        }
    }
    ...
    private sealed class TempRepository : IDisposable
    {
        ...
        public TempRepository()
        {
            ...
            File.Copy(Path.Combine(sourceRepository, "profiles", "catalog", "catalog.json"), Path.Combine(Root, "profiles", "catalog", "catalog.json"));
            File.Copy(Path.Combine(sourceRepository, "profiles", "versions.props"), Path.Combine(Root, "profiles", "versions.props"));
            File.Copy(Path.Combine(sourceRepository, "profiles", "base-images.json"), Path.Combine(Root, "profiles", "base-images.json"));
            foreach (var channelPath in Directory.EnumerateFiles(Path.Combine(sourceRepository, "profiles", "channels"), "*.yaml"))
                File.Copy(channelPath, Path.Combine(Root, "profiles", "channels", Path.GetFileName(channelPath)));
            foreach (var runtimeProfileId in ActiveRuntimeProfileIds)
            {
            ...
            File.WriteAllText(
                ...
                """);
            var sourceLock = JsonSerializer.Deserialize<ReleaseLockDocument>(File.ReadAllText(Path.Combine(sourceRepository, "profiles", "lock.json")), WebJsonOptions) ?? throw new InvalidOperationException("Repository release lock is invalid.");
            ...
            foreach (var (componentId, component) in sourceLock.Components.Where(static pair => pair.Key.StartsWith("netfx", StringComparison.Ordinal) && pair.Key.EndsWith("-managed-ref", StringComparison.Ordinal)))
            ...
            File.WriteAllText(LockPath, JsonSerializer.Serialize(document, LockJsonOptions) + Environment.NewLine);
            var catalogPath = Path.Combine(Root, "profiles", "catalog", "catalog.json");
            var template = JsonSerializer.Deserialize<CatalogDocument>(File.ReadAllText(catalogPath), WebJsonOptions) ?? throw new InvalidOperationException("Repository catalog is invalid.");
            File.WriteAllText(catalogPath, JsonSerializer.Serialize(RestrictSelectableRuntimesToLock(template, document), LockJsonOptions) + Environment.NewLine);
        ...
        public ProfileUpdateWorkflow CreateWorkflow(IProfileUpdateCommandRunner runner) => new(Root, LockPath, StateRoot, new FakeProfileSourceClient(), runner, workspaceManager: new CopyProfileCandidateWorkspaceManager());
        ...
        private static CatalogDocument RestrictSelectableRuntimesToLock(CatalogDocument catalog, ReleaseLockDocument releaseLock)
        {
            ...
            return catalog with
            {
                Toolchains = catalog.Toolchains.Select(toolchain => toolchain with { AllowedReferenceSetIds = toolchain.AllowedReferenceSetIds.Where(referenceSetIds.Contains).ToArray() })
                    .ToArray(),
                ...
                Runtimes = catalog.Runtimes.Select(runtime => HasComponent(releaseLock, runtime.Id, "runtime")
                        ? runtime : runtime with { Availability = new ComponentAvailability { Installed = false, Health = "unavailable", Reason = "Not represented by this synthetic release lock." } })
                    .ToArray(),
    ...
    {
        public Task<DotNetChannelResolution> ResolveDotNetChannelAsync(string channel, CancellationToken cancellationToken = default)
        {
        ...
        public Task<NuGetPackageResolution> ResolveLatestStablePackageAsync(string packageId, CancellationToken cancellationToken = default) =>
        ...
        public Task<NuGetPackageResolution> ResolveExactPackageAsync(string packageId, string version, CancellationToken cancellationToken = default) => Task.FromResult(Package(packageId, version));

        public Task<GitCommitResolution> ResolveGitCommitAsync(string owner, string repository, string branch, CancellationToken cancellationToken = default)
        {
    ...
    {
        public Task PrepareAsync(string repositoryRoot, string workspaceRoot, CancellationToken cancellationToken = default)
        {
    ...
    private sealed class RecordingSourceDateEpochReader(string? epoch) : ISourceDateEpochReader
    {
        ...
        public Task<string?> ReadAsync(string repositoryRoot, string revision, CancellationToken cancellationToken = default)
        {
