...
{
    private static readonly Ed25519PrivateKeyParameters TestPlanPrivateKey = new(Enumerable.Range(0, 32).Select(static value => (byte)value).ToArray(), 0);
    private static readonly byte[] TestPlanSpki =
    [
        0x30, 0x2a, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x03, 0x21, 0x00, ..TestPlanPrivateKey.GeneratePublicKey().GetEncoded()
    ];
    private static readonly byte[] TestPlanPem = Encoding.ASCII.GetBytes($"-----BEGIN PUBLIC KEY-----\n{Convert.ToBase64String(TestPlanSpki)}\n-----END PUBLIC KEY-----\n");

    internal static RuntimePromotionPlanSignatureVerifier CreateTestPlanVerifier() => new("sha256:" + Convert.ToHexStringLower(SHA256.HashData(TestPlanSpki)), "eng/profiles/trust/runtime-promotion-plan-test-public.pem", TestPlanPem, TestPlanSpki, static (_, _, _) => Task.CompletedTask);
    ...
    public void ProductionSignedPromotionPlanVectorVerifiesAndRejectsMutation()
    {
        ...
        var plan = File.ReadAllBytes(Path.Combine(root, "tests", "Fixtures", "RuntimePromotionPlanSigning", "plan.json"));
        var signature = File.ReadAllBytes(Path.Combine(root, "tests", "Fixtures", "RuntimePromotionPlanSigning", "plan.json.sig"));

        RuntimePromotionTrust.RuntimePromotionPlanSignatureTrust.VerifyDetachedForTests(plan, signature);

        plan[^2] ^= 1;
        Assert.Throws<BundleValidationException>(() => RuntimePromotionTrust.RuntimePromotionPlanSignatureTrust.VerifyDetachedForTests(plan, signature));
    }
    ...
    public void PromotionPlanRejectsWineOperatorReceiptForNonWineRuntime()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => RuntimePromotionPlanWorkflow.CreateContext(inputs.ProfileBytes, inputs.PreflightProfileBytes, PromotionFixture.JsonBytes(plan), inputs.PerformancePolicyBytes));
    ...
    public void WineOperatorReceiptCandidateLabelsMustAllMatchTheSignedBinding()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => WineCoreClrOperatorReceiptTrust.ValidateCandidateImage(runtimeId, image, snapshot));
        ...
        var rowBinding = binding with { LineageKind = "framework-row", IntermediaryReference = $"registry.example/row@sha256:{new string('6', 64)}", IntermediaryImageId = $"sha256:{new string('7', 64)}", IntermediarySizeBytes = 1 };
        var rowSnapshot = snapshot with { Binding = rowBinding };
        Assert.Throws<BundleValidationException>(() => WineCoreClrOperatorReceiptTrust.ValidateCandidateImage(runtimeId, image, rowSnapshot));
    ...
    public void WineOperatorReceiptLineageIsFamilySpecific()
    {
        ...
        Assert.Throws<BundleValidationException>(() => WineCoreClrOperatorReceiptTrust.ValidateBinding("wine", direct, "netfx-clr-wine"));
        ...
        Assert.Throws<BundleValidationException>(() => WineCoreClrOperatorReceiptTrust.ValidateBinding("wine", framework, "coreclr-wine"));
    }
    ...
    public async Task PromotionPlanFinalizerProducesReceiptAcceptedByReleaseTrust()
    {
        ...
        var context = RuntimePromotionPlanWorkflow.CreateContext(inputs.ProfileBytes, inputs.PreflightProfileBytes, inputs.PlanBytes, inputs.PerformancePolicyBytes);
        var result = RuntimePromotionPlanWorkflow.Finalize(inputs.ProfileBytes, inputs.PreflightProfileBytes, inputs.PlanBytes, inputs.PerformancePolicyBytes, inputs.CapabilityEvidence, inputs.PerformanceEvidenceBytes, fixture.RequestBinding);
    ...
    public void PromotionPlanFinalizerRejectsEvidenceBoundToAnotherPlan()
    {
        using var fixture = new PromotionFixture();
        var inputs = fixture.CreatePromotionWorkflowInputs();
        var evidence = inputs.CapabilityEvidence.ToDictionary(static item => item.Key, static item => item.Value, StringComparer.Ordinal);
        ...
        var exception = Assert.Throws<BundleValidationException>(() => RuntimePromotionPlanWorkflow.Finalize(inputs.ProfileBytes, inputs.PreflightProfileBytes, inputs.PlanBytes, inputs.PerformancePolicyBytes, evidence, inputs.PerformanceEvidenceBytes, fixture.RequestBinding));
    ...
    public void PromotionPlanExpandsInstrumentationOnlyInTheImmutablePreflightProfile()
    {
        ...
        Assert.Equal(["run", "jit-asm"], candidate["capabilities"]!.AsArray().Select(static item => item!.GetValue<string>()));
        Assert.Equal(["execution-flow", "inspection", "jit-asm", "run"], preflight["capabilities"]!.AsArray().Select(static item => item!.GetValue<string>()));
    ...
    public void PromotionPlanAllowsImmutableFrameworkRangeNormalization()
    {
        ...
        var inputs = fixture.CreatePromotionWorkflowInputs();
        var candidate = JsonNode.Parse(inputs.ProfileBytes)!.AsObject();
        candidate["acceptedFrameworks"] = new JsonArray { new JsonObject { ["name"] = "Microsoft.NETCore.App", ["minimumVersion"] = "10.0.0", ["maximumVersion"] = "10.0.9" } };
        var preflight = JsonNode.Parse(inputs.PreflightProfileBytes)!.AsObject();
        preflight["acceptedFrameworks"] = new JsonArray { new JsonObject { ["name"] = "Microsoft.NETCore.App", ["exactVersion"] = "10.0.9" } };
        ...
        var context = RuntimePromotionPlanWorkflow.CreateContext(candidateBytes, preflightBytes, PromotionFixture.JsonBytes(plan), inputs.PerformancePolicyBytes);
    ...
    public void PromotionPlanRejectsInstrumentationLeakingIntoBlockedCandidateProfile()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => RuntimePromotionPlanWorkflow.CreateContext(candidateBytes, inputs.PreflightProfileBytes, PromotionFixture.JsonBytes(plan), inputs.PerformancePolicyBytes));
    ...
    public void PromotionBoundRuntimeImageMayBindImplementationRevisionBeforeReleaseRevision()
    {
        ...
        ReleaseBundleBuilder.ValidateInspectionSourceRevision(definition, labels, releaseRevision, promotionBoundRuntime: true);

        var ordinary = Assert.Throws<BundleValidationException>(() => ReleaseBundleBuilder.ValidateInspectionSourceRevision(definition, labels, releaseRevision, promotionBoundRuntime: false));
    ...
    public void PromotionBoundRuntimeImageRejectsDisagreeingImplementationRevisionLabels()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => ReleaseBundleBuilder.ValidateInspectionSourceRevision(PromotionImageDefinition(), labels, "cccccccccccccccccccccccccccccccccccccccc", promotionBoundRuntime: true));
    ...
    public async Task SourceClosureAcceptsExactReceiptDerivedImplementationToReleaseTransaction()
    {
        using var fixture = new PromotionFixture();
        var inputs = fixture.CreatePromotionWorkflowInputs();
        var finalized = RuntimePromotionPlanWorkflow.Finalize(inputs.ProfileBytes, inputs.PreflightProfileBytes, inputs.PlanBytes, inputs.PerformancePolicyBytes, inputs.CapabilityEvidence, inputs.PerformanceEvidenceBytes, fixture.RequestBinding);
        ...
        Assert.Equal(fixture.SourceClosurePaths().Order(StringComparer.Ordinal), snapshot.Files.Select(static file => file.RelativePath));

        await RuntimePromotionSourceClosure.RevalidateAsync(fixture.Root, snapshot, inspector, TestContext.Current.CancellationToken);
    ...
    public void SignedPlanBindsExactlyOneCandidateProfile()
    {
        ...
        var candidates = Directory.EnumerateFiles(Path.Combine(fixture.Root, "profiles", "runtimes", "candidates"), "*.json", SearchOption.TopDirectoryOnly).ToArray();
        var candidate = Assert.Single(candidates);
        using var plan = JsonDocument.Parse(inputs.PlanBytes);

        Assert.Equal(plan.RootElement.GetProperty("profileSha256").GetString(), PromotionFixture.Digest(File.ReadAllBytes(candidate)));
    }
    ...
    public async Task SourceClosureRejectsAllowedPlanPathWhoseBytesDoNotBindTheReceipt()
    {
        ...
        var exception = await Assert.ThrowsAsync<BundleValidationException>(() => RuntimePromotionSourceClosure.RevalidateAsync(fixture.Root, closure, new PromotionSourceInspector(isAncestor: true, fixture.SourceClosurePaths().Select(static path => new RuntimePromotionSourceChange("M", path)).ToArray()), TestContext.Current.CancellationToken));
    ...
    public async Task SourceClosureRevalidationRejectsBaselineInputChangedAfterCapture(string input)
    {
        ...
        Assert.DoesNotContain(closure!.Files, file => file.RelativePath == candidatePath);
        Assert.Contains(closure.CapturedFiles, file => file.RelativePath == candidatePath && file.Bytes.SequenceEqual(inputs.ProfileBytes));
        ...
        await File.AppendAllTextAsync(Path.Combine(fixture.Root, inputPath.Replace('/', Path.DirectorySeparatorChar)), " ", TestContext.Current.CancellationToken);

        var exception = await Assert.ThrowsAsync<BundleValidationException>(() => RuntimePromotionSourceClosure.RevalidateAsync(fixture.Root, closure, inspector, TestContext.Current.CancellationToken));
        Assert.Contains("changed before release finalization", exception.Message, StringComparison.Ordinal);
    }
    ...
    {
        var profile = new RuntimeProfileDefinition { Family = family, Capabilities = ["jit-asm"] };
    ...
    {
        var profile = new RuntimeProfileDefinition { Id = "wine-netfx48-linux-x64", Family = "netfx-clr-wine", Operations = new RuntimeProfileOperations { Jit = new RuntimeJitOperationDefinition { ImplementationId = RuntimeOperationImplementationIds.DesktopClrJitInspector, SourceMappingKind = RuntimeJitSourceMappingKinds.None } } };
        ...
        var provider = Assert.Throws<BundleValidationException>(() => RuntimePromotionTrust.RequirePermittedJitOperation(profile));
        ...
        var mapping = Assert.Throws<BundleValidationException>(() => RuntimePromotionTrust.RequirePermittedJitOperation(profile));
        Assert.Contains("source mapping kind 'none'", mapping.Message, StringComparison.Ordinal);

        profile.Operations.Jit = null;
        var missingOperation = Assert.Throws<BundleValidationException>(() => RuntimePromotionTrust.RequirePermittedJitOperation(profile));
    ...
    {
        var profile = new RuntimeProfileDefinition { Family = "mono", Capabilities = ["jit-asm"] };

        var exception = Assert.Throws<BundleValidationException>(() => RuntimePromotionPlanWorkflow.ValidateJitLibraryPath(profile, jitLibraryPath));
    ...
    public void PromotionPlanRejectsPreflightImageIdentityThatDiffersFromPlan(string field)
    {
        ...
        preflight[field] = field == "image"
            ? $"registry.example/other@sha256:{new string('8', 64)}" : $"sha256:{new string('9', 64)}";
        var preflightBytes = PromotionFixture.JsonBytes(preflight);
        var plan = JsonNode.Parse(inputs.PlanBytes)!.AsObject();
        plan["preflightProfile"]!["sha256"] = PromotionFixture.Digest(preflightBytes);

        var exception = Assert.Throws<BundleValidationException>(() => RuntimePromotionPlanWorkflow.CreateContext(inputs.ProfileBytes, preflightBytes, PromotionFixture.JsonBytes(plan), inputs.PerformancePolicyBytes));
    ...
    public void CapabilityEvidenceSandboxUserBindsExactProfileExecutionIdentity(string executionUser)
    {
        ...
        fixture.ValidateRunEvidenceSandboxUser(profileExecutionUser: executionUser, receiptPlatform: "wine", evidenceUser: executionUser);
    }
    ...
    public void CapabilityEvidenceRejectsSandboxUserThatDiffersFromProfileExecutionIdentity()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => fixture.ValidateRunEvidenceSandboxUser(profileExecutionUser: RuntimeContainerExecutionUsers.NonRoot, receiptPlatform: "wine", evidenceUser: RuntimeContainerExecutionUsers.Root));
    ...
    public void CapabilityEvidenceRejectsUnapprovedSupervisorSandboxPolicy()
    {
        ...
        var exception = Assert.Throws<BundleValidationException>(() => fixture.ValidateRunEvidenceSandboxPolicy(supervisorPolicyId: "runtime-job-weak", seccompSha256: "sha256:" + new string('8', 64)));
    ...
    public void TargetRuntimeRunnerExecutableIsAcceptedForOperatorFamilies(string family, string profileId)
    {
        ...
        var profile = new RuntimeProfileDefinition { Id = profileId, Family = family, Layout = new RuntimeImageLayout { RunnerAssemblyPath = helperPath }, Operations = new RuntimeProfileOperations { Run = new RuntimeRunOperationDefinition { ImplementationId = RuntimeOperationImplementationIds.TargetRuntimeRunner, Command = new RuntimeOperationCommandDefinition { Executable = family == "mono" ? "/usr/bin/mono" : "/usr/bin/wine", Argv = [helperPath, "run", RuntimeOperationPlaceholders.EntryAssembly] } } } };
        var receipt = new RuntimePromotionReceiptDocument
        {
            ...
            Image = new RuntimePromotionImageIdentity { Reference = $"registry.example/runtime@sha256:{new string('1', 64)}", ImageId = $"sha256:{new string('2', 64)}", SizeBytes = 1 },
            ComponentIdentity = new RuntimePromotionComponentIdentity { SourceUri = $"docker://registry.example/operator@sha256:{new string('3', 64)}", SourceDigest = $"sha256:{new string('3', 64)}" },
            RuntimeIdentity = new RuntimePromotionRuntimeIdentity { RuntimeCommit = "not-applicable", JitVersion = "not-applicable", JitCommit = "not-applicable" },
            Operations = new RuntimePromotionOperations { Run = new RuntimePromotionOperationHelper { Implementation = RuntimeOperationImplementationIds.TargetRuntimeRunner, AssemblyPath = helperPath, AssemblySha256 = $"sha256:{new string('4', 64)}" } },
            Performance = new RuntimePromotionPerformanceBinding { Result = "passed", PolicyId = "test", PolicyPath = "profiles/runtime-performance-policies/test.json", PolicySha256 = $"sha256:{new string('5', 64)}", EvidencePath = $"profiles/runtime-promotion-evidence/{profileId}/performance.json", EvidenceSha256 = $"sha256:{new string('6', 64)}" },
    ...
    public async Task CapturesReceiptAndRevalidatesEvidenceImageAndHelpers()
    {
        ...
        await RuntimePromotionTrust.RevalidateAsync(fixture.Root, snapshots, fixture.Docker, TestContext.Current.CancellationToken);

        var snapshot = Assert.Single(snapshots);
        Assert.Equal(PromotionFixture.ProfileId, snapshot.RuntimeId);
        Assert.Equal(PromotionFixture.MeasurementHelperReference, snapshot.MeasurementHelper.Reference);
        Assert.Equal(PromotionFixture.MeasurementHelperImageId, snapshot.MeasurementHelper.ImageId);
        Assert.Equal([PromotionFixture.ImmutableReference, PromotionFixture.MeasurementHelperReference], fixture.Docker.InspectedReferences);
        Assert.Equal(8, fixture.Docker.InspectedFiles.Count);
        Assert.Equal(2, fixture.Docker.InspectedFiles.Count(static item => item.ImageId == PromotionFixture.MeasurementHelperImageId));
        Assert.Equal(6, fixture.Docker.InspectedFiles.Count(static item => item.ImageId == PromotionFixture.ImageId));
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.ImageId == PromotionFixture.MeasurementHelperImageId && item.Path == RuntimeMeasurementHelperContract.Entrypoint);
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/SharpLabNext.Runner.dll");
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/SharpLabNext.JitInspector.dll");
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/SharpLabNext.JitProfiler.so");
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/target-dotnet/dotnet");
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/SharpLab.Runtime.dll");
        Assert.Contains(fixture.Docker.InspectedFiles, static item => item.Path.EndsWith("/libclrjit.so", StringComparison.Ordinal));
    }
    ...
    {
        using var fixture = new PromotionFixture();
        var receiptPath = Path.Combine(fixture.Root, fixture.Profile.PromotionReceipt!.Path.Replace('/', Path.DirectorySeparatorChar));
    ...
    {
        using var fixture = new PromotionFixture();
        var evidencePath = Path.Combine(fixture.Root, "profiles", "runtime-promotion-evidence", PromotionFixture.ProfileId, "run.json");
    ...
    public async Task RevalidationRejectsEvidenceChangedAfterCapture()
    {
        ...
        await File.AppendAllTextAsync(Path.Combine(fixture.Root, evidence.RelativePath.Replace('/', Path.DirectorySeparatorChar)), " \n", TestContext.Current.CancellationToken);

        var exception = await Assert.ThrowsAsync<BundleValidationException>(() => RuntimePromotionTrust.RevalidateAsync(fixture.Root, snapshots, fixture.Docker, TestContext.Current.CancellationToken));
    ...
    public async Task RevalidationRejectsImmutableReferenceResolvingToAnotherImage()
    {
        using var fixture = new PromotionFixture();
        var snapshots = await fixture.CaptureAsync();
        fixture.Docker.Inspection = fixture.Docker.Inspection with { ImageId = $"sha256:{new string('9', 64)}" };
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.Component = fixture.Component with { SourceUri = "https://example.invalid/a-different-runtime.tar.gz" };
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["scenarios"]!.AsObject().Remove("mapping"));
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["scenarios"]!["run"]!["cold"]!.AsArray().RemoveAt(0));
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["environment"]!["runnerId"] = "runtime-preflight-linux-x64-v1");
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence.Remove("measurementHelper"));
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["measurementHelper"]!["contentSha256"] = $"sha256:{new string('9', 64)}");
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["measurementHelper"]!["image"]!["reference"] = $"registry.example/sharplabnext/not-the-supervisor@sha256:{new string('7', 64)}");
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["measurementHelper"]!["image"]!["imageId"] = PromotionFixture.ImageId);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["scenarios"]!["run"]!["cold"]![0]!.AsObject().Remove("completionPeakMemoryBytes"));
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["scenarios"]!["run"]!["cold"]![0]!.AsObject().Remove("postCompletionResourceSampleCount"));
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdateReceipt(static receipt => receipt["image"]!.AsObject()["sizeBytes"] = 536870913);
    ...
    public async Task RevalidationRejectsMeasurementHelperImageDrift()
    {
        using var fixture = new PromotionFixture();
        var snapshots = await fixture.CaptureAsync();
        fixture.Docker.HelperInspection = fixture.Docker.HelperInspection with { ImageId = $"sha256:{new string('9', 64)}" };
    ...
    public async Task RevalidationRejectsPerformancePolicyDrift()
    {
        using var fixture = new PromotionFixture();
        var snapshots = await fixture.CaptureAsync();
        await File.AppendAllTextAsync(fixture.PerformancePolicyPath, " \n", TestContext.Current.CancellationToken);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.ReplaceEvidence("run", Encoding.UTF8.GetBytes("{\"schemaVersion\":1,\"profileId\":\"dotnet-10-linux-x64\",\"capability\":\"run\",\"result\":\"passed\"}\n"));
    ...
    public async Task CaptureRejectsJitEvidenceWithoutJitLibrary()
    {
        ...
        fixture.UpdateCapabilityEvidence("jit-asm", static evidence =>
        {
            ...
            var jitLibrary = artifacts.Single(node => node!["role"]!.GetValue<string>() == "jit-library");
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdateCapabilityEvidence("run", static evidence => evidence["lifecycle"]!["timeout"]!["processTreeRemoved"] = false);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdateCapabilityEvidence("run", static evidence => evidence["run"]!["expectedStdoutMarker"] = null);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdateReceipt(static receipt => receipt["componentIdentity"]!["sourceUri"] = null);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformanceEvidence(static evidence => evidence["scenarios"]!["run"]!["cold"]![0]!["peakMemoryBytes"] = null);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdatePerformancePolicy(static policy => policy["scenarios"]!["run"]!["cold"]!["maximumP95LatencyMilliseconds"] = null);
    ...
    {
        using var fixture = new PromotionFixture();
        fixture.UpdateCapabilityEvidence("jit-asm", evidence => evidence["jit"]!["pdb"]!["path"] = pdbPath);
    ...
    public async Task CaptureRejectsSubstitutedExecutableHostArtifact()
    {
        ...
        {
            var runtimeHost = evidence["artifacts"]!.AsArray().Single(node => node!["role"]!.GetValue<string>() == "runtime-host");
    ...
    public async Task CaptureRejectsCommandAndSecurityPolicyDrift()
    {
        ...
        commandFixture.UpdateCapabilityEvidence("run", static evidence => evidence["invocation"]!["command"]![0] = "/opt/sharplabnext/substituted-dotnet");

        var commandException = await Assert.ThrowsAsync<BundleValidationException>(() => commandFixture.CaptureAsync());
        ...
        policyFixture.UpdateCapabilityEvidence("run", static evidence => evidence["sandbox"]!["deadlineMilliseconds"] = 10_001);

        var policyException = await Assert.ThrowsAsync<BundleValidationException>(() => policyFixture.CaptureAsync());
    ...
    public async Task CaptureRejectsMissingOrUnexpectedRunProbeArguments()
    {
        ...
        var missingException = await Assert.ThrowsAsync<BundleValidationException>(() => missingFixture.CaptureAsync());
        ...
        var unexpectedException = await Assert.ThrowsAsync<BundleValidationException>(() => unexpectedFixture.CaptureAsync());
    ...
    {
        using var fixture = new PromotionFixture(includeInstrumentation: false, includeSupportAssembly: false);

        var snapshots = await fixture.CaptureAsync();
        await RuntimePromotionTrust.RevalidateAsync(fixture.Root, snapshots, fixture.Docker, TestContext.Current.CancellationToken);

        Assert.DoesNotContain(fixture.Docker.InspectedFiles, static item => item.Path == "/opt/sharplabnext/SharpLab.Runtime.dll");
    }
    ...
    private static RuntimePromotionWineOperatorBinding CreateWineOperatorBinding(string revision, string lineageKind) => new()
    {
    ...
    internal sealed class PromotionFixture : IDisposable
    {
        ...
        public const string ImmutableReference = "registry.example/sharplabnext/runtime-dotnet-10@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        public const string ImageId = "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        public const string MeasurementHelperReference = "registry.example/sharplabnext/runtime-supervisor@sha256:7777777777777777777777777777777777777777777777777777777777777777";
        public const string MeasurementHelperImageId = "sha256:8888888888888888888888888888888888888888888888888888888888888888";
        ...
        private const string DefaultComponentSourceUri = "https://builds.dotnet.microsoft.com/dotnet/Runtime/10.0.9/dotnet-runtime-10.0.9-linux-x64.tar.gz";
        private const string DefaultComponentSha512 = "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
        private const string RunnerDigest = "sha256:1111111111111111111111111111111111111111111111111111111111111111";
        private const string JitInspectorDigest = "sha256:2222222222222222222222222222222222222222222222222222222222222222";
        private const string ProfilerDigest = "sha256:3333333333333333333333333333333333333333333333333333333333333333";
        public const string RuntimeHostDigest = "sha256:4444444444444444444444444444444444444444444444444444444444444444";
        private const string SupportAssemblyDigest = "sha256:5555555555555555555555555555555555555555555555555555555555555555";
        private const string JitLibraryDigest = "sha256:6666666666666666666666666666666666666666666666666666666666666666";
        ...
        public PromotionFixture(bool includeInstrumentation = true, bool includeSupportAssembly = true, RuntimeProfileDefinition? profileTemplate = null, LockedComponent? componentTemplate = null)
        {
            ...
            Profile = profileTemplate is null ? new RuntimeProfileDefinition
            {
                ...
                Capabilities = includeInstrumentation
                    ? ["run", "jit-asm", "inspection", "execution-flow"] : ["run", "jit-asm"],
                Container = new RuntimeContainerDefinition { IsolationKind = RuntimeContainerIsolationKinds.Standard, EnvironmentKind = RuntimeContainerEnvironmentKinds.CoreClr, ExecutionUser = RuntimeContainerExecutionUsers.NonRoot },
                Layout = new RuntimeImageLayout { DotNetHostPath = RuntimeHostPath, RunnerAssemblyPath = "/opt/sharplabnext/SharpLabNext.Runner.dll", JitInspectorAssemblyPath = "/opt/sharplabnext/SharpLabNext.JitInspector.dll" },
            ...
            Component = componentTemplate is null ? new LockedComponent { Kind = "runtime", ResolvedVersion = DefaultRuntimeVersion, Commit = DefaultRuntimeCommit, JitCommit = DefaultRuntimeCommit, SourceUri = DefaultComponentSourceUri, Sha512 = DefaultComponentSha512 } : componentTemplate with { };
            ...
            {
                var jitInspectorPath = Profile.Layout.JitInspectorAssemblyPath ?? throw new InvalidOperationException("Promotion fixture JIT profile has no inspector path.");
        ...
        public string PerformancePolicyPath => Path.Combine(Root, "profiles", "runtime-performance-policies", $"{PerformancePolicyId}.json");
        public RuntimeProfileDefinition Profile { get; }
        public LockedComponent Component { get; set; }
        public PromotionDocker Docker { get; }
        public RuntimeCapabilityRequestBinding RequestBinding => new($"sha256:{new string('d', 64)}", Profile.Capabilities.Contains("execution-flow", StringComparer.Ordinal) ? $"sha256:{new string('e', 64)}" : null, Profile.Capabilities.Contains("jit-asm", StringComparer.Ordinal) ? JitMethodFilter : null);

        public void SetRuntimeCandidatePromotionLabels(string? sourceContext, string? promotionEligible)
        {
        ...
        {
            var runtime = new RuntimeManifest { Id = ProfileId, DisplayName = ".NET 10", Family = "coreclr", ResolvedVersion = RuntimeVersion, Rid = "linux-x64", Architecture = "x64", AcceptedArtifactFormats = ["dotnet-managed-pe-v1"], Capabilities = Profile.Capabilities, Availability = new ComponentAvailability { Installed = true, Health = "healthy" } };
            var catalog = new CatalogDocument { SchemaVersion = 1, Revision = "test", ReleaseId = "test", Languages = [], Toolchains = [], ReferenceSets = [], Runtimes = [runtime], ArtifactProcessors = [], Outputs = [], Compatibility = [], Presets = [] };
            ...
            var deployment = new DeploymentImageManifest
            {
                ...
                Images =
                [
                    new DeploymentImageDefinition { Id = ProfileId, Repository = "registry.example/sharplabnext/runtime-dotnet-10", ImmutableReference = ImmutableReference, RuntimeId = ProfileId }
                ]
            };
        ...
        {
            var receiptPath = Path.Combine(Root, Profile.PromotionReceipt!.Path.Replace('/', Path.DirectorySeparatorChar));
        ...
        {
            var evidencePath = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, capability + ".json");
            File.WriteAllBytes(evidencePath, bytes);
            UpdateReceipt(receipt =>
            {
                var check = receipt["checks"]!.AsArray().Select(static node => node!.AsObject()).Single(item => string.Equals(item["capability"]!.GetValue<string>(), capability, StringComparison.Ordinal));
        ...
        {
            var path = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, "performance.json");
            var evidence = JsonNode.Parse(File.ReadAllText(path))!.AsObject();
            update(evidence);
            var bytes = JsonBytes(evidence);
            File.WriteAllBytes(path, bytes);
            UpdateReceipt(receipt => receipt["performance"]!.AsObject()["evidenceSha256"] = Digest(bytes));
        }
        ...
        public void UpdatePerformancePolicy(Action<JsonObject> update)
        {
            ...
            var evidencePath = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, "performance.json");
            var evidence = JsonNode.Parse(File.ReadAllText(evidencePath))!.AsObject();
        ...
        {
            var candidateProfile = JsonNode.Parse(JsonSerializer.SerializeToUtf8Bytes(Profile, ProfileJsonOptions))!.AsObject();
            candidateProfile.Remove("promotionReceipt");
            candidateProfile["capabilities"] = new JsonArray(Profile.Capabilities.Where(static capability => capability is not ("inspection" or "execution-flow")).Select(static capability => (JsonNode?)JsonValue.Create(capability)).ToArray());
            ...
            preflightProfile["capabilities"] = new JsonArray(Profile.Capabilities.Order(StringComparer.Ordinal).Select(static capability => (JsonNode?)JsonValue.Create(capability)).ToArray());
            var preflightProfileBytes = JsonBytes(preflightProfile);
            var buildInputs = new JsonObject { ["IMAGE_PREFIX"] = "registry.example/sharplabnext", ["RELEASE_ID"] = "test-release", ["RUNTIME_MATRIX_PROFILE_ID"] = ProfileId, ["RUNTIME_MATRIX_RUNTIME_VERSION"] = RuntimeVersion, ["SOURCE_DATE_EPOCH"] = "0" };
            var plan = new JsonObject
            {
                ...
                ["producer"] = new JsonObject { ["id"] = RuntimePromotionPlanWorkflow.ProducerId, ["sourceRevision"] = SourceRevision },
                ["securityPolicyId"] = "runtime-job-default",
                ["capabilities"] = new JsonArray(Profile.Capabilities.Order(StringComparer.Ordinal).Select(static value => (JsonNode?)JsonValue.Create(value)).ToArray()),
                ...
                ["preflightProfile"] = new JsonObject { ["path"] = $"profiles/runtime-promotion-plans/{ProfileId}.profile.json", ["sha256"] = Digest(preflightProfileBytes) },
                ["performance"] = new JsonObject { ["policyId"] = PerformancePolicyId, ["policyPath"] = $"profiles/runtime-performance-policies/{PerformancePolicyId}.json", ["policySha256"] = Digest(performancePolicyBytes), ["evidencePath"] = $"profiles/runtime-promotion-evidence/{ProfileId}/performance.json" }
            };
            ...
            {
                var path = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, capability + ".json");
            ...
            var performanceEvidencePath = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, "performance.json");
            var performanceEvidence = JsonNode.Parse(File.ReadAllBytes(performanceEvidencePath))!.AsObject();
            ...
            return new PromotionWorkflowInputs(profileBytes, preflightProfileBytes, planBytes, planSha256, performancePolicyBytes, capabilityEvidence, performanceEvidenceBytes);
        }

        public void InstallFinalizedReceipt(byte[] receiptBytes)
        {
            var path = Path.Combine(Root, Profile.PromotionReceipt!.Path.Replace('/', Path.DirectorySeparatorChar));
        ...
        public void ExportCompletedPromotionMaterial(string destinationRoot)
        {
            ...
            var finalized = RuntimePromotionPlanWorkflow.Finalize(inputs.ProfileBytes, inputs.PreflightProfileBytes, inputs.PlanBytes, inputs.PerformancePolicyBytes, inputs.CapabilityEvidence, inputs.PerformanceEvidenceBytes, RequestBinding);
            ...
            var paths = new[]
            {
                ...
                $"profiles/runtime-promotion-evidence/{ProfileId}/{capability}.json")).Append($"profiles/runtime-promotion-evidence/{ProfileId}/performance.json");
        ...
        {
            WriteRepositoryFile($"profiles/runtime-promotion-plans/{ProfileId}.json", inputs.PlanBytes);
            WriteRepositoryFile($"profiles/runtime-promotion-plans/{ProfileId}.json.sig", SignTestPlan(inputs.PlanBytes));
            WriteRepositoryFile($"profiles/runtime-promotion-plans/{ProfileId}.profile.json", inputs.PreflightProfileBytes);
            WriteRepositoryFile($"profiles/runtimes/candidates/{ProfileId}.json", inputs.ProfileBytes);
            WriteRepositoryFile($"profiles/runtimes/{ProfileId}.json", JsonSerializer.SerializeToUtf8Bytes(Profile, ProfileJsonOptions));
        ...
        [
            Profile.PromotionReceipt!.Path, ..Profile.Capabilities.Select(capability => $"profiles/runtime-promotion-evidence/{ProfileId}/{capability}.json"),
        ...
        private void InstallSignedPlan(byte[] planBytes, string planSha256)
        {
            ...
            WriteRepositoryFile("eng/profiles/trust/runtime-promotion-plan-test-public.pem", TestPlanPem);
            _planSha256 = planSha256;
            _planSignature = new JsonObject { ["path"] = signaturePath, ["sha256"] = Digest(signature), ["keyId"] = CreateTestPlanVerifier().KeyId };
        ...
        public void ValidateRunEvidenceSandboxUser(string profileExecutionUser, string receiptPlatform, string evidenceUser)
        {
            ...
            var receipt = JsonSerializer.Deserialize<RuntimePromotionReceiptDocument>(JsonBytes(receiptJson), ProfileJsonOptions)!;
            var check = receipt.Checks.Select(static item => item!).Single(static item => item.Capability == "run");
            var evidencePath = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, "run.json");
            var evidence = JsonNode.Parse(File.ReadAllBytes(evidencePath))!.AsObject();
            evidence["sandbox"]!["user"] = evidenceUser;

            _ = RuntimeCapabilityEvidenceValidation.Validate(JsonBytes(evidence), Profile, receipt, check);
        }

        public void ValidateRunEvidenceSandboxPolicy(string supervisorPolicyId, string seccompSha256)
        {
            var receiptPath = Path.Combine(Root, Profile.PromotionReceipt!.Path.Replace('/', Path.DirectorySeparatorChar));
            var receipt = JsonSerializer.Deserialize<RuntimePromotionReceiptDocument>(File.ReadAllBytes(receiptPath), ProfileJsonOptions)!;
            var check = receipt.Checks.Select(static item => item!).Single(static item => item.Capability == "run");
            var evidencePath = Path.Combine(Root, "profiles", "runtime-promotion-evidence", ProfileId, "run.json");
            var evidence = JsonNode.Parse(File.ReadAllBytes(evidencePath))!.AsObject();
        ...
        private void WritePromotionMaterial()
        {
            ...
            foreach (var capability in Profile.Capabilities)
            {
                ...
                var evidenceBytes = JsonBytes(CreateCapabilityEvidence(capability, sourceMappingKind, mappingSource));
                File.WriteAllBytes(Path.Combine(Root, evidencePath.Replace('/', Path.DirectorySeparatorChar)), evidenceBytes);
                checks.Add(new JsonObject { ["capability"] = capability, ["result"] = "passed", ["networkDisabled"] = true, ["supervisorSandbox"] = true, ["outputLimitValidated"] = true, ["sourceMappingKind"] = sourceMappingKind, ["mappingSource"] = mappingSource, ["evidencePath"] = evidencePath, ["evidenceSha256"] = Digest(evidenceBytes) });
            }

            var performancePolicy = CreatePerformancePolicy();
            var performancePolicyBytes = JsonBytes(performancePolicy);
            var performancePolicyPath = Path.Combine(Root, "profiles", "runtime-performance-policies", $"{PerformancePolicyId}.json");
            File.WriteAllBytes(performancePolicyPath, performancePolicyBytes);
            var performancePolicyDigest = Digest(performancePolicyBytes);

            var performanceScenarios = new JsonObject { ["run"] = CreatePerformanceScenario(), ["jit"] = CreatePerformanceScenario(), ["mapping"] = CreatePerformanceScenario() };
            var performanceEvidence = new JsonObject
            {
                ["schemaVersion"] = 1,
                ["profileId"] = ProfileId,
                ["planSha256"] = PlanSha256,
                ["image"] = new JsonObject { ["reference"] = ImmutableReference, ["imageId"] = ImageId, ["sizeBytes"] = 536870912 },
                ["measurementHelper"] = new JsonObject { ["implementation"] = "sharplabnext-runtime-cgroup-sidecar-v1", ["image"] = new JsonObject { ["reference"] = MeasurementHelperReference, ["imageId"] = MeasurementHelperImageId, ["sizeBytes"] = 342000000 }, ["entrypoint"] = "/usr/local/bin/sharplabnext-runtime-measurement", ["sourceRevision"] = SourceRevision, ["contentSha256"] = RuntimeMeasurementHelperContract.ContentSha256 },
                ["sourceRevision"] = SourceRevision,
                ["policy"] = new JsonObject { ["id"] = PerformancePolicyId, ["sha256"] = performancePolicyDigest },
                ["capabilities"] = new JsonArray(Profile.Capabilities.Order(StringComparer.Ordinal).Select(static value => (JsonNode?)JsonValue.Create(value)).ToArray()),
                ["sourceMappingKind"] = "linux-profiler",
                ["environment"] = new JsonObject { ["runnerId"] = "runtime-preflight-linux-x64-v2", ["operatingSystem"] = "linux", ["architecture"] = "x64", ["nanoCpus"] = 1000000000, ["memoryLimitBytes"] = 268435456 },
            ...
            var performanceEvidencePath =
                $"profiles/runtime-promotion-evidence/{ProfileId}/performance.json";
            File.WriteAllBytes(Path.Combine(Root, performanceEvidencePath.Replace('/', Path.DirectorySeparatorChar)), performanceEvidenceBytes);
            ...
            var receipt = new JsonObject
            {
                ...
                ["image"] = new JsonObject { ["reference"] = ImmutableReference, ["imageId"] = ImageId, ["sizeBytes"] = 536870912 },
                ["componentIdentity"] = new JsonObject { ["sourceUri"] = ComponentSourceUri, ["sourceDigest"] = $"sha512:{ComponentSha512}" },
                ["runtimeIdentity"] = new JsonObject { ["runtimeCommit"] = RuntimeCommit, ["jitVersion"] = RuntimeVersion, ["jitCommit"] = RuntimeCommit },
                ["operations"] = new JsonObject { ["run"] = new JsonObject { ["implementation"] = RuntimeOperationImplementationIds.Runner, ["assemblyPath"] = Profile.Layout.RunnerAssemblyPath, ["assemblySha256"] = RunnerDigest }, ["jit"] = new JsonObject { ["implementation"] = RuntimeOperationImplementationIds.JitInspector, ["assemblyPath"] = Profile.Layout.JitInspectorAssemblyPath, ["assemblySha256"] = JitInspectorDigest, ["profilerPath"] = Profile.Operations!.Jit!.ProfilerPath, ["profilerSha256"] = ProfilerDigest } },
                ["performance"] = new JsonObject { ["result"] = "passed", ["policyId"] = PerformancePolicyId, ["policyPath"] = $"profiles/runtime-performance-policies/{PerformancePolicyId}.json", ["policySha256"] = performancePolicyDigest, ["evidencePath"] = performanceEvidencePath, ["evidenceSha256"] = Digest(performanceEvidenceBytes) },
                ["sourceRevision"] = SourceRevision,
                ["checks"] = checks
            };
            var receiptPath = $"profiles/runtime-promotion-receipts/{ProfileId}.json";
            Profile.PromotionReceipt = new RuntimePromotionReceiptReference { Path = receiptPath, Sha256 = string.Empty };
            WriteReceipt(Path.Combine(Root, receiptPath.Replace('/', Path.DirectorySeparatorChar)), receipt);
        }

        private JsonObject CreateCapabilityEvidence(string capability, string sourceMappingKind, string mappingSource)
        {
            ...
            var command = isJit
                ? RuntimeProfileCommandBuilder.CreateJitCommand(Profile, "SharpLabNext.Preflight.dll", JitMethodFilter) : RuntimeProfileCommandBuilder.CreateRunCommand(
                    Profile,
                    ...
                    capability switch
                    {
                        ...
                        _ => throw new InvalidOperationException($"Unknown Run capability '{capability}'.")
                    });
            var artifacts = new JsonArray { CreateArtifact("helper", isJit ? Profile.Layout.JitInspectorAssemblyPath! : Profile.Layout.RunnerAssemblyPath, isJit ? JitInspectorDigest : RunnerDigest, "managed-pe", "anycpu"), CreateArtifact("runtime-host", RuntimeHostPath, RuntimeHostDigest, "elf", "x64") };
            if (_includeSupportAssembly)
                artifacts.Add(CreateArtifact("support-assembly", SupportAssemblyPath, SupportAssemblyDigest, "managed-pe", "anycpu"));
            if (isJit)
            {
                artifacts.Add(CreateArtifact("jit-library", JitLibraryPath, JitLibraryDigest, "elf", "x64"));
                artifacts.Add(CreateArtifact("profiler", Profile.Operations!.Jit!.ProfilerPath!, ProfilerDigest, "elf", "x64"));
            }
            ...
            var evidence = new JsonObject
            {
                ...
                ["image"] = new JsonObject { ["reference"] = ImmutableReference, ["imageId"] = ImageId },
                ["sourceRevision"] = SourceRevision,
                ["completedAtUtc"] = "2026-07-22T00:00:00Z",
                ["result"] = "passed",
                ["producer"] = new JsonObject { ["id"] = "sharplabnext-runtime-preflight-v1", ["sourceRevision"] = SourceRevision, ["planSha256"] = PlanSha256 },
                ["artifacts"] = artifacts,
                ["invocation"] = new JsonObject { ["implementation"] = operationImplementation, ["command"] = new JsonArray(command.Select(static value => (JsonNode?)JsonValue.Create(value)).ToArray()), ["entryAssembly"] = new JsonObject { ["path"] = EntryAssemblyPath, ["sha256"] = $"sha256:{new string('7', 64)}" }, ["outcome"] = "succeeded", ["exitCode"] = 0, ["runtimeFrameCount"] = 3, ["terminalFrameKind"] = "Exit", ["terminalStatus"] = "completed", ["stdoutBytes"] = 32, ["stderrBytes"] = 16 },
                ...
                ["lifecycle"] = new JsonObject { ["outputOverflow"] = CreateLifecycleProbe("output-limit-exceeded"), ["timeout"] = CreateLifecycleProbe("timeout"), ["cancellation"] = CreateLifecycleProbe("cancelled"), ["processTreeCleanup"] = CreateLifecycleProbe("completed") }
            };
            var sourceProbeArtifactSha256 = $"sha256:{new string('d', 64)}";
            var probeArtifactSha256 = capability == "execution-flow"
                ? $"sha256:{new string('e', 64)}" : sourceProbeArtifactSha256;
            var probeArtifact = new JsonObject { ["contract"] = RuntimeCapabilityProbeContract.ContractId, ["sourceArtifactSha256"] = sourceProbeArtifactSha256, ["artifactSha256"] = probeArtifactSha256, ["entryAssemblySha256"] = $"sha256:{new string('7', 64)}", ["planSha256"] = PlanSha256, ["preflightProfileSha256"] = PreflightProfileSha256 };
            if (capability == "execution-flow")
            {
                probeArtifact["derivation"] = new JsonObject { ["parentArtifactSha256"] = sourceProbeArtifactSha256, ["processorId"] = RuntimeCapabilityProbeContract.ExecutionFlowProcessorId, ["processorVersion"] = RuntimeCapabilityProbeContract.ExecutionFlowProcessorVersion, ["optionsSha256"] = RuntimeCapabilityProbeContract.ExecutionFlowOptionsDigest, ["transformId"] = RuntimeCapabilityProbeContract.ExecutionFlowTransformId, ["profileId"] = RuntimeCapabilityProbeContract.ExecutionFlowProfileId, ["applied"] = true };
            }
            ...
            {
                case "run":
                    evidence["run"] = new JsonObject { ["expectedStdoutMarker"] = "stdout-marker", ["observedStdoutMarker"] = "stdout-marker", ["expectedStderrMarker"] = "stderr-marker", ["observedStderrMarker"] = "stderr-marker", ["exceptionFrameValidated"] = true };
                    break;
                case "jit-asm":
                {
                    var ranges = new JsonArray { CreateSourceRange(0, 0, 8, 10), CreateSourceRange(4, 8, 16, 11) };
                    evidence["jit"] = new JsonObject
                    {
                        ["runtimeVersion"] = RuntimeVersion,
                        ["jitVersion"] = RuntimeVersion,
                        ["pdb"] = new JsonObject { ["path"] = "/workspace/SharpLabNext.Preflight.pdb", ["sha256"] = $"sha256:{new string('a', 64)}", ["contentId"] = new string('b', 40), ["sequencePointCount"] = 2 },
                        ["methods"] = new JsonArray { new JsonObject { ["metadataToken"] = "0x06000001", ["displayName"] = "SharpLabNext.Preflight.MultipleSequencePoints", ["nativeCodeBytes"] = 32, ["instructionCount"] = 12, ["sourceRanges"] = ranges } },
                        ["mapping"] = new JsonObject { ["kind"] = sourceMappingKind, ["source"] = mappingSource, ["rangeCount"] = 2, ["distinctSourceRangeCount"] = 2, ["allRangesMatchPdb"] = true }
                    };
                    break;
                }
                case "inspection":
                    evidence["inspection"] = new JsonObject { ["recordCount"] = 2, ["kinds"] = new JsonArray("Value", "MemoryGraph"), ["valueProbePassed"] = true, ["memoryGraphProbePassed"] = true };
                    break;
                case "execution-flow":
        ...
        private static JsonObject CreateArtifact(string role, string path, string sha256, string format, string architecture) => new()
        {
            ["role"] = role,
            ["path"] = path,
            ["sha256"] = sha256,
        ...
        private static JsonObject CreateSourceRange(int ilOffset, int nativeStartOffset, int nativeEndOffset, int startLine) => new()
        {
            ["ilOffset"] = ilOffset,
        ...
        private static JsonObject CreatePerformancePolicy() => new()
        {
            ...
            ["resourceLimits"] = new JsonObject { ["nanoCpus"] = 1000000000, ["allowedMemoryBytes"] = new JsonArray(268435456, 1073741824) },
            ["image"] = new JsonObject { ["maximumSizeBytes"] = 8589934592 },
            ["scenarios"] = new JsonObject { ["run"] = CreatePerformanceBudget(30000, 45000, 10000, 20000), ["jit"] = CreatePerformanceBudget(45000, 60000, 20000, 30000), ["mapping"] = CreatePerformanceBudget(60000, 90000, 30000, 45000) }
        };

        private static JsonObject CreatePerformanceBudget(double coldP95, double coldSample, double warmP95, double warmSample) => new()
        {
            ["cold"] = new JsonObject { ["maximumP95LatencyMilliseconds"] = coldP95, ["maximumSampleLatencyMilliseconds"] = coldSample, ["maximumPeakMemoryBytes"] = 1073741824 },
            ["warm"] = new JsonObject { ["maximumP95LatencyMilliseconds"] = warmP95, ["maximumSampleLatencyMilliseconds"] = warmSample, ["maximumPeakMemoryBytes"] = 1073741824 }
        };
        ...
        private static JsonArray CreateSamples(int count, double latency) => new(Enumerable.Range(0, count).Select(_ => (JsonNode)new JsonObject { ["latencyMilliseconds"] = latency, ["peakMemoryBytes"] = 134217728, ["completionPeakMemoryBytes"] = 134217728, ["operationId"] = $"op_{System.Threading.Interlocked.Increment(ref PerformanceSampleSequence):x32}", ["resourceSampleCount"] = 1, ["postCompletionResourceSampleCount"] = 1, ["completedAtUtc"] = "2026-07-22T00:00:00.0000000Z" }).ToArray());

        public static byte[] JsonBytes(JsonObject value) => Encoding.UTF8.GetBytes(value.ToJsonString(new JsonSerializerOptions { WriteIndented = true }).ReplaceLineEndings("\n") + "\n");
    ...
    private sealed class PromotionSourceInspector(bool isAncestor, IReadOnlyList<RuntimePromotionSourceChange> changes) : IRuntimePromotionSourceInspector
    {
    ...
    internal sealed class PromotionDocker : IDockerCli
    {
        ...
        public Task<DockerImageInspection> InspectImageAsync(string reference, CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(reference == PromotionFixture.MeasurementHelperReference ? HelperInspection : Inspection);
        }
        ...
        public Task SaveImagesAsync(IReadOnlyList<string> references, string outputPath, CancellationToken cancellationToken) => Task.CompletedTask;
    }
}
