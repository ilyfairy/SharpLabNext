...
public sealed class PascalCaseProblemDetailsTests
{
    ...
    public async Task ProblemDetailsServiceWritesPascalCaseStandardMembers()
    {
        ...
        var written = await service.TryWriteAsync(new ProblemDetailsContext { HttpContext = context, ProblemDetails = new ProblemDetails { Type = "https://sharplabnext.dev/problems/invalid-argument", Title = "invalid-argument", Status = StatusCodes.Status400BadRequest, Detail = "The request is invalid.", Instance = "/api/v1/build", Extensions = { ["TraceId"] = "trace-1" } } });
    ...
    public async Task ResultsProblemUsesThePascalCaseWriter()
    {
        ...
        Assert.False(root.TryGetProperty("status", out _));
        Assert.False(root.TryGetProperty("title", out _));
        Assert.All(root.EnumerateObject(), property => Assert.True(property.Name.Length == 0 || char.IsUpper(property.Name[0]), $"ProblemDetails member '{property.Name}' is not PascalCase."));
    }
}
