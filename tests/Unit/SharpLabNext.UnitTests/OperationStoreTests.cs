...
public sealed class OperationStoreTests
{
    ...
    public void TerminalOperationRejectsFurtherEvents()
    {
        ...
        Assert.Throws<InvalidOperationException>(() =>
            store.Append(start.Handle.OperationId, new ProgressOperationEventPayload("late", null, null), DateTimeOffset.UnixEpoch));
    }

    [Fact]
    public void EventLimitReservesTheFinalSlotForATerminalFailure()
    {
        var store = new OperationStore(new OperationStoreOptions
        {
            MaximumEventsPerOperation = 3
        });
        ...
        store.Append(
            start.Handle.OperationId,
            new ProgressOperationEventPayload("first", null, 0.25),
            DateTimeOffset.UnixEpoch.AddSeconds(1));

        var terminal = store.Append(
            start.Handle.OperationId,
            new ProgressOperationEventPayload("overflow", null, 0.5),
    ...
    public void CapacityExhaustionDoesNotEvictOrLeakAnActiveOperation()
    {
        ...
        {
            MaximumOperations = 1,
            TerminalOperationTimeToLive = TimeSpan.FromHours(1)
        });
        var active = store.Start(
            "active-request",
            "active-key",
            OperationKind.Build,
            "active-trace",
        ...
        var exception = Assert.Throws<OperationCapacityExceededException>(() => store.Start(
            "rejected-request",
            "rejected-key",
            OperationKind.Run,
            "rejected-trace",
    ...
    public void CleanupRemovesOnlyExpiredTerminalOperationsAndTheirIdempotencyKeys()
    {
        ...
        store.Append(
            expired.Handle.OperationId,
            new CompletedOperationEventPayload(OperationCompletionStatus.Completed, TimeSpan.Zero),
            DateTimeOffset.UnixEpoch);
        store.Append(
            recent.Handle.OperationId,
            new FailedOperationEventPayload(new WorkerError(
                "recent-failure",
                ...
                "test",
                "test")),
            DateTimeOffset.UnixEpoch.AddMinutes(90));
        ...
        var replacement = store.Start(
            "replacement",
            "expired-key",
            OperationKind.Build,
            "trace-4",
            DateTimeOffset.UnixEpoch.AddHours(2));
        var recentReplay = store.Start(
            "recent-replay",
            "recent-key",
            OperationKind.Build,
            "trace-5",
    ...
    public void QueueOverloadFailuresUseTheShortRetentionWindow()
    {
        var store = new OperationStore(new OperationStoreOptions
        {
            MaximumOperations = 3,
            TerminalOperationTimeToLive = TimeSpan.FromHours(1),
            OverloadOperationTimeToLive = TimeSpan.FromMinutes(1)
        });
        ...
        store.Append(
            ...
            DateTimeOffset.UnixEpoch);
        store.Append(
            ordinaryFailure.Handle.OperationId,
            new FailedOperationEventPayload(new WorkerError(
                "ordinary-failure",
                WorkerErrorCategory.Internal,
                "Ordinary failure.",
        ...
        Assert.Equal(OperationStatus.Accepted, store.Get(active.Handle.OperationId)?.Status);
        Assert.False(store.Start(
            "replacement",
            "overloaded-key",
            OperationKind.Build,
            "trace-4",
            DateTimeOffset.UnixEpoch.AddMinutes(2)).Handle.IsExisting);
    }

    [Fact]
    public void OverloadRetentionCannotExceedTerminalRetention()
    {
        Assert.Throws<InvalidOperationException>(() => new OperationStore(new OperationStoreOptions
        {
            TerminalOperationTimeToLive = TimeSpan.FromMinutes(1),
            OverloadOperationTimeToLive = TimeSpan.FromMinutes(2)
        }));
    }
}
