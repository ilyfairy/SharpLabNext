...
public sealed class GatewayBuildPipelineTests
{
    ...
    public async Task RealWorkerBuildCompileCheckAndAstFlowThroughGatewayAndCas()
    {
        ...
        var artifactResult = Assert.IsType<BuildResult>(Assert.Single(
            artifactEvents.Select(item => item.Payload).OfType<TypedResultOperationEventPayload>()).Result);
        ...
        var compileCheckEvents = await ExecuteBuildAsync(
            gateway.HttpClient,
            workspace,
            "compile-check",
            BuildTarget.CompileCheck,
            "compile-check");
        var compileCheck = Assert.IsType<CompilationCheckResult>(Assert.Single(
            compileCheckEvents.Select(item => item.Payload).OfType<TypedResultOperationEventPayload>()).Result);
        ...
        var ast = Assert.IsType<AstResult>(Assert.Single(
            astEvents.Select(item => item.Payload).OfType<TypedResultOperationEventPayload>()).Result);
        ...
        var explain = Assert.IsType<ExplainResult>(Assert.Single(
            explainEvents.Select(item => item.Payload).OfType<TypedResultOperationEventPayload>()).Result);
        ...
        Assert.Contains(
            explain.Document.Files.SelectMany(static file => file.Nodes),
            static node => node.Kind == "InvocationExpression");
    }

    private static async Task<IReadOnlyList<OperationEvent>> ExecuteExplainAsync(
        HttpClient client,
        WorkspaceSnapshot workspace)
    {
        ...
        var handle = await startResponse.Content.ReadFromJsonAsync<OperationHandle>(
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken)
        ...
        Assert.True(
            ...
            $"Explain failed: {state.Error?.Code}: {state.Error?.PublicMessage}");
        return await GetEventsAsync(client, handle.OperationId);
    }

    private static async Task<IReadOnlyList<OperationEvent>> ExecuteBuildAsync(
        HttpClient client,
        WorkspaceSnapshot workspace,
        string outputId,
        BuildTarget target,
    ...
    {
        var catalogRevision = await GatewayTestCatalog.GetRevisionAsync(client);
        var resolveRequest = new ResolveSelectionRequest(
            "csharp",
            "roslyn-stable",
            "net10-ref",
            outputId,
        ...
        Assert.True(
            state.Status == OperationStatus.Completed,
            $"Build failed: {state.Error?.Code}: {state.Error?.PublicMessage}");

        return await GetEventsAsync(client, handle.OperationId);
    }

    private static async Task<IReadOnlyList<OperationEvent>> GetEventsAsync(
        HttpClient client,
        string operationId)
    {
        ...
        var events = body.Split('\n', StringSplitOptions.RemoveEmptyEntries)
            ...
            .Select(line => JsonSerializer.Deserialize<OperationEvent>(
...
internal sealed class DotNetWebServiceProcess : IAsyncDisposable
{
    ...
    private DotNetWebServiceProcess(
        Process process,
        Task<string> standardOutput,
        Task<string> standardError,
        HttpClient httpClient,
        string? internalServiceTokenFile)
    {
        _process = process;
    ...
    public static async Task<DotNetWebServiceProcess> StartAsync(
        string projectPath,
        ...
        string? internalServiceToken = null)
    {
        ...
        try
        {
            process = Process.Start(startInfo)
                ?? throw new InvalidOperationException($"Could not start service project '{projectPath}'.");
        }
        catch
        {
            ...
            for (var attempt = 0; attempt < 300; attempt++)
            {
                if (process.HasExited)
                {
                    throw new InvalidOperationException(
                        $"Service '{projectPath}' exited during startup.{Environment.NewLine}stdout:{Environment.NewLine}{await output}{Environment.NewLine}stderr:{Environment.NewLine}{await error}");
                }
