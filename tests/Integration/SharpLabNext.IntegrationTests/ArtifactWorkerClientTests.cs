...
public sealed class ArtifactWorkerClientTests
{
    ...
    public async Task ExactProcessorIdentityIsAccepted()
    {
        ...
        var handle = await client.StartRenderAsync(
            CreateRequest(),
    ...
    public async Task MismatchedDescriptorReleaseIsRejectedBeforeOperationStart()
    {
        ...
        var exception = await Assert.ThrowsAsync<ArtifactWorkerClientException>(() =>
            client.StartRenderAsync(CreateRequest(), TestContext.Current.CancellationToken));
    ...
    public async Task MissingProcessorIdentityIsRejected()
    {
        using var httpClient = CreateHttpClient(
            CreateDescriptor(),
            CreateRenderResult(identity: null));
        var client = CreateClient(httpClient);
        var handle = await client.StartRenderAsync(
            CreateRequest(),
            TestContext.Current.CancellationToken);
        ...
        var exception = await Assert.ThrowsAsync<ArtifactWorkerClientException>(() =>
            client.GetEventsAsync(handle.OperationId, 0, TestContext.Current.CancellationToken));
    ...
    private static ArtifactWorkerClient CreateClient(HttpClient httpClient) => new(
        httpClient,
        new ArtifactPipelineOptions(),
        new ArtifactWorkerClientSettings(WorkerId, ReleaseId, WorkerImageId));

    private static HttpClient CreateHttpClient(
        WorkerDescriptor descriptor,
        RenderArtifactResult result,
        Action? onStart = null) => new(new ArtifactDelegateHandler(request =>
    {
        ...
        if (path == "/api/v1/artifact-renders")
        {
            ...
            return new HttpResponseMessage(HttpStatusCode.Accepted)
            {
                Content = JsonContent.Create(
                    new OperationHandle(OperationId, "artifact-client-request", DateTimeOffset.UtcNow, false),
        ...
        if (path == $"/api/v1/operations/{OperationId}/events")
        {
            ...
            [
                new(
                    ...
                    "artifact-client-trace",
                    new AcceptedOperationEventPayload("artifact-client-request", OperationKind.RenderArtifact)),
                new(
                    OperationId,
                    2,
                    DateTimeOffset.UtcNow,
    ...
    private static WorkerDescriptor CreateDescriptor() => new(
        new ServiceIdentity(
            ...
            ["il"],
            "ready"),
        "artifact-worker-instance",
        WorkerKind.ArtifactProcessor,
        WorkerImageId,
        ProtocolVersion.WorkerV1,
        [ProtocolVersion.WorkerV1],
        [new WorkerCapabilityDescriptor("il", 1, true, [WorkerId])],
        [WorkerId],
        DateTimeOffset.UtcNow);

    private static RenderArtifactRequest CreateRequest() => new(
        "artifact-client-request",
        "artifact-client-key",
        "artifact-client-pipeline",
    ...
    private static RenderArtifactResult CreateRenderResult(ArtifactProcessorIdentity? identity) => new(
        ArtifactJobOutcome.Succeeded,
        new ContentRef($"sha256:{new string('c', 64)}"),
        "text/plain; charset=utf-8",
        [],
        [],
        identity);

    private static ArtifactProcessorIdentity CreateIdentity() => new(
        ReleaseId,
        WorkerId,
        "ilspy/10.1.0.8386",
        WorkerImageId);

    private sealed class ArtifactDelegateHandler(Func<HttpRequestMessage, HttpResponseMessage> handler)
        : HttpMessageHandler
    {
