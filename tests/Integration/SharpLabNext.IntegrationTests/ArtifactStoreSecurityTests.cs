...
public sealed class ArtifactStoreSecurityTests
{
    ...
    public async Task InternalEndpointsRequireSharedBearerTokenWhileHealthRemainsAnonymous()
    {
        ...
        await using var server = await ArtifactStoreProcess.StartAsync(TestContext.Current.CancellationToken, internalServiceToken: token);
        using var anonymousClient = new HttpClient { BaseAddress = server.HttpClient.BaseAddress, Timeout = server.HttpClient.Timeout };

        using var health = await anonymousClient.GetAsync("/health/ready", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.OK, health.StatusCode);

        using var unauthorized = await anonymousClient.GetAsync("/api/v1/artifacts/status", TestContext.Current.CancellationToken);
        ...
        using var authorized = await anonymousClient.SendAsync(authorizedRequest, TestContext.Current.CancellationToken);
    ...
    public async Task DigestMismatchAndTraversalManifestAreRejectedWithoutPublishing()
    {
        ...
        using (var put = new HttpRequestMessage(HttpMethod.Put, $"/internal/v1/contents/sha256/{wrongDigest}?TtlSeconds=60")
        {
        ...
        var unsafeManifest = safeManifest with { ArtifactId = new ArtifactRef($"sha256:{new string('0', ArtifactStoreProtocol.Sha256HexLength)}"), EntryAssembly = "../app.dll", Files = [safeManifest.Files[0] with { Path = "../app.dll" }] };
        using var form = new MultipartFormDataContent();
        form.Add(new StringContent(JsonSerializer.Serialize(unsafeManifest, ContractJson.CreateSerializerOptions()), Encoding.UTF8, "application/json"), ArtifactStoreProtocol.ManifestPartName);
        var file = new ByteArrayContent(bytes);
        file.Headers.ContentType = new MediaTypeHeaderValue("application/octet-stream");
        form.Add(file, ArtifactStoreProtocol.FilesPartName, "../app.dll");
        using var traversalResponse = await server.HttpClient.PutAsync($"/internal/v1/artifacts/sha256/{new string('0', ArtifactStoreProtocol.Sha256HexLength)}", form, TestContext.Current.CancellationToken);
    ...
    public async Task LegacyLowerCamelMultipartNamesAreRejected()
    {
        ...
        using var form = new MultipartFormDataContent();
        form.Add(new StringContent(JsonSerializer.Serialize(manifest, ContractJson.CreateSerializerOptions()), Encoding.UTF8, "application/json"), "manifest");
        var file = new ByteArrayContent(bytes);
        file.Headers.ContentType = new MediaTypeHeaderValue("application/octet-stream");
        form.Add(file, "files", "app.dll");

        using var response = await server.HttpClient.PutAsync($"/internal/v1/artifacts/sha256/{ArtifactStoreProtocol.GetDigest(manifest.ArtifactId)}", form, TestContext.Current.CancellationToken);
    ...
    public async Task CorruptedOrSymlinkedCasContentIsNeverReturned()
    {
        ...
        _ = await server.Client.PutContentAsync(contentRef, new MemoryStream(bytes, writable: false), bytes.LongLength, TimeSpan.FromMinutes(1), TestContext.Current.CancellationToken);
        ...
        using (var corrupted = await server.HttpClient.GetAsync($"/internal/v1/contents/sha256/{digest}", TestContext.Current.CancellationToken))
        {
        ...
        using var symlinked = await server.HttpClient.GetAsync($"/internal/v1/contents/sha256/{digest}", TestContext.Current.CancellationToken);
