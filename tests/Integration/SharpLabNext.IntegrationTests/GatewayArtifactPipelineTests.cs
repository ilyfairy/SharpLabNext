...
public sealed class GatewayArtifactPipelineTests
{
    ...
    public async Task ExecutionFlowAcceptsItsIntermediateInstrumentationTransform()
    {
        ...
        var request = new TransformArtifactRequest("artifact-transform-request", "artifact-transform-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "runtime-instrumentation-v1", new TransformArtifactOptions(RewriterProfileId: "execution-flow-v1"), DateTimeOffset.UtcNow.AddMinutes(1));
        ...
        var typed = Assert.IsType<TypedResultOperationEventPayload>(Assert.Single(events, item => item.Payload is TypedResultOperationEventPayload).Payload);
    ...
    public async Task ThirdPartyArtifactTransformRoutesToResolvedProcessorWorker()
    {
        ...
        await using var factory = new GatewayArtifactTestFactory(worker);
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "run", languageId: "minilang", toolchainId: "minilang-stable", runtimeId: "dotnet-10-linux-x64");
        var transform = Assert.Single(resolution.PipelinePlan.Stages, static stage => stage.Kind == PipelineStageKind.Transform);
        var request = new TransformArtifactRequest("minilang-assemble-request", "minilang-assemble-key", resolution.PipelineResolutionId, Artifact(), transform.ProviderId, transform.Id, new TransformArtifactOptions(), DateTimeOffset.UtcNow.AddMinutes(1));
    ...
    public async Task InstrumentationTransformRejectsAnUnpinnedRewriterProfile()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "execution-flow");
        var request = new TransformArtifactRequest("artifact-transform-invalid", "artifact-transform-invalid-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "runtime-instrumentation-v1", new TransformArtifactOptions(RewriterProfileId: "arbitrary"), DateTimeOffset.UtcNow.AddMinutes(1));

        using var response = await client.PostAsJsonAsync("/api/v1/artifact-transforms", request, ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
    ...
    public async Task RenderRelaysTypedContentResultAndTerminalEvent()
    {
        ...
        var request = new RenderArtifactRequest("artifact-render-request", "artifact-render-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "il", new RenderArtifactOptions(), DateTimeOffset.UtcNow.AddMinutes(1));
        ...
        var content = Assert.IsType<ContentProducedOperationEventPayload>(Assert.Single(events, item => item.Payload is ContentProducedOperationEventPayload).Payload);
        ...
        Assert.DoesNotContain(events, item => item.Payload is AcceptedOperationEventPayload accepted && accepted.RequestId == FakeArtifactWorkerClient.RemoteRequestId);
    }
    ...
    public async Task TerminalStateObservedBetweenEventPollsStillRelaysFinalEvents()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "il");
        var request = new RenderArtifactRequest("artifact-render-race-request", "artifact-render-race-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "il", new RenderArtifactOptions(), DateTimeOffset.UtcNow.AddMinutes(1));
    ...
    public async Task VerificationRequiresTheResolvedVerificationStage()
    {
        ...
        using var rejected = await client.PostAsJsonAsync("/api/v1/verifications", request, ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
    ...
    public async Task ClientCancellationCancelsTheRemoteArtifactOperation()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "il");
        var request = new RenderArtifactRequest("artifact-cancel-request", "artifact-cancel-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "il", new RenderArtifactOptions(), DateTimeOffset.UtcNow.AddMinutes(1));
        var handle = await StartAsync(client, "/api/v1/artifact-renders", request);
        await worker.WatchStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);

        using var cancel = await client.PostAsJsonAsync($"/api/v1/operations/{handle.OperationId}/cancel", new CancelOperationRequest(handle.OperationId, "test"), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
    ...
    public async Task UnexpectedRemoteResultFailsTheGatewayOperation()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "il");
        var request = new RenderArtifactRequest("artifact-wrong-result", "artifact-wrong-result-key", resolution.PipelineResolutionId, Artifact(), "artifacts-default", "il", new RenderArtifactOptions(), DateTimeOffset.UtcNow.AddMinutes(1));
    ...
    private static VerifyArtifactRequest VerificationRequest(string pipelineId) => new("artifact-verify-request", "artifact-verify-key", pipelineId, Artifact(), "artifacts-default", new VerifyArtifactOptions("net10-default"), DateTimeOffset.UtcNow.AddMinutes(1));

    private static ArtifactRef Artifact() => new($"sha256:{new string('a', 64)}");

    private static async Task<ResolveSelectionResponse> ResolveAsync(HttpClient client, string outputId, string languageId = "csharp", string toolchainId = "roslyn-stable", string? runtimeId = null)
    {
        var catalogRevision = await GatewayTestCatalog.GetRevisionAsync(client);
        using var response = await client.PostAsJsonAsync("/api/v1/selections/resolve", new ResolveSelectionRequest(languageId, toolchainId, "net10-ref", outputId, runtimeId, BuildConfiguration.Release, catalogRevision, 1), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
...
internal sealed class GatewayArtifactTestFactory(FakeArtifactWorkerClient worker) : WebApplicationFactory<Program>
{
...
internal sealed class FakeArtifactWorkerClient(FakeArtifactScenario scenario) : IArtifactWorkerClient, IArtifactWorkerClientFactory
{
    ...
    public Task<OperationHandle> StartTransformAsync(TransformArtifactRequest request, CancellationToken cancellationToken = default)
    {
    ...
    public Task<OperationHandle> StartRenderAsync(RenderArtifactRequest request, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        RenderStartCount++;
        _requestId = request.RequestId;
    ...
    public Task<OperationHandle> StartVerifyAsync(VerifyArtifactRequest request, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        VerifyStartCount++;
        _requestId = request.RequestId;
    ...
    public Task<OperationState?> GetOperationAsync(string operationId, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var terminal = scenario == FakeArtifactScenario.TerminalSnapshotBeforeFinalEvents &&
            Volatile.Read(ref _eventPollCount) >= 2;
        return Task.FromResult<OperationState?>(new OperationState(operationId, _requestId ?? string.Empty, _kind, terminal ? OperationStatus.Completed : OperationStatus.Running, terminal ? 5 : 2, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, terminal ? DateTimeOffset.UtcNow : null, "remote-artifact-trace", null));
    }
    ...
    public async Task<IReadOnlyList<OperationEvent>> GetEventsAsync(string operationId, long fromSequence, CancellationToken cancellationToken = default)
    {
        ...
        var events = new List<OperationEvent> { Event(1, new AcceptedOperationEventPayload(requestId, _kind)), Event(2, new ProgressOperationEventPayload("processor", null, 0.5)) };
        ...
        {
            events.Add(Event(3, new ArtifactProducedOperationEventPayload(DerivedArtifactRef, "dotnet-managed-pe-v1", "runtime-instrumented")));
            events.Add(Event(4, new TypedResultOperationEventPayload(new TransformArtifactResult(ArtifactJobOutcome.Succeeded, DerivedArtifactRef, new ArtifactRef($"sha256:{new string('a', 64)}"), "dotnet-managed-pe-v1", []))));
            events.Add(Event(5, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, TimeSpan.FromMilliseconds(10))));
        }
        else if (scenario is FakeArtifactScenario.CompletedRender or FakeArtifactScenario.TerminalSnapshotBeforeFinalEvents)
        {
            ...
            events.Add(Event(4, new TypedResultOperationEventPayload(new RenderArtifactResult(ArtifactJobOutcome.Succeeded, ContentRef, "text/plain", [], []))));
            events.Add(Event(5, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, TimeSpan.FromMilliseconds(10))));
        }
        else if (scenario == FakeArtifactScenario.CompletedVerification)
        {
            events.Add(Event(3, new TypedResultOperationEventPayload(new VerifyArtifactResult(ArtifactVerificationOutcome.Findings, [new VerificationFinding("stack-unbalanced", "Stack is unbalanced.", null, null, null, null, null)], "microsoft-ilverification", "10.0.9"))));
            events.Add(Event(4, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, TimeSpan.FromMilliseconds(10))));
        }
        else
        {
            events.Add(Event(3, new TypedResultOperationEventPayload(new VerifyArtifactResult(ArtifactVerificationOutcome.Valid, [], "microsoft-ilverification", "10.0.9"))));
            events.Add(Event(4, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, TimeSpan.FromMilliseconds(10))));
        }
        return events.Where(item => item.Sequence > fromSequence).ToArray();
    }
    ...
    private static OperationEvent Event(long sequence, OperationEventPayload payload) => new(RemoteOperationId, sequence, DateTimeOffset.UtcNow, "remote-artifact-trace", payload);
}
