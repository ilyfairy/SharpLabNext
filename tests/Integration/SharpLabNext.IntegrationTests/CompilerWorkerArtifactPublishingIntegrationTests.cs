...
public sealed class CompilerWorkerArtifactPublishingIntegrationTests
{
    ...
    public async Task FSharpWorkerPublishesEntrySymbolsAndFSharpCoreDirectly()
    {
        ...
        try
        {
            ...
            await using var worker = await StartWorkerAsync("src/Workers/FSharp/SharpLabNext.Worker.FSharp/SharpLabNext.Worker.FSharp.csproj", workerEnvironment);
            var request = CreateRequest("fsharp", "fsharp-stable", "Program.fs", "module Program\nopen System\n[<EntryPoint>]\nlet main _ = Console.WriteLine(42); 0\n", NullableContextMode.Disable, "9.0");
            ...
            Assert.Equal(["portable-pdb", "primary-assembly", "support-assembly"], descriptor.Entries.Select(static entry => entry.Role).Order(StringComparer.Ordinal));
            Assert.Contains(descriptor.Entries, static entry => entry.Path == "FSharp.Core.dll" && entry.Role == "support-assembly");
        }
        finally
        {
    ...
    public async Task GatewayPublisherAcceptsPeachPieManagedPeAndSupportClosure()
    {
        ...
        try
        {
            ...
            await using var worker = await StartWorkerAsync("src/Workers/PeachPie/SharpLabNext.Worker.PeachPie/SharpLabNext.Worker.PeachPie.csproj", workerEnvironment);
            var request = CreateRequest("php", "peachpie-stable", "index.php", "<?php function square($value) { return $value * $value; } echo square(7);", NullableContextMode.Disable, "8.5");
            ...
            var artifactRef = Assert.IsType<ArtifactRef>(result.ArtifactRef);
            var published = await new BuildArtifactPublisher(artifactStore.Client, new BuildPipelineOptions()).PublishAsync(envelope, TestContext.Current.CancellationToken);
            ...
            Assert.Contains(descriptor.Entries, static entry => entry.Path == "Peachpie.Runtime.dll" && entry.Role == "support-assembly");
            Assert.Contains(descriptor.Entries, static entry => entry.Path == "Peachpie.Library.dll" && entry.Role == "support-assembly");
            var primary = Assert.Single(descriptor.Entries, static entry => entry.Role == "primary-assembly");
            await using var content = await artifactStore.Client.OpenContentReadAsync(primary.ContentRef, TestContext.Current.CancellationToken);
    ...
    public async Task IlWorkerPublishesManagedPeDirectly()
    {
        ...
        try
        {
            ...
            referenceSets.AddToEnvironment(workerEnvironment, "net10-ref", "net11-preview-ref");
            await using var worker = await StartWorkerAsync("src/Workers/IL/SharpLabNext.Worker.IL/SharpLabNext.Worker.IL.csproj", workerEnvironment);
            ...
            await using var content = await artifactStore.Client.OpenContentReadAsync(entry.ContentRef, TestContext.Current.CancellationToken);
            var header = new byte[2];
            await content.Content.ReadExactlyAsync(header, TestContext.Current.CancellationToken);
            Assert.Equal([0x4d, 0x5a], header);
        }
    ...
    private static Task<DotNetWebServiceProcess> StartWorkerAsync(string project, IReadOnlyDictionary<string, string?> environment)
    {
        var allEnvironment = new Dictionary<string, string?>(environment, StringComparer.Ordinal)
        {
            ["ASPNETCORE_ENVIRONMENT"] = "Production"
        };
        return DotNetWebServiceProcess.StartAsync(project, "/health/ready", allEnvironment, TestContext.Current.CancellationToken, noBuild: false, internalServiceToken: InternalServiceToken);
    }

    private static async Task<ToolchainBuildResponse> BuildAsync(HttpClient client, BuildRequest request, bool expectDevelopmentArtifact = false)
    {
        ...
        return JsonSerializer.Deserialize<ToolchainBuildResponse>(body, ContractJson.CreateSerializerOptions()) ?? throw new InvalidOperationException("Worker build response was empty.");
    }

    private static BuildRequest CreateRequest(string languageId, string toolchainId, string fileName, string source, NullableContextMode nullableContext, string languageVersion)
    {
        var options = new BuildOptions(BuildConfiguration.Release, Optimize: true, BuildOutputKind.Console, AllowUnsafe: false, EmitPortablePdb: true, nullableContext, languageVersion);
        var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, Revision: 94, SelectionRevision: 13, languageId, [new WorkspaceFile(fileName, 1, source)], fileName, [fileName], "net10-ref", options);
        return new BuildRequest($"direct-{languageId}", $"direct-{languageId}-key", $"direct-{languageId}-pipeline", toolchainId, "net10-ref", workspace, DateTimeOffset.UtcNow.AddMinutes(1), options, BuildTarget.Artifact);
    }
