...
public sealed class GatewayRuntimePipelineTests
{
    ...
    public async Task RunForwardsPayloadsWithGatewayIdentityAndPreservesBase64Output()
    {
        ...
        var output = Assert.IsType<OutputChunkOperationEventPayload>(Assert.Single(
            events,
            operationEvent => operationEvent.Payload is OutputChunkOperationEventPayload).Payload);
        Assert.Equal(supervisor.EncodedOutput, output.Chunk.Data);
        Assert.IsType<RunResult>(Assert.IsType<TypedResultOperationEventPayload>(Assert.Single(
            events,
            operationEvent => operationEvent.Payload is TypedResultOperationEventPayload).Payload).Result);
    ...
    public async Task JitEndpointForwardsTypedResultAndContent()
    {
        ...
        var jit = Assert.IsType<JitResult>(Assert.IsType<TypedResultOperationEventPayload>(Assert.Single(
            events,
            operationEvent => operationEvent.Payload is TypedResultOperationEventPayload).Payload).Result);
    ...
    public async Task JSharpRunResolvesAndForwardsTheDedicatedClr2RuntimeContract()
    {
        ...
        var request = new RunRequest(
            "gateway-jsharp-run",
            "gateway-jsharp-run-key",
            resolution.PipelineResolutionId,
            Artifact(),
            "wine-jsharp20-linux-x64",
            new RunOptions([], null, RunInstrumentation.None, "runtime-job-wine-jsharp20"),
        ...
        Assert.Equal(
            ...
            supervisor.LastRunRequest?.Options.SecurityPolicyId);
    }
    ...
    public async Task JSharpUnsupportedOutputsAreRejectedBeforeRemoteRuntimeCalls(
        string outputId,
        string? runtimeId)
    {
        var supervisor = new FakeRuntimeSupervisorClient(FakeRuntimeScenario.CompletedRun);
    ...
    public async Task OperationWebSocketReusesRuntimeSessionAcrossWorkspaceRevisionsAndReleasesOnDisconnect()
    {
        ...
        using var socket = await webSocketClient.ConnectAsync(
            ...
            TestContext.Current.CancellationToken);
        var selection = new ResolveSelectionRequest(
            "csharp",
            "roslyn-stable",
            "net10-ref",
            "run",
        ...
        var firstHandle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(firstResolution.PipelineResolutionId, "session-run-1", "session-run-key-1"),
            "start-session-1");
        ...
        var secondHandle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(secondResolution.PipelineResolutionId, "session-run-2", "session-run-key-2"),
            "start-session-2");
        ...
        await socket.CloseOutputAsync(
            ...
            "runtime session test complete",
            TestContext.Current.CancellationToken);
        var releasedSessionId = await supervisor.SessionReleased.Task.WaitAsync(
    ...
    public async Task OperationWebSocketRejectsRuntimeStartForAnOlderValidResolution()
    {
        ...
        var firstHandle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(firstResolution.PipelineResolutionId, "current-run-1", "current-run-key-1"),
            "start-current-1");
        _ = await WaitForTerminalAsync(client, firstHandle.OperationId);
        var secondResolution = await ResolveOverWebSocketAsync(
            socket,
        ...
        {
            type = "start",
            commandId = "start-stale-resolution",
            operation = "run",
            request = CreateRunRequest(
                firstResolution.PipelineResolutionId,
                "stale-resolution-run",
                "stale-resolution-run-key")
        });
        using (var response = await ReceiveWebSocketJsonAsync(socket))
        {
            ...
            Assert.Equal(
                ...
                response.RootElement.GetProperty("Error").GetProperty("Error").GetString());
        }
        Assert.Equal(1, supervisor.RunStartCount);
        ...
        var secondHandle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(secondResolution.PipelineResolutionId, "current-run-2", "current-run-key-2"),
            "start-current-2");
        _ = await WaitForTerminalAsync(client, secondHandle.OperationId);
        ...
        await socket.CloseOutputAsync(
            WebSocketCloseStatus.NormalClosure,
            "resolution binding test complete",
            TestContext.Current.CancellationToken);
        _ = await supervisor.SessionReleased.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
    }
    ...
    public async Task OperationWebSocketCancelsRuntimeAndReleasesSessionWhenRuntimeSelectionChanges()
    {
        ...
        using var socket = await webSocketClient.ConnectAsync(
            new Uri("ws://localhost/api/v1/operations/ws"),
            TestContext.Current.CancellationToken);
        var firstSelection = new ResolveSelectionRequest(
            "csharp",
            "roslyn-stable",
            "net10-ref",
            "run",
        ...
        var runHandle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(firstResolution.PipelineResolutionId, "runtime-change-run", "runtime-change-run-key"),
            "start-runtime-change");
        await supervisor.WatchStarted.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
        ...
        var secondResolution = await ResolveOverWebSocketAsync(
            ...
            firstSelection with
            {
                ...
                RuntimeId = "dotnet-11-preview-linux-x64",
                WorkspaceRevision = 2
            },
            "resolve-runtime-2");
        ...
        var secondHandle = await StartOverWebSocketAsync(
            ...
            "run",
            CreateRunRequest(
                secondResolution.PipelineResolutionId,
                "runtime-change-run-2",
                "runtime-change-run-key-2") with
            {
                RuntimeProfileId = "dotnet-11-preview-linux-x64"
            },
    ...
    public async Task OperationWebSocketCancelsActiveRuntimeAndReleasesSessionOnDisconnect(
        string operation,
        string outputId)
    {
        var supervisor = new FakeRuntimeSupervisorClient(FakeRuntimeScenario.CancelledRun);
        ...
        using var socket = await webSocketClient.ConnectAsync(
            new Uri("ws://localhost/api/v1/operations/ws"),
            TestContext.Current.CancellationToken);
        var resolution = await ResolveOverWebSocketAsync(
            socket,
            new ResolveSelectionRequest(
                "csharp",
                "roslyn-stable",
        ...
        var operationHandle = operation == "run"
            ? await StartOverWebSocketAsync(
                socket,
                operation,
                CreateRunRequest(resolution.PipelineResolutionId, "disconnect-run", "disconnect-run-key"),
                "start-disconnect")
            : await StartOverWebSocketAsync(
                socket,
                operation,
                CreateJitRequest(resolution.PipelineResolutionId, "disconnect-jit", "disconnect-jit-key"),
                "start-disconnect");
        await supervisor.WatchStarted.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
        ...
        Assert.Equal(runtimeSessionId, await supervisor.SessionReleased.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken));
        Assert.Equal(1, supervisor.ReleaseCount);
    }
    ...
    public async Task OperationWebSocketAbortCancelsRuntimeAndReleasesSession()
    {
        ...
        var resolution = await ResolveOverWebSocketAsync(
            ...
            new ResolveSelectionRequest(
                ...
                1),
            "resolve-abort");
        var handle = await StartOverWebSocketAsync(
            socket,
            "run",
            CreateRunRequest(resolution.PipelineResolutionId, "abort-run", "abort-run-key"),
            "start-abort");
        await supervisor.WatchStarted.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
    ...
    public async Task LocalCancellationIsPropagatedAndEndsCancelled()
    {
        ...
        using var cancelResponse = await client.PostAsJsonAsync(
            ...
            new CancelOperationRequest(handle.OperationId, "test-cancel"),
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        ...
        Assert.Equal(1, supervisor.CancelCount);
        Assert.Equal(FakeRuntimeSupervisorClient.RemoteOperationId, supervisor.CancelledOperationId);
        var result = Assert.IsType<RunResult>(Assert.IsType<TypedResultOperationEventPayload>(Assert.Single(
            events,
            operationEvent => operationEvent.Payload is TypedResultOperationEventPayload).Payload).Result);
    ...
    public async Task RuntimeAndOutputMustMatchResolvedPipeline()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "run", "dotnet-10-linux-x64");
        var wrongRuntime = CreateRunRequest(
            resolution.PipelineResolutionId,
            "wrong-runtime",
            "wrong-runtime-key") with
        {
            RuntimeProfileId = "dotnet-11-preview-linux-x64"
        };
        var wrongOutput = CreateJitRequest(
            resolution.PipelineResolutionId,
            "wrong-output",
            "wrong-output-key");

        using var runtimeResponse = await client.PostAsJsonAsync(
            "/api/v1/runs",
            wrongRuntime,
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        using var outputResponse = await client.PostAsJsonAsync(
            "/api/v1/jits",
            wrongOutput,
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
    ...
    public async Task CompletionWithoutTypedResultBecomesProtocolFailure()
    {
        ...
        using var client = factory.CreateClient();
        var resolution = await ResolveAsync(client, "jit-asm", "dotnet-10-linux-x64");
        var request = CreateJitRequest(
            resolution.PipelineResolutionId,
            "gateway-missing-result",
            "gateway-missing-result-key");
    ...
    public async Task CancellationWhileStartingInterruptsTheSupervisorRequest()
    {
        ...
        using var cancelResponse = await client.PostAsJsonAsync(
            $"/api/v1/operations/{handle.OperationId}/cancel",
            new CancelOperationRequest(handle.OperationId, "cancel-during-start"),
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        cancelResponse.EnsureSuccessStatusCode();
        var state = await WaitForTerminalAsync(client, handle.OperationId);
    ...
    public async Task DispatchProgressPrecedesABlockedSupervisorStart()
    {
        ...
        var request = CreateRunRequest(
            resolution.PipelineResolutionId,
            "gateway-dispatch-progress",
            "gateway-dispatch-progress-key");
        ...
        using var cancelResponse = await client.PostAsJsonAsync(
            $"/api/v1/operations/{handle.OperationId}/cancel",
            new CancelOperationRequest(handle.OperationId, "dispatch-progress-test-complete"),
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        cancelResponse.EnsureSuccessStatusCode();
        Assert.Equal(
            OperationStatus.Cancelled,
            (await WaitForTerminalAsync(client, handle.OperationId)).Status);
    }
    ...
    public async Task CancellationHasAHardBoundaryWhenSupervisorCallsIgnoreTokens()
    {
        ...
        var runtimeOptions = new RuntimePipelineOptions
        {
            MaximumDuration = TimeSpan.FromSeconds(5),
            ControlRequestTimeout = TimeSpan.FromMilliseconds(25),
        ...
        var request = CreateRunRequest(
            resolution.PipelineResolutionId,
            "gateway-unresponsive-cancel",
            "gateway-unresponsive-cancel-key");
        var handle = await StartAsync<RunRequest>(client, "/api/v1/runs", request);
        await supervisor.WatchStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);

        using var cancelResponse = await client.PostAsJsonAsync(
            $"/api/v1/operations/{handle.OperationId}/cancel",
            new CancelOperationRequest(handle.OperationId, "unresponsive-supervisor"),
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        cancelResponse.EnsureSuccessStatusCode();
        var state = await WaitForTerminalAsync(client, handle.OperationId);
    ...
    public async Task RuntimeTimeoutResultCanArriveDuringGatewayDeadlineGrace()
    {
        ...
        {
            MaximumDuration = TimeSpan.FromSeconds(5),
            ControlRequestTimeout = TimeSpan.FromMilliseconds(50),
        ...
        var request = CreateRunRequest(
            resolution.PipelineResolutionId,
            "gateway-runtime-timeout",
            "gateway-runtime-timeout-key") with
        {
    ...
    public async Task GatewayDeadlineFailsBoundedlyWhenSupervisorNeverFinishes()
    {
        ...
        var request = CreateRunRequest(
            resolution.PipelineResolutionId,
            "gateway-hard-deadline",
            "gateway-hard-deadline-key");
    ...
    private static async Task<ResolveSelectionResponse> ResolveOverWebSocketAsync(
        WebSocket socket,
        ...
        string commandId)
    {
        ...
        return response.RootElement.GetProperty("Payload")
            .Deserialize<ResolveSelectionResponse>(ContractJson.CreateSerializerOptions())
            ?? throw new InvalidOperationException("Selection response was empty.");
    }

    private static async Task<OperationHandle> StartOverWebSocketAsync<TRequest>(
        WebSocket socket,
        string operation,
        TRequest request,
        string commandId)
    {
        ...
        return response.RootElement.GetProperty("Payload")
            .Deserialize<OperationHandle>(ContractJson.CreateSerializerOptions())
    ...
    private static async Task<IReadOnlyList<OperationEvent>> ReadEventsAsync(
        HttpClient client,
        string operationId)
    {
    ...
    private static RunRequest CreateRunRequest(string pipelineId, string requestId, string key) => new(
        requestId,
        key,
        pipelineId,
        Artifact(),
    ...
    private static JitRequest CreateJitRequest(string pipelineId, string requestId, string key) => new(
        requestId,
        key,
        pipelineId,
        Artifact(),
...
internal sealed class GatewayRuntimeTestFactory(
    FakeRuntimeSupervisorClient supervisor,
    RuntimePipelineOptions? runtimeOptions = null)
...
internal sealed class FakeRuntimeSupervisorClient(FakeRuntimeScenario scenario) : IRuntimeSupervisorClient
{
    ...
    public Task<OperationHandle> StartRunAsync(
        ...
        => StartRunCoreAsync(request, cancellationToken);
    ...
    private Task<OperationHandle> StartRunCoreAsync(
        RunRequest request,
        CancellationToken cancellationToken)
    {
        ...
        {
            return Task.FromException<OperationHandle>(new RuntimeSupervisorClientException(new WorkerError(
    ...
    public Task<OperationHandle> StartJitAsync(
        JitRequest request,
        CancellationToken cancellationToken = default)
        => StartJitCoreAsync(request, cancellationToken);
    ...
    private Task<OperationHandle> StartJitCoreAsync(
        JitRequest request,
        CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var acceptedKind = scenario == FakeRuntimeScenario.InvalidAccepted
            ? (_kind == OperationKind.Run ? OperationKind.Jit : OperationKind.Run)
            : _kind;
        ...
        {
            yield return Event(90, new CompletedOperationEventPayload(
        ...
        if (scenario == FakeRuntimeScenario.CancelledRun)
        {
            ...
            yield return Event(70, new TypedResultOperationEventPayload(
                _kind == OperationKind.Run
                    ? CancelledRunResult()
                    : CancelledJitResult()));
            yield return Event(90, new CompletedOperationEventPayload(
        ...
        {
            yield return Event(60, new OutputChunkOperationEventPayload(new OutputChunk(
    ...
    private static RunResult CancelledRunResult() => new(
    ...
    private static JitResult CancelledJitResult() => new(
        JitTerminalStatus.Cancelled,
        null,
        null,
        [],
        TimeSpan.FromMilliseconds(10),
    ...
    private static RunResult TimeoutRunResult() => new(
    ...
    private static JitResult CompletedJitResult(ContentRef contentRef) => new(
        ...
        [new JitMethodSummary(
            ...
            [new LinkedRange(
                "Program.cs",
                new TextRange(2, 0, 2, 12),
                new TextRange(4, 0, 4, 18),
                "sequence-point")])],
        TimeSpan.FromMilliseconds(10),
        RuntimeJitIdentity());
    ...
    private static RuntimeIdentity RuntimeIdentity() => new(
        "10.0.9",
        "runtime-commit",
        "runtime-image",
        "linux-x64",
