...
public sealed class GatewayLanguageSessionTests
{
    ...
    public void LanguageWorkerEndpointsAreCatalogBoundAndConfigurationDriven()
    {
        ...
        var registry = LanguageWorkerEndpointRegistry.FromConfiguration(
            configuration,
            "release-test",
            ["roslyn-stable", "fsharp-stable", "gsharp-stable", "mobius-ilasm-stable"],
            defaultServiceToken: "shared-token");
    ...
    public void LanguageWorkerEndpointsRejectWorkersOutsideTheCatalog()
    {
        ...
        var exception = Assert.Throws<InvalidOperationException>(() =>
            LanguageWorkerEndpointRegistry.FromConfiguration(
                configuration,
                "release-test",
                ["roslyn-stable"]));
    ...
    public async Task RealGatewayProxiesCSharpAndVisualBasicAndEnforcesOneShotMessageLimits()
    {
        ...
        await AssertLanguageAsync(
            ...
            "using System;\nclass Demo\n{\n    void Run()\n    {\n        Console.\n    }\n}",
            completionLine: 5,
            completionCharacter: 16,
            expectedDiagnosticPrefix: "CS");
        await AssertLanguageAsync(
            gateway,
            gatewayCatalog,
            "visual-basic",
            ...
            completionLine: 3,
            completionCharacter: 16,
            expectedDiagnosticPrefix: "BC");

        var limitedSession = await OpenSessionAsync(
            gateway.HttpClient,
            gatewayCatalog,
            "csharp",
            "Program.cs",
        ...
        var binarySession = await OpenSessionAsync(
            gateway.HttpClient,
            gatewayCatalog,
            "csharp",
            "Program.cs",
    ...
    private static async Task AssertLanguageAsync(
        DotNetWebServiceProcess gateway,
        ...
        string expectedDiagnosticPrefix)
    {
        ...
        using (var diagnostics = await ReceiveUntilMethodAsync(socket, "textDocument/publishDiagnostics"))
        {
            ...
            Assert.Contains(
                ...
                diagnostic => diagnostic.GetProperty("code").GetString()!
                    .StartsWith(expectedDiagnosticPrefix, StringComparison.Ordinal));
        }
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new
            {
                textDocument = new { uri = documentUri, version = 3 },
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new
            {
                ...
                position = new { line = completionLine, character = completionCharacter },
    ...
    private static async Task<GatewayLanguageSessionResponse> OpenSessionAsync(
        HttpClient client,
        CatalogDocument catalog,
        string languageId,
        string fileName,
        string source)
    {
        var resolve = new ResolveSelectionRequest(
            languageId,
            "roslyn-stable",
            "net10-ref",
            "ast",
        ...
        var request = new OpenLanguageSessionRequest(
            $"gateway-lsp-{languageId}-{Guid.NewGuid():N}",
        ...
        return await response.Content.ReadFromJsonAsync<GatewayLanguageSessionResponse>(
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken)
            ?? throw new InvalidOperationException("Gateway language session response was empty.");
    }
    ...
    private static async Task<JsonDocument> ReceiveUntilMethodAsync(WebSocket socket, string method)
    {
        for (var attempt = 0; attempt < 20; attempt++)
        {
            ...
            if (message.RootElement.TryGetProperty("method", out var actualMethod) &&
                actualMethod.GetString() == method)
            {
                return message;
            }
