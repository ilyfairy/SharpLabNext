...
public sealed class GatewayGistEndpointTests : IClassFixture<GatewayGistTestFactory>
{
    ...
    public async Task PublicAndPrivateGistsFollowAnonymousAndOAuthReadRules()
    {
        ...
        var publicGist = await gists.CreateAsync(
            new CreateGistRequest("public", true, Workspace()),
            owner,
            TestContext.Current.CancellationToken);
        var privateGist = await gists.CreateAsync(
            new CreateGistRequest("private", false, Workspace()),
            owner,
            TestContext.Current.CancellationToken);
        ...
        using var publicResponse = await anonymous.GetAsync(
            $"/api/v1/shares/gists/{publicGist.Id}",
            TestContext.Current.CancellationToken);
        using var privateResponse = await anonymous.GetAsync(
            $"/api/v1/shares/gists/{privateGist.Id}",
            TestContext.Current.CancellationToken);
        ...
        var loadedPrivate = await authenticated.GetFromJsonAsync<GistDocument>(
            $"/api/v1/shares/gists/{privateGist.Id}",
    ...
    public async Task CreateAndExplicitUpdateRequireSessionCsrfAndDoNotExposeAccessToken()
    {
        ...
        using var createResponse = await authenticated.PostAsJsonAsync(
            "/api/v1/shares/gists",
            request,
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        ...
        var created = System.Text.Json.JsonSerializer.Deserialize<GistDocument>(
            body,
            ContractJson.CreateSerializerOptions());
        ...
        using var updateRequest = new HttpRequestMessage(
            HttpMethod.Patch,
            $"/api/v1/shares/gists/{created.Id}")
        {
            Content = JsonContent.Create(
                new UpdateGistRequest("updated", Workspace() with { OutputId = "ast" }),
                options: ContractJson.CreateSerializerOptions())
        };
        using var updateResponse = await authenticated.SendAsync(
            updateRequest,
            TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.OK, updateResponse.StatusCode);
        var updated = await updateResponse.Content.ReadFromJsonAsync<GistDocument>(
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
        ...
        var statusBody = await authenticated.GetStringAsync(
    ...
    public async Task AnonymousCreateIsRejected()
    {
        ...
        using var response = await client.PostAsJsonAsync(
            "/api/v1/shares/gists",
            new CreateGistRequest("anonymous", false, Workspace()),
            ContractJson.CreateSerializerOptions(),
            TestContext.Current.CancellationToken);
    ...
    private static GistWorkspaceState Workspace() =>
        GistShareServiceTests.Workspace(
            [new GistSourceFile("Program.cs", "System.Console.WriteLine(42);")],
            "Program.cs");
}
...
public sealed class GatewayGistTestFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureTestServices(services =>
        {
            ...
            services.AddSingleton<IGitHubGistClient>(static provider =>
                provider.GetRequiredService<FakeGitHubGistClient>());
        });
    }
}
