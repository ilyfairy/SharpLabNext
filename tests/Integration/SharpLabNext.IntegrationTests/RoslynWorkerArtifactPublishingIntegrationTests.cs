...
public sealed class RoslynWorkerArtifactPublishingIntegrationTests
{
    ...
    public async Task ProductionBuildPublishesDirectlyAndReturnsOnlyArtifactRef()
    {
        ...
        using var response = await worker.HttpClient.PostAsJsonAsync(
            "/api/v1/build",
            request,
        ...
        var build = System.Text.Json.JsonSerializer.Deserialize<ToolchainBuildResponse>(
            responseText,
            ContractJson.CreateSerializerOptions());
        ...
        var primaryAssembly = Assert.Single(
            descriptor.Entries,
            static entry => entry.Role == "primary-assembly");
        await using var content = await artifactStore.Client.OpenContentReadAsync(
            primaryAssembly.ContentRef,
            TestContext.Current.CancellationToken);
    ...
    private static BuildRequest CreateRequest()
    {
        ...
        var workspace = new WorkspaceSnapshot(
            ...
            LanguageId: "csharp",
            Files: [new WorkspaceFile("Program.cs", 1, "System.Console.WriteLine(42);")],
        ...
        return new BuildRequest(
            ...
            "worker-direct-publish-key",
            "worker-direct-publish-pipeline",
            "roslyn-stable",
            "net10-ref",
            workspace,
