...
public sealed class ArtifactStoreIntegrationTests
{
    ...
    {
        var rootPath = Path.Combine(Path.GetTempPath(), "SharpLabNext-ArtifactStoreTests", Guid.NewGuid().ToString("N"));
        ...
        await using (var first = await ArtifactStoreProcess.StartAsync(TestContext.Current.CancellationToken, rootPath, deleteRootOnDispose: false))
        {
            _ = await first.Client.PutArtifactAsync(manifest, [new ArtifactFileUpload("app.dll", new MemoryStream(assembly, writable: false), assembly.LongLength)], TimeSpan.FromMinutes(1), TestContext.Current.CancellationToken);
        }

        await using var second = await ArtifactStoreProcess.StartAsync(TestContext.Current.CancellationToken, rootPath);
        var descriptor = await second.Client.GetArtifactAsync(manifest.ArtifactId, TestContext.Current.CancellationToken);
        Assert.NotNull(descriptor);
        await using var content = await second.Client.OpenArtifactFileReadAsync(manifest.ArtifactId, "app.dll", TestContext.Current.CancellationToken);
    ...
    public async Task ArtifactRoundTripIsContentAddressedAndIdempotent()
    {
        ...
        var manifest = ArtifactStoreTestData.CreateManifest(("app.dll", assembly, "primary-assembly"), ("symbols/app.pdb", symbols, "portable-pdb"));
        ...
        await using (var content = await server.Client.OpenArtifactFileReadAsync(manifest.ArtifactId, "symbols/app.pdb", TestContext.Current.CancellationToken))
        {
            using var copy = new MemoryStream();
            await content.Content.CopyToAsync(copy, TestContext.Current.CancellationToken);
        ...
        var lease = await server.Client.AcquireLeaseAsync(manifest.ArtifactId, "integration-test", TimeSpan.FromSeconds(20), TestContext.Current.CancellationToken);
        var renewed = await server.Client.RenewLeaseAsync(lease.LeaseToken, TimeSpan.FromSeconds(30), TestContext.Current.CancellationToken);
    ...
    public async Task LeaseProtectsExpiredArtifactUntilRelease()
    {
        ...
        var manifest = ArtifactStoreTestData.CreateManifest(("app.dll", assembly, "primary-assembly"));
        _ = await server.Client.PutArtifactAsync(manifest, [new ArtifactFileUpload("app.dll", new MemoryStream(assembly, writable: false), assembly.LongLength)], TimeSpan.FromSeconds(1), TestContext.Current.CancellationToken);
        var lease = await server.Client.AcquireLeaseAsync(manifest.ArtifactId, "ttl-test", TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
        ...
        using var response = await server.HttpClient.GetAsync($"/internal/v1/contents/sha256/{digest}", TestContext.Current.CancellationToken);
