...
public sealed record TestReferenceSet(
    string Id,
...
public static class TestReferenceSets
{
    ...
    private static readonly Lazy<TestReferenceSet> Net10Value = new(() =>
        Resolve("net10-ref", "net10.0", "SHARPLABNEXT_NET10_REF_PATH"));
    private static readonly Lazy<TestReferenceSet> Net11Value = new(() =>
        Resolve("net11-preview-ref", "net11.0", "SHARPLABNEXT_NET11_REF_PATH"));

    public static TestReferenceSet Net10 => Net10Value.Value;

    public static TestReferenceSet Net11 => Net11Value.Value;

    public static TestReferenceSet Resolve(
        string id,
        string targetFramework,
        string explicitPathVariable)
    {
        ...
        var manifestPath = System.IO.Path.Combine(path, ManifestFileName);
        ...
        if (string.IsNullOrWhiteSpace(version))
        {
            throw new InvalidOperationException(
                $"Reference set '{id}' at '{path}' has no attestation and its package version cannot be inferred.");
        }

        return new TestReferenceSet(
            id,
            targetFramework,
            path,
            version,
        ...
        string explicitPathVariable)
    {
        ...
        {
            var materialized = System.IO.Path.Combine(materializedRoot, id);
            if (Directory.Exists(materialized))
                return System.IO.Path.GetFullPath(materialized);
        }
        ...
        {
            var packRoot = System.IO.Path.Combine(root, "packs", "Microsoft.NETCore.App.Ref");
            if (!Directory.Exists(packRoot))
                continue;

            var match = Directory.EnumerateDirectories(packRoot)
                .Select(static path => new DirectoryInfo(path))
                .Where(directory => Directory.Exists(System.IO.Path.Combine(
                    directory.FullName,
                    "ref",
                    targetFramework)))
                .OrderByDescending(static directory => VersionPrefix(directory.Name))
                .ThenByDescending(static directory => directory.Name, StringComparer.Ordinal)
                .FirstOrDefault();
        ...
        throw new InvalidOperationException(
            $"Reference set '{id}' for '{targetFramework}' was not found. " +
            $"Set {MaterializedRootVariable} or {explicitPathVariable}.");
    }
    ...
    private static IEnumerable<string> DotNetRoots()
    {
        ...
        return values
            ...
            .Select(static value => System.IO.Path.GetFullPath(value!))
    ...
    private static TestReferenceSet ReadManifest(
        string manifestPath,
        string path,
        string expectedId,
        string expectedTargetFramework)
    {
            ...
            string.IsNullOrWhiteSpace(digest))
        {
            throw new InvalidDataException(
                $"Reference set manifest '{manifestPath}' does not match '{expectedId}/{expectedTargetFramework}'.");
        }
