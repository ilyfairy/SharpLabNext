...
{
    public static ResolveSelectionResponse Resolve(
        ...
        DateTimeOffset now)
    {
        ...
        var route = ResolveArtifactRoute(
            catalog,
            toolchain,
            referenceSet,
            output,
        ...
        foreach (var transformProcessor in route.Transformations
                     .Select(static transformation => transformation.Processor)
        ...
        var effectiveSelection = new ResolvedSelection(
            language.Id,
            toolchain.Id,
            referenceSet.Id,
            output.Id,
        ...
        var plan = new PipelinePlanDescriptor(
            catalog.ReleaseId,
            toolchain.WorkerId,
            toolchain.WorkerId,
            referenceSet.Id,
            stages,
            runtime?.Id,
            SecurityPolicyId(output, runtime),
            []);
        ...
        return new ResolveSelectionResponse(
            effectiveSelection,
            changes,
            capabilities,
            resolutionId,
    ...
    private static string SecurityPolicyId(OutputManifest output, RuntimeManifest? runtime)
    {
        ...
        if (runtime is not null &&
            string.Equals(runtime.Id, "wine-jsharp20-linux-x64", StringComparison.Ordinal))
        {
    ...
    private static ToolchainManifest ResolveToolchain(
        CatalogDocument catalog,
        ...
        List<SelectionChange> changes)
    {
        ...
        {
            changes.Add(new SelectionChange(
                SelectionField.Toolchain,
                null,
                language.DefaultToolchainId,
                SelectionChangeReason.DefaultApplied,
                $"Selected the default toolchain for {language.DisplayName}."));
        ...
        if (!string.Equals(toolchain.Id, requestedId, StringComparison.Ordinal))
        {
            changes.Add(new SelectionChange(
                ...
                requestedId,
                toolchain.Id,
                SelectionChangeReason.LegacyAliasResolved,
                $"Legacy toolchain alias '{requestedId}' resolved to '{toolchain.Id}'."));
        }
        ...
        changes.Add(new SelectionChange(
            ...
            requestedId,
            fallback.Id,
            SelectionChangeReason.UnsupportedByLanguage,
            $"{toolchain.DisplayName} does not support {language.DisplayName}; selected {fallback.DisplayName}."));
        return fallback;
    }

    private static ReferenceSetManifest ResolveReferenceSet(
        CatalogDocument catalog,
        ...
        List<SelectionChange> changes)
    {
        ...
        {
            changes.Add(new SelectionChange(
                SelectionField.ReferenceSet,
                null,
                effectiveId,
                SelectionChangeReason.DefaultApplied,
                $"Selected the default reference set for {toolchain.DisplayName}."));
        }
        ...
        var hasRule = catalog.Compatibility.Any(rule =>
            rule.Kind == CompatibilityRuleKind.ToolchainReferenceSet &&
            rule.Allowed &&
            string.Equals(rule.FromId, toolchain.Id, StringComparison.Ordinal) &&
            string.Equals(rule.ToId, effectiveId, StringComparison.Ordinal));
        ...
        if (requestedId is not null)
        {
            changes.Add(new SelectionChange(
                ...
                requestedId,
                fallback.Id,
                SelectionChangeReason.IncompatibleReferenceSet,
                $"Reference set '{requestedId}' is not compatible with {toolchain.DisplayName}; selected {fallback.DisplayName}."));
        }

        return fallback;
    }

    private static ArtifactRoute ResolveArtifactRoute(
        CatalogDocument catalog,
        ToolchainManifest toolchain,
        ...
        List<SelectionChange> changes)
    {
        ...
        {
            throw SelectionResolutionException.Incompatible(
                SelectionField.Output,
                output.Id,
                $"Toolchain '{toolchain.Id}' does not declare an artifact format.");
        }
        ...
        if (!output.RequiresRuntime && requestedRuntimeId is not null)
        {
            changes.Add(new SelectionChange(
                ...
                requestedRuntimeId,
                null,
                SelectionChangeReason.RuntimeNotRequired,
                $"Output '{output.DisplayName}' does not use a runtime."));
        }
        ...
        return new ArtifactRoute(
            toolchain.ProducesArtifactFormats[0],
            toolchain.ProducesArtifactFormats[0],
            [],
            null,
            null);
    ...
    private static ArtifactRoute ResolveProcessorRoute(
        CatalogDocument catalog,
        ToolchainManifest toolchain,
        ReferenceSetManifest referenceSet,
        OutputManifest output)
    {
        ...
        {
            if (!processor.Capabilities.Contains(output.Id, StringComparer.Ordinal) ||
                !ContainsAll(processor.AcceptedMetadataFeatureTags, requiredTags))
            {
                continue;
            }

            var targetFormats = processor.AcceptsArtifactFormats.Where(format =>
                output.AcceptedArtifactFormats.Count == 0 ||
                output.AcceptedArtifactFormats.Contains(format, StringComparer.Ordinal));
            foreach (var targetFormat in targetFormats)
            {
                if (!HasProcessorCompatibility(catalog, targetFormat, processor, requiredTags))
                    continue;
                var conversion = FindConversionRoute(
                    catalog,
                    toolchain.ProducesArtifactFormats,
                    [targetFormat],
                    requiredTags);
        ...
        if (best is null)
        {
            throw SelectionResolutionException.Incompatible(
                SelectionField.Output,
                output.Id,
                $"No approved artifact route provides output '{output.Id}' for toolchain '{toolchain.Id}'.");
        }

        return new ArtifactRoute(
            best.Conversion.SourceFormat,
            best.Conversion.FinalFormat,
            best.Conversion.Transformations,
            best.Processor,
            null);
    }

    private static ArtifactRoute ResolveRuntimeRoute(
        CatalogDocument catalog,
        ToolchainManifest toolchain,
        ReferenceSetManifest referenceSet,
        OutputManifest output,
        ...
        List<SelectionChange> changes)
    {
        ...
        {
            var requested = FindByIdOrAlias(
                catalog.Runtimes,
                requestedId,
                static item => item.Id,
                static item => item.LegacyAliases)
                ?? throw SelectionResolutionException.Unknown(SelectionField.Runtime, requestedId);
            ...
            if (!string.Equals(requested.Id, requestedId, StringComparison.Ordinal))
            {
                changes.Add(new SelectionChange(
                    ...
                    requested.Id,
                    SelectionChangeReason.LegacyAliasResolved,
                    $"Legacy runtime alias '{requestedId}' resolved to '{requested.Id}'."));
            }
        }
        ...
        foreach (var runtime in candidates)
        {
            ...
            {
                if (output.AcceptedArtifactFormats.Count > 0 &&
                    !output.AcceptedArtifactFormats.Contains(targetFormat, StringComparer.Ordinal))
                {
                    continue;
                }
                ...
                if (NeedsArtifactProcessor(catalog, output))
                {
                    processor = FindOutputProcessorForFormat(
                        catalog,
                        output,
                        targetFormat,
                        requiredTags);
                ...
                if (bestConversion is null ||
                    conversion.Transformations.Count < bestConversion.Transformations.Count)
                {
            ...
            if (requestedId is null)
            {
                changes.Add(new SelectionChange(
                    ...
                    runtime.Id,
                    SelectionChangeReason.DefaultApplied,
                    $"Selected compatible runtime {runtime.DisplayName}."));
            }
            return new ArtifactRoute(
                bestConversion.SourceFormat,
                bestConversion.FinalFormat,
                bestConversion.Transformations,
                bestProcessor,
                runtime);
        }
        ...
        throw SelectionResolutionException.Incompatible(
            ...
            requestedId,
            requestedId is null
                ? "No compatible runtime and artifact conversion route is registered."
                : $"Runtime '{requestedId}' is not compatible with toolchain '{toolchain.Id}' and reference set '{referenceSet.Id}'.");
    }

    private static ArtifactProcessorManifest? FindOutputProcessorForFormat(
        CatalogDocument catalog,
        OutputManifest output,
        string artifactFormat,
        IReadOnlyList<string> requiredTags) =>
        catalog.ArtifactProcessors.FirstOrDefault(processor =>
            processor.Capabilities.Contains(output.Id, StringComparer.Ordinal) &&
            processor.AcceptsArtifactFormats.Contains(artifactFormat, StringComparer.Ordinal) &&
            ContainsAll(processor.AcceptedMetadataFeatureTags, requiredTags) &&
            HasProcessorCompatibility(catalog, artifactFormat, processor, requiredTags));

    private static ConversionRoute? FindConversionRoute(
        CatalogDocument catalog,
        ...
        IReadOnlyList<string> requiredTags)
    {
        ...
        while (queue.TryDequeue(out var current))
        {
            foreach (var processor in catalog.ArtifactProcessors)
            {
                if (!ContainsAll(processor.AcceptedMetadataFeatureTags, requiredTags) ||
                    !HasProcessorCompatibility(catalog, current.FinalFormat, processor, requiredTags))
                {
                    continue;
                }
                ...
                {
                    if (!string.Equals(
                            transformation.InputArtifactFormat,
                            current.FinalFormat,
                            StringComparison.Ordinal) ||
                        !visited.Add(transformation.OutputArtifactFormat))
                    {
                        continue;
                    }
                    var next = new ConversionRoute(
                        current.SourceFormat,
                        transformation.OutputArtifactFormat,
                        [.. current.Transformations, new ArtifactTransformStep(processor, transformation)]);
    ...
    private static bool HasProcessorCompatibility(
        CatalogDocument catalog,
        string artifactFormat,
        ArtifactProcessorManifest processor,
        IReadOnlyList<string> artifactMetadataTags) =>
        catalog.Compatibility.Any(rule =>
            rule.Kind == CompatibilityRuleKind.ArtifactProcessor &&
            rule.Allowed &&
            string.Equals(rule.FromId, artifactFormat, StringComparison.Ordinal) &&
            string.Equals(rule.ToId, processor.Id, StringComparison.Ordinal) &&
            ContainsAll(artifactMetadataTags, rule.RequiredMetadataFeatureTags));

    private static string[] RequiredMetadataTags(
        ToolchainManifest toolchain,
        ReferenceSetManifest referenceSet) =>
        toolchain.MetadataFeatureTags
            .Concat(referenceSet.MetadataFeatureTags)
        ...
        RuntimeManifest runtime)
    {
        ...
        var hasEdge = catalog.Compatibility.Any(rule =>
            rule.Kind == CompatibilityRuleKind.ArtifactRuntime &&
            rule.Allowed &&
            string.Equals(rule.FromId, artifactFormat, StringComparison.Ordinal) &&
            string.Equals(rule.ToId, runtime.Id, StringComparison.Ordinal) &&
            ContainsAll(artifactMetadataTags, rule.RequiredMetadataFeatureTags) &&
        ...
        ArtifactRoute route)
    {
        foreach (var capability in output.RequiredCapabilities)
        {
            if (capability == "explain" &&
                (!language.Capabilities.Contains(capability, StringComparer.Ordinal) ||
                 !toolchain.Capabilities.Contains(capability, StringComparer.Ordinal)))
            {
                throw SelectionResolutionException.Incompatible(
                    SelectionField.Output,
                    output.Id,
                    $"Language '{language.Id}' and toolchain '{toolchain.Id}' do not jointly provide capability '{capability}'.");
            }
            if (language.Capabilities.Contains(capability, StringComparer.Ordinal) ||
                toolchain.Capabilities.Contains(capability, StringComparer.Ordinal) ||
                processor?.Capabilities.Contains(capability, StringComparer.Ordinal) == true ||
                (capability == "managed-pe" &&
                 string.Equals(route.FinalFormat, "dotnet-managed-pe-v1", StringComparison.Ordinal)) ||
                capability is "run" or "jit-asm")
            {
                continue;
            }
            ...
            throw SelectionResolutionException.Incompatible(
                SelectionField.Output,
                output.Id,
                $"Toolchain '{toolchain.Id}' does not provide capability '{capability}'.");
        }
    }

    private static List<PipelineStageDescriptor> BuildStages(
        ToolchainManifest toolchain,
        OutputManifest output,
        ArtifactRoute route)
    {
        if (output.Id == "explain")
        {
            ...
            [
                new PipelineStageDescriptor(
                    "explain",
                    PipelineStageKind.Explain,
                    toolchain.WorkerId,
                    null,
                    "explanation-document-v1")
            ];
        }

        var stages = new List<PipelineStageDescriptor>
        {
            new("build", PipelineStageKind.Build, toolchain.WorkerId, null, route.SourceFormat)
        };
        foreach (var step in route.Transformations)
        {
            stages.Add(new PipelineStageDescriptor(
                step.Transformation.Id,
                PipelineStageKind.Transform,
                step.Processor.WorkerId,
                step.Transformation.InputArtifactFormat,
                step.Transformation.OutputArtifactFormat));
        }
        ...
        if (output.Id is "execution-flow" or "run-il")
        {
            ...
            var instrumentation = processor.Transformations.SingleOrDefault(transformation =>
                string.Equals(transformation.Id, "runtime-instrumentation-v1", StringComparison.Ordinal) &&
                string.Equals(transformation.InputArtifactFormat, artifactFormat, StringComparison.Ordinal));
            ...
            stages.Add(new PipelineStageDescriptor(
                ...
                PipelineStageKind.Transform,
                processor.WorkerId,
                instrumentation.InputArtifactFormat,
                instrumentation.OutputArtifactFormat));
    ...
    private static EffectiveCapabilities CreateCapabilities(
        CatalogDocument catalog,
        LanguageManifest language,
        ToolchainManifest toolchain,
        ReferenceSetManifest referenceSet,
        RuntimeManifest? runtime)
    {
        var outputIds = catalog.Outputs
            .Where(output => CanOfferOutput(catalog, language, toolchain, referenceSet, output))
            .Select(static output => output.Id)
            .ToArray();
        return new EffectiveCapabilities(
            language.Capabilities.Where(capability => capability is not ("ast" or "multi-file" or "source-order")).ToArray(),
            toolchain.Capabilities.ToArray(),
            outputIds,
            runtime?.Capabilities.ToArray() ?? []);
    }

    private static bool CanOfferOutput(
        CatalogDocument catalog,
        LanguageManifest language,
        ToolchainManifest toolchain,
        ReferenceSetManifest referenceSet,
        OutputManifest output)
    {
        if (output.RequiredCapabilities.Contains("explain", StringComparer.Ordinal) &&
            (!language.Capabilities.Contains("explain", StringComparer.Ordinal) ||
             !toolchain.Capabilities.Contains("explain", StringComparer.Ordinal)))
        {
            return false;
        }
        try
        ...
        List<SelectionChange> changes)
    {
        ...
        changes.Add(new SelectionChange(
            ...
            availability.Reason ?? $"Profile '{id}' is not available in this release."));
    }
    ...
    private static T FindRequired<T>(
        ...
        SelectionField field)
    {
        return items.FirstOrDefault(item => string.Equals(getId(item), id, StringComparison.Ordinal))
            ?? throw SelectionResolutionException.Unknown(field, id);
    }

    private static T? FindByIdOrAlias<T>(
        IEnumerable<T> items,
        string id,
        Func<T, string> getId,
        ...
        where T : class
    {
        return items.FirstOrDefault(item =>
            string.Equals(getId(item), id, StringComparison.Ordinal) ||
            getAliases(item).Contains(id, StringComparer.Ordinal));
    }
    ...
    private static bool NeedsArtifactProcessor(CatalogDocument catalog, OutputManifest output) =>
        catalog.ArtifactProcessors.Any(processor =>
            processor.Capabilities.Contains(output.Id, StringComparer.Ordinal));
        ...
        IReadOnlyList<PipelineStageDescriptor> stages)
    {
        var value = string.Join('|',
            ...
            string.Join(',', stages.Select(static stage => $"{stage.Kind}:{stage.ProviderId}")));
        var digest = SHA256.HashData(Encoding.UTF8.GetBytes(value));
        return $"pr_{Convert.ToHexStringLower(digest.AsSpan(0, 16))}";
    }

    private sealed record ArtifactRoute(
        string SourceFormat,
        string FinalFormat,
        IReadOnlyList<ArtifactTransformStep> Transformations,
        ArtifactProcessorManifest? OutputProcessor,
        RuntimeManifest? Runtime);

    private sealed record ArtifactTransformStep(
        ArtifactProcessorManifest Processor,
        ArtifactTransformationManifest Transformation);

    private sealed record ConversionRoute(
        string SourceFormat,
        string FinalFormat,
        IReadOnlyList<ArtifactTransformStep> Transformations);

    private sealed record ProcessorRouteCandidate(
        ArtifactProcessorManifest Processor,
        ConversionRoute Conversion);
}

public sealed class SelectionResolutionException : Exception
{
    private SelectionResolutionException(string code, SelectionField field, string? value, string message)
        : base(message)
    {
        Code = code;
        Field = field;
