...
internal sealed record OperationControlResponse(int StatusCode, object? Payload, string? Location = null)
{
    public IResult ToHttpResult() => StatusCode == StatusCodes.Status202Accepted
        ? Results.Accepted(Location, Payload)
        : StatusCode == StatusCodes.Status404NotFound && Payload is null
            ? Results.NotFound()
        : Results.Json(Payload, ContractJson.CreateSerializerOptions(), statusCode: StatusCode);
}

internal sealed class OperationControlService(
    CatalogDocument catalog,
    PipelineResolutionRegistry registry,
    OperationStore operations,
    BuildOperationExecutor buildExecutor,
    ArtifactOperationExecutor artifactExecutor,
    RuntimeOperationExecutor runtimeExecutor,
    ExplainOperationExecutor explainExecutor,
    LanguageWorkerEndpointRegistry workerEndpoints,
    GatewayDependencyHealthService dependencyHealth)
{
    ...
    public Task<OperationControlResponse> StartAsync(
        string operation,
        JsonElement request,
        string traceId,
    ...
    public async Task<OperationControlResponse> StartBuildAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (resolution is null)
        {
            return Problem(
                StatusCodes.Status400BadRequest,
                "invalid-pipeline-resolution",
                "Resolve the selection again before building.");
        }

        if (!string.Equals(resolution.EffectiveSelection.ToolchainId, request.ToolchainId, StringComparison.Ordinal) ||
            !string.Equals(resolution.EffectiveSelection.ReferenceSetId, request.ReferenceSetId, StringComparison.Ordinal))
        {
            return Problem(StatusCodes.Status400BadRequest, "pipeline-mismatch", "Build request does not match the resolved pipeline.");
        }

        var toolchain = catalog.Toolchains.FirstOrDefault(candidate =>
            string.Equals(candidate.Id, resolution.EffectiveSelection.ToolchainId, StringComparison.Ordinal));
        var compilerWorkerId = resolution.PipelinePlan.CompilerWorkerId;
        if (toolchain is null ||
            string.IsNullOrWhiteSpace(compilerWorkerId) ||
            !string.Equals(toolchain.WorkerId, compilerWorkerId, StringComparison.Ordinal))
        {
            return Problem(
                StatusCodes.Status400BadRequest,
                "pipeline-mismatch",
                "The resolved compiler worker does not match the active catalog.");
        }
        ...
        var unavailableReason = GatewayPipelineAvailability.GetUnavailableReason(
            snapshot,
            resolution,
            request.Target == BuildTarget.Artifact);
    ...
    public async Task<OperationControlResponse> StartExplainAsync(
        ...
        CancellationToken cancellationToken)
    {
            ...
            !StringComparer.Ordinal.Equals(stage.ProviderId, resolution.PipelinePlan.CompilerWorkerId))
        {
            return Problem(
                StatusCodes.Status400BadRequest,
                "pipeline-mismatch",
                "Explain request does not match an active resolved C# pipeline.");
        }
    ...
    public async Task<OperationControlResponse> StartTransformAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!HasOperationIdentity(request.RequestId, request.IdempotencyKey))
            return InvalidOperationIdentity();
        var resolution = registry.Get(request.PipelineResolutionId, DateTimeOffset.UtcNow);
        if (resolution is null || !MatchesArtifactPipeline(
                resolution,
                request.ProcessorId,
                request.TransformId,
                PipelineStageKind.Transform) ||
            !MatchesTransformOptions(request))
        {
    ...
    public async Task<OperationControlResponse> StartRenderAsync(
    ...
    public async Task<OperationControlResponse> StartVerificationAsync(
    ...
    public async Task<OperationControlResponse> StartRunAsync(
        RunRequest request,
        string traceId,
        CancellationToken cancellationToken) =>
        await StartRunAsync(
    ...
    private async Task<OperationControlResponse> StartRunAsync(
        RunRequest request,
        string traceId,
        string? runtimeSessionId,
        CancellationToken cancellationToken)
    {
        ...
        var expectedOutputId = request.Options?.Instrumentation == RunInstrumentation.ExecutionFlow
            ? "execution-flow"
            : "run";
        if (resolution is null || request.Options is null || !MatchesRuntimePipeline(
                resolution,
    ...
    public async Task<OperationControlResponse> StartJitAsync(
        JitRequest request,
        string traceId,
        CancellationToken cancellationToken) =>
        await StartJitAsync(
    ...
    private async Task<OperationControlResponse> StartJitAsync(
        JitRequest request,
        string traceId,
        string? runtimeSessionId,
        CancellationToken cancellationToken)
    ...
    public Task ReleaseRuntimeSessionAsync(
        string runtimeSessionId,
    ...
    public OperationControlResponse GetState(string operationId)
    {
        ...
        return state is null
            ...
            : new OperationControlResponse(StatusCodes.Status200OK, state);
    }
    ...
    public OperationControlResponse Cancel(string operationId, string? requestOperationId, string? reason)
    {
        ...
        return result.Disposition == CancelDisposition.NotFound
            ? new OperationControlResponse(StatusCodes.Status404NotFound, null)
            : new OperationControlResponse(StatusCodes.Status200OK, result);
    }

    private static T Deserialize<T>(JsonElement request) =>
        request.Deserialize<T>(SerializerOptions)
        ?? throw new JsonException($"The {typeof(T).Name} command request was empty.");

    private static OperationControlResponse Accepted(OperationHandle handle) => new(
        StatusCodes.Status202Accepted,
        handle,
        $"/api/v1/operations/{handle.OperationId}");

    private static OperationControlResponse InvalidOperationIdentity() => Problem(
        StatusCodes.Status400BadRequest,
        "invalid-request-identity",
        "RequestId and IdempotencyKey are required.");

    private static OperationControlResponse DependencyUnavailable(string message) => Problem(
        StatusCodes.Status503ServiceUnavailable,
        "profile-unavailable",
        message);
    ...
    private static bool MatchesRuntimePipeline(
    ...
    {
        if (!string.Equals(resolution.EffectiveSelection.RuntimeId, runtimeProfileId, StringComparison.Ordinal) ||
            !string.Equals(resolution.EffectiveSelection.OutputId, outputId, StringComparison.Ordinal) ||
            !string.Equals(resolution.PipelinePlan.RuntimeId, runtimeProfileId, StringComparison.Ordinal) ||
            !string.Equals(resolution.PipelinePlan.SecurityPolicyId, securityPolicyId, StringComparison.Ordinal) ||
            resolution.PipelinePlan.Stages.Count == 0)
        {
            return false;
        }
    ...
    private static bool MatchesArtifactPipeline(
        ResolveSelectionResponse resolution,
        string processorId,
        string outputId,
        PipelineStageKind stageKind)
    {
        ...
        var stage = stageKind == PipelineStageKind.Transform
            ? resolution.PipelinePlan.Stages.FirstOrDefault(candidate =>
                candidate.Kind == stageKind && StringComparer.Ordinal.Equals(candidate.Id, outputId))
            : StringComparer.Ordinal.Equals(resolution.EffectiveSelection.OutputId, outputId)
                ? resolution.PipelinePlan.Stages[^1]
                : null;
        if (stage is null)
            return false;
        return stage.Kind == stageKind
               && StringComparer.Ordinal.Equals(stage.Id, outputId)
               && StringComparer.Ordinal.Equals(stage.ProviderId, processorId);
    }
    ...
    private static bool MatchesTransformOptions(TransformArtifactRequest request) =>
        ...
        (request.Options.PreservePortablePdb &&
         request.Options.PreserveSequencePoints &&
         StringComparer.Ordinal.Equals(request.Options.RewriterProfileId, "execution-flow-v1"));
