...
{
    Task<OperationHandle> StartTransformAsync(
        TransformArtifactRequest request,
        CancellationToken cancellationToken = default);

    Task<OperationHandle> StartRenderAsync(
        RenderArtifactRequest request,
        CancellationToken cancellationToken = default);

    Task<OperationHandle> StartVerifyAsync(
        VerifyArtifactRequest request,
        CancellationToken cancellationToken = default);

    Task<OperationState?> GetOperationAsync(
        string operationId,
        CancellationToken cancellationToken = default);
...
public sealed record ArtifactWorkerClientSettings(
    string WorkerId,
    string ExpectedReleaseId,
    string? ExpectedWorkerImageId);

public sealed class ArtifactWorkerClient(
    HttpClient httpClient,
    ArtifactPipelineOptions options,
    ArtifactWorkerClientSettings settings) : IArtifactWorkerClient
{
    ...
    public Task<OperationHandle> StartTransformAsync(
        TransformArtifactRequest request,
        CancellationToken cancellationToken = default) =>
        StartAsync("/api/v1/artifact-transforms", request, request.RequestId, cancellationToken);

    public Task<OperationHandle> StartRenderAsync(
        RenderArtifactRequest request,
        CancellationToken cancellationToken = default) =>
        StartAsync("/api/v1/artifact-renders", request, request.RequestId, cancellationToken);

    public Task<OperationHandle> StartVerifyAsync(
        VerifyArtifactRequest request,
        CancellationToken cancellationToken = default) =>
        StartAsync("/api/v1/verifications", request, request.RequestId, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"/api/v1/operations/{Uri.EscapeDataString(operationId)}/events?FromSequence={fromSequence}");
        using var response = await SendAsync(request, cancellationToken).ConfigureAwait(false);
        await EnsureSuccessAsync(response, cancellationToken).ConfigureAwait(false);
        ...
        {
            if (!StringComparer.Ordinal.Equals(operationEvent.OperationId, operationId)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(handle.RequestId)
            || (!handle.IsExisting && !StringComparer.Ordinal.Equals(handle.RequestId, requestId)))
        {
    ...
    private async Task<WorkerDescriptor> GetDescriptorAsync(CancellationToken cancellationToken)
    {
        ...
        using var response = await SendAsync(request, cancellationToken).ConfigureAwait(false);
        await EnsureSuccessAsync(response, cancellationToken).ConfigureAwait(false);
        var descriptor = await ReadRequiredJsonAsync<WorkerDescriptor>(response, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw Failure(
                "artifact-worker-control-timeout",
                WorkerErrorCategory.DeadlineExceeded,
                "The artifact worker did not respond in time.",
                retryable: true,
                safeToRetry: true,
                exception);
        }
        catch (HttpRequestException exception)
        {
            throw Failure(
                ...
                "The artifact worker is unavailable.",
                retryable: true,
                safeToRetry: true,
                exception);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new ArtifactWorkerClientException(new WorkerError(
            code ?? $"artifact-worker-http-{statusCode}",
            category,
            message ?? PublicMessage(category),
            retryable,
            retryable,
            traceId ?? "artifact-worker-client",
            settings.WorkerId,
            "unknown"), statusCode);
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            return await response.Content.ReadFromJsonAsync<T>(JsonOptions, cancellationToken).ConfigureAwait(false)
                ?? throw ProtocolFailure("Artifact worker returned an empty JSON response.");
        }
    ...
    private void ValidateDescriptor(WorkerDescriptor descriptor)
    {
        if (!string.Equals(descriptor.Service.Id, settings.WorkerId, StringComparison.Ordinal)
            || descriptor.Service.Kind != ServiceKind.ArtifactWorker
            || descriptor.WorkerKind != WorkerKind.ArtifactProcessor
            || !descriptor.ProfileIds.Contains(settings.WorkerId, StringComparer.Ordinal))
        {
        ...
        if (string.IsNullOrWhiteSpace(descriptor.WorkerImageId)
            || descriptor.WorkerImageId.Length > 512
            || settings.ExpectedWorkerImageId is not null
                && !string.Equals(
                    descriptor.WorkerImageId,
                    settings.ExpectedWorkerImageId,
        ...
        if (descriptor.NegotiatedProtocol.Major != ProtocolVersion.WorkerV1.Major
            ...
            || !descriptor.SupportedProtocolVersions.Any(
                static version => version.Major == ProtocolVersion.WorkerV1.Major))
        {
            throw Failure(
                "artifact-worker-protocol-incompatible",
                WorkerErrorCategory.UnsupportedCapability,
                "The artifact worker protocol is incompatible with the Gateway.",
                retryable: false,
                safeToRetry: false);
        }
    }
    ...
    private void ValidateProcessorIdentity(OperationResult result, WorkerDescriptor descriptor)
    {
        ...
        if (!string.Equals(identity.ReleaseId, settings.ExpectedReleaseId, StringComparison.Ordinal)
            || !string.Equals(identity.ProcessorId, settings.WorkerId, StringComparison.Ordinal)
            || string.IsNullOrWhiteSpace(identity.ProcessorVersion)
            || identity.ProcessorVersion.Length > 512
            || !string.Equals(identity.WorkerImageId, descriptor.WorkerImageId, StringComparison.Ordinal))
        {
            throw ProtocolFailure("The artifact worker result identity did not match the selected processor.");
        }
    }

    private ArtifactWorkerClientException ProtocolFailure(
        string message,
    ...
    private ArtifactWorkerClientException Failure(
        string code,
        ...
        new(new WorkerError(
            ...
            retryable,
            safeToRetry,
            "artifact-worker-client",
            settings.WorkerId,
            "unknown"), null, innerException);
...
{
    public ArtifactWorkerClientException(
        WorkerError error,
        int? statusCode = null,
        Exception? innerException = null)
