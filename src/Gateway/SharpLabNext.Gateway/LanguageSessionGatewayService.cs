...
public sealed class GatewayLanguageSessionException(
    string code,
    string message,
    int statusCode,
...
public sealed class LanguageSessionGatewayService(
    LanguageSessionGatewayOptions options,
    LanguageWorkerEndpointRegistry endpoints,
    ...
    ILogger<LanguageSessionGatewayService> logger) : BackgroundService
{
    ...
    public async Task<GatewayLanguageSessionResponse> OpenAsync(
        OpenLanguageSessionRequest request,
        CancellationToken cancellationToken)
    {
        ValidateRequest(request);
        var resolution = resolutions.Get(request.PipelineResolutionId, DateTimeOffset.UtcNow)
            ?? throw Failure(
                "invalid-pipeline-resolution",
                "Resolve the selection again before opening a language session.",
        ...
        var toolchain = catalog.Toolchains.FirstOrDefault(item =>
            string.Equals(item.Id, request.ToolchainId, StringComparison.Ordinal));
            ...
            !toolchain.Capabilities.Contains("lsp", StringComparer.Ordinal))
        {
            throw Failure(
                ...
                "The selected toolchain does not have an installed language server.",
                StatusCodes.Status503ServiceUnavailable);
        }
        ...
        {
            throw Failure(
                "language-worker-unavailable",
                "The selected language worker is not configured on this Gateway.",
                StatusCodes.Status503ServiceUnavailable);
        }
        ...
        {
            throw Failure(
                "language-session-limit",
                "The Gateway language session limit has been reached.",
        ...
        try
        {
            ...
            var expiresAt = Min(
                upstream.ExpiresAtUtc,
                DateTimeOffset.UtcNow.Add(options.MaximumSessionLifetime));
            var state = new GatewayLanguageSessionState(
                sessionId,
                upstream.SessionId,
                endpoint,
                upstream.LanguageId,
                upstream.ToolchainId,
                upstream.CompilerBuildIdentity,
                upstream.LspVersion,
                upstream.WorkspaceRevision,
                upstream.SelectionRevision,
                expiresAt);
            ...
            return new GatewayLanguageSessionResponse(
                sessionId,
                state.LanguageId,
                state.ToolchainId,
                state.CompilerBuildIdentity,
                state.LspVersion,
                state.WorkspaceRevision,
                state.SelectionRevision,
                state.ExpiresAtUtc,
                $"/api/v1/language-sessions/{Uri.EscapeDataString(sessionId)}/lsp",
                resolution.EffectiveCapabilities.LanguageServerCapabilities);
        }
    ...
    public async Task<ClientWebSocket> ConnectUpstreamAsync(
        GatewayLanguageSessionState state,
        CancellationToken cancellationToken)
    {
        ...
        {
            return await client
                .ConnectAsync(state.UpstreamSessionId, options.KeepAliveInterval, timeout.Token)
                .ConfigureAwait(false);
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested && !state.Closed.IsCancellationRequested)
        {
            throw Failure(
                "language-worker-timeout",
                "The language worker did not accept the WebSocket connection in time.",
        ...
        catch (WebSocketException exception)
        {
            throw Failure(
                "language-worker-unavailable",
                "The language worker WebSocket is unavailable.",
    ...
    private async Task<bool> CloseAsyncCore(
        string sessionId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await CreateClient(state.Endpoint)
                .CloseAsync(state.UpstreamSessionId, timeout.Token)
                .ConfigureAwait(false);
        }
    ...
    public override async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var sessionId in _sessions.Keys)
        {
            await CloseAsyncCore(
                sessionId,
                SharpLabNextTelemetryOutcome.Cancelled,
    ...
    private ToolchainLanguageSessionClient CreateClient(LanguageWorkerEndpoint endpoint)
    {
        ...
        return new ToolchainLanguageSessionClient(
            httpClient,
    ...
    private void ValidateRequest(OpenLanguageSessionRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.RequestId) ||
            ...
            string.IsNullOrWhiteSpace(request.ToolchainId) ||
            string.IsNullOrWhiteSpace(request.ReferenceSetId) ||
        ...
        if (request.RequestId.Length > 128 ||
            request.PipelineResolutionId.Length > 256 ||
            request.LanguageId.Length > 128 ||
            request.ToolchainId.Length > 128 ||
            request.ReferenceSetId.Length > 128)
        {
            throw Failure(
                "invalid-language-session",
                "The language session identity is too long.",
                StatusCodes.Status400BadRequest);
        }
        ...
        if (workspace.SchemaVersion != ContractSchemaVersions.WorkspaceSnapshot ||
            ...
            workspace.Files is null ||
            workspace.SourceOrder is null ||
            workspace.BuildOptions is null)
        {
            throw Failure(
                "invalid-language-session",
                "The workspace snapshot is invalid or uses an unsupported schema version.",
                StatusCodes.Status400BadRequest);
        }
        ...
        {
            throw Failure(
                "language-session-source-limit",
                $"A language session must contain between 1 and {options.MaxWorkspaceFiles} files.",
        ...
        foreach (var file in workspace.Files)
        {
            if (file is null || string.IsNullOrWhiteSpace(file.Path) || file.Text is null || file.Version < 0)
            {
                throw Failure(
                    "invalid-language-session",
                    "Every workspace file must have a path, non-negative version, and source text.",
                    StatusCodes.Status400BadRequest);
            }
            ...
            if (fileBytes > options.MaxFileSourceUtf8Bytes)
            {
                throw Failure(
                    "language-session-source-limit",
                    $"Workspace file '{file.Path}' exceeds the Gateway source limit.",
                    StatusCodes.Status413PayloadTooLarge);
            }
            ...
            if (totalBytes > options.MaxTotalSourceUtf8Bytes)
            {
                throw Failure(
                    "language-session-source-limit",
                    "The workspace exceeds the Gateway total source limit.",
                    StatusCodes.Status413PayloadTooLarge);
            }
        }
    }
        ...
        ResolveSelectionResponse resolution)
    {
            ...
            string.IsNullOrWhiteSpace(resolution.PipelinePlan.LanguageWorkerId))
        {
            throw Failure(
                "pipeline-mismatch",
                "The language session does not match the server-resolved pipeline.",
                StatusCodes.Status400BadRequest);
        }
    }
    ...
    private async Task CloseUpstreamIgnoringFailureAsync(
        LanguageWorkerEndpoint endpoint,
    ...
    private static GatewayLanguageSessionException Failure(
        string code,
        string message,
        int statusCode,
...
public sealed class GatewayLanguageSessionState : IDisposable
{
    ...
    public GatewayLanguageSessionState(
        string sessionId,
        string upstreamSessionId,
        LanguageWorkerEndpoint endpoint,
        string languageId,
...
public sealed class GatewayLanguageSessionConnection(GatewayLanguageSessionState state) : IDisposable
{
    ...
    public GatewayLanguageSessionState State =>
        _state ?? throw new ObjectDisposedException(nameof(GatewayLanguageSessionConnection));
