...
var profileUpdateStatusPath = ResolveProfileUpdateStatusFile(
    builder.Environment.ContentRootPath,
    builder.Configuration[$"{ProfileUpdateStatusOptions.SectionName}:StatusPath"]);
...
var githubOAuthOptions = GitHubOAuthOptions.FromConfiguration(
    builder.Configuration,
    builder.Environment);
var githubApiBaseAddress = GitHubExternalEndpoint.Parse(
    ...
    builder.Environment);
var artifactStoreBaseAddress = RequiredServiceUri(
    builder.Configuration["Services:ArtifactStore:BaseAddress"],
    "Services:ArtifactStore:BaseAddress");
var runtimeSupervisorBaseAddress = RequiredServiceUri(
    builder.Configuration["Services:RuntimeSupervisor:BaseAddress"],
    "Services:RuntimeSupervisor:BaseAddress");
var dependencyHealthOptions = new GatewayDependencyHealthOptions(
    builder.Configuration.GetValue("DependencyHealth:Enabled", true),
    artifactStoreBaseAddress,
    runtimeSupervisorBaseAddress,
    builder.Configuration.GetValue("DependencyHealth:CacheDuration", TimeSpan.FromSeconds(2)),
    builder.Configuration.GetValue("DependencyHealth:ProbeTimeout", TimeSpan.FromSeconds(2)),
    internalServiceAuthentication.Token);
dependencyHealthOptions.Validate();
var languageWorkerEndpoints = LanguageWorkerEndpointRegistry.FromConfiguration(
    builder.Configuration,
    catalog.ReleaseId,
    catalog.Toolchains,
    expectedReferenceSetDigests,
    internalServiceAuthentication.Token);
var artifactWorkerEndpoints = ArtifactWorkerEndpointRegistry.FromConfiguration(
    builder.Configuration,
    catalog.ReleaseId,
    catalog.ArtifactProcessors.Select(static processor => processor.WorkerId),
    internalServiceAuthentication.Token);
var runtimeSupervisorSettings = new RuntimeSupervisorClientSettings(
    runtimePipelineOptions.ControlRequestTimeout,
    runtimePipelineOptions.MaximumEventCharacters);
...
var descriptor = new ServiceIdentity(
    ...
    ["health", "static-web", "catalog", "profile-updates", "selection-resolver", "language-sessions", "operations", "artifact-operations", "runtime-operations", "explain", "github-gists"],
    "ready");
...
{
    KeepAliveInterval = languageSessionOptions.KeepAliveInterval
});
...
app.MapGet("/api/v1/profile-updates", (
    ServiceIdentity service,
    ProfileUpdateStatusReader status,
    CancellationToken cancellationToken) =>
    status.ReadAsync(service.ReleaseId, cancellationToken));
app.MapGet("/api/v1/catalog", async (
    GatewayDependencyHealthService dependencyHealth,
    CancellationToken cancellationToken) =>
    (await dependencyHealth.GetSnapshotAsync(cancellationToken)).Catalog);

app.MapPost("/api/v1/selections/resolve", async (
    ResolveSelectionRequest request,
    PipelineResolutionRegistry registry,
    GatewayDependencyHealthService dependencyHealth,
    CancellationToken cancellationToken) =>
{
    try
    ...
    catch (SelectionResolutionException exception)
    {
        return Results.BadRequest(new
        {
            ...
            Field = exception.Field.ToString(),
...
app.MapPost("/api/v1/language-sessions", async (
    OpenLanguageSessionRequest request,
    HttpContext context,
    LanguageSessionGatewayService sessions) =>
{
    try
    {
...
app.MapDelete("/api/v1/language-sessions/{sessionId}", async (
    string sessionId,
    HttpContext context,
    LanguageSessionGatewayService sessions) =>
{
...
app.MapGet("/api/v1/language-sessions/{sessionId}/lsp", async (
    string sessionId,
    HttpContext context,
    LanguageWebSocketProxy proxy) =>
{
    await proxy.RunAsync(sessionId, context);
});

app.MapPost("/api/v1/builds", async (BuildRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartBuildAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/explanations", async (ExplainRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartExplainAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/artifact-transforms", async (TransformArtifactRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartTransformAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/artifact-renders", async (RenderArtifactRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartRenderAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/verifications", async (VerifyArtifactRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartVerificationAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/runs", async (RunRequest request, HttpContext context, OperationControlService control) =>
    (await control.StartRunAsync(request, context.TraceIdentifier, context.RequestAborted)).ToHttpResult());
app.MapPost("/api/v1/jit", StartJit);
app.MapPost("/api/v1/jits", StartJit);
app.MapGet("/api/v1/operations/ws", (HttpContext context, OperationCommandWebSocket commands) =>
    commands.RunAsync(context));

app.MapGet("/api/v1/operations/{operationId}", (string operationId, OperationControlService control) =>
    control.GetState(operationId).ToHttpResult());

app.MapGet("/api/v1/operations/{operationId}/contents/sha256/{digest}", async (
    string operationId,
    string digest,
    HttpContext context,
    OperationStore operations,
    IArtifactStoreClient artifactStore,
    ArtifactPipelineOptions contentOptions) =>
{
    ...
    var produced = events?
        ...
        .OfType<ContentProducedOperationEventPayload>()
        .FirstOrDefault(item => item.ContentRef == contentRef);
    if (produced is null)
    {
    ...
    try
    {
        ...
        if (content.Length is null || content.Length < 0
            || content.Length > contentOptions.MaximumPublicContentBytes
            || produced.Size != content.Length)
        {
...
app.MapGet("/api/v1/operations/{operationId}/events", async (
    string operationId,
    HttpContext context,
    OperationStore operations) =>
{
...
app.MapPost("/api/v1/operations/{operationId}/cancel", (
    string operationId,
    CancelOperationRequest request,
    OperationControlService control) => control.Cancel(operationId, request.OperationId, request.Reason).ToHttpResult());

app.MapGet("/api/v1/auth/github/status", (
    HttpContext context,
    GitHubOAuthOptions options,
    GitHubOAuthSessionStore sessions) =>
{
    context.Response.Headers.CacheControl = "no-store";
    var session = GetGitHubSession(context, sessions);
    return Results.Ok(new GitHubAuthStatus(
        options.Available,
        session is not null,
        session?.Login,
        session?.CsrfToken));
...
app.MapGet("/api/v1/auth/github/start", (
    HttpContext context,
    GitHubOAuthOptions options,
    GitHubOAuthSessionStore sessions,
    GitHubOAuthClient oauth) =>
{
    ...
    if (!PascalCaseQuery.TryGetOptionalSingle(context.Request, "ReturnPath", out var returnPath))
    {
        ...
        {
            Error = "invalid-query",
            Message = "ReturnPath must use its exact PascalCase spelling and appear at most once."
        });
    }
    ...
    try
    {
        ...
        context.Response.Cookies.Append(
            "SharpLabNext.GitHubOAuthState",
            pending.State,
            GitHubCookie(context, options, httpOnly: true, sameSite: SameSiteMode.Lax, path: "/api/v1/auth/github/callback"));
        var callback = GitHubCallbackUri(context, options);
        return Results.Ok(new GitHubOAuthStartResponse(
            oauth.CreateAuthorizationUri(pending.State, callback).AbsoluteUri));
    }
...
app.MapGet("/api/v1/auth/github/callback", async (
    string? code,
    string? state,
    string? error,
    HttpContext context,
    ...
    IGitHubGistClient github) =>
{
    ...
    if (!string.IsNullOrWhiteSpace(error) || string.IsNullOrWhiteSpace(code) || string.IsNullOrWhiteSpace(state))
    {
        ...
        {
            Error = "oauth-denied",
            Message = "GitHub authorization was denied or incomplete."
        });
    }
    ...
    if (stateCookie is null || !sessions.TryTakePending(state, stateCookie, DateTimeOffset.UtcNow, out var pending))
    {
        return Results.BadRequest(new
        {
            ...
            Message = "The GitHub OAuth state is invalid or expired."
        });
    }
    try
    {
        ...
        context.Response.Cookies.Append(
            "SharpLabNext.GitHubSession",
            session.SessionId,
            GitHubCookie(context, options, httpOnly: true, sameSite: SameSiteMode.Lax, path: "/"));
...
app.MapPost("/api/v1/auth/github/logout", (
    HttpContext context,
    GitHubOAuthOptions options,
    GitHubOAuthSessionStore sessions) =>
{
...
app.MapGet("/api/v1/shares/gists/{id}", async (
    string id,
    HttpContext context,
    GitHubOAuthSessionStore sessions,
    GistShareService gists) =>
{
    context.Response.Headers.CacheControl = "private, no-store";
    if (!PascalCaseQuery.TryGetOptionalSingle(context.Request, "Target", out var target) ||
        !PascalCaseQuery.TryGetOptionalSingle(context.Request, "Branch", out var branch) ||
        !PascalCaseQuery.TryGetOptionalSingle(context.Request, "Mode", out var mode))
    {
        return Results.BadRequest(new
        {
            Error = "invalid-query",
            Message = "Target, Branch, and Mode must use their exact PascalCase spellings and appear at most once."
        });
    }
    ...
    try
    {
        var document = await gists.GetAsync(
            id,
            new GistLoadOverrides(target, branch, buildMode),
            GetGitHubSession(context, sessions),
            context.RequestAborted);
...
app.MapPost("/api/v1/shares/gists", async (
...
app.MapPatch("/api/v1/shares/gists/{id}", async (
    string id,
...
static GitHubOAuthSession? GetGitHubSession(
    HttpContext context,
    GitHubOAuthSessionStore sessions) =>
    sessions.TryGetSession(
        context.Request.Cookies["SharpLabNext.GitHubSession"],
        DateTimeOffset.UtcNow,
        out var session)
        ? session
        : null;

static IResult? RequireGitHubMutationSession(
    HttpContext context,
    GitHubOAuthSessionStore sessions,
...
static Uri GitHubCallbackUri(HttpContext context, GitHubOAuthOptions options)
{
    ...
    return new UriBuilder(
        ...
        $"{context.Request.PathBase}/api/v1/auth/github/callback").Uri;
}

static CookieOptions GitHubCookie(
    HttpContext context,
    GitHubOAuthOptions options,
    bool httpOnly,
...
static async Task<IResult> StartJit(
    JitRequest request,
    HttpContext context,
