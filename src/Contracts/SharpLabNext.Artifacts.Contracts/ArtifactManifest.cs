...
public sealed record ArtifactRuntimeRequirement(
    string Family,
    IReadOnlyList<FrameworkRequirement> Frameworks,
    string Architecture,
    IReadOnlyList<string> RequiredRuntimeFeatureTags);

public sealed record FrameworkRequirement(
    string Name,
    string MinimumVersion);

public sealed record ArtifactFileDescriptor(
    string Role,
    string Path,
    long Size,
    string Digest);
