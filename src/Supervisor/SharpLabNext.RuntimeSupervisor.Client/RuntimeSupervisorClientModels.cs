...
{
    Task<OperationHandle> StartRunAsync(
        RunRequest request,
    ...
    Task<OperationHandle> StartJitAsync(
        JitRequest request,
        CancellationToken cancellationToken = default);
    ...
    Task<OperationHandle> StartJitAsync(
        ...
        string? runtimeSessionId,
        CancellationToken cancellationToken = default) =>
        StartJitAsync(request, cancellationToken);

    Task<OperationState?> GetOperationAsync(
        string operationId,
        CancellationToken cancellationToken = default);

    IAsyncEnumerable<OperationEvent> WatchEventsAsync(
        string operationId,
        long fromSequence = 0,
        CancellationToken cancellationToken = default);
...
public sealed record RuntimeSupervisorClientSettings(
    TimeSpan ControlRequestTimeout,
    int MaximumEventCharacters = 4 * 1024 * 1024)
{
    public void Validate()
    {
        if (ControlRequestTimeout <= TimeSpan.Zero || ControlRequestTimeout > TimeSpan.FromMinutes(1))
        {
            throw new ArgumentOutOfRangeException(
                ...
                "The runtime supervisor control request timeout is outside the supported range.");
        }

        if (MaximumEventCharacters is < 1024 or > 64 * 1024 * 1024)
        {
            throw new ArgumentOutOfRangeException(
                nameof(MaximumEventCharacters),
                "The runtime supervisor event size limit is outside the supported range.");
        }
    }
}
...
{
    public RuntimeSupervisorClientException(
        WorkerError error,
        int? statusCode = null,
