...
public static class RuntimeJobRequestValidator
{
    public static object? Validate(RunRequest request)
    {
        ...
        if (request.Options.Arguments.Count > 32 ||
            request.Options.Arguments.Any(static argument => argument.Length > 4096 || argument.Contains('\0')))
        {
    ...
    private static object? ValidateCommon(
        string requestId,
        ...
        DateTimeOffset deadlineUtc)
    {
        ...
        return deadlineUtc <= DateTimeOffset.UtcNow
            ? Error("deadline-expired", "The request deadline has already expired.")
            : null;
    }
...
public sealed partial class RuntimeJobExecutor(
    ...
    ILogger<RuntimeJobExecutor> logger)
{
    ...
    internal bool QueueRunForMeasurement(
        ...
        RuntimeJobMeasurementRegistration? measurement)
    {
        ...
        var queued = scheduler.TryQueue(
            operation,
            () => ExecuteJitAsync(operation, request, runtimeSessionId, measurement));
        if (!queued)
        {
            measurement?.Reject(
                "operation-queue-rejected",
        ...
        TimeSpan? runtimeDeadlineAfterMarker = null)
    {
        ...
        try
        {
            ...
            descriptor = await artifactStore.GetArtifactAsync(artifactRef, execution.Token)
                ?? throw new RuntimeJobFailureException(
                    "artifact-not-found",
            ...
            var lease = await artifactStore.AcquireLeaseAsync(
                ...
                TimeSpan.FromSeconds(_options.ArtifactLeaseSeconds),
                execution.Token);
            leaseToken = lease.LeaseToken;

            Append(operation, new ProgressOperationEventPayload("artifact", "Preparing isolated artifact archive.", 0.15));
            await using var archive = await BuildArtifactArchiveAsync(
                descriptor,
            ...
            implementation = kind == RuntimeJobKind.Run
                ? profile.Operations?.Run?.ImplementationId ?? profile.Layout.RunnerKind
                : profile.Operations?.Jit?.ImplementationId ?? "legacy-layout-jit";
            var environment = new Dictionary<string, string>(
                createEnvironment(profile, descriptor),
                StringComparer.Ordinal)
            {
                ...
                ["SHARPLABNEXT_MAX_OUTPUT_BYTES"] = policy.MaximumOutputBytes.ToString(
            ...
            try
            {
                ...
                {
                    sessionLease = await runtimeSessions.AcquireAsync(
                        new RuntimeSessionRequest(
                            runtimeSessionId,
                ...
                else
                {
                    ...
                    {
                        oneShotAdmission = await runtimeSessions.AcquireOneShotAdmissionAsync(
                            runtimeSessionId,
                            execution.Token).ConfigureAwait(false);
                    }
            ...
            stopwatch.Stop();
            var result = (OperationResult)await ResolveResultAsync(
                createResult(capture, exit, stopwatch.Elapsed, profile, capture.OutputTruncated));
        ...
        catch (OperationCanceledException) when (operation.CancellationToken.IsCancellationRequested)
        {
            ...
            if (profile is not null)
            {
                OperationResult result = kind == RuntimeJobKind.Run
                    ? CreateCancelledRunResult(stopwatch.Elapsed, profile)
                    : CreateCancelledJitResult(stopwatch.Elapsed, profile);
                measurementResult = result;
                Append(operation, new TypedResultOperationEventPayload(result));
            }
        ...
        catch (OperationCanceledException)
        {
            ...
            {
                OperationResult result = kind == RuntimeJobKind.Run
                    ? CreateTimeoutRunResult(stopwatch.Elapsed, profile)
                    : CreateTimeoutJitResult(stopwatch.Elapsed, profile);
                measurementResult = result;
                Append(operation, new TypedResultOperationEventPayload(result));
                Append(operation, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, stopwatch.Elapsed));
            }
        ...
        catch (RuntimeOutputLimitException)
        {
            ...
            {
                OperationResult result = kind == RuntimeJobKind.Run
                    ? CreateOutputLimitRunResult(stopwatch.Elapsed, profile)
                    : CreateOutputLimitJitResult(stopwatch.Elapsed, profile);
                measurementResult = result;
                Append(operation, new TypedResultOperationEventPayload(result));
                Append(operation, new CompletedOperationEventPayload(OperationCompletionStatus.Completed, stopwatch.Elapsed));
            }
        ...
        catch (Exception exception)
        {
            ...
            Fail(
                ...
                "The isolated runtime job could not be completed.",
                retryable: true);
        }
        finally
        {
            ...
            SharpLabNextTelemetry.Metrics.RecordRuntime(
                kind == RuntimeJobKind.Run
                    ? SharpLabNextRuntimeOperation.Run
            ...
            var audit = measurement is null
                ...
                : await CreateAuditAsync(
                    ...
                    capture,
                    kind,
                    cleanupSucceeded).ConfigureAwait(false);
    ...
    private async Task<RuntimeJobAudit?> CreateAuditAsync(
        string? containerId,
        IReadOnlyList<string>? command,
        string? implementation,
        ArtifactBundleDescriptor? descriptor,
        RuntimeProfileOptions? profile,
        RuntimeFrameCapture? capture,
        RuntimeJobKind kind,
        bool cleanupSucceeded)
    {
        if (containerId is null || command is null || implementation is null ||
            descriptor is null || profile is null || capture is null)
        {
            return null;
        }

        var normalizedEntry = ArtifactPath.Normalize(descriptor.Manifest.EntryAssembly);
        var entry = descriptor.Manifest.Files.SingleOrDefault(file =>
            StringComparer.Ordinal.Equals(ArtifactPath.Normalize(file.Path), normalizedEntry));
        if (entry is null)
            return null;

        var entryPath = kind == RuntimeJobKind.Jit
            ? RuntimeProfileCommandBuilder.CreateJitCommand(profile, normalizedEntry, null)
                .FirstOrDefault(token => IsWorkspaceAssemblyPath(token, profile.Operations?.Jit?.PathStyle))
            : RuntimeProfileCommandBuilder.CreateRunCommand(profile, normalizedEntry, [])
                .FirstOrDefault(token => IsWorkspaceAssemblyPath(token, profile.Operations?.Run?.PathStyle));
        entryPath ??= command.FirstOrDefault(token => IsWorkspaceAssemblyPath(
            token,
            kind == RuntimeJobKind.Jit
    ...
    private static RuntimeObservedException? MapObservedException(ChildExceptionPayload? exception) =>
        exception is null
            ? null
            : new RuntimeObservedException(
                exception.TypeName,
                exception.Message,
                exception.StackTrace,
                MapObservedException(exception.InnerException));
    ...
    private static RuntimeJitAuditMethod[] ParseJitAuditMethods(ReadOnlySpan<byte> summaryBytes)
    {
        ...
        return summary?.Methods
            .Where(static method => method.Status == "prepared")
            .Select(static method => new RuntimeJitAuditMethod(
                method.Method,
                method.DisplayName ?? method.Method,
                method.NativeCodeSize,
                method.InstructionCount,
                method.EvidenceRanges?
                    .Where(static range =>
                        range.IlOffset >= 0 &&
                        range.NativeStartOffset >= 0 &&
                    ...
                    .Select(static range => new RuntimeJitEvidenceRange(
                        range.IlOffset,
                        range.NativeStartOffset,
                        range.NativeEndOffset,
                        range.Document,
                        range.StartLine,
                        range.StartColumn,
                        range.EndLine,
    ...
    private RuntimeSecurityPolicyOptions GetPolicy(string id)
    {
        ...
        {
            throw new RuntimeJobFailureException(
                "security-policy-not-installed",
                WorkerErrorCategory.InvalidArgument,
                "The selected runtime security policy is not installed.",
                retryable: false);
        }
    }
    ...
    internal static void ValidateCompatibility(ArtifactManifest manifest, RuntimeProfileOptions profile)
    {
            ...
            BuildOutputKind.WindowsApplication))
        {
            throw new RuntimeJobFailureException(
                ...
                "The artifact manifest does not declare a concrete output kind.",
                retryable: false);
        }
        ...
        if (!profile.AcceptedArtifactFormats.Contains(manifest.ArtifactFormat, StringComparer.Ordinal) ||
            !acceptedRuntimeFamilies.Contains(manifest.RuntimeRequirement.Family, StringComparer.Ordinal))
        {
            throw new RuntimeJobFailureException(
                "incompatible-artifact",
                WorkerErrorCategory.IncompatibleArtifact,
                "The selected runtime cannot load this artifact format or runtime family.",
                retryable: false);
        }

        var frameworkNames = new HashSet<string>(StringComparer.Ordinal);
        var incompatibleFramework = manifest.RuntimeRequirement.Frameworks.Any(framework =>
        ...
        if (incompatibleFramework)
        {
            throw new RuntimeJobFailureException(
                ...
                WorkerErrorCategory.IncompatibleArtifact,
                "The selected runtime does not satisfy the artifact's framework requirements.",
                retryable: false);
        }

        if (!RuntimeArchitectureCompatibility.IsCompatible(
                manifest.RuntimeRequirement.Architecture,
                profile.Architecture))
        {
            throw new RuntimeJobFailureException(
                "incompatible-architecture",
                WorkerErrorCategory.IncompatibleArtifact,
                "The artifact architecture is incompatible with the selected runtime.",
                retryable: false);
        }

        var missingRuntimeFeatures = manifest.RuntimeRequirement.RequiredRuntimeFeatureTags
            .Except(profile.ProvidedRuntimeFeatureTags, StringComparer.Ordinal)
            .ToArray();
        var missingMetadataFeatures = manifest.MetadataFeatureTags
            .Except(profile.ProvidedMetadataFeatureTags, StringComparer.Ordinal)
            .ToArray();
        if (missingRuntimeFeatures.Length > 0 || missingMetadataFeatures.Length > 0)
        {
            throw new RuntimeJobFailureException(
                "incompatible-feature-tags",
                WorkerErrorCategory.IncompatibleArtifact,
    ...
    internal static void ValidateCapability(RuntimeProfileOptions profile, string capability)
    {
        ...
        throw new RuntimeJobFailureException(
            "runtime-capability-not-supported",
            WorkerErrorCategory.UnsupportedCapability,
            $"The selected runtime does not support the '{capability}' operation.",
            retryable: false);
    }
    ...
    internal static RuntimeContainerIsolationKind ResolveIsolationKind(RuntimeProfileOptions profile) =>
        (profile.Container.IsolationKind, profile.Container.ExecutionUser) switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Runtime container isolation kind '{profile.Container.IsolationKind}' and execution user " +
                $"'{profile.Container.ExecutionUser}' are not a supported combination.")
        };

    internal static void ValidateInstrumentation(
        ...
        RunInstrumentation? instrumentation)
    {
        ...
        {
            throw new RuntimeJobFailureException(
                "execution-flow-artifact-required",
                WorkerErrorCategory.IncompatibleArtifact,
                "Execution Flow requires an artifact derived by runtime-instrumentation-v1.",
                retryable: false);
        }
    }

    private async Task<MemoryStream> BuildArtifactArchiveAsync(
        ArtifactBundleDescriptor descriptor,
        long maximumBytes,
        string? stdin,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (declaredSize < 0 || declaredSize + stdinBytes > maximumBytes)
        {
            throw new RuntimeJobFailureException(
                ...
                "The artifact exceeds the runtime job size limit.",
                retryable: false);
        }
        ...
        using (var writer = new TarWriter(archive, leaveOpen: true))
        {
            ...
            foreach (var entry in descriptor.Entries)
            {
                ...
                if (data.Length != entry.Size)
                {
                    throw new RuntimeJobFailureException(
                        ...
                        "The stored artifact failed integrity validation.",
                        retryable: true);
                }
            ...
            if (stdin is not null)
            {
                WriteArchiveEntry(
                    ...
                    new MemoryStream(Encoding.UTF8.GetBytes(stdin), writable: false),
                    workspaceOwner.Uid,
                    workspaceOwner.Gid);
            }
    ...
    internal static void WriteArchiveEntry(
        TarWriter writer,
        ISet<string> writtenDirectories,
        string path,
        Stream data,
        int uid = 1654,
        int gid = 1654)
        ...
        while (await reader.ReadAsync(cancellationToken: cancellationToken) is { } frame)
        {
            ...
            switch (frame.Kind)
            {
                ...
                case RuntimeFrameKind.MemoryGraph:
                    ...
                    capture.InspectionPayloads.Add(
                        RuntimeStructuredPayloadCodec.DeserializeInspection(frame.Payload.Span));
                ...
                case RuntimeFrameKind.Exception:
                    capture.Exception = JsonSerializer.Deserialize<ChildExceptionPayload>(frame.Payload.Span, JsonOptions)
                        ?? throw new InvalidDataException("Runtime child returned an empty exception payload.");
                    break;
                case RuntimeFrameKind.Exit:
                    capture.Exit = JsonSerializer.Deserialize<ChildExitPayload>(frame.Payload.Span, JsonOptions)
                        ?? throw new InvalidDataException("Runtime child returned an empty exit payload.");
                    break;
        ...
        long maximumOutputBytes)
    {
        ...
        if (observed > maximumOutputBytes)
        {
            ...
            Append(operation, new OutputTruncatedOperationEventPayload(
                channel,
        ...
        if (!payload.IsEmpty)
        {
            Append(operation, new OutputChunkOperationEventPayload(new OutputChunk(
                channel,
                OutputEncoding.Utf8,
    ...
    private static IReadOnlyList<string> CreateRunCommand(
        RuntimeProfileOptions profile,
        RunRequest request,
        string entryAssembly) =>
        RuntimeProfileCommandBuilder.CreateRunCommand(
            profile,
            ArtifactPath.Normalize(entryAssembly),
            request.Options.Arguments);

    private static IReadOnlyList<string> CreateJitCommand(
        RuntimeProfileOptions profile,
        JitRequest request,
        string entryAssembly) =>
        RuntimeProfileCommandBuilder.CreateJitCommand(
            profile,
            ArtifactPath.Normalize(entryAssembly),
            request.Options.MethodFilter);

    internal static Dictionary<string, string> CreateRunEnvironment(
        RuntimeProfileOptions profile,
        RunInstrumentation instrumentation)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Runtime environment kind '{profile.Container.EnvironmentKind}' is not supported.");
        }

        return environment;
    }

    private static void ValidateJSharp20Compatibility(
        ArtifactManifest manifest,
        RuntimeProfileOptions profile)
    {
        var jsharpProfile = profile.ProvidedRuntimeFeatureTags.Contains(
        ...
        {
            throw new RuntimeJobFailureException(
                "incompatible-jsharp20-contract",
                WorkerErrorCategory.IncompatibleArtifact,
                "J# execution requires the exact x64 CLR 2.0 artifact and dedicated Wine runtime profile.",
                retryable: false);
        }
    }

    internal static Dictionary<string, string> CreateJitEnvironment(
        RuntimeProfileOptions profile,
        JitRequest request,
        string entryAssembly)
    {
        ...
        {
            if (!usesMonoJitInspector ||
                mappingKind != RuntimeJitSourceMappingKinds.None)
            {
                throw new InvalidOperationException(
                    "Mono JIT inspection requires the bounded Mono helper with no claimed source mapping.");
            }
            environment["DOTNET_EnableDiagnostics"] = "0";
            environment["COMPlus_EnableDiagnostics"] = "0";
        ...
        bool outputTruncated)
    {
        ...
        var exception = capture.Exception is null
            ? null
            : MapUserException(capture.Exception);
        return new RunResult(
            status,
            capture.Exit?.ExitCode is null ? checked((int?)exit.StatusCode) : capture.Exit.ExitCode,
            exception,
            elapsed,
            outputTruncated,
    ...
    private static UserExceptionInfo MapUserException(ChildExceptionPayload exception, int depth = 0) =>
        new(
            ...
            depth < MaximumExceptionDepth && exception.InnerException is { } inner
                ? MapUserException(inner, depth + 1)
                : null);

    internal static RunTerminalStatus ClassifyRunStatus(
        ...
        RuntimeContainerExit exit)
    {
        ...
        return reportedExitCode == 0 && exit.StatusCode == 0
            ...
            : RunTerminalStatus.NonZeroExit;
    }

    private async Task<object> CreateJitResultAsync(
        string operationId,
        JitRequest request,
        RuntimeFrameCapture capture,
        RuntimeContainerExit exit,
        ...
        bool outputTruncated)
    {
        ...
        return new JitResult(
            ...
            methods,
            elapsed,
            JitIdentity(profile, request.Options));
    }
    ...
    internal static JitMethodSummary[] ParseJitMethods(ReadOnlySpan<byte> summaryBytes)
    {
        ...
        return summary?.Methods
            .Where(static method => method.Status == "prepared")
            .Select(static method => new JitMethodSummary(
                method.Method,
                method.DisplayName ?? method.Method,
                method.NativeCodeSize,
                method.InstructionCount,
                MapJitLinkedRanges(method.LinkedRanges)))
            .ToArray() ?? [];
    }

    private static LinkedRange[] MapJitLinkedRanges(
        IReadOnlyList<JitLinkedRangePayload>? ranges) =>
        ranges?.Take(4_096)
            .Select(static range => MapJitLinkedRange(range))
            .OfType<LinkedRange>()
            .ToArray() ?? [];

    private static LinkedRange? MapJitLinkedRange(JitLinkedRangePayload range)
    {
        if (!TryMapJitTextRange(range.SourceRange, out var sourceRange) ||
            !TryMapJitTextRange(range.OutputRange, out var outputRange))
        {
            return null;
        }
    ...
    private static bool TryMapJitTextRange(JitTextRangePayload? range, out TextRange mapped)
    {
        ...
        if (range is null ||
            ...
            range.EndLine > 1_000_000 || range.EndCharacter > 1_000_000)
        {
            return false;
        }

        mapped = new TextRange(
            range.StartLine,
    ...
    private static string? SanitizeJitSourcePath(string? path)
    {
        ...
        var segments = path.Replace('\\', '/')
            ...
            .Where(static segment => segment is not "." and not ".." && !segment.EndsWith(':'))
            .TakeLast(8)
            .ToArray();
    ...
    internal static JitTerminalStatus ClassifyJitStatus(
        string? reportedStatus,
        RuntimeContainerExit exit,
        int methodCount,
        bool outputTruncated)
    ...
    private async Task<ContentRef?> PublishContentAsync(string operationId, byte[] content, string mediaType)
    {
        ...
        operations.Append(
            operationId,
            new ContentProducedOperationEventPayload(contentRef, mediaType, content.Length),
    ...
    private static RunResult TerminalRunResult(
    ...
    private static JitResult TerminalJitResult(
        ...
        new(
            ...
            JitIdentity(
                profile,
                new JitOptions(null, "default", "default", "coreclr-jitdisasm", "runtime-job-default")));
    ...
    private void Fail(
        ...
        bool retryable) =>
        Append(operation, new FailedOperationEventPayload(new WorkerError(
            code,
            category,
            publicMessage,
            retryable,
            retryable,
            operation.Handle.RequestId,
            "runtime-supervisor",
            "runtime-supervisor")));
    ...
    private static async Task<RuntimeContainerResourceUsage> StopResourceMonitorAsync(
        IRuntimeContainerResourceMonitor monitor)
    {
        try
        {
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(5));
    ...
    private sealed record ChildExceptionPayload(
        string TypeName,
        string Message,
        string? StackTrace,
        ChildExceptionPayload? InnerException = null,
        double? ElapsedMilliseconds = null);

    private sealed record JitSummaryPayload(
        string RuntimeVersion,
        string? Assembly,
        string? MethodFilter,
        IReadOnlyList<JitMethodPayload> Methods);
    ...
    private sealed record JitEvidenceRangePayload(
        int IlOffset,
    ...
    private sealed record JitLinkedRangePayload(
        string? SourceFilePath,
        JitTextRangePayload? SourceRange,
        JitTextRangePayload? OutputRange,
        string? Precision);

    private sealed record JitTextRangePayload(
        int StartLine,
        int StartCharacter,
        int EndLine,
        int EndCharacter);
...
public sealed partial class RuntimeContainerReaper(
    IDockerEngineClient docker,
    IOptions<RuntimeSupervisorOptions> configuredOptions,
    ILogger<RuntimeContainerReaper> logger) : BackgroundService
{
    private readonly RuntimeSupervisorOptions _options = configuredOptions.Value;
    ...
    private async Task ReapAsync(CancellationToken cancellationToken)
    {
        ...
        {
            stopwatch.Stop();
            SharpLabNextTelemetry.Metrics.RecordReaperPass(
                _options.ResourceScope,
                stopwatch.Elapsed,
                removedContainers,
                failures);
...
internal sealed class RuntimeJobFailureException(
    string code,
    WorkerErrorCategory category,
    string publicMessage,
    bool retryable) : Exception(publicMessage)
{
