...
public sealed record RuntimeImageFileInspection(
    string Role,
    string Path,
    string Sha256,
    long SizeBytes,
...
public interface IDockerEngineClient
{
    ...
    Task<RuntimeImageInspection> InspectImageAsync(
        string immutableReference,
        CancellationToken cancellationToken = default);

    Task<IReadOnlyList<RuntimeImageFileInspection>> InspectImageFilesAsync(
        string imageId,
    ...
    Task<RuntimeWorkspaceMaterialization> MaterializeWorkspaceAsync(
        string jobId,
        string releaseId,
        string image,
        Stream archive,
    ...
    Task<IRuntimeContainerResourceMonitor> StartContainerResourceMonitorAsync(
        string containerId,
        CancellationToken cancellationToken = default);
...
public sealed class DockerEngineClient : IDockerEngineClient, IDisposable
{
    ...
    public DockerEngineClient(
        IOptions<RuntimeSupervisorOptions> options,
        RuntimeSandboxPolicy sandbox)
        : this(options.Value, sandbox, CreateDockerHandler(options.Value.DockerSocketPath))
    {
    }
    ...
    public async Task<RuntimeImageInspection> InspectImageAsync(
        string immutableReference,
        CancellationToken cancellationToken = default)
    {
        ValidateImmutableImageReference(immutableReference);
        using var response = await _client.GetAsync(
            Api($"/images/{Uri.EscapeDataString(immutableReference)}/json"),
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        var inspection = await response.Content.ReadFromJsonAsync<InspectImageResponse>(
            JsonOptions,
            cancellationToken)
            ?? throw new DockerEngineException("Docker returned an empty image-inspect response.");
        if (!IsSha256Digest(inspection.Id) || inspection.Size <= 0 ||
        ...
        var repoDigests = inspection.RepoDigests?
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var environment = effectiveEnvironment
            .OrderBy(static pair => pair.Key, StringComparer.Ordinal)
            .Select(static pair => $"{pair.Key}={pair.Value}")
            .ToArray();
        var isolation = RuntimeContainerIsolation.Resolve(
            spec.IsolationKind,
            spec.SecurityPolicy,
            spec.WinePrefixPath);
        ...
        using var response = await _client.PostAsJsonAsync(
            ...
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        var result = await response.Content.ReadFromJsonAsync<CreateContainerResponse>(JsonOptions, cancellationToken)
            ?? throw new DockerEngineException("Docker returned an empty create-container response.");
    ...
    public async Task<IReadOnlyList<RuntimeImageFileInspection>> InspectImageFilesAsync(
        string imageId,
        IReadOnlyList<RuntimeImageFileRequest> files,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (!IsStableArtifactRole(file.Role) || !IsCanonicalContainerPath(file.Path) ||
                !roles.Add(file.Role) || !paths.Add(file.Path))
            {
                throw new ArgumentException(
                    "Image-file requests require unique stable roles and canonical absolute paths.",
                    nameof(files));
            }
        }
        ...
        try
        {
                       ...
                       cancellationToken).ConfigureAwait(false))
            {
                ...
                var created = await response.Content.ReadFromJsonAsync<CreateContainerResponse>(
                    JsonOptions,
                    cancellationToken).ConfigureAwait(false)
                    ?? throw new DockerEngineException("Docker returned an empty inspection-container response.");
            ...
            foreach (var file in files)
            {
                result.Add(await InspectContainerFileAsync(
                    containerId,
                    file,
                    cancellationToken).ConfigureAwait(false));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using (var volumeResponse = await _client.PostAsJsonAsync(
                       Api("/volumes/create"),
                       volumeBody,
                       JsonOptions,
                       cancellationToken))
            ...
            using (var createResponse = await _client.PostAsJsonAsync(
                       Api($"/containers/create?name={helperName}"),
                       helperBody,
                       JsonOptions,
                       cancellationToken))
            {
                await EnsureSuccessAsync(createResponse, cancellationToken);
                var created = await createResponse.Content.ReadFromJsonAsync<CreateContainerResponse>(
                    JsonOptions,
                    cancellationToken)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            using var request = new HttpRequestMessage(
                HttpMethod.Get,
                Api($"/containers/{Uri.EscapeDataString(containerId)}/stats?stream=true&one-shot=false"));
            ...
            var monitor = new DockerContainerResourceMonitor(
                response,
                stream,
                lifetime,
                () => ReadOneShotMemoryAsync(containerId));
            response = null;
            return monitor;
        }
        catch
    ...
    private async Task<long> ReadOneShotMemoryAsync(string containerId)
    {
        ...
        using var response = await _client.GetAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/stats?stream=false&one-shot=true"),
            timeout.Token).ConfigureAwait(false);
        await EnsureSuccessAsync(response, timeout.Token).ConfigureAwait(false);
        DockerStatsResponse? sample;
        try
        {
            sample = await response.Content.ReadFromJsonAsync<DockerStatsResponse>(
                JsonOptions,
                timeout.Token).ConfigureAwait(false);
        }
        ...
        {
            throw new DockerEngineException(
                $"Docker returned malformed one-shot container-stats JSON: {exception.Message}");
        }
        ...
        return observed > 0
            ? observed
            : throw new DockerEngineException(
                "Docker one-shot container-stats returned no positive memory sample.");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var response = await _client.PostAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/stop?t={timeoutSeconds}"),
            content: null,
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ValidateContainerId(containerId);
        using var response = await _client.PostAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/wait?condition=not-running"),
            content: null,
            cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        var wait = await response.Content.ReadFromJsonAsync<WaitContainerResponse>(JsonOptions, cancellationToken)
            ?? throw new DockerEngineException("Docker returned an empty wait response.");

        using var inspectResponse = await _client.GetAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/json"),
            cancellationToken);
        await EnsureSuccessAsync(inspectResponse, cancellationToken);
        var inspect = await inspectResponse.Content.ReadFromJsonAsync<InspectContainerResponse>(JsonOptions, cancellationToken)
            ?? throw new DockerEngineException("Docker returned an empty inspect response.");
    ...
    public async Task KillContainerAsync(string containerId, CancellationToken cancellationToken = default)
    {
        ValidateContainerId(containerId);
        using var response = await _client.PostAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/kill?signal=KILL"),
            content: null,
            cancellationToken);
    ...
    public async Task RemoveContainerAsync(string containerId, CancellationToken cancellationToken = default)
    {
        ...
        using var response = await _client.DeleteAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}?force=true&v=true"),
            cancellationToken);
    ...
    public async Task RemoveWorkspaceVolumeAsync(
        string volumeName,
        CancellationToken cancellationToken = default)
    {
        ValidateVolumeName(volumeName);
        using var response = await _client.DeleteAsync(
            Api($"/volumes/{Uri.EscapeDataString(volumeName)}?force=true"),
            cancellationToken);
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            return;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var containers = await response.Content.ReadFromJsonAsync<ListContainerResponse[]>(JsonOptions, cancellationToken)
            ?? [];
        return containers
            .Where(static container => !string.IsNullOrWhiteSpace(container.Id))
            .Select(static container => new ManagedRuntimeContainer(
                container.Id!,
                ParseContainerCreatedAt(container),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return result?.Volumes?
            ...
            .Select(static volume => new ManagedWorkspaceVolume(
                volume.Name!,
                ParseVolumeCreatedAt(volume)))
            .ToArray() ?? [];
    }

    public void Dispose() => _client.Dispose();

    internal static IReadOnlyDictionary<string, string> CreateManagedLabels(
        string managementLabel,
        ...
        string? traceParent = null)
    {
        ...
        var labels = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["com.sharplabnext.created-at"] = DateTimeOffset.UtcNow.ToString(
                "O",
                System.Globalization.CultureInfo.InvariantCulture)
        };
        if (materializer)
    ...
    private async Task<RuntimeImageFileInspection> InspectContainerFileAsync(
        string containerId,
        RuntimeImageFileRequest requested,
        CancellationToken cancellationToken)
    {
        using var response = await _client.GetAsync(
            Api($"/containers/{Uri.EscapeDataString(containerId)}/archive?path=" +
                Uri.EscapeDataString(requested.Path)),
            HttpCompletionOption.ResponseHeadersRead,
            cancellationToken).ConfigureAwait(false);
        await EnsureSuccessAsync(response, cancellationToken).ConfigureAwait(false);
        ...
        while (await reader.GetNextEntryAsync(copyData: false, cancellationToken).ConfigureAwait(false) is { } entry)
        {
            if (entry.EntryType is TarEntryType.GlobalExtendedAttributes or TarEntryType.ExtendedAttributes)
                continue;
            if (bytes is not null || entry.EntryType is not (
                    TarEntryType.RegularFile or TarEntryType.V7RegularFile or TarEntryType.ContiguousFile) ||
                entry.DataStream is null)
            {
                throw new DockerEngineException(
                    $"Image path '{requested.Path}' must resolve to exactly one regular non-link file.");
            }

            bytes = new MemoryStream(entry.Length > 0 && entry.Length <= 268_435_456
            ...
            while (true)
            {
                ...
                {
                    throw new DockerEngineException(
                        $"Image file '{requested.Path}' exceeds the evidence size limit.");
                }
    ...
    private static (string Format, string Architecture) ClassifyImageFile(
        string path,
        ReadOnlySpan<byte> content)
    {
        if (content.Length >= 4 && content[..4].SequenceEqual("\u007fELF"u8))
        {
            if (content.Length < 20 || content[4] != 2 || content[5] != 1 ||
                BinaryPrimitives.ReadUInt16LittleEndian(content[18..20]) != 0x3e)
            {
        ...
        throw new DockerEngineException(
            $"Image file '{path}' is not a supported ELF, PE, managed PE, or script artifact.");
    }
    ...
    private static bool IsCanonicalContainerPath(string path) =>
        ...
        path[1..].Split('/').All(static segment =>
    ...
    private static void ValidateImmutableImageReference(string immutableReference)
    {
        ...
        if (markerIndex <= 0 || markerIndex + marker.Length + 64 != immutableReference.Length ||
            immutableReference.Any(char.IsWhiteSpace) ||
            immutableReference[(markerIndex + marker.Length)..].Any(static character =>
                !char.IsAsciiHexDigit(character) || char.IsAsciiLetterUpper(character)))
        {
            throw new ArgumentException(
                "The Docker image reference must contain one canonical immutable sha256 repository digest.",
                nameof(immutableReference));
        }
    }
    ...
    private static void ValidateVolumeName(string volumeName)
    {
        ...
        if (volumeName.Length > 128 ||
            volumeName.Any(static character =>
    ...
    {
        if (volume.Labels is not null &&
            volume.Labels.TryGetValue("com.sharplabnext.created-at", out var labeled) &&
            DateTimeOffset.TryParse(labeled, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.RoundtripKind, out var labeledTime))
        {
            return labeledTime;
        }
    ...
    private static DateTimeOffset ParseContainerCreatedAt(ListContainerResponse container)
    {
        if (container.Labels is not null &&
            container.Labels.TryGetValue("com.sharplabnext.created-at", out var labeled) &&
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new DockerEngineException(
            $"Docker Engine returned HTTP {(int)response.StatusCode} ({response.StatusCode}): {body}");
    }
    ...
    private sealed record DockerMemoryStats(
        long Usage,
        [property: JsonPropertyName("max_usage")] long MaxUsage);

    private sealed record ListContainerResponse(
        string? Id,
        long Created,
        string? State,
    ...
    private sealed record ListVolumeResponse(
        string? Name,
        string? CreatedAt,
        IReadOnlyDictionary<string, string>? Labels);
    ...
    private sealed class DockerContainerResourceMonitor : IRuntimeContainerResourceMonitor
    {
        ...
        public DockerContainerResourceMonitor(
            HttpResponseMessage response,
            Stream stream,
            CancellationTokenSource lifetime,
            Func<Task<long>> oneShotMemoryReader)
        {
            _response = response;
            _stream = stream;
            _lifetime = lifetime;
        ...
        private async Task<RuntimeContainerResourceUsage> ReadUsageAsync(
            Stream stream,
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                while (await reader.ReadLineAsync(cancellationToken).ConfigureAwait(false) is { } line)
                {
                    ...
                    var sample = JsonSerializer.Deserialize<DockerStatsResponse>(line, JsonOptions)
                        ?? throw new DockerEngineException("Docker returned an empty container-stats sample.");
                    var observed = Math.Max(
                        sample.MemoryStats?.Usage ?? 0,
                        sample.MemoryStats?.MaxUsage ?? 0);
                    if (observed <= 0)
                        continue;
...
internal sealed record RuntimeContainerIsolation(
    string User,
    IReadOnlyDictionary<string, string> Tmpfs)
{
    ...
    public static RuntimeContainerIsolation Resolve(
        RuntimeContainerIsolationKind kind,
        RuntimeSecurityPolicyOptions securityPolicy,
        string? winePrefixPath = null)
    {
        ArgumentNullException.ThrowIfNull(securityPolicy);
    ...
    private static RuntimeContainerIsolation ResolveWineNonRoot(
        RuntimeSecurityPolicyOptions securityPolicy,
    ...
    {
        if (string.IsNullOrWhiteSpace(winePrefixPath) ||
            !winePrefixPath.StartsWith("/opt/", StringComparison.Ordinal) ||
            winePrefixPath.EndsWith('/') ||
            winePrefixPath.Contains("..", StringComparison.Ordinal) ||
            winePrefixPath.Any(static character =>
                !char.IsAsciiLetterOrDigit(character) && character is not ('/' or '-' or '_' or '.')))
        {
            throw new InvalidOperationException(
                "Wine isolation requires a safe absolute prefix below /opt.");
        }
    }
}
