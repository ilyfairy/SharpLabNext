...
internal sealed class LocalArtifactStore : IDisposable
{
    ...
    private static readonly Action<ILogger, string, Exception?> AbandonedStagingCleanupFailed =
        LoggerMessage.Define<string>(
            LogLevel.Warning,
            new EventId(1001, nameof(AbandonedStagingCleanupFailed)),
            "Could not remove abandoned Artifact Store staging directory {Directory}");
    private static readonly Action<ILogger, string, Exception?> ArtifactDeleteFailed =
        LoggerMessage.Define<string>(
            LogLevel.Error,
            new EventId(1002, nameof(ArtifactDeleteFailed)),
            "Could not delete collected artifact directory {Directory}");
    private static readonly Action<ILogger, string, Exception?> ContentDeleteFailed =
        LoggerMessage.Define<string>(
            LogLevel.Error,
            new EventId(1003, nameof(ContentDeleteFailed)),
            "Could not delete collected content file {File}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var staged = await StageContentAsync(
                content,
                expectedContentRef,
                declaredSize,
                _options.MaxContentBytes,
                Path.Combine(stagingDirectory, "content.tmp"),
    ...
    public async Task<PutArtifactResponse> PutArtifactAsync(
        ...
        IReadOnlyList<ArtifactUploadSource> uploads,
        TimeSpan? timeToLive,
        CancellationToken cancellationToken)
    {
        await InitializeAsync(cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            uploadByPath = uploads.ToDictionary(
                upload => ArtifactPath.Normalize(upload.Path),
        ...
        try
        {
            ...
            foreach (var (path, file) in manifestFiles.OrderBy(pair => pair.Key, StringComparer.Ordinal))
            {
                ...
                await using (stream.ConfigureAwait(false))
                {
                    ...
                    var staged = await StageContentAsync(
                        ...
                        file.Size,
                        _options.MaxContentBytes,
                        Path.Combine(stagingDirectory, $"content-{fileIndex++}.tmp"),
                        cancellationToken).ConfigureAwait(false);
            ...
            try
            {
                ...
                var entries = stagedContents.Select(item => new ArtifactBundleEntry(
                    item.Path,
                    item.Content.Size,
                    item.File.Digest,
                    item.File.Role,
                    item.Content.ContentRef)).ToArray();
                var descriptor = new ArtifactBundleDescriptor(manifest, entries);
                var artifactRelativePath = await PublishArtifactDirectoryAsync(
                    descriptor,
                    stagingDirectory,
                    cancellationToken).ConfigureAwait(false);
                var inserted = await _database.CommitArtifactAsync(
                    descriptor,
                    ...
                    expiresAt,
                    cancellationToken).ConfigureAwait(false);
                var stored = await _database.GetArtifactAsync(expectedArtifactRef, now, cancellationToken).ConfigureAwait(false)
                    ?? throw new ArtifactCorruptedException("Artifact metadata disappeared immediately after commit.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var metadata = await _database.GetArtifactAsync(artifactRef, DateTimeOffset.UtcNow, cancellationToken)
    ...
    public async Task<VerifiedContent> OpenContentReadAsync(
        ContentRef contentRef,
        CancellationToken cancellationToken)
    {
        await InitializeAsync(cancellationToken).ConfigureAwait(false);
        _ = ArtifactStoreProtocol.GetDigest(contentRef);
        var metadata = await _database.GetContentAsync(contentRef, DateTimeOffset.UtcNow, cancellationToken)
            .ConfigureAwait(false)
            ?? throw new ArtifactNotFoundException($"Content '{contentRef}' was not found.");
        return await OpenAndVerifyContentAsync(metadata.ContentRef, metadata.Size, metadata.RelativePath, cancellationToken)
            .ConfigureAwait(false);
    }

    public async Task<VerifiedContent> OpenArtifactFileReadAsync(
        ArtifactRef artifactRef,
        string path,
        CancellationToken cancellationToken)
    {
        ...
        var metadata = await _database.GetArtifactEntryAsync(
            artifactRef,
            normalizedPath,
            DateTimeOffset.UtcNow,
            cancellationToken).ConfigureAwait(false)
            ?? throw new ArtifactNotFoundException($"Artifact file '{normalizedPath}' was not found.");
        return await OpenAndVerifyContentAsync(metadata.ContentRef, metadata.Size, metadata.RelativePath, cancellationToken)
            .ConfigureAwait(false);
    }

    public async Task<LeaseMetadata> AcquireLeaseAsync(
        ArtifactRef artifactRef,
        string owner,
        TimeSpan duration,
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 0; attempt < 3; attempt++)
        {
            ...
            var created = await _database.CreateLeaseAsync(
                artifactRef,
                HashLeaseToken(token),
                owner,
                now,
                expiresAt,
                cancellationToken).ConfigureAwait(false);
    ...
    public async Task<LeaseMetadata> RenewLeaseAsync(
        string leaseToken,
        TimeSpan duration,
        CancellationToken cancellationToken)
    {
        ...
        var now = DateTimeOffset.UtcNow;
        var expiresAt = now.Add(effectiveDuration);
        var lease = await _database.RenewLeaseAsync(
            HashLeaseToken(leaseToken),
            now,
            expiresAt,
            cancellationToken).ConfigureAwait(false)
            ?? throw new ArtifactNotFoundException("The lease was not found or has expired.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var plan = await _database.CollectGarbageAsync(
                DateTimeOffset.UtcNow,
                maxArtifacts,
                maxContents,
                cancellationToken).ConfigureAwait(false);
            ...
            return new GarbageCollectionResponse(
                plan.ExpiredLeasesDeleted,
                plan.ArtifactRelativePaths.Count,
                plan.Contents.Count,
                plan.Contents.Sum(content => content.Size));
        }
        finally
        {
            _mutationGate.Release();
    ...
    private static async Task<StagedContent> StageContentAsync(
        Stream source,
        ContentRef expectedContentRef,
        long? expectedSize,
        long maximumSize,
    ...
    private async Task<PublishedContent> PublishContentAsync(
        StagedContent staged,
        CancellationToken cancellationToken)
    {
        var destinationPath = GetContentPath(staged.Digest);
        var destinationDirectory = Path.GetDirectoryName(destinationPath)
            ?? throw new ArtifactCorruptedException("Content path did not have a parent directory.");
        ...
        if (alreadyExisted)
        {
            await VerifyExistingFileAsync(destinationPath, staged.ContentRef, staged.Size, cancellationToken)
                .ConfigureAwait(false);
        ...
        return new PublishedContent(
            staged.ContentRef,
            staged.Digest,
            StorageSafety.ToDatabaseRelativePath(_root, destinationPath),
            staged.Size,
            alreadyExisted);
    }

    private async Task<string> PublishArtifactDirectoryAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var destinationParent = Path.GetDirectoryName(destination)
            ?? throw new ArtifactCorruptedException("Artifact path did not have a parent directory.");
    ...
    private async Task<VerifiedContent> OpenAndVerifyContentAsync(
        ContentRef contentRef,
        long expectedSize,
        string relativePath,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (actualSize != expectedSize
                || !string.Equals(actualDigest, ArtifactStoreProtocol.GetDigest(contentRef), StringComparison.Ordinal))
            {
    ...
    private async Task VerifyExistingFileAsync(
        string path,
        ContentRef contentRef,
        long expectedSize,
        CancellationToken cancellationToken)
    {
        ...
        await using var verified = await OpenAndVerifyContentAsync(contentRef, expectedSize, relativePath, cancellationToken)
            .ConfigureAwait(false);
    }

    private async Task VerifyArtifactMetadataAsync(
    ...
    private static void ValidateLeaseToken(string token)
    {
        if (string.IsNullOrWhiteSpace(token)
            || token.Length is < 20 or > 128
            || !token.StartsWith("lease_", StringComparison.Ordinal)
    ...
    private static void ValidateManifest(ArtifactRef expectedArtifactRef, ArtifactManifest manifest)
    {
        if (manifest.ManifestVersion != ArtifactStoreProtocol.ArtifactManifestVersion)
        {
            throw new ArtifactValidationException(
                $"Artifact manifest version {manifest.ManifestVersion} is not supported.");
        }
