...
internal sealed record WineRuntimePackageManifestSnapshot(
...
{
    Task<WineRuntimePackageManifestSnapshot> LoadValidatedAsync(
        string repositoryRoot,
        ReleaseLockDocument releaseLock,
        CancellationToken cancellationToken);
}

internal sealed class RepositoryWineRuntimePackageManifestSnapshotProvider
    : IWineRuntimePackageManifestSnapshotProvider
{
    public async Task<WineRuntimePackageManifestSnapshot> LoadValidatedAsync(
        string repositoryRoot,
        ReleaseLockDocument releaseLock,
        CancellationToken cancellationToken)
    {
...
internal static class WineRuntimePackageManifestLoader
{
    ...
    internal const string RequiredNoticeArchiveImagePath =
        "/usr/local/share/sharplabnext/wine-coreclr-copyright-notices.tar";
    ...
    private const string RequiredComponentVersion =
        "wine-9.0~repack-4build3+xvfb-2:21.1.12-1ubuntu1.6";
    private const string RequiredComponentSourceUri =
        "https://snapshot.ubuntu.com/ubuntu/20260810T000000Z/";
    private const string RequiredWineVersion = "9.0~repack-4build3";
    private const string RequiredWineSourceBaseUri =
        "https://snapshot.ubuntu.com/ubuntu/20260810T000000Z/pool/universe/w/wine/";
    private const string RequiredResolvedPackageListSha256 =
        "sha256:fa83c245764fc09102029b249f5149a48baeda53a40c0432de973ebe09e39dee";
    private const string RequiredUbuntuArchiveSigningKeyFingerprint =
    ...
    private static readonly Dictionary<string, ReviewedDirectPackage> RequiredDirectPackages =
        new Dictionary<string, ReviewedDirectPackage>(StringComparer.Ordinal)
        {
            ["wine"] = new(
                RequiredWineVersion,
                "all",
                "pool/universe/w/wine/wine_9.0~repack-4build3_all.deb",
                "5606f0f5677ac0fb882e3fdc8ce03a60232cdf2930e912ed3e42db2b9e0d5f7f",
                "wine",
                "LGPL-2.1+",
                RequiredWineSourceBaseUri + "wine_9.0~repack-4build3.dsc",
                "5d720edb86a3069749efe89c3a9d886c7faa19aa3f55f1e9c4a8e0abda8bda85",
                3826),
            ["wine64"] = new(
                RequiredWineVersion,
                "amd64",
                "pool/universe/w/wine/wine64_9.0~repack-4build3_amd64.deb",
                "7f4f912b21917f28c281c99e3244ee801be2e20f0a23cd9f6d37acc97f5041ff",
                "wine",
                "LGPL-2.1+",
                RequiredWineSourceBaseUri + "wine_9.0~repack-4build3.dsc",
                "5d720edb86a3069749efe89c3a9d886c7faa19aa3f55f1e9c4a8e0abda8bda85",
            ...
            ["fonts-wine"] = new(
                RequiredWineVersion,
                "all",
                "pool/universe/w/wine/fonts-wine_9.0~repack-4build3_all.deb",
                "7dcb227d236eed96c707f532dd498ef227da27b2950ee49bbea37efc309e2bcc",
                "wine",
                "LGPL-2.1+",
                RequiredWineSourceBaseUri + "wine_9.0~repack-4build3.dsc",
                "5d720edb86a3069749efe89c3a9d886c7faa19aa3f55f1e9c4a8e0abda8bda85",
            ...
            ["xvfb"] = new(
                ...
                "amd64",
                "pool/universe/x/xorg-server/xvfb_21.1.12-1ubuntu1.6_amd64.deb",
                "9188c2ab6394dfe6aa53f782f6bfa22b7eb6febfe51a48e0e36af35cd2f64307",
                "xorg-server",
                "MIT",
                "https://snapshot.ubuntu.com/ubuntu/20260810T000000Z/pool/main/x/xorg-server/xorg-server_21.1.12-1ubuntu1.6.dsc",
                "a863f83234ff1bb20147b774820ffac461b01dc3747de8e4275f7f76b3004094",
                4381)
        };
    ...
    private static readonly Dictionary<string, ReviewedSourceFile> RequiredWineSourceFiles =
        ...
        {
            ["wine_9.0~repack-4build3.dsc"] = new(
                "5d720edb86a3069749efe89c3a9d886c7faa19aa3f55f1e9c4a8e0abda8bda85",
                3826),
            ["wine_9.0~repack.orig.tar.xz"] = new(
                "b956a23e00a5083f46c5c5ce0fbb3428460548a55ec1414cc20c6c21c7c8d0a7",
                26988196),
            ["wine_9.0~repack-4build3.debian.tar.xz"] = new(
                "0e1ac34c2272c560df213602495e2792de8a1c31bf27a6b6fbea39289dfc145a",
                58753032)
        };
    ...
    internal static async Task<WineRuntimePackageManifest> LoadAsync(
        string repositoryRoot,
        CancellationToken cancellationToken) =>
        (await LoadSnapshotAsync(repositoryRoot, cancellationToken)).Manifest;

    internal static async Task<WineRuntimePackageManifestSnapshot> LoadSnapshotAsync(
        string repositoryRoot,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var manifest = JsonSerializer.Deserialize<WineRuntimePackageManifest>(
                bytes,
                JsonOptions)
                ?? throw new BundleValidationException("Wine package inventory manifest is empty.");
            Validate(manifest);
            return new WineRuntimePackageManifestSnapshot(
                manifest,
        ...
        catch (JsonException exception)
        {
            throw new BundleValidationException(
                $"Wine package inventory manifest is invalid: {exception.Message}");
        }
    }

    internal static void ValidateBaseImage(
        WineRuntimePackageManifest manifest,
        BaseImageManifest baseImages)
    {
        if (!baseImages.Images.Any(image =>
                string.Equals(image.Id, manifest.BaseImageId, StringComparison.Ordinal)))
        {
            throw new BundleValidationException(
                $"Wine package inventory references unknown base image '{manifest.BaseImageId}'.");
        }
    }

    internal static void ValidateResolvedPackagesForBundle(WineRuntimePackageManifest manifest)
    {
        if (manifest.ResolvedPackages.Count != RequiredResolvedPackageCount ||
            manifest.SourcePackages.Count != RequiredSourcePackageCount)
        {
            throw new BundleValidationException(
                $"Wine operating-system inventory must contain exactly {RequiredResolvedPackageCount} binary packages and {RequiredSourcePackageCount} source packages.");
        }
    ...
    internal static void ValidateReleaseLock(
        WineRuntimePackageManifestSnapshot snapshot,
        ReleaseLockDocument releaseLock)
    {
        var manifest = snapshot.Manifest;
        if (!releaseLock.Components.TryGetValue(manifest.Component.Id, out var component) ||
            !string.Equals(component.Kind, manifest.Component.Kind, StringComparison.Ordinal) ||
            !string.Equals(component.ResolvedVersion, manifest.Component.ResolvedVersion, StringComparison.Ordinal) ||
            !string.Equals(component.Digest, snapshot.ManifestSha256, StringComparison.Ordinal) ||
            !string.Equals(component.SourceUri, manifest.Component.SourceUri, StringComparison.Ordinal))
        {
            throw new BundleValidationException(
                $"Wine package inventory '{manifest.Component.Id}' does not match its release lock identity.");
        }
    }
    ...
    internal static Uri SourceUri(WineRuntimePackageManifest manifest, WineSourceOfferFile file)
    {
        ...
        if (!string.Equals(uri.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase) ||
            ...
            !string.Equals(uri.AbsolutePath, new Uri(manifest.SourceOffer.BaseUri).AbsolutePath + file.Path, StringComparison.Ordinal))
        {
    ...
    internal static Uri ArchiveUri(
        WineRuntimePackageManifest manifest,
        string archiveSnapshotId,
        string relativePath)
    {
        var snapshot = manifest.ArchiveSnapshots.SingleOrDefault(item =>
            string.Equals(item.Id, archiveSnapshotId, StringComparison.Ordinal))
            ?? throw new BundleValidationException(
                $"Wine package evidence references unknown archive snapshot '{archiveSnapshotId}'.");
    ...
    private static void Validate(WineRuntimePackageManifest manifest)
    {
        if (manifest.SchemaVersion != 1 ||
            ...
            manifest.SourcePackages.Count != RequiredSourcePackageCount ||
            !string.Equals(
                manifest.ResolvedPackageListSha256,
                RequiredResolvedPackageListSha256,
    ...
    private static void ValidateArchiveSnapshots(IReadOnlyList<WineArchiveSnapshot> snapshots)
    {
        ...
        {
            if (!actual.TryAdd(snapshot.Id, snapshot) ||
                !RequiredArchiveSnapshots.TryGetValue(snapshot.Id, out var expected) ||
                !string.Equals(snapshot.Purpose, expected.Purpose, StringComparison.Ordinal) ||
                !string.Equals(snapshot.Uri, expected.Uri, StringComparison.Ordinal) ||
                snapshot.Suites.Count != expected.Suites.Count)
            {
            ...
            if (suites.Count != snapshot.Suites.Count || expected.Suites.Any(pair =>
                    !suites.TryGetValue(pair.Key, out var suite) ||
                    !string.Equals(suite.InReleaseSha256, pair.Value.Sha256, StringComparison.Ordinal) ||
                    suite.InReleaseSizeBytes != pair.Value.SizeBytes ||
                    !string.Equals(
                        suite.SigningKeyFingerprint,
                        RequiredUbuntuArchiveSigningKeyFingerprint,
                        StringComparison.Ordinal)))
            {
                throw new BundleValidationException(
                    $"Wine archive snapshot '{snapshot.Id}' does not match its reviewed signed index identity.");
            }
    ...
    private static void ValidateArchiveIndexes(string snapshotId, WineArchiveSnapshotSuite suite)
    {
        ...
        foreach (var index in suite.Indexes)
        {
            ...
            var architectureValid = binary
                ? string.Equals(index.Architecture, "amd64", StringComparison.Ordinal)
                : index.Architecture is null;
            var expectedPrefix = source
                ? $"{index.Component}/source/Sources."
                : $"{index.Component}/binary-{index.Architecture}/Packages.";
            if ((!binary && !source) || !IsArchiveComponent(index.Component) || !architectureValid ||
                !IsCanonicalRelativePath(index.Path) ||
                !index.Path.StartsWith(expectedPrefix, StringComparison.Ordinal) ||
                !(index.Path.EndsWith(".gz", StringComparison.Ordinal) ||
                  index.Path.EndsWith(".xz", StringComparison.Ordinal)) ||
                !IsSha256(index.Sha256) || index.SizeBytes <= 0 ||
                !identities.Add($"{index.Kind}\0{index.Component}\0{index.Architecture}") ||
                !paths.Add(index.Path))
            {
                throw new BundleValidationException(
                    $"Wine archive suite '{snapshotId}/{suite.Name}' has an invalid or ambiguous signed index.");
            }
        }
    }
    ...
    private static void ValidateDirectPackages(IReadOnlyList<WineDirectPackage> packages)
    {
        ...
        foreach (var package in packages)
        {
            ...
            {
                throw new BundleValidationException(
                    $"Wine direct package '{package.Name}' does not match its reviewed public identity.");
            }
        }
    }

    private static void ValidateSourcePackages(
        IReadOnlyList<WineSourcePackage> packages,
        IReadOnlyList<WineArchiveSnapshot> snapshots,
        bool requireReviewedTotal)
    {
        ...
        foreach (var package in packages)
        {
            ...
            if (!IsStableId(package.Name) || string.IsNullOrWhiteSpace(package.Version) ||
                StringComparer.Ordinal.Compare(previous, identity) >= 0 || package.Files.Count == 0 ||
                !HasSnapshotIndex(
                    snapshots,
                    package.ArchiveSnapshotId,
                    package.ArchiveSuite,
                    package.ArchiveComponent,
                    package.ArchiveIndexPath,
                    "source",
                    null))
            {
                throw new BundleValidationException(
                    "Wine source package inventory must be complete, ordinally sorted, and snapshot bound.");
            }
            ...
            foreach (var file in package.Files)
            {
                ...
                if (!IsCanonicalPoolPath(file.Path) || !IsSha256(file.Sha256) ||
                    file.SizeBytes <= 0 || file.SizeBytes > MaximumClosureSourceFileBytes ||
                    !filePaths.Add(file.Path) ||
                    sourceDirectory is not null && !string.Equals(sourceDirectory, directory, StringComparison.Ordinal))
                {
                    throw new BundleValidationException(
                        $"Wine source package '{package.Name}@{package.Version}' has invalid or duplicate source material.");
                }
            ...
            {
                throw new BundleValidationException(
                    $"Wine source package '{package.Name}@{package.Version}' has no source descriptor.");
            }
            previous = identity;
        }

        if (fileCount != RequiredSourceFileCount ||
            requireReviewedTotal && totalBytes != RequiredSourceTotalBytes)
        {
            throw new BundleValidationException(
                $"Wine source closure must contain exactly {RequiredSourcePackageCount} sources, {RequiredSourceFileCount} files, and the reviewed total byte count.");
        }
    }

    private static void ValidateResolvedPackages(
        IReadOnlyList<WineResolvedPackage> packages,
        IReadOnlyList<WineSourcePackage> sourcePackages,
        IReadOnlyList<WineArchiveSnapshot> snapshots)
    {
        var sources = sourcePackages.ToDictionary(
            static package => $"{package.Name}\0{package.Version}",
        ...
        foreach (var package in packages)
        {
            ...
            {
                throw new BundleValidationException(
                    "Wine resolved package inventory must contain complete binary, source, snapshot, and copyright evidence.");
            }
            if (copyrights.TryGetValue(package.CopyrightPath, out var existing) &&
            ...
            {
                throw new BundleValidationException(
                    $"Wine copyright path '{package.CopyrightPath}' has conflicting identities.");
            }
    ...
    private static void ValidateNoticeArchive(
        WineNoticeArchive archive,
        IReadOnlyList<WineResolvedPackage> packages)
    {
        var entries = packages.Select(static package => package.CopyrightPath)
            .Distinct(StringComparer.Ordinal)
            .Count();
        if (!string.Equals(archive.ImagePath, RequiredNoticeArchiveImagePath, StringComparison.Ordinal) ||
            !IsSha256(archive.Sha256) || archive.SizeBytes <= 0 ||
            archive.SizeBytes > MaximumNoticeArchiveBytes || archive.EntryCount != entries)
        {
            throw new BundleValidationException(
                "Wine notice archive does not bind the exact deduplicated copyright inventory.");
        }
    }

    private static bool HasSnapshotIndex(
        IReadOnlyList<WineArchiveSnapshot> snapshots,
        string snapshotId,
        string suite,
        string component,
        ...
        string kind,
        string? architecture) =>
        snapshots.Any(snapshot => string.Equals(snapshot.Id, snapshotId, StringComparison.Ordinal) &&
            snapshot.Suites.Any(item => string.Equals(item.Name, suite, StringComparison.Ordinal) &&
                item.Indexes.Any(index =>
                    string.Equals(index.Kind, kind, StringComparison.Ordinal) &&
                    string.Equals(index.Component, component, StringComparison.Ordinal) &&
                    string.Equals(index.Architecture, architecture, StringComparison.Ordinal) &&
                    string.Equals(index.Path, indexPath, StringComparison.Ordinal))));

    private static void ValidateDirectPackagesAreResolved(
        IReadOnlyList<WineDirectPackage> directPackages,
        IReadOnlyList<WineResolvedPackage> resolvedPackages)
    {
        var resolved = resolvedPackages
            .Select(static package => $"{package.Name}\0{package.Version}")
    ...
    private static void ValidateSourceOffer(
        WineSourceOffer offer,
        IReadOnlyList<WineSourcePackage> sourcePackages)
    {
        ...
        {
            if (!IsCanonicalFileName(file.Path) || !IsSha256(file.Sha256) || file.SizeBytes <= 0 ||
                file.SizeBytes > MaximumSourceFileBytes || !names.Add(file.Path))
            {
                throw new BundleValidationException("Wine source offer has an invalid file identity.");
            }
            if (!RequiredWineSourceFiles.TryGetValue(file.Path, out var expected) ||
                !string.Equals(file.Sha256, expected.Sha256, StringComparison.Ordinal) ||
                file.SizeBytes != expected.SizeBytes)
            {
                throw new BundleValidationException(
                    $"Wine source material '{file.Path}' does not match its reviewed public identity.");
            }
            total = checked(total + file.SizeBytes);
        }

        if (total > MaximumSourceTotalBytes ||
            !names.SetEquals(RequiredWineSourceOfferPaths) ||
            !names.SetEquals(RequiredWineSourceFiles.Keys))
        {
            throw new BundleValidationException("Wine source offer must contain exactly the reviewed three-file source closure.");
        }

        var wineSource = sourcePackages.SingleOrDefault(source =>
            string.Equals(source.Name, offer.Package, StringComparison.Ordinal) &&
            string.Equals(source.Version, offer.Version, StringComparison.Ordinal));
        if (wineSource is null || wineSource.Files.Count != offer.Files.Count ||
            offer.Files.Any(offerFile => !wineSource.Files.Any(sourceFile =>
                sourceFile.Path.EndsWith('/' + offerFile.Path, StringComparison.Ordinal) &&
                string.Equals(sourceFile.Sha256, offerFile.Sha256, StringComparison.Ordinal) &&
                sourceFile.SizeBytes == offerFile.SizeBytes)))
        {
            throw new BundleValidationException(
                "Wine source offer is not the same material as the complete operating-system source closure.");
        }
    }

    private static void ValidateResolvedPackageDigest(
        IReadOnlyList<WineResolvedPackage> packages,
    ...
    private static bool IsSha256Digest(string value) =>
        value.Length == 71 && value.StartsWith("sha256:", StringComparison.Ordinal) && IsSha256(value[7..]);
    ...
    private static bool IsCanonicalRelativePath(string value) =>
        ...
        value.Split('/').All(static segment => segment is not "" and not "." and not ".." &&
            segment.All(static character => char.IsAsciiLetterOrDigit(character) || character is '.' or '_' or '-' or '~' or '+'));

    private static bool IsCanonicalFileName(string value) =>
        !value.Contains('/') && IsCanonicalRelativePath(value);

    private static bool IsCanonicalPoolPath(string value) =>
        value.StartsWith("pool/", StringComparison.Ordinal) && IsCanonicalRelativePath(value);

    private static bool IsArchiveComponent(string value) =>
        value is "main" or "universe" or "restricted" or "multiverse";
    ...
    private sealed record ReviewedDirectPackage(
        string Version,
        string Architecture,
        string Path,
        string Sha256,
    ...
    private sealed record ReviewedArchiveSnapshot(
        string Purpose,
        string Uri,
        IReadOnlyDictionary<string, ReviewedArchiveSuite> Suites);
