...
public sealed class ReleaseBundleBuilder
{
    ...
    public const string WineNoticeArchiveBundleRelativePath = "notices/wine-coreclr-copyright-notices.tar";
    ...
    public async Task<BundleBuildResult> BuildAsync(BundleBuilderCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var runtimeProfilesPath = command.RuntimeProfilesPath ?? Path.Combine(command.RepositoryRoot, "profiles", "runtimes");
        ...
        var runtimeMatrixBaseImagesTask = RuntimeMatrixBaseImageBindings.LoadAsync(command.RepositoryRoot, cancellationToken);
        var runtimeProfilesTask = LoadRuntimeProfilesAsync(runtimeProfilesPath, cancellationToken);
        await Task.WhenAll(catalogTask, lockTask, deploymentTask, baseImagesTask, runtimeMatrixBaseImagesTask, runtimeProfilesTask);
        ...
        var wineManifestSnapshot = await wineManifestProvider.LoadValidatedAsync(command.RepositoryRoot, releaseLock, cancellationToken);
        ...
        var maintainedProvenance = await MaintainedProvenanceLoader.LoadAsync(command.RepositoryRoot, releaseLock, baseImages, cancellationToken);
        var dependencies = (await DependencyInventory.LoadAsync(command.RepositoryRoot, command.LicensePolicyPath, cancellationToken)).Components;
        var expectedReferenceSetDigests = ValidateInputs(catalog, releaseLock, deployment, command.ImageOverrides);
        ...
        var promotionBoundRuntimeIds = activeRuntimeProfiles.Where(static profile => profile.PromotionReceipt is not null).Select(static profile => profile.Id).ToHashSet(StringComparer.Ordinal);

        var imagePlan = CreateImagePlan(catalog, deployment, command.ImagePrefix, command.ImageOverrides);
        var definitionsById = deployment.Images.ToDictionary(static image => image.Id, StringComparer.Ordinal);
        var pendingInspections = new List<(DeploymentImageDefinition Definition, string Reference, DockerImageInspection Inspection)>(imagePlan.Images.Count);
        ...
        var runtimePromotionSourceClosure = await RuntimePromotionSourceClosure.CaptureAsync(command.RepositoryRoot, source, runtimePromotionTrust, runtimePromotionSourceInspector, cancellationToken);
        var releaseRuntimeProfiles = MaterializeRuntimeProfiles(catalog, releaseLock, inspectedImages, activeRuntimeProfiles);
        ...
        var outputParent = Path.GetDirectoryName(output) ?? throw new BundleValidationException("Bundle output has no parent directory.");
        ...
        return new BundleBuildResult(output, catalog.ReleaseId, inspectedImages, !command.MetadataOnly, command.SigningKeyPath is not null);
    }

    public static async Task<ReleaseImagePlan> CreateImagePlanAsync(BundleBuilderCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
        EnsureInputFile(command.CatalogPath);
        EnsureInputFile(command.LockPath);
        ...
        var baseImagesTask = LoadBaseImagesAsync(baseImagesPath, cancellationToken);
        var runtimeProfilesTask = LoadRuntimeProfilesAsync(runtimeProfilesPath, cancellationToken);
        await Task.WhenAll(catalogTask, lockTask, deploymentTask, baseImagesTask, runtimeProfilesTask);
        var catalog = await catalogTask;
        var releaseLock = await lockTask;
        var deployment = await deploymentTask;
        var baseImages = await baseImagesTask;
        var runtimeProfiles = await runtimeProfilesTask;
        var wineManifestSnapshot = await new RepositoryWineRuntimePackageManifestSnapshotProvider().LoadValidatedAsync(command.RepositoryRoot, releaseLock, cancellationToken);
        ValidateBaseImages(baseImages);
        WineRuntimePackageManifestLoader.ValidateResolvedPackagesForBundle(wineManifestSnapshot.Manifest);
        WineRuntimePackageManifestLoader.ValidateBaseImage(wineManifestSnapshot.Manifest, baseImages);
        _ = await MaintainedProvenanceLoader.LoadAsync(command.RepositoryRoot, releaseLock, baseImages, cancellationToken);
        _ = await DependencyInventory.LoadAsync(command.RepositoryRoot, command.LicensePolicyPath, cancellationToken);
        ...
        return CreateImagePlan(catalog, deployment, command.ImagePrefix, command.ImageOverrides);
    }

    public static ReleaseImagePlan CreateImagePlan(CatalogDocument catalog, DeploymentImageManifest deployment, string? imagePrefix, IReadOnlyDictionary<string, string>? imageOverrides = null)
    {
    ...
    public static IReadOnlyList<DeploymentImageDefinition> SelectImages(CatalogDocument catalog, DeploymentImageManifest deployment)
    {
        ...
        foreach (var definition in deployment.Images)
        {
            ...
            if (definition.ToolchainId is not null)
            {
                ...
                selected |= catalog.Toolchains.Any(toolchain => string.Equals(toolchain.WorkerId, workerId, StringComparison.Ordinal) && toolchain.Availability.IsSelectable);
            }
    ...
    public static string CreateImageReference(DeploymentImageDefinition definition, string releaseId, string? imagePrefix)
    {
    ...
    private static string ResolveImageReference(DeploymentImageDefinition definition, string releaseId, string? imagePrefix, IReadOnlyDictionary<string, string> imageOverrides)
    {
        if (imageOverrides.TryGetValue(definition.Id, out var configured))
        {
            if (definition.ImmutableReference is not null && !string.Equals(configured, definition.ImmutableReference, StringComparison.Ordinal))
            {
                throw new BundleValidationException($"Deployment image '{definition.Id}' is promotion-bound to " + $"'{definition.ImmutableReference}' and cannot be overridden with '{configured}'.");
            }

            return configured;
        }

        return definition.ImmutableReference ?? CreateImageReference(definition, releaseId, imagePrefix);
    }

    public static string CreateComposeOverlay(CatalogDocument catalog, IReadOnlyList<InspectedImage> images, IReadOnlyList<RuntimeProfileDefinition>? runtimeProfiles = null)
    {
        var builder = new StringBuilder();
        var toolchainWorkerImages = IndexToolchainWorkerImages(catalog, images);
        var toolchainWorkerExpectations = catalog.Toolchains.Where(static toolchain => toolchain.Availability.IsSelectable).Select(static toolchain => toolchain.WorkerId).Distinct(StringComparer.Ordinal).OrderBy(static workerId => workerId, StringComparer.Ordinal).Select(workerId => new { WorkerId = workerId, ImageId = toolchainWorkerImages.TryGetValue(workerId, out var image) ? image.ImageId : throw new BundleValidationException($"Selectable language worker '{workerId}' has no inspected deployment image.") })
            .ToArray();
        var artifactWorkerImages = images.Where(static image => image.ArtifactProcessorId is not null).ToDictionary(static image => image.ArtifactProcessorId!, StringComparer.Ordinal);
        var artifactWorkerExpectations = catalog.ArtifactProcessors.Where(static processor => processor.Availability.IsSelectable).OrderBy(static processor => processor.WorkerId, StringComparer.Ordinal).Select(processor => new { processor.WorkerId, ImageId = artifactWorkerImages[processor.Id].ImageId })
            .ToArray();
        builder.AppendLine("# Generated by SharpLabNext.BundleBuilder. Do not edit.");
        builder.AppendLine("services:");
        foreach (var image in images.Where(static item => item.ComposeService is not null && item.ComposeService != "runtime-supervisor").OrderBy(static item => item.ComposeService, StringComparer.Ordinal))
        {
            ...
            if (image.ReleaseIdEnvironment is not null || image.ImageIdEnvironment is not null || isGateway && (toolchainWorkerExpectations.Length > 0 || artifactWorkerExpectations.Length > 0))
            {
                builder.AppendLine("    environment:");
                if (image.ReleaseIdEnvironment is not null)
                {
                    builder.Append("      ").Append(image.ReleaseIdEnvironment).Append(": \"").Append(EscapeYaml(catalog.ReleaseId)).AppendLine("\"");
                }
                if (image.ImageIdEnvironment is not null)
                {
                    builder.Append("      ").Append(image.ImageIdEnvironment).Append(": \"").Append(image.ImageId).AppendLine("\"");
                }
                if (isGateway)
                {
                    foreach (var expectation in toolchainWorkerExpectations)
                    {
                        builder.Append("      Services__LanguageWorkers__").Append(expectation.WorkerId).Append("__ExpectedWorkerImageId: \"").Append(expectation.ImageId).AppendLine("\"");
                    }
                    foreach (var expectation in artifactWorkerExpectations)
                    {
                        builder.Append("      Services__ArtifactWorkers__").Append(expectation.WorkerId).Append("__ExpectedWorkerImageId: \"").Append(expectation.ImageId).AppendLine("\"");
                    }
                }
            }
        }

        var runtimes = catalog.Runtimes.Where(static runtime => runtime.Availability.IsSelectable).ToArray();
        runtimeProfiles ??= [];
        var runtimeProfileIndex = IndexRuntimeProfiles(runtimeProfiles);
        var selectableRuntimeIds = runtimes.Select(static runtime => runtime.Id).ToHashSet(StringComparer.Ordinal);
        foreach (var profile in runtimeProfiles)
        {
            if (!selectableRuntimeIds.Contains(profile.Id))
            {
                throw new BundleValidationException($"Runtime profile '{profile.Id}' is not a selectable Catalog runtime.");
            }
        }
        foreach (var runtime in runtimes)
        {
            ...
            {
                throw new BundleValidationException($"Selectable runtime '{runtime.Id}' has no materialized active runtime profile.");
            }
        }
        ...
        if (runtimes.Length > 0)
        {
            ...
            {
                AppendSecurityPolicy(builder, $"RuntimeSupervisorProfileOverlay__SecurityPolicies__{index}", securityPolicies[index]);
            }
        }
    ...
    private static Dictionary<string, InspectedImage> IndexToolchainWorkerImages(CatalogDocument catalog, IReadOnlyList<InspectedImage> images)
    {
        var toolchains = catalog.Toolchains.ToDictionary(static item => item.Id, StringComparer.Ordinal);
        ...
        foreach (var image in images.Where(static item => item.ToolchainId is not null))
        {
            ...
            {
                throw new BundleValidationException($"Inspected image '{image.Id}' references missing toolchain '{image.ToolchainId}'.");
            }
            if (!byWorkerId.TryAdd(toolchain.WorkerId, image))
            {
                throw new BundleValidationException($"Language worker '{toolchain.WorkerId}' is represented by more than one inspected image.");
            }
        }
        return byWorkerId;
    }

    public static async Task WriteChecksumsAsync(string root, CancellationToken cancellationToken = default)
    {
        var checksumPath = Path.Combine(root, "checksums.sha256");
        var files = Directory.EnumerateFiles(root, "*", SearchOption.AllDirectories).Where(path => !string.Equals(path, checksumPath, StringComparison.OrdinalIgnoreCase)).OrderBy(path => Path.GetRelativePath(root, path), StringComparer.Ordinal).ToArray();
        var builder = new StringBuilder();
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            File.Copy(provenance.FullPath, Path.Combine(staging, "provenance", "maintained", Path.GetFileName(provenance.FullPath)));
        }
        ...
        var signingPublicKeySha256 = command.SigningPublicKeyPath is null
            ? null : await ComputeFileSha256Async(command.SigningPublicKeyPath, cancellationToken);
            ...
            Images = images.Select(image => new BundleImageDocument
            {
                ...
                RuntimeCommit = image.RuntimeId is null ||
                    !RequiresCommitIdentity(runtimeCatalog[image.RuntimeId])
                        ? null : RuntimeIdentity(image, RuntimeCommitLabel),
                JitCommit = image.RuntimeId is null ||
                    !RequiresCommitIdentity(runtimeCatalog[image.RuntimeId])
                        ? null : RuntimeIdentity(image, JitCommitLabel)
            }).ToArray()
        };
        await WriteJsonAsync(Path.Combine(staging, "bundle.json"), bundle, cancellationToken);
        var expectedImages = string.Join('\n', images.OrderBy(static image => image.Id, StringComparer.Ordinal).Select(static image => $"{image.Id} {image.ImageId}")) + '\n';
        await File.WriteAllTextAsync(Path.Combine(staging, "images.expected"), expectedImages, cancellationToken);
        await WriteJsonAsync(Path.Combine(staging, "sbom", "dependencies.json"), new DependencyInventoryDocument(1, DateTimeOffset.UtcNow, dependencies), cancellationToken);
        await File.WriteAllBytesAsync(Path.Combine(staging, "sbom", "runtime-wine-packages.json"), wineManifestSnapshot.ManifestBytes, cancellationToken);
        ...
        {
            await docker.SaveImagesAsync(images.Select(static image => image.ImageId).ToArray(), Path.Combine(staging, "images.tar"), cancellationToken);
        }

        await RevalidateRuntimePromotionTrustAsync(command, source, effectiveSourceInspector, runtimePromotionTrust, runtimePromotionSourceClosure, cancellationToken);
        ...
        if (command.SigningKeyPath is not null)
        {
            ...
            await effectiveSigner.SignAndVerifyAsync(Path.Combine(staging, "checksums.sha256"), Path.Combine(staging, "checksums.sha256.sig"), command.SigningKeyPath, Path.Combine(staging, "signing-public-key.pem"), cancellationToken);
        }
    }

    private static async Task WriteRuntimePromotionEvidenceAsync(IReadOnlyList<RuntimePromotionTrustSnapshot> trust, RuntimePromotionSourceClosureSnapshot? sourceClosure, string staging, CancellationToken cancellationToken)
    {
        if (trust.Count == 0)
            return;
        if (sourceClosure is null)
        {
            throw new BundleValidationException("Promotion-bound release material has no captured source closure for offline evidence.");
        }

        var snapshotFiles = sourceClosure.Files.ToDictionary(static item => item.RelativePath, StringComparer.Ordinal);
        var captured = sourceClosure.CapturedFiles.ToDictionary(static item => item.RelativePath, StringComparer.Ordinal);
        if (snapshotFiles.Count != sourceClosure.Files.Count || captured.Count != sourceClosure.CapturedFiles.Count || captured.Count < snapshotFiles.Count)
        {
            throw new BundleValidationException("Promotion source closure has duplicate or incomplete captured files.");
        }

        foreach (var snapshot in snapshotFiles.Values)
        {
            if (!captured.TryGetValue(snapshot.RelativePath, out var bytes) || !StringComparer.Ordinal.Equals(snapshot.Sha256, bytes.Sha256) || !StringComparer.Ordinal.Equals(snapshot.Sha256, $"sha256:{Convert.ToHexStringLower(SHA256.HashData(bytes.Bytes))}"))
            {
                throw new BundleValidationException($"Promotion source closure capture is incomplete or corrupted for '{snapshot.RelativePath}'.");
            }
        }
        ...
        foreach (var runtime in trust.OrderBy(static item => item.RuntimeId, StringComparer.Ordinal))
        {
            ...
            AddBinding(bindings, $"profiles/runtimes/candidates/{runtime.RuntimeId}.json", runtime.RuntimeId, "candidate-profile");
            ...
            {
                AddBinding(bindings, evidence.RelativePath, runtime.RuntimeId, evidence.RelativePath.EndsWith("/performance.json", StringComparison.Ordinal) ? "performance-evidence" : "capability-evidence");
            }
            AddBinding(bindings, $"profiles/runtime-promotion-plans/{runtime.RuntimeId}.json", runtime.RuntimeId, "plan");
            AddBinding(bindings, $"profiles/runtime-promotion-plans/{runtime.RuntimeId}.profile.json", runtime.RuntimeId, "preflight-profile");
            AddBinding(bindings, $"profiles/runtimes/{runtime.RuntimeId}.json", runtime.RuntimeId, "active-profile");
        }
        var expectedCapturedPaths = snapshotFiles.Keys.Concat(bindings.Keys).ToHashSet(StringComparer.Ordinal);
        if (!expectedCapturedPaths.SetEquals(captured.Keys))
        {
            throw new BundleValidationException("Promotion source closure has unexpected or missing captured evidence inputs.");
        }

        var root = Path.Combine(staging, PromotionEvidenceDirectoryName);
        var sourceRoot = Path.Combine(root, "source");
        Directory.CreateDirectory(sourceRoot);
        var expectedRuntimeIds = trust.Select(static item => item.RuntimeId).OrderBy(static item => item, StringComparer.Ordinal).ToArray();
        ...
        foreach (var file in captured.Values.OrderBy(static item => item.RelativePath, StringComparer.Ordinal))
        {
            ...
            var destination = Path.Combine(sourceRoot, file.RelativePath.Replace('/', Path.DirectorySeparatorChar));
            var destinationDirectory = Path.GetDirectoryName(destination) ?? throw new BundleValidationException("Promotion evidence destination has no parent directory.");
            ...
            entries.Add(new PromotionEvidenceManifestEntry { Kind = binding?.Kind ?? "source-closure", SourcePath = file.RelativePath, BundlePath = bundlePath, Sha256 = file.Sha256, SizeBytes = file.Bytes.LongLength, ProfileIds = binding?.RuntimeIds.ToArray() ?? expectedRuntimeIds, RuntimeIds = binding?.RuntimeIds.ToArray() ?? expectedRuntimeIds });
        }

        var actualRuntimeIds = entries.Where(static item => item.RuntimeIds is not null).SelectMany(static item => item.RuntimeIds!).Distinct(StringComparer.Ordinal).OrderBy(static item => item, StringComparer.Ordinal).ToArray();
        if (!expectedRuntimeIds.SequenceEqual(actualRuntimeIds, StringComparer.Ordinal))
        {
            throw new BundleValidationException("Promotion evidence manifest does not bind every promoted runtime.");
        }

        var manifestPath = Path.Combine(root, "manifest.json");
        await WriteJsonAsync(
            manifestPath,
            new PromotionEvidenceManifest { SchemaVersion = 1, BuildSourceRevision = sourceClosure.BuildSourceRevision, ReleaseSourceRevision = sourceClosure.ReleaseSourceRevision, PromotedRuntimeIds = expectedRuntimeIds, Entries = entries },
            cancellationToken);
        await WritePromotionEvidenceVerificationManifestAsync(Path.Combine(root, "manifest.tsv"), sourceClosure, $"sha256:{await ComputeFileSha256Async(manifestPath, cancellationToken)}", expectedRuntimeIds, entries, cancellationToken);
    }

    private static Task WritePromotionEvidenceVerificationManifestAsync(string path, RuntimePromotionSourceClosureSnapshot sourceClosure, string manifestJsonSha256, IReadOnlyList<string> promotedRuntimeIds, IReadOnlyList<PromotionEvidenceManifestEntry> entries, CancellationToken cancellationToken)
    {
        ...
        {
            lines.Add(string.Join('\t', "entry", entry.Kind, entry.ProfileIds.Length == 0 ? "-" : string.Join(',', entry.ProfileIds), entry.RuntimeIds.Length == 0 ? "-" : string.Join(',', entry.RuntimeIds), entry.SourcePath, entry.BundlePath, entry.Sha256, entry.SizeBytes.ToString(System.Globalization.CultureInfo.InvariantCulture)));
        }
        return File.WriteAllTextAsync(path, (string.Join('\n', lines) + "\n").ReplaceLineEndings("\n"), new UTF8Encoding(encoderShouldEmitUTF8Identifier: false), cancellationToken);
    }

    private static void AddBinding(Dictionary<string, PromotionEvidenceBinding> bindings, string path, string runtimeId, string kind)
    {
        ...
        if (bindings.TryGetValue(path, out var existing))
        {
            ...
            {
                throw new BundleValidationException($"Promotion evidence source '{path}' has conflicting evidence kinds.");
            }
    ...
    {
        if (string.IsNullOrWhiteSpace(path) || path.Contains('\\') || path.Contains('\t') || path.Contains('\r') || path.Contains('\n') || Path.IsPathRooted(path) || path.Split('/').Any(static segment => segment is "" or "." or ".."))
        {
            throw new BundleValidationException($"Promotion evidence path '{path}' is not canonical.");
        }
    }

    private async Task RevalidateRuntimePromotionTrustAsync(BundleBuilderCommand command, RepositorySourceProvenance source, IRepositorySourceInspector effectiveSourceInspector, IReadOnlyList<RuntimePromotionTrustSnapshot> runtimePromotionTrust, RuntimePromotionSourceClosureSnapshot? runtimePromotionSourceClosure, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BundleValidationException("Promotion-bound release material has no captured A-to-B source closure.");
        }

        await RuntimePromotionSourceClosure.RevalidateAsync(command.RepositoryRoot, runtimePromotionSourceClosure, runtimePromotionSourceInspector, cancellationToken);

        await RuntimePromotionTrust.RevalidateAsync(command.RepositoryRoot, runtimePromotionTrust, docker, cancellationToken);
    }

    private static async Task WriteProfileUpdateStatusAsync(BundleBuilderCommand command, string releaseId, string staging, CancellationToken cancellationToken)
    {
        const int maximumStatusBytes = 64 * 1024;
        var defaultPath = Path.Combine(command.RepositoryRoot, "artifacts", "profile-updater", "status.public.json");
        ...
        if (!File.Exists(inputPath))
        {
            ...
            {
                throw new BundleValidationException("The configured public profile update status file does not exist.");
            }
        ...
        else
        {
            ...
            try
            {
                await using var stream = new FileStream(inputPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete, 16 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan);
                status = await JsonSerializer.DeserializeAsync<ProfileUpdateStatusDocument>(stream, JsonOptions, cancellationToken) ?? throw new BundleValidationException("The public profile update status file is empty.");
            }
            catch (JsonException)
            {
                throw new BundleValidationException("The public profile update status file is invalid JSON.");
            }

            if (status.SchemaVersion != 1 || !string.Equals(status.Active.ReleaseId, releaseId, StringComparison.Ordinal))
            {
                throw new BundleValidationException("The public profile update status does not match the bundle release.");
            }
            status = CreatePublicStatusProjection(status);
        }

        await WriteJsonAsync(Path.Combine(staging, "profile-update-status.json"), status, cancellationToken);
    }
    ...
    private static ProfileUpdateStatusDocument CreatePublicStatusProjection(ProfileUpdateStatusDocument source) => new()
    {
        ...
        LastStage = new ProfileUpdatePublicStageStatus { Stage = source.LastStage.Stage, Outcome = source.LastStage.Outcome, StartedAt = source.LastStage.StartedAt, CompletedAt = source.LastStage.CompletedAt, Error = source.LastStage.Outcome == ProfileUpdatePublicStageOutcome.Failed ? CreatePublicProfileUpdateError(source.LastStage.Error) : null }
    };
    ...
    private static ProfileUpdatePublicError CreatePublicProfileUpdateError(ProfileUpdatePublicError? source) => source?.Code switch
    {
        "profile-update.check-failed" => PublicProfileUpdateError(source.Code, "Profile update check failed; update availability is unknown."),
        "profile-update.resolve-failed" => PublicProfileUpdateError(source.Code, "Profile candidate resolution failed; the approved release remains active."),
        "profile-update.build-failed" => PublicProfileUpdateError(source.Code, "Profile candidate build failed; the approved release remains active."),
        "profile-update.test-failed" => PublicProfileUpdateError(source.Code, "Profile candidate validation failed; the approved release remains active."),
        "profile-update.promote-failed" => PublicProfileUpdateError(source.Code, "Profile candidate promotion failed; the previous approved release remains active."),
        "profile-update.failed" => PublicProfileUpdateError(source.Code, "Profile update failed; the approved release remains active."),
        _ => PublicProfileUpdateError("profile-update.failed", "The profile update check did not complete successfully.")
    };
    ...
    internal async Task WriteWineNoticeArchiveAsync(WineRuntimePackageManifestSnapshot manifestSnapshot, IReadOnlyList<InspectedImage> images, string staging, CancellationToken cancellationToken)
    {
        ...
        var sourceImage = images.Where(image => image.Labels.TryGetValue(labelPrefix + "version", out var version) && string.Equals(version, manifest.Component.ResolvedVersion, StringComparison.Ordinal) && image.Labels.TryGetValue(labelPrefix + "digest", out var digest) && string.Equals(digest, componentDigest, StringComparison.Ordinal) && image.Labels.TryGetValue(labelPrefix + "source-uri", out var sourceUri) && string.Equals(sourceUri, manifest.Component.SourceUri, StringComparison.Ordinal))
            .OrderBy(static image => image.Id, StringComparer.Ordinal).FirstOrDefault() ?? throw new BundleValidationException("No inspected final Wine image carries the exact userspace component identity required for notice extraction.");

        var destination = Path.Combine(staging, WineNoticeArchiveBundleRelativePath.Replace('/', Path.DirectorySeparatorChar));
        Directory.CreateDirectory(Path.GetDirectoryName(destination)!);
        var inspection = await docker.CopyImageFileAsync(sourceImage.ImageId, manifest.NoticeArchive.ImagePath, destination, WineRuntimePackageManifestLoader.MaximumNoticeArchiveBytes, cancellationToken);
        if (!string.Equals(inspection.Sha256, $"sha256:{manifest.NoticeArchive.Sha256}", StringComparison.Ordinal) || inspection.Length != manifest.NoticeArchive.SizeBytes)
        {
            throw new BundleValidationException("Wine notice archive copied from the final image does not match its manifest identity.");
        }

        await ValidateWineNoticeArchiveAsync(destination, manifest, cancellationToken);
    }

    internal static async Task ValidateWineNoticeArchiveAsync(string archivePath, WineRuntimePackageManifest manifest, CancellationToken cancellationToken)
    {
        ...
        foreach (var package in manifest.ResolvedPackages)
        {
            ...
            if (expected.TryGetValue(path, out var existing) && (existing.SizeBytes != package.CopyrightSizeBytes || !string.Equals(existing.Sha256, package.CopyrightSha256, StringComparison.Ordinal)))
            {
                throw new BundleValidationException($"Wine notice '{package.CopyrightPath}' has conflicting package identities.");
            }
        ...
        await using var stream = new FileStream(archivePath, FileMode.Open, FileAccess.Read, FileShare.Read, 64 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan);
        ...
        {
            if (entry is not UstarTarEntry ustar || entry.EntryType is not (TarEntryType.RegularFile or TarEntryType.V7RegularFile) || entry.DataStream is null || entry.ModificationTime != DateTimeOffset.UnixEpoch || ustar.Uid != 0 || ustar.Gid != 0 || !IsCanonicalBundlePath(entry.Name) || !actual.Add(entry.Name) || !expected.TryGetValue(entry.Name, out var identity) || entry.Length != identity.SizeBytes)
            {
                throw new BundleValidationException("Wine notice archive must contain only the exact deterministic regular-file copyright closure.");
            }
    ...
    private static bool IsCanonicalBundlePath(string value) =>
        ...
        !value.Contains('\\') &&
        value.Split('/').All(static segment => segment.Length > 0 && segment is not "." and not ".." && segment.All(static character => !char.IsControl(character)));

    private static ReleaseLockDocument CreateBundleLock(ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images)
    {
        ...
        foreach (var image in images)
        {
            ...
            components[image.LockComponentId] = existing is null
                ? new LockedComponent { Kind = "image", ResolvedVersion = releaseLock.ReleaseId, Digest = digest, ImageId = image.ImageId }
    ...
    private static string SelectDigest(InspectedImage image)
    {
        var digest = image.RepoDigests
            .Select(static value => value[(value.LastIndexOf('@') + 1)..]).FirstOrDefault(static value => IsSha256(value));
        return digest ?? image.ImageId;
    }

    private static object CreateSpdx(string releaseId, ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images, IReadOnlyList<DependencyComponent> dependencies, WineRuntimePackageManifest wineManifest)
    {
        var packages = new List<object>();
        packages.AddRange(releaseLock.Components.OrderBy(static pair => pair.Key, StringComparer.Ordinal).Select(pair => (object)new { SPDXID = $"SPDXRef-Component-{SafeSpdxId(pair.Key)}", name = pair.Key, versionInfo = pair.Value.ResolvedVersion, downloadLocation = pair.Value.SourceUri ?? "NOASSERTION", filesAnalyzed = false, licenseConcluded = "NOASSERTION", licenseDeclared = "NOASSERTION", checksums = ComponentChecksums(pair.Value) }));
        packages.AddRange(dependencies.Select(dependency => (object)new { SPDXID = $"SPDXRef-{SafeSpdxId(dependency.PackageManager)}-{SafeSpdxId(dependency.Name)}-{SafeSpdxId(dependency.Version)}", name = dependency.Name, versionInfo = dependency.Version, downloadLocation = dependency.SourceUri ?? "NOASSERTION", filesAnalyzed = false, licenseConcluded = dependency.License, licenseDeclared = dependency.License, checksums = IntegrityChecksums(dependency.Integrity, spdx: true) }));
        packages.AddRange(wineManifest.ResolvedPackages.Select(package => (object)new
        {
            ...
            downloadLocation = WineRuntimePackageManifestLoader.ArchiveUri(wineManifest, package.ArchiveSnapshotId, package.Path).AbsoluteUri,
            filesAnalyzed = false,
        ...
        packages.AddRange(images.Select(image => (object)new { SPDXID = $"SPDXRef-Image-{SafeSpdxId(image.Id)}", name = image.Id, versionInfo = releaseId, downloadLocation = "NOASSERTION", filesAnalyzed = false, licenseConcluded = "NOASSERTION", licenseDeclared = "NOASSERTION", checksums = (object[])[new { algorithm = "SHA256", checksumValue = image.ImageId[7..] }] }));
        return new { spdxVersion = "SPDX-2.3", dataLicense = "CC0-1.0", SPDXID = "SPDXRef-DOCUMENT", name = $"SharpLabNext-{releaseId}", documentNamespace = $"https://sharplabnext.dev/sbom/{Uri.EscapeDataString(releaseId)}", creationInfo = new { created = DateTimeOffset.UtcNow, creators = new[] { "Tool: SharpLabNext.BundleBuilder" } }, packages };
    }

    private static object CreateCycloneDx(string releaseId, ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images, IReadOnlyList<DependencyComponent> dependencies, WineRuntimePackageManifest wineManifest)
    {
        var components = new List<object>();
        components.AddRange(releaseLock.Components.OrderBy(static pair => pair.Key, StringComparer.Ordinal).Select(pair => (object)new { type = pair.Value.Kind == "runtime" ? "framework" : "library", name = pair.Key, version = pair.Value.ResolvedVersion, purl = PackageUrl(pair.Value), hashes = CycloneHashes(pair.Value) }));
        components.AddRange(dependencies.Select(dependency => (object)new { type = "library", name = dependency.Name, version = dependency.Version, purl = DependencyPackageUrl(dependency), hashes = IntegrityChecksums(dependency.Integrity, spdx: false), licenses = new[] { new { expression = dependency.License } }, properties = new[] { new { name = "sharplabnext:direct", value = dependency.Direct.ToString().ToLowerInvariant() } } }));
        ...
        components.AddRange(images.Select(image => (object)new { type = "container", name = image.Id, version = releaseId, purl = (string?)null, hashes = (object[])[new { alg = "SHA-256", content = image.ImageId[7..] }] }));
        return new { bomFormat = "CycloneDX", specVersion = "1.6", serialNumber = $"urn:uuid:{Guid.NewGuid()}", version = 1, metadata = new { timestamp = DateTimeOffset.UtcNow, component = new { type = "application", name = "SharpLabNext", version = releaseId }, tools = new[] { new { vendor = "SharpLabNext", name = "BundleBuilder", version = "1" } } }, components };
    }

    private static object CreateProvenance(BundleBuilderCommand command, CatalogDocument catalog, ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images, IReadOnlyList<DependencyComponent> dependencies, WineRuntimePackageManifestSnapshot wineManifestSnapshot, BaseImageManifest baseImages, RuntimeMatrixBaseImageBindings runtimeMatrixBaseImages, IReadOnlyList<MaintainedProvenanceInput> maintainedProvenance, RepositorySourceProvenance source)
    {
        var resolvedDependencies = new List<object>();
        resolvedDependencies.AddRange(images.Select(image => (object)new { uri = $"pkg:docker/{Uri.EscapeDataString(image.Id)}", digest = new Dictionary<string, string> { ["sha256"] = image.ImageId[7..] } }));
        foreach (var operatorImage in images.Select(static image => image.Labels.TryGetValue("io.sharplabnext.framework.row-operator-image", out var reference) ? reference : null).OfType<string>().Distinct(StringComparer.Ordinal))
        {
        ...
        resolvedDependencies.AddRange(dependencies.Select(dependency => (object)new { uri = $"pkg:{dependency.PackageManager}/{Uri.EscapeDataString(dependency.Name)}@{Uri.EscapeDataString(dependency.Version)}", digest = IntegrityDigest(dependency.Integrity) }));
        resolvedDependencies.AddRange(wineManifestSnapshot.Manifest.ResolvedPackages.Select(package => (object)new { uri = OsPackageUrl(package), downloadLocation = WineRuntimePackageManifestLoader.ArchiveUri(wineManifestSnapshot.Manifest, package.ArchiveSnapshotId, package.Path).AbsoluteUri, digest = new Dictionary<string, string> { ["sha256"] = package.Sha256 }, sourcePackage = package.SourcePackage, sourceVersion = package.SourceVersion }));
        ...
        var resolvedBaseImageReferences = baseImages.Images.Select(static image => image.Reference).ToHashSet(StringComparer.Ordinal);
        foreach (var image in images)
        {
            if (image.RuntimeId is not null && runtimeMatrixBaseImages.LinuxRuntimeBaseImages.TryGetValue(image.RuntimeId, out var reference))
            {
                resolvedBaseImageReferences.Add(reference);
            }
        }
        resolvedDependencies.AddRange(resolvedBaseImageReferences.Order(StringComparer.Ordinal).Select(reference => (object)new { uri = $"pkg:docker/{Uri.EscapeDataString(reference[..reference.LastIndexOf('@')])}", digest = new Dictionary<string, string> { ["sha256"] = BaseImageDigest(reference) } }));
        resolvedDependencies.AddRange(maintainedProvenance.SelectMany(static provenance => provenance.ReferencedComponentIds).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal).Select(componentId =>
            {
                var component = releaseLock.Components[componentId];
                return (object)new { name = componentId, uri = component.SourceUri ?? PackageUrl(component) ?? $"urn:sharplabnext:component:{componentId}", digest = LockedComponentDigest(component) };
            }));
        var maintainedParameters = maintainedProvenance.Select(provenance => new
        {
            ...
            relatedComponents = provenance.ReferencedComponentIds.Where(id => !string.Equals(id, provenance.ComponentId, StringComparison.Ordinal) && !string.Equals(id, provenance.SourceComponentId, StringComparison.Ordinal)).Select(id => LockIdentity(id, releaseLock.Components[id])).ToArray()
        }).ToArray();
        return new
        {
            _type = "https://in-toto.io/Statement/v1",
            subject = images.Select(image => new { name = image.Id, digest = new Dictionary<string, string> { ["sha256"] = image.ImageId[7..] } }).ToArray(),
            ...
            predicate = new
            {
                ...
                runDetails = new { builder = new { id = "https://sharplabnext.dev/builders/bundle-builder/v1" }, metadata = new { invocationId = Guid.NewGuid(), startedOn = DateTimeOffset.UtcNow } }
            }
        };
    }

    private static object LockIdentity(string componentId, LockedComponent component) => new { componentId, component.Kind, component.ResolvedVersion, component.Commit, component.JitCommit, component.Digest, component.SourceUri, component.Package, component.PackageContentHash, component.Sha512 };
    ...
    private static string WineManifestSha256(WineRuntimePackageManifestSnapshot snapshot)
    {
        ...
        {
            throw new BundleValidationException("Wine package inventory bytes changed after their release identity was validated.");
        }
        return actual;
    }
    ...
    private static string? PackageUrl(LockedComponent component) => component.Package is null
        ? null : $"pkg:nuget/{Uri.EscapeDataString(component.Package)}@{Uri.EscapeDataString(component.ResolvedVersion)}";
    ...
    private static string SafeSpdxId(string value) =>
        new(value.Select(static character => char.IsAsciiLetterOrDigit(character) || character is '.' or '-' ? character : '-').ToArray());
    ...
    private static string RuntimeIdentity(InspectedImage image, string label) =>
        image.Labels.TryGetValue(label, out var commit) && IsCommit(commit)
            ? commit : throw new BundleValidationException($"Runtime image '{image.Id}' does not carry a valid '{label}' identity.");

    private static void AppendRuntimeProfile(StringBuilder builder, int index, RuntimeProfileDefinition profile)
    {
        ...
        AppendConfiguration(builder, prefix + "__Layout__JitInspectorAssemblyPath", profile.Layout.JitInspectorAssemblyPath);
        ...
        {
            AppendSecurityPolicy(builder, $"{prefix}__SecurityPolicies__{policyIndex}", profile.SecurityPolicies[policyIndex]);
        }
    }

    private static void AppendRuntimeOperation(StringBuilder builder, string prefix, RuntimeOperationDefinition operation)
    {
    ...
    private static void AppendSecurityPolicy(StringBuilder builder, string prefix, RuntimeSecurityPolicyDefinition policy)
    {
    ...
    private static void AppendConfiguration(StringBuilder builder, string key, long value) => AppendConfiguration(builder, key, value.ToString(System.Globalization.CultureInfo.InvariantCulture));

    private static void AppendConfiguration(StringBuilder builder, string key, string? value)
    {
        if (value is null)
            return;
        builder.Append("      ").Append(key).Append(": \"").Append(EscapeYaml(value)).AppendLine("\"");
    }
    ...
    private static string EscapeYaml(string value) => value.Replace("\\", "\\\\", StringComparison.Ordinal).Replace("\"", "\\\"", StringComparison.Ordinal).Replace("\r", string.Empty, StringComparison.Ordinal).Replace("\n", "\\n", StringComparison.Ordinal);

    public static async Task<IReadOnlyList<RuntimeProfileDefinition>> LoadRuntimeProfilesAsync(string directory, CancellationToken cancellationToken = default)
    {
        EnsureInputDirectory(directory);
        var profiles = new List<RuntimeProfileDefinition>();
        foreach (var path in Directory.EnumerateFiles(directory, "*.json", SearchOption.TopDirectoryOnly).OrderBy(static path => path, StringComparer.Ordinal))
        {
            cancellationToken.ThrowIfCancellationRequested();
            try
            {
                ...
                {
                    throw new BundleValidationException($"Active runtime profile '{Path.GetFileName(path)}' must be a JSON object.");
                }
                foreach (var property in RequiredRuntimeProfileProperties)
                {
                    if (!document.RootElement.TryGetProperty(property, out _))
                    {
                        throw new BundleValidationException($"Active runtime profile '{Path.GetFileName(path)}' is incomplete: missing '{property}'.");
                    }
                }

                var profile = document.RootElement.Deserialize<RuntimeProfileDefinition>(RuntimeProfileJsonOptions) ?? throw new BundleValidationException($"Active runtime profile '{Path.GetFileName(path)}' is empty.");
                var failures = RuntimeProfileValidation.ValidatePackage(profile, requireDigestPinnedImage: false).ToList();
                if (profile.Operations is null)
                {
                    failures.Add($"Active runtime profile '{profile.Id}' must use explicit operation definitions.");
                }
                if (profile.SecurityPolicies.Count == 0)
                {
                    failures.Add($"Active runtime profile '{profile.Id}' must include its security policy definitions.");
                }
                if (failures.Count > 0)
                {
                    throw new BundleValidationException($"Active runtime profile '{Path.GetFileName(path)}' is invalid: " + string.Join(" ", failures));
                }
                profiles.Add(profile);
            }
            catch (JsonException exception)
            {
                throw new BundleValidationException($"Active runtime profile '{Path.GetFileName(path)}' is invalid JSON: {exception.Message}");
            }
        }
    ...
    internal static void ValidateRuntimeProfileBindings(CatalogDocument catalog, ReleaseLockDocument releaseLock, IReadOnlyList<RuntimeProfileDefinition> profiles)
    {
        var profileIndex = IndexRuntimeProfiles(profiles);
        var selectableRuntimes = catalog.Runtimes.Where(static runtime => runtime.Availability.IsSelectable).ToDictionary(static runtime => runtime.Id, StringComparer.Ordinal);
        foreach (var profile in profiles)
        {
            if (!selectableRuntimes.ContainsKey(profile.Id))
            {
                throw new BundleValidationException($"Active runtime profile '{profile.Id}' does not map to a selectable Catalog runtime.");
            }
        }
        ...
        foreach (var runtime in selectableRuntimes.Values)
        {
            ...
            {
                throw new BundleValidationException($"Selectable runtime '{runtime.Id}' has no active profile in profiles/runtimes.");
            }
            if (!releaseLock.Components.TryGetValue(runtime.Id, out var component) || !StringComparer.Ordinal.Equals(component.Kind, "runtime"))
            {
                throw new BundleValidationException($"Selectable runtime '{runtime.Id}' has no matching runtime component in the release lock.");
            }
            ...
            RequireRuntimeProfileValues(runtime.Id, "accepted artifact formats", runtime.AcceptedArtifactFormats, profile.AcceptedArtifactFormats);
            RequireRuntimeProfileValues(runtime.Id, "capabilities", runtime.Capabilities, profile.Capabilities);
            RequireRuntimeProfileValues(runtime.Id, "accepted runtime families", runtime.AcceptedRuntimeFamilies, profile.AcceptedRuntimeFamilies);
            RequireRuntimeProfileValues(runtime.Id, "runtime feature tags", runtime.ProvidedRuntimeFeatureTags, profile.ProvidedRuntimeFeatureTags);
            RequireRuntimeProfileValues(runtime.Id, "metadata feature tags", runtime.ProvidedMetadataFeatureTags, profile.ProvidedMetadataFeatureTags);
            ...
            {
                RequireRuntimeProfileValue(runtime.Id, "JIT source mapping kind", sourceMappingKind, profile.Operations?.Jit?.SourceMappingKind);
            }

            if (RequiresCommitIdentity(runtime))
            {
                RequireRuntimeProfileValue(runtime.Id, "runtime commit", component.Commit, profile.RuntimeCommit, StringComparison.OrdinalIgnoreCase);
                RequireRuntimeProfileValue(runtime.Id, "JIT version", runtime.JitVersion ?? component.ResolvedVersion, profile.JitVersion);
                RequireRuntimeProfileValue(runtime.Id, "JIT commit", component.JitCommit, profile.JitCommit, StringComparison.OrdinalIgnoreCase);
            }
            else
            {
            ...
            {
                RequireRuntimeProfileValue(runtime.Id, "Catalog runtime commit", catalogRuntimeCommit, profile.RuntimeCommit, StringComparison.OrdinalIgnoreCase);
            }
            ...
            {
                RequireRuntimeProfileValue(runtime.Id, "Catalog JIT commit", catalogJitCommit, profile.JitCommit, StringComparison.OrdinalIgnoreCase);
            }
    ...
    private static List<RuntimeProfileDefinition> MaterializeRuntimeProfiles(CatalogDocument catalog, ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images, IReadOnlyList<RuntimeProfileDefinition> profiles)
    {
        var profileIndex = IndexRuntimeProfiles(profiles);
        var runtimeImages = images.Where(static image => image.RuntimeId is not null).ToDictionary(static image => image.RuntimeId!, StringComparer.Ordinal);
        ...
        foreach (var runtime in catalog.Runtimes.Where(static runtime => runtime.Availability.IsSelectable))
        {
            ...
            var profile = JsonSerializer.SerializeToElement(source, RuntimeProfileJsonOptions).Deserialize<RuntimeProfileDefinition>(RuntimeProfileJsonOptions) ?? throw new BundleValidationException($"Active runtime profile '{runtime.Id}' could not be materialized.");
            if (!runtimeImages.TryGetValue(runtime.Id, out var image))
            {
                throw new BundleValidationException($"Selectable runtime '{runtime.Id}' has no inspected deployment image.");
            }
            ...
            if (failures.Count > 0)
            {
                throw new BundleValidationException($"Materialized runtime profile '{runtime.Id}' is invalid: {string.Join(" ", failures)}");
            }
            result.Add(profile);
        }
        return result;
    ...
    private static Dictionary<string, RuntimeProfileDefinition> IndexRuntimeProfiles(IReadOnlyList<RuntimeProfileDefinition> profiles)
    {
        ...
        foreach (var profile in profiles)
        {
            ...
            {
                throw new BundleValidationException($"Active runtime profile '{profile.Id}' is declared more than once.");
            }
        }
        return result;
    }

    private static RuntimeSecurityPolicyDefinition[] MergeRuntimeSecurityPolicies(IReadOnlyList<RuntimeProfileDefinition> profiles)
    {
        ...
        foreach (var profile in profiles)
        {
            foreach (var policy in profile.SecurityPolicies)
            {
                if (policies.TryGetValue(policy.Id, out var existing))
                {
                    ...
                    {
                        throw new BundleValidationException($"Runtime security policy '{policy.Id}' has conflicting active profile definitions.");
                    }
                    continue;
                }
    ...
    private static bool RuntimeSecurityPoliciesEqual(RuntimeSecurityPolicyDefinition left, RuntimeSecurityPolicyDefinition right) =>
    ...
    private static void RequireRuntimeProfileFrameworks(RuntimeManifest runtime, RuntimeProfileDefinition profile)
    {
        if (runtime.AcceptedFrameworks.Count != profile.AcceptedFrameworks.Count)
        {
            throw new BundleValidationException($"Active runtime profile '{runtime.Id}' accepted frameworks do not match the Catalog.");
        }
        foreach (var expected in runtime.AcceptedFrameworks)
        {
            var actual = profile.AcceptedFrameworks.SingleOrDefault(framework => StringComparer.Ordinal.Equals(framework.Name, expected.Name));
            if (actual is null || !StringComparer.Ordinal.Equals(actual.MinimumVersion, expected.MinimumVersion) || !StringComparer.Ordinal.Equals(actual.MaximumVersion, expected.MaximumVersion) || !StringComparer.Ordinal.Equals(actual.ExactVersion, expected.ExactVersion))
            {
                throw new BundleValidationException($"Active runtime profile '{runtime.Id}' accepted framework '{expected.Name}' does not match the Catalog.");
            }
        }
    }

    private static void RequireRuntimeProfileValues(string runtimeId, string field, IReadOnlyList<string> expected, List<string> actual)
    {
        if (expected.Count == actual.Count && expected.ToHashSet(StringComparer.Ordinal).SetEquals(actual))
            return;
        throw new BundleValidationException($"Active runtime profile '{runtimeId}' {field} do not match the Catalog.");
    }

    private static void RequireRuntimeProfileValue(string runtimeId, string field, string? expected, string? actual, StringComparison comparison = StringComparison.Ordinal)
    {
        if (expected is not null && string.Equals(expected, actual, comparison))
            return;
        throw new BundleValidationException($"Active runtime profile '{runtimeId}' {field} is '{actual}', but requires '{expected}'.");
    }

    private static async Task<DeploymentImageManifest> LoadDeploymentImagesAsync(string path, CancellationToken cancellationToken)
    {
        await using var stream = File.OpenRead(path);
        var manifest = await JsonSerializer.DeserializeAsync<DeploymentImageManifest>(stream, JsonOptions, cancellationToken);
        return manifest ?? throw new BundleValidationException("Deployment image manifest is empty.");
    }

    private static async Task<BaseImageManifest> LoadBaseImagesAsync(string path, CancellationToken cancellationToken)
    {
        await using var stream = File.OpenRead(path);
        var manifest = await JsonSerializer.DeserializeAsync<BaseImageManifest>(stream, JsonOptions, cancellationToken);
    ...
    internal static string BaseImageDigest(string reference)
    {
        ...
        if (digest.Length != 64 || digest.Any(static character => character is not (>= '0' and <= '9' or >= 'a' and <= 'f')))
        {
    ...
    private static IReadOnlyDictionary<string, string> ValidateInputs(CatalogDocument catalog, ReleaseLockDocument releaseLock, DeploymentImageManifest deployment, IReadOnlyDictionary<string, string> imageOverrides)
    {
        ...
        foreach (var image in deployment.Images)
        {
            ...
            {
                throw new BundleValidationException($"Deployment image '{image.Id}' must be always-on or select exactly one catalog component.");
            }
            ...
            {
                throw new BundleValidationException($"Worker image '{image.Id}' must declare release and immutable image identity environment keys.");
            }

            if (image.ToolchainId is not null && !toolchains.ContainsKey(image.ToolchainId) || image.RuntimeId is not null && !runtimes.ContainsKey(image.RuntimeId) || image.ArtifactProcessorId is not null && !processors.ContainsKey(image.ArtifactProcessorId))
            {
            ...
            if (image.ImmutableReference is not null)
            {
                ...
                {
                    throw new BundleValidationException($"Only a runtime image can carry promotion-bound immutable reference '{image.ImmutableReference}'.");
                }
                ValidateImmutableImageReference(image);
                if (imageOverrides.TryGetValue(image.Id, out var overridden) && !string.Equals(overridden, image.ImmutableReference, StringComparison.Ordinal))
                {
                    throw new BundleValidationException($"Deployment image '{image.Id}' is promotion-bound and cannot use a different override.");
                }
            }
            ...
            if (image.ToolchainId is not null)
            {
                ...
                {
                    throw new BundleValidationException($"Language worker '{workerId}' is represented by both deployment images " + $"'{toolchainImagesByWorkerId[workerId]}' and '{image.Id}'.");
                }
            }
            ...
            {
                var workerId = toolchains[image.ToolchainId].WorkerId;
                foreach (var profileId in toolchains.Values.Where(toolchain => string.Equals(toolchain.WorkerId, workerId, StringComparison.Ordinal)).Select(static toolchain => toolchain.Id))
                {
                    if (!componentIds.Contains(profileId, StringComparer.Ordinal))
                    {
                        throw new BundleValidationException($"Language worker image '{image.Id}' does not declare lock component " + $"'{profileId}' for its hosted toolchain profile.");
                    }
                }
            }
            foreach (var componentId in componentIds)
            {
                ...
                {
                    throw new BundleValidationException($"Deployment image '{image.Id}' references missing lock component '{componentId}'.");
                }
            }
        }

        foreach (var workerId in catalog.Toolchains.Where(static item => item.Availability.IsSelectable).Select(static item => item.WorkerId).Distinct(StringComparer.Ordinal))
        {
            if (!toolchainImagesByWorkerId.ContainsKey(workerId))
            {
                throw new BundleValidationException($"Selectable language worker '{workerId}' has no deployment image definition.");
            }
        }
    ...
    private static void ValidateRuntimePromotionBindings(CatalogDocument catalog, DeploymentImageManifest deployment, IReadOnlyList<RuntimeProfileDefinition> profiles)
    {
        var profileIndex = IndexRuntimeProfiles(profiles);
        foreach (var runtime in catalog.Runtimes.Where(static item => item.Availability.IsSelectable))
        {
            var definitions = deployment.Images.Where(item => string.Equals(item.RuntimeId, runtime.Id, StringComparison.Ordinal)).ToArray();
            if (definitions.Length != 1)
            {
                throw new BundleValidationException($"Selectable runtime '{runtime.Id}' must have exactly one deployment image definition.");
            }
            ...
            {
                throw new BundleValidationException($"Runtime '{runtime.Id}' must declare its promotion receipt and immutable deployment reference together.");
            }
            if (receipt is null)
            {
                continue;
            ...
            {
                throw new BundleValidationException($"Runtime '{runtime.Id}' promotion receipt path must be '{expectedPath}'.");
            }
            if (!IsSha256(receipt.Sha256))
            {
                throw new BundleValidationException($"Runtime '{runtime.Id}' promotion receipt has an invalid SHA-256 digest.");
            }
            ...
            {
                throw new BundleValidationException($"Runtime '{runtime.Id}' profile image does not match its promotion-bound deployment reference.");
            }
        }
    }
    ...
    {
        var reference = definition.ImmutableReference ?? throw new BundleValidationException($"Deployment image '{definition.Id}' has no immutable reference.");
        var expectedPrefix = definition.Repository + "@sha256:";
        if (!reference.StartsWith(expectedPrefix, StringComparison.Ordinal) || reference.Length != expectedPrefix.Length + 64 || reference.AsSpan(expectedPrefix.Length).ToArray().Any(static character => character is not (>= '0' and <= '9' or >= 'a' and <= 'f')))
        {
            throw new BundleValidationException($"Deployment image '{definition.Id}' immutable reference must pin its declared repository by SHA-256.");
        }
    }

    private static void RequireDefinition(DeploymentImageManifest deployment, Func<DeploymentImageDefinition, string, bool> matches, string id, string kind)
    {
        ...
        bool allowDevelopmentImageInputs)
    {
        ...
        if (!string.Equals(inspection.OperatingSystem, "linux", StringComparison.Ordinal) || !string.Equals(inspection.Architecture, "amd64", StringComparison.Ordinal))
        {
            throw new BundleValidationException($"Image '{reference}' is {inspection.OperatingSystem}/{inspection.Architecture}; linux/amd64 is required.");
        }

        if (definition.ImmutableReference is not null && !inspection.RepoDigests.Contains(definition.ImmutableReference, StringComparer.Ordinal))
        {
            throw new BundleValidationException($"Promotion-bound image '{definition.Id}' does not resolve its immutable reference " + $"'{definition.ImmutableReference}' to inspected image ID '{inspection.ImageId}'.");
        }
        ...
        if (definition.RuntimeId is not null)
        {
            ...
            var runtime = catalog.Runtimes.Single(item => string.Equals(item.Id, definition.RuntimeId, StringComparison.Ordinal));
            if (RequiresCommitIdentity(runtime))
            {
            ...
            {
                throw new BundleValidationException($"Runtime image '{definition.Id}' must have an exact locked digest when commit identity is not applicable.");
            }
        }

        if (definition.ToolchainId is not null)
        {
            ValidateToolchainReferenceSetLabels(definition, inspection.Labels, catalog, expectedReferenceSetDigests);
        }
    }
    ...
    internal static void ValidateInspectionSourceRevision(DeploymentImageDefinition definition, IReadOnlyDictionary<string, string> labels, string releaseRevision, bool promotionBoundRuntime)
    {
        ...
        {
            if (definition.RuntimeId is null || definition.ImmutableReference is null || !IsCommit(sourceRevision) || !labels.TryGetValue("org.opencontainers.image.revision", out var ociRevision) || !StringComparer.Ordinal.Equals(ociRevision, sourceRevision))
            {
                throw new BundleValidationException($"Promotion-bound runtime image '{definition.Id}' does not carry one canonical, " + "matching implementation revision in both source labels.");
            }
            return;
        }
    ...
    private static void ValidateBaseImageLabels(DeploymentImageDefinition definition, IReadOnlyDictionary<string, string> labels, BaseImageManifest baseImages, RuntimeMatrixBaseImageBindings runtimeMatrixBaseImages)
    {
        ...
        var matrixBaseImageBound = definition.RuntimeId is not null &&
            runtimeMatrixBaseImages.LinuxRuntimeBaseImages.TryGetValue(definition.RuntimeId, out matrixBaseImage);
        if (matrixBaseImageBound)
            expected[RuntimeMatrixBaseImageBindings.LinuxRuntimeBaseImageId] = matrixBaseImage!;

        var observed = labels.Where(static pair => pair.Key.StartsWith(BaseImageLabelPrefix, StringComparison.Ordinal)).ToArray();
        ...
        foreach (var pair in observed)
        {
            ...
            if (!string.Equals(reference, pair.Value, StringComparison.Ordinal))
            {
                var requiredBy = matrixBaseImageBound &&
                    StringComparer.Ordinal.Equals(baseImageId, RuntimeMatrixBaseImageBindings.LinuxRuntimeBaseImageId)
                            ? "runtime matrix" : "base image manifest";
                throw new BundleValidationException($"Image '{definition.Id}' base image '{baseImageId}' is '{pair.Value}', " + $"but the {requiredBy} requires '{reference}'.");
            }
        }
    }

    private static void ValidateComponentIdentityLabels(DeploymentImageDefinition definition, IReadOnlyDictionary<string, string> labels, ReleaseLockDocument releaseLock)
    {
        foreach (var componentId in ComponentIds(definition))
        {
            var component = releaseLock.Components[componentId];
            ValidateComponentIdentityLabel(definition.Id, componentId, labels, "version", component.ResolvedVersion, ignoreCase: false);
            ValidateOptionalComponentIdentityLabel(definition.Id, componentId, labels, "commit", component.Commit, ignoreCase: true);
            ValidateOptionalComponentIdentityLabel(definition.Id, componentId, labels, "digest", component.Digest, ignoreCase: false);
            ValidateOptionalComponentIdentityLabel(definition.Id, componentId, labels, "source-uri", component.SourceUri, ignoreCase: false);
            ValidateOptionalComponentIdentityLabel(definition.Id, componentId, labels, "patch-digest", component.PatchDigest, ignoreCase: false);
        }
    }
    ...
    private static ReleaseLockDocument ResolveDevelopmentFrameworkComponentIdentities(CatalogDocument catalog, ReleaseLockDocument releaseLock, IReadOnlyList<InspectedImage> images, bool allowDevelopmentImageInputs)
    {
        ...
        foreach (var image in images)
        {
            ...
            var runtime = catalog.Runtimes.Single(item => StringComparer.Ordinal.Equals(item.Id, image.RuntimeId));
            ...
            if (!image.Labels.TryGetValue(componentPrefix + "digest", out var observedDigest) || !image.Labels.TryGetValue(componentPrefix + "source-uri", out var observedSourceUri))
            {
                continue;
            }
            ...
            var hasRowBinding = image.Labels.ContainsKey("io.sharplabnext.framework.row-operator-image");
            if (!identityDiffers && !hasRowBinding)
                continue;

            var rowOperatorImage = RequiredImageLabel(image, "io.sharplabnext.framework.row-operator-image");
            var operatorDigest = $"sha256:{BaseImageDigest(rowOperatorImage)}";
            var operatorSourceUri = $"docker://{rowOperatorImage}";
            RequireImageLabel(image, "io.sharplabnext.framework.matrix-selector", "true");
            RequireImageLabel(image, $"{ComponentLabelPrefix}runtime-matrix.profile-id", image.RuntimeId);
            RequireImageLabel(image, $"{ComponentLabelPrefix}runtime-matrix.version", component.ResolvedVersion);
            RequireImageLabel(image, $"{ComponentLabelPrefix}runtime-matrix.digest", operatorDigest);
            RequireImageLabel(image, $"{ComponentLabelPrefix}runtime-matrix.source-uri", operatorSourceUri);
            ...
            {
                throw new BundleValidationException($"Image '{image.Id}' has invalid Framework row digest '{rowDigest}'.");
            }
            _ = BaseImageDigest(RequiredImageLabel(image, "io.sharplabnext.framework.matrix-parent"));
            var matrixSourceUri = RequiredImageLabel(image, "io.sharplabnext.framework.matrix-source-uri");
            if (!matrixSourceUri.StartsWith("docker://", StringComparison.Ordinal))
            {
                throw new BundleValidationException($"Image '{image.Id}' Framework matrix source is not an immutable docker URI.");
            }
    ...
    private static string RequiredImageLabel(InspectedImage image, string label)
    {
        ...
        {
            throw new BundleValidationException($"Image '{image.Id}' does not carry required label '{label}'.");
        }
        return value;
    }
    ...
    private static void RequireImageLabel(InspectedImage image, string label, string expected)
    {
        ...
        {
            throw new BundleValidationException($"Image '{image.Id}' label '{label}' is '{actual}', but its verified Framework row requires '{expected}'.");
        }
    }

    private static void ValidateOptionalComponentIdentityLabel(string imageId, string componentId, IReadOnlyDictionary<string, string> labels, string field, string? expected, bool ignoreCase)
    {
        if (!string.IsNullOrWhiteSpace(expected))
            ValidateComponentIdentityLabel(imageId, componentId, labels, field, expected, ignoreCase);
    }

    private static void ValidateComponentIdentityLabel(string imageId, string componentId, IReadOnlyDictionary<string, string> labels, string field, string expected, bool ignoreCase)
    {
        ...
        {
            throw new BundleValidationException($"Image '{imageId}' label '{label}' is '{actual}', but lock requires '{expected}'.");
        }
    }

    private static IReadOnlyList<string> ComponentIds(DeploymentImageDefinition definition)
    {
        var primary = definition.LockComponentId ?? definition.ToolchainId ?? definition.RuntimeId ?? definition.ArtifactProcessorId;
        return primary is null
            ? definition.LockComponentIds : [primary, ..definition.LockComponentIds];
    }

    private static void ValidateToolchainReferenceSetLabels(DeploymentImageDefinition definition, IReadOnlyDictionary<string, string> labels, CatalogDocument catalog, IReadOnlyDictionary<string, string> expectedReferenceSetDigests)
    {
        var toolchain = catalog.Toolchains.Single(item => string.Equals(item.Id, definition.ToolchainId, StringComparison.Ordinal));
        var referenceSetIds = catalog.Toolchains.Where(item => string.Equals(item.WorkerId, toolchain.WorkerId, StringComparison.Ordinal)).SelectMany(static item => item.AllowedReferenceSetIds).Distinct(StringComparer.Ordinal).OrderBy(static id => id, StringComparer.Ordinal);
        foreach (var referenceSetId in referenceSetIds)
        {
            ...
            {
                throw new BundleValidationException($"Toolchain image '{definition.Id}' has no locked identity for reference set '{referenceSetId}'.");
            }

            var label = ReferenceSetLabelPrefix + referenceSetId;
            if (!labels.TryGetValue(label, out var actualDigest))
            {
                throw new BundleValidationException($"Toolchain image '{definition.Id}' does not carry required label '{label}'.");
            }
            if (!string.Equals(actualDigest, expectedDigest, StringComparison.Ordinal))
            {
                throw new BundleValidationException($"Toolchain image '{definition.Id}' label '{label}' is '{actualDigest}', but lock requires '{expectedDigest}'.");
            }
        }
    }

    private static void ValidateDeclaredReferenceSetLabels(DeploymentImageDefinition definition, IReadOnlyDictionary<string, string> labels, ReleaseLockDocument releaseLock, IReadOnlyDictionary<string, string> expectedReferenceSetDigests)
    {
        foreach (var referenceSetId in ComponentIds(definition).Where(componentId => StringComparer.Ordinal.Equals(releaseLock.Components[componentId].Kind, "reference-set")))
        {
            if (!expectedReferenceSetDigests.TryGetValue(referenceSetId, out var expectedDigest))
            {
                throw new BundleValidationException($"Image '{definition.Id}' has no locked identity for declared reference set '{referenceSetId}'.");
            }

            var label = ReferenceSetLabelPrefix + referenceSetId;
            if (!labels.TryGetValue(label, out var actualDigest))
            {
                throw new BundleValidationException($"Image '{definition.Id}' does not carry required label '{label}'.");
            }
            if (!StringComparer.Ordinal.Equals(actualDigest, expectedDigest))
            {
                throw new BundleValidationException($"Image '{definition.Id}' label '{label}' is '{actualDigest}', but lock requires '{expectedDigest}'.");
            }
        }
    }

    private static void ValidateRuntimeIdentityLabel(string imageId, IReadOnlyDictionary<string, string> labels, string label, string? expected)
    {
        ...
        {
            throw new BundleValidationException($"Runtime image '{imageId}' label '{label}' is '{actual}', but lock requires '{expected}'.");
        }
    }
    ...
    private static void CopyDirectory(string source, string destination)
    {
        ...
        foreach (var directory in Directory.EnumerateDirectories(sourceRoot, "*", SearchOption.AllDirectories))
        {
            ...
            {
                throw new BundleValidationException($"Security asset directory '{directory}' cannot be a reparse point.");
            }
        }
        ...
        foreach (var sourcePath in Directory.EnumerateFiles(sourceRoot, "*", SearchOption.AllDirectories))
        {
            ...
            {
                throw new BundleValidationException($"Security asset '{sourcePath}' cannot be a reparse point.");
            }
    ...
    private static Task WriteJsonAsync(string path, object value, CancellationToken cancellationToken) => WriteJsonAsync(path, value, JsonOptions, cancellationToken);

    private static async Task WriteJsonAsync(string path, object value, JsonSerializerOptions options, CancellationToken cancellationToken)
    {
    ...
    private static void DeleteStagingDirectory(string staging, string parent)
    {
        ...
        if (!fullStaging.StartsWith(fullParent, StringComparison.OrdinalIgnoreCase) || !Path.GetFileName(fullStaging).StartsWith('.'))
        {
