...
public sealed class OpenSslBundleSigner(string command) : IBundleSigner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var privateDescription = await RunAsync(
            ["pkey", "-in", privateKeyPath, "-text", "-noout"],
            cancellationToken);
        var publicDescription = await RunAsync(
            ["pkey", "-pubin", "-in", publicKeyPath, "-text", "-noout"],
            cancellationToken);
        if (!privateDescription.Contains("ED25519", StringComparison.OrdinalIgnoreCase) ||
            !publicDescription.Contains("ED25519", StringComparison.OrdinalIgnoreCase))
        {
            throw new BundleValidationException("Bundle signing keys must use Ed25519.");
        }

        _ = await RunAsync(
            ["pkeyutl", "-sign", "-rawin", "-inkey", privateKeyPath, "-in", contentPath, "-out", signaturePath],
            cancellationToken);
        _ = await RunAsync(
            ["pkeyutl", "-verify", "-rawin", "-pubin", "-inkey", publicKeyPath, "-in", contentPath, "-sigfile", signaturePath],
            cancellationToken);
    }
    ...
    private async Task<string> RunAsync(IReadOnlyList<string> arguments, CancellationToken cancellationToken)
    {
        ...
        if (process.ExitCode != 0)
        {
            throw new BundleValidationException(
                $"OpenSSL failed with exit code {process.ExitCode}: {Limit(error.Trim())}");
        }
