...
public static class DependencyInventory
{
    ...
    public static async Task<(LicensePolicy Policy, IReadOnlyList<DependencyComponent> Components)> LoadAsync(string repositoryRoot, string policyPath, CancellationToken cancellationToken = default)
    {
        await using var policyStream = File.OpenRead(policyPath);
        var policy = await JsonSerializer.DeserializeAsync<LicensePolicy>(policyStream, JsonOptions, cancellationToken) ?? throw new BundleValidationException("License policy is empty.");
        ...
        var result = components.Values.OrderBy(static item => item.PackageManager, StringComparer.Ordinal).ThenBy(static item => item.Name, StringComparer.OrdinalIgnoreCase).ThenBy(static item => item.Version, StringComparer.Ordinal).ToArray();
    ...
    private static async Task ReadVendoredAsync(string repositoryRoot, Dictionary<string, DependencyComponent> components, CancellationToken cancellationToken)
    {
        ...
        var inventory = await JsonSerializer.DeserializeAsync<ReviewedDependencyInventory>(stream, JsonOptions, cancellationToken) ?? throw new BundleValidationException("The reviewed vendored dependency inventory is empty.");
        ...
        foreach (var component in inventory.Components)
        {
            ...
            await using (var source = File.OpenRead(sourcePath))
            {
                ...
                {
                    throw new BundleValidationException($"Vendored dependency '{component.Id}' does not match its reviewed SHA-256 identity.");
                }
            }
            ...
            if (!licenseText.Contains("Apache License", StringComparison.Ordinal) || !licenseText.Contains("END OF TERMS AND CONDITIONS", StringComparison.Ordinal) || licenseText.Length < 10_000)
            {
                throw new BundleValidationException($"Vendored dependency '{component.Id}' does not include the complete reviewed license text.");
            }

            Add(components, new DependencyComponent(component.PackageManager, component.Name, component.Version, $"sha256-{Convert.ToBase64String(Convert.FromHexString(component.Sha256))}", component.License, component.SourceUri, Direct: true, Optional: false));
        }
    }
    ...
    private static void ValidateVendoredIdentity(ReviewedDependencyComponent component)
    {
        ...
        {
            throw new BundleValidationException($"Vendored dependency '{component.Id}' has an invalid reviewed identity.");
        }
    }
    ...
    private static string ResolveRepositoryFile(string repositoryRoot, string relativePath, string componentId)
    {
        ...
        {
            throw new BundleValidationException($"Vendored dependency '{componentId}' has an unsafe repository path.");
        }
        ...
        {
            throw new BundleValidationException($"Vendored dependency '{componentId}' references a missing or unsafe repository file.");
        }

        return path;
    }

    private static bool IsStableId(string value) => value.Length is > 0 and <= 128 && char.IsAsciiLetterOrDigit(value[0]) && value.All(static character => char.IsAsciiLetterOrDigit(character) || character is '.' or '_' or '-');

    private static bool IsCommit(string value) => value.Length is 40 or 64 && value.All(static character => character is >= '0' and <= '9' or >= 'a' and <= 'f');

    private static bool IsSha256(string value) => value.Length == 64 && value.All(static character => character is >= '0' and <= '9' or >= 'a' and <= 'f');

    private static async Task ReadNpmAsync(string repositoryRoot, LicensePolicy policy, Dictionary<string, DependencyComponent> components, CancellationToken cancellationToken)
    {
        ...
        foreach (var package in packages.EnumerateObject())
        {
            ...
            var version = OptionalString(value, "version") ?? throw new BundleValidationException($"npm package '{name}' has no exact version.");
            var rawLicense = OptionalString(value, "license") ?? throw new BundleValidationException($"npm package '{name}@{version}' has no license metadata.");
            var license = ResolveLicense(policy, $"npm:{name}@{version}", rawLicense);
            Add(components, new DependencyComponent("npm", name, version, OptionalString(value, "integrity"), license, OptionalString(value, "resolved"), directNames.Contains(name), OptionalBoolean(value, "optional")));
        }
    }
    ...
    private static void ReadNuGet(string repositoryRoot, LicensePolicy policy, Dictionary<string, DependencyComponent> components)
    {
            ...
            foreach (var framework in frameworks.EnumerateObject())
            {
                foreach (var package in framework.Value.EnumerateObject())
                {
                    ...
                    var license = policy.Overrides.TryGetValue(overrideKey, out var configured)
                        ? configured : ReadNuGetLicense(globalPackages, package.Name, version);
    ...
    private static IEnumerable<string> EnumerateLockFiles(string root)
    {
        ...
        while (pending.Count > 0)
        {
            ...
            foreach (var path in Directory.EnumerateFiles(directory).Where(path => IsNuGetLockFileName(Path.GetFileName(path))).OrderBy(static path => path, StringComparer.Ordinal))
            {
                yield return path;
            }
    ...
    private static string ReadNuGetLicense(string globalPackages, string name, string version)
    {
        ...
        var nuspecPath = Directory.Exists(directory)
            ? Directory.EnumerateFiles(directory, "*.nuspec", SearchOption.TopDirectoryOnly).FirstOrDefault() : null;
        if (nuspecPath is null)
        {
            throw new BundleValidationException($"NuGet package '{name}@{version}' is not restored; its license cannot be audited.");
        }

        var document = XDocument.Load(nuspecPath, LoadOptions.None);
        var metadata = document.Descendants().FirstOrDefault(static element => element.Name.LocalName == "metadata") ?? throw new BundleValidationException($"NuGet package '{name}@{version}' has an invalid nuspec.");
        var license = metadata.Elements().FirstOrDefault(static element => element.Name.LocalName == "license");
        if (license is not null && string.Equals(license.Attribute("type")?.Value, "expression", StringComparison.OrdinalIgnoreCase) && !string.IsNullOrWhiteSpace(license.Value))
        {
            return license.Value.Trim();
        }

        throw new BundleValidationException($"NuGet package '{name}@{version}' has no SPDX license expression; add a reviewed policy override.");
    }

    private static void Add(Dictionary<string, DependencyComponent> components, DependencyComponent component)
    {
        ...
        if (components.TryGetValue(key, out var existing))
        {
            ...
            {
                throw new BundleValidationException($"Dependency '{component.Name}@{component.Version}' has conflicting license metadata.");
            }
            components[key] = existing with { Direct = existing.Direct || component.Direct, Optional = existing.Optional && component.Optional };
            return;
        }
    ...
    private static void ValidateLicense(DependencyComponent component, LicensePolicy policy)
    {
        ...
        {
            throw new BundleValidationException($"Dependency '{component.Name}@{component.Version}' has an invalid license expression.");
        }
        ...
        foreach (var identifier in identifiers)
        {
            ...
            {
                throw new BundleValidationException($"Dependency '{component.Name}@{component.Version}' uses denied license '{identifier}'.");
            }
            if (!allowed.Contains(identifier))
            {
                throw new BundleValidationException($"Dependency '{component.Name}@{component.Version}' uses unapproved license '{identifier}'.");
            }
        }
    }
    ...
    private static bool OptionalBoolean(JsonElement parent, string propertyName) => parent.TryGetProperty(propertyName, out var value) && value.ValueKind == JsonValueKind.True;
