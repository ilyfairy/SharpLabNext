...
public sealed record ProfileCandidateVerificationResult(
    string ReleaseId,
    string CatalogRevision,
    int WorkersVerified,
    int RuntimesVerified);
...
public sealed class ProfileCandidateDeploymentVerifier(HttpClient httpClient)
{
    ...
    public async Task<ProfileCandidateVerificationResult> VerifyAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var expectedReferenceSetDigests = ReferenceSetIdentityResolver.ResolveExpectedDigests(
            candidateCatalog,
            releaseLock);
        ...
        workersVerified += await VerifyWorkerAsync(
            ...
            new Dictionary<string, ExpectedIdentity>(StringComparer.Ordinal)
            {
                ...
                ["compilerCommit"] = new(
                    Required(Component(releaseLock, "peachpie-stable").Commit, "peachpie-stable.commit"),
                    true)
            },
            candidateCatalog,
            expectedReferenceSetDigests,
        ...
        workersVerified += await VerifyWorkerAsync(
            ...
            {
                ["compilerCommit"] = new(
                    Required(Component(releaseLock, "roslyn-const-generics").Commit, "roslyn-const-generics.commit"),
                    true)
            },
            candidateCatalog,
            expectedReferenceSetDigests,
        ...
        var runtimesVerified = await VerifyRuntimesAsync(
            endpoints,
            images,
            releaseLock,
            candidateCatalog,
            timeout.Token);
        return new ProfileCandidateVerificationResult(
            releaseLock.ReleaseId,
            candidateCatalog.Revision,
            workersVerified,
            runtimesVerified);
    }
    ...
    private async Task VerifyGatewayAsync(
        ...
        CatalogDocument candidateCatalog,
        CancellationToken cancellationToken)
    {
        ...
        RequireEqual(
            releaseLock.ReleaseId,
            RequiredString(system.RootElement, "ReleaseId"),
            "gateway.system.ReleaseId");

        var remoteCatalog = await GetAsync<CatalogDocument>(
            Endpoint(gatewayEndpoint, "/api/v1/catalog"),
            cancellationToken);
    ...
    private async Task<int> VerifyWorkerAsync(
        CandidateValidationEndpoints endpoints,
        IReadOnlyDictionary<string, CandidateBundleImage> images,
        ReleaseLockDocument releaseLock,
        string profileId,
        string imageId,
        IReadOnlyDictionary<string, ExpectedIdentity> expectedIdentity,
        CatalogDocument catalog,
        IReadOnlyDictionary<string, string> expectedReferenceSetDigests,
        IDictionary<string, string> verifiedReferenceSetContents,
        CancellationToken cancellationToken)
    {
        ...
        var descriptor = await GetAsync<WorkerDescriptor>(
            Endpoint(endpoint, "/api/v1/worker/describe"),
            cancellationToken);
        ...
        foreach (var pair in expectedIdentity)
        {
            ...
            RequireEqual(
                pair.Value.Value,
                actual,
                $"worker[{profileId}].identity.{pair.Key}",
                pair.Value.IgnoreCase);
        }

        if (descriptor.WorkerKind == WorkerKind.Toolchain)
        {
            ValidateReferenceSetAttestations(
                profileId,
                descriptor.ReferenceSets,
                releaseLock,
                catalog,
                expectedReferenceSetDigests,
                verifiedReferenceSetContents);
    ...
    private static void ValidateReferenceSetAttestations(
        string profileId,
        IReadOnlyList<ReferenceSetAttestation>? referenceSets,
        ReleaseLockDocument releaseLock,
        CatalogDocument catalog,
        IReadOnlyDictionary<string, string> expectedReferenceSetDigests,
        IDictionary<string, string> verifiedReferenceSetContents)
    {
        ...
        foreach (var referenceSetId in toolchain.AllowedReferenceSetIds)
        {
            ...
            var manifest = catalog.ReferenceSets.Single(item =>
            ...
            var expectedDigest = expectedReferenceSetDigests.TryGetValue(referenceSetId, out var selectableDigest)
                ? selectableDigest
                : ReferenceSetIdentityResolver.ResolveLockedDigest(component, referenceSetId);
            ...
            if (verifiedReferenceSetContents.TryGetValue(referenceSetId, out var verifiedContentDigest))
            {
                RequireEqual(
                    ...
                    $"worker[{profileId}].referenceSets[{referenceSetId}].contentDigest");
            }
            else if (isOperatorImage)
            {
                RequireEqual(
                    "operator-image",
                    attestation.Provenance.Kind,
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.kind");
                RequireEqual(
                    Required(component.SourceUri, $"{referenceSetId}.sourceUri"),
                    attestation.Provenance.SourceUri ?? string.Empty,
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.sourceUri");
                RequireEqual(
                    expectedDigest,
            ...
            RequireEqual(
                ...
                attestation.TargetFramework,
                $"worker[{profileId}].referenceSets[{referenceSetId}].targetFramework");
            RequireEqual(
                component.ResolvedVersion,
                attestation.Provenance.ResolvedVersion,
                $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.resolvedVersion");
            ...
            if (!string.IsNullOrWhiteSpace(component.Package))
            {
                ...
                RequireEqual(
                    component.Package,
                    attestation.Provenance.Package ?? string.Empty,
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.package");
                RequireEqual(
                    Required(component.SourceUri, $"{referenceSetId}.sourceUri"),
                    attestation.Provenance.SourceUri ?? string.Empty,
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.sourceUri");
            ...
            else if (!isOperatorImage)
            {
                RequireEqual(
                    ...
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.kind");
                RequireEqual(
                    Required(component.Commit, $"{referenceSetId}.commit"),
                    attestation.Provenance.Commit ?? string.Empty,
                    $"worker[{profileId}].referenceSets[{referenceSetId}].provenance.commit",
                    ignoreCase: true);
                RequireEqual(
                    Required(component.SourceUri, $"{referenceSetId}.sourceUri"),
                    attestation.Provenance.SourceUri ?? string.Empty,
    ...
    private static void ValidateCompositeReferenceSet(
        ...
        ReferenceSetAttestation attestation)
    {
        ...
        if (sources is null || sources.Count != 2 ||
            !ValidCompositeSource(sources[0], "base", "all") ||
            !ValidCompositeSource(sources[1], "extension", "assembly-version:3.0.0.0"))
        {
        ...
        RequireEqual(
            component.Digest,
            actualDigest,
            $"referenceSets[{referenceSetId}].compositeSourceIdentity");
    }

    private static bool ValidCompositeSource(
    ...
    private async Task<int> VerifyRuntimesAsync(
        CandidateValidationEndpoints endpoints,
        IReadOnlyDictionary<string, CandidateBundleImage> images,
        ReleaseLockDocument releaseLock,
        CatalogDocument catalog,
        CancellationToken cancellationToken)
    {
        ...
        var byId = profiles.EnumerateArray().ToDictionary(
            static profile => RequiredString(profile, "Id"),
            static profile => profile.Clone(),
        ...
        foreach (var runtime in catalog.Runtimes.Where(static item => item.Availability.IsSelectable))
        {
            ...
            if (!RequiresCommitIdentity(runtime))
            {
                ...
                {
                    throw Failure(
                        $"Candidate bundle must not claim CoreCLR commit identities for runtime '{runtime.Id}'.");
                }
    ...
    private async Task<T> GetAsync<T>(Uri uri, CancellationToken cancellationToken)
    {
        ...
        return document.Deserialize<T>(WireJsonOptions)
            ?? throw Failure($"Endpoint '{uri}' returned an empty JSON document.");
    }
    ...
    private static async Task<T> ReadJsonAsync<T>(string path, CancellationToken cancellationToken)
    {
        ...
        return await JsonSerializer.DeserializeAsync<T>(stream, CanonicalJsonOptions, cancellationToken)
            ?? throw Failure($"JSON document '{path}' is empty.");
    }

    private static LockedComponent Component(ReleaseLockDocument releaseLock, string id) =>
        releaseLock.Components.TryGetValue(id, out var component)
            ? component
            : throw Failure($"Candidate lock is missing component '{id}'.");

    private static JsonElement RequiredProperty(JsonElement element, string propertyName) =>
        element.TryGetProperty(propertyName, out var value)
            ? value
            : throw Failure($"JSON response is missing '{propertyName}'.");
    ...
    private static string RequiredString(JsonElement element, string propertyName)
    {
        ...
        return value.ValueKind == JsonValueKind.String && !string.IsNullOrWhiteSpace(value.GetString())
            ? value.GetString()!
            : throw Failure($"JSON response field '{propertyName}' must be a non-empty string.");
    }
    ...
    private static string Required(string? value, string field) =>
        !string.IsNullOrWhiteSpace(value)
            ? value
            : throw Failure($"Candidate lock field '{field}' is required.");
