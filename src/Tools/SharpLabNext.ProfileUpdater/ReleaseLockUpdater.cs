...
public sealed record ReleaseLockUpdateResult(
...
public sealed class ReleaseLockUpdater(IProfileSourceClient sourceClient, string channelRoot)
{
    ...
    public async Task<ReleaseLockUpdateResult> ResolveAsync(
        ReleaseLockDocument current,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var runtimeTasks = manifests.Runtimes.ToDictionary(
            static intent => intent.Id,
            intent => sourceClient.ResolveDotNetChannelAsync(intent.Channel, cancellationToken),
            StringComparer.Ordinal);
        var nugetTasks = manifests.Components
            .Where(static intent => string.Equals(intent.SourceType, "nuget", StringComparison.Ordinal))
            .ToDictionary(
                static intent => intent.Id,
                intent => ResolveNuGetAsync(intent, cancellationToken),
                StringComparer.Ordinal);
        var gitTasks = manifests.Components
            .Where(static intent => string.Equals(intent.SourceType, "github-commit", StringComparison.Ordinal))
            .ToDictionary(
                static intent => intent.Id,
                intent => sourceClient.ResolveGitCommitAsync(
                    intent.GitOwner!,
                    intent.GitRepository!,
                    intent.GitReference!,
                    cancellationToken),
                StringComparer.Ordinal);
        ...
        var referenceTasks = manifests.Runtimes.ToDictionary(
            static intent => intent.ReferenceSetId,
            intent => sourceClient.ResolveExactPackageAsync(
                intent.ReferencePackage,
                runtimeTasks[intent.Id].Result.RuntimeVersion,
                cancellationToken),
            StringComparer.Ordinal);
        await Task.WhenAll(referenceTasks.Values);

        var components = current.Components
            .Where(static pair => !IsPackageManagerOwnedComponent(pair.Key, pair.Value))
            .ToDictionary(
        ...
        foreach (var intent in manifests.Components)
        {
            ...
            {
                var component = NuGetComponent(intent.Kind, await nugetTasks[intent.Id]) with
                {
                    Commit = intent.ProvenanceCommit
                };
            ...
            if (intent.Version is not null &&
                !string.Equals(intent.Version, source.ProductVersion, StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    $"Profile channel '{intent.Id}' expected Git product version '{intent.Version}', " +
                    $"but source '{intent.GitOwner}/{intent.GitRepository}@{intent.GitReference}' resolved '{source.ProductVersion}'.");
            }

            var digest = $"sha256:{source.ArchiveSha256}";
            Set(components, intent.Id, new LockedComponent
            {
                Kind = intent.Kind,
                ResolvedVersion = resolvedVersion,
                Commit = source.Commit,
                SourceUri = intent.SourceComponentId is null
                    ? source.ArchiveUri.AbsoluteUri
                    : $"{source.RepositoryUri.AbsoluteUri.TrimEnd('/')}/tree/{source.Commit}",
                Digest = digest
            });
            if (intent.SourceComponentId is not null)
            {
                Set(components, intent.SourceComponentId, new LockedComponent
                {
                    Kind = "source",
                    ResolvedVersion = resolvedVersion,
                    Commit = source.Commit,
                    SourceUri = source.ArchiveUri.AbsoluteUri,
                    Digest = digest
                });
            }
        }
        ...
        {
            var resolvedVersion = ProfileChannelManifestLoader.VersionPlaceholderRegex().Replace(
                derived.VersionTemplate,
                match => RequiredResolvedVersion(components, match.Groups["id"].Value));
            var component = derived.IdentitySourceComponentId is null
                ...
                : components[derived.IdentitySourceComponentId] with
                {
                    Kind = derived.Kind,
                    ResolvedVersion = resolvedVersion,
                    PatchDigest = null,
        ...
        var candidate = new ReleaseLockDocument
        {
            SchemaVersion = current.SchemaVersion,
            ReleaseId = string.IsNullOrWhiteSpace(releaseId) ? current.ReleaseId : releaseId,
            ResolvedAt = DateTimeOffset.UtcNow,
            Components = components
        };
        var changes = current.Components.Keys
            .Union(components.Keys, StringComparer.Ordinal)
            .Where(componentId =>
            {
            ...
            .Select(componentId => new ReleaseLockChange(
                componentId,
                current.Components.TryGetValue(componentId, out var previous) ? previous.ResolvedVersion : null,
                components.TryGetValue(componentId, out var next) ? next.ResolvedVersion : null))
            .OrderBy(static change => change.ComponentId, StringComparer.Ordinal)
            .ToArray();
        return new ReleaseLockUpdateResult(candidate, changes);
    }

    private Task<NuGetPackageResolution> ResolveNuGetAsync(
        ComponentChannelIntent intent,
        CancellationToken cancellationToken) =>
        string.Equals(intent.Policy, "latest-stable", StringComparison.Ordinal)
            ? sourceClient.ResolveLatestStablePackageAsync(intent.Package!, cancellationToken)
            : sourceClient.ResolveExactPackageAsync(intent.Package!, intent.Version!, cancellationToken);

    private static string RequiredResolvedVersion(
        Dictionary<string, LockedComponent> components,
        string componentId) =>
        components.TryGetValue(componentId, out var component) && !string.IsNullOrWhiteSpace(component.ResolvedVersion)
            ...
            : throw new InvalidDataException(
                $"Derived profile component references '{componentId}', which has no resolved version.");
    ...
    private static void Set(
        Dictionary<string, LockedComponent> components,
        string id,
        LockedComponent value) => components[id] = value;
}
