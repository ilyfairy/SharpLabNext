...
internal static class CheckedJitDisassemblyDocument
{
    ...
    public static string SelectPreparedMethods(
        ...
        IReadOnlyDictionary<int, CheckedMethodSourceMap> sourceMaps)
    {
        ...
        var preparedMethods = methods
            .Where(static method => string.Equals(method.Status, "prepared", StringComparison.Ordinal))
            .ToArray();
        var preparedHeaders = preparedMethods
            .Where(static method => method.SignatureIdentity.HeaderKey is not null)
            .GroupBy(static method => method.SignatureIdentity.HeaderKey!, StringComparer.Ordinal)
            .ToDictionary(static group => group.Key, static group => group.ToArray(), StringComparer.Ordinal);
        var namespaceShortenedPreparedHeaders = preparedMethods
            .Where(static method => method.SignatureIdentity.NamespaceShortenedHeaderKey is not null)
            .GroupBy(static method => method.SignatureIdentity.NamespaceShortenedHeaderKey!, StringComparer.Ordinal)
            .ToDictionary(static group => group.Key, static group => group.ToArray(), StringComparer.Ordinal);
        var unsupportedPreparedNames = preparedMethods
            .Where(static method => method.SignatureIdentity.HeaderKey is null)
            .SelectMany(static method => GetNameKeys(method.SignatureIdentity))
            .ToHashSet(StringComparer.Ordinal);
        var retainedSections = sections
            .Where(section =>
                section.SignatureIdentity.HeaderKey is { } headerKey &&
                (preparedHeaders.ContainsKey(headerKey) ||
                 namespaceShortenedPreparedHeaders.ContainsKey(headerKey) ||
                 unsupportedPreparedNames.Contains(section.SignatureIdentity.NameKey)))
            .ToArray();
        var sectionsByHeader = retainedSections
            .Where(static section => section.SignatureIdentity.HeaderKey is not null)
            .GroupBy(static section => section.SignatureIdentity.HeaderKey!, StringComparer.Ordinal)
            .ToDictionary(static group => group.Key, static group => group.ToArray(), StringComparer.Ordinal);
        var bindings = CreateBindings(
            preparedHeaders,
            namespaceShortenedPreparedHeaders,
            sectionsByHeader);
        ...
        foreach (var section in retainedSections)
        {
            ...
            if (sourceMaps.TryGetValue(result.MetadataToken, out var sourceMap))
            {
                ...
                result.LinkedRanges = mapping.Ranges
                    .Select(range => OffsetOutputRange(range, sectionOutputStart))
                    .ToList();
    ...
    private static Dictionary<JitAssemblySection, JitMethodResult> CreateBindings(
        Dictionary<string, JitMethodResult[]> preparedHeaders,
        Dictionary<string, JitMethodResult[]> namespaceShortenedPreparedHeaders,
        Dictionary<string, JitAssemblySection[]> sectionsByHeader)
    {
        ...
        foreach (var (headerKey, candidates) in preparedHeaders)
        {
            if (candidates.Length != 1 ||
                !sectionsByHeader.TryGetValue(headerKey, out var matchingSections) ||
        ...
        {
            if (preparedHeaders.ContainsKey(headerKey) ||
                candidates.Length != 1 ||
                exactlyBoundMethods.Contains(candidates[0]) ||
                !sectionsByHeader.TryGetValue(headerKey, out var matchingSections) ||
                matchingSections.Length != 1 ||
                bindings.ContainsKey(matchingSections[0]))
            {
                continue;
            }
    ...
    private static List<JitAssemblySection> ParseSections(string[] lines)
    {
        ...
        {
            if (!TryGetHeaderName(lines[start], out var header) ||
                !JitMethodSignatures.TryParseHeader(header, out var signatureIdentity))
            {
                continue;
            }
    ...
    private static JitLinkedRange OffsetOutputRange(JitLinkedRange range, int lineOffset) =>
        new(
            ...
            range.SourceRange,
            new JitTextRange(
                checked(range.OutputRange.StartLine + lineOffset),
    ...
    private static bool TryGetHeaderName(string line, out string name)
    {
        string? prefix = line.StartsWith(HeaderPrefix, StringComparison.Ordinal)
            ? HeaderPrefix
            : line.StartsWith(BareHeaderPrefix, StringComparison.Ordinal)
                ? BareHeaderPrefix
                : null;
        if (prefix is null)
        {
    ...
    private static int ParseNativeCodeSize(string[] lines, int start, int end)
    {
        for (var index = start; index < end; index++)
        {
            ...
            if (digitEnd > digitStart && int.TryParse(
                    trimmed.AsSpan(digitStart, digitEnd - digitStart),
                    NumberStyles.None,
    ...
    private sealed record JitAssemblySection(
        int Start,
        int End,
