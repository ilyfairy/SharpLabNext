...
internal static partial class JitSourceMapping
{
    ...
    public static IReadOnlyDictionary<int, IReadOnlyList<JitSourcePoint>> LoadSiblingPortablePdb(
        string assemblyPath)
    {
        ...
        try
        {
            ...
            for (var row = 1; row <= methodCount; row++)
            {
                ...
                foreach (var point in information.GetSequencePoints())
                {
                    ...
                    points.Add(new JitSourcePoint(
                        point.Offset,
                        SanitizeDocumentPath(reader.GetString(document.Name)),
    ...
    public static IReadOnlyList<JitSourceLinkedRange> MapSection(
        string sectionText,
        IReadOnlyList<JitSourcePoint> sequencePoints)
    {
        ...
        return markerRanges.Count > 0 || sawRootMarker
            ? markerRanges
            : MapMethodFallback(sectionText, sequencePoints);
    }

    internal static IReadOnlyList<JitSourceLinkedRange> MapTextMarkers(
        string sectionText,
        IReadOnlyList<JitSourcePoint> sequencePoints) =>
        MapTextMarkers(sectionText, sequencePoints, out _);

    private static List<JitSourceLinkedRange> MapTextMarkers(
        string sectionText,
        IReadOnlyList<JitSourcePoint> sequencePoints,
        out bool sawRootMarker)
    {
        sawRootMarker = false;
        ...
        void CompleteRange()
        {
            ...
            ranges.Add(new JitSourceLinkedRange(
        ...
        for (var lineIndex = 0; lineIndex < lines.Length; lineIndex++)
        {
            ...
            if (marker.Success)
            {
                ...
                currentPoint = marker.Groups["unknown"].Success ||
                    !int.TryParse(
                        marker.Groups["offset"].Value,
                        NumberStyles.AllowHexSpecifier,
                        CultureInfo.InvariantCulture,
                        out var offset)
                    ? null
                    : FindSequencePoint(sequencePoints, offset);
                continue;
            }
    ...
    internal static IReadOnlyList<JitSourceLinkedRange> MapMethodFallback(
        string sectionText,
        IReadOnlyList<JitSourcePoint> sequencePoints)
    {
        if (sectionText.Length == 0 || sequencePoints.Count == 0)
    ...
    internal static IReadOnlyList<JitSourceLinkedRange> MapSection(
        string assemblyPath,
        int metadataToken,
        string sectionText)
    {
    ...
    private static JitSourcePoint? FindSequencePoint(
        IReadOnlyList<JitSourcePoint> sequencePoints,
        int offset)
    {
        var low = 0;
        ...
        return match >= 0 && sequencePoints[match].SourceRange is not null
            ? sequencePoints[match]
            : null;
    }

    private static JitSourceLinkedRange? CreateMethodFallback(
        string[] lines,
        IReadOnlyList<JitSourcePoint> sequencePoints)
    {
        var firstPoint = sequencePoints.FirstOrDefault(static point =>
        ...
        var points = sequencePoints
            .Where(point => point.DocumentPath == documentPath && point.SourceRange is not null)
            .Select(static point => point.SourceRange!)
            .ToArray();
        if (points.Length == 0)
            return null;
        ...
        return new JitSourceLinkedRange(
            documentPath,
            new JitSourceTextRange(
                start.StartLine,
...
internal sealed record JitSourcePoint(
    int Offset,
    string? DocumentPath,
    JitSourceTextRange? SourceRange);

internal sealed record JitSourceTextRange(
    int StartLine,
    int StartCharacter,
    int EndLine,
    int EndCharacter);

internal sealed record JitSourceLinkedRange(
