...
public sealed class LanguageWorkerConformanceRunner(
    HttpClient httpClient,
    Func<Uri, CancellationToken, Task<WebSocket>> connectWebSocketAsync)
{
    ...
    public async Task<LanguageWorkerConformanceReport> VerifyAsync(
        LanguageWorkerConformanceScenario scenario,
        CancellationToken cancellationToken = default)
    {
        ...
        await VerifyHealthAsync(
            ...
            scenario.ExpectedWorkerImageId,
            scenario.ExpectedManifest.ToolchainIds,
            scenario.ExpectedManifest.SupportedReferenceSetIds,
    ...
    private async Task VerifyHealthAsync(
        ServiceIdentity expected,
        string expectedWorkerImageId,
        IReadOnlyList<string> expectedToolchainIds,
        IReadOnlyList<string> expectedReferenceSetIds,
        CancellationToken cancellationToken)
    {
        ...
        Require(
            "health-and-identity",
            descriptor.ProfileIds.SequenceEqual(expectedToolchainIds, StringComparer.Ordinal),
            "The toolchain profiles differ from the capability manifest.");
        ...
        foreach (var referenceSetId in expectedReferenceSetIds)
        {
            Require(
                ...
                descriptor.ReferenceSets!.Any(item =>
                    item.Id == referenceSetId &&
                    !string.IsNullOrWhiteSpace(item.Digest) &&
                    item.ContentDigest.StartsWith("sha256:", StringComparison.Ordinal) &&
                    !string.IsNullOrWhiteSpace(item.Provenance.Kind) &&
                    !string.IsNullOrWhiteSpace(item.Provenance.ResolvedVersion)),
                $"Reference set '{referenceSetId}' is not attested by the worker descriptor.");
        }
        foreach (var capability in expected.Capabilities)
        {
            Require(
                ...
                descriptor.Capabilities.Any(item =>
                    item.Id == capability &&
                    item.Available &&
                    item.ProfileIds.SequenceEqual(expectedToolchainIds, StringComparer.Ordinal)),
                $"Capability '{capability}' is missing or unavailable in the worker descriptor.");
        }

    }

    private async Task VerifyCapabilitiesAsync(
        ServiceIdentity identity,
        LanguageWorkerCapabilityManifest expected,
        CancellationToken cancellationToken)
    {
        var actual = await GetAsync<LanguageWorkerCapabilityManifest>(
            "capability-manifest",
        ...
        Require(
            "capability-manifest",
            actual.ToolchainIds.SequenceEqual(expected.ToolchainIds, StringComparer.Ordinal),
            "The endpoint manifest toolchain IDs differ.");
    ...
    private async Task VerifyCompileCheckAsync(
        LanguageWorkerConformanceScenario scenario,
        CancellationToken cancellationToken)
    {
        ...
        var response = await PostAsync<BuildRequest, LanguageWorkerBuildHttpResponse>(
            "compile-check",
            "/api/v1/build",
            scenario.CompileCheckRequest,
            cancellationToken).ConfigureAwait(false);
        ...
        var result = response.Result as CompilationCheckResult
            ?? throw new LanguageWorkerConformanceException("compile-check", "The endpoint did not return a CompilationCheckResult.");
        ...
        Require(
            "compile-check",
            result.Identity.ToolchainId == scenario.CompileCheckRequest.ToolchainId,
            "The result used another toolchain identity.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var response = await PostAsync<BuildRequest, LanguageWorkerBuildHttpResponse>(
            "artifact-envelope",
            "/api/v1/build",
            scenario.ArtifactRequest,
            cancellationToken).ConfigureAwait(false);
        var result = response.Result as BuildResult
            ?? throw new LanguageWorkerConformanceException("artifact-envelope", "The endpoint did not return a BuildResult.");
        Require("artifact-envelope", result.Outcome == BuildOutcome.Succeeded, "The known-valid workspace did not emit an artifact.");
        var envelope = response.DevelopmentArtifact
            ?? throw new LanguageWorkerConformanceException("artifact-envelope", "The worker omitted its conformance artifact envelope.");
    ...
    private async Task VerifyLspAsync(
        LanguageWorkerConformanceScenario scenario,
        CancellationToken cancellationToken)
    {
        var session = await PostAsync<OpenLanguageSessionRequest, LanguageSession>(
            "lsp-lifecycle",
            "/api/v1/language-sessions",
            scenario.LanguageSessionRequest,
            cancellationToken).ConfigureAwait(false);
        Require("lsp-lifecycle", session.LanguageId == scenario.ExpectedManifest.LanguageId, "The session language differs.");
        Require(
            "lsp-lifecycle",
            session.ToolchainId == scenario.LanguageSessionRequest.ToolchainId,
            "The session toolchain differs.");
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new
            {
                textDocument = new
                {
                    uri = scenario.DocumentUri,
                    languageId = scenario.ExpectedManifest.LanguageId,
                    version = 1,
                    text = scenario.OpenText
                }
            }
        }, cancellationToken).ConfigureAwait(false);
        using (var diagnostics = await ReceiveDiagnosticsAsync(socket, cancellationToken).ConfigureAwait(false))
        {
            Require(
                ...
                diagnostics.RootElement.GetProperty("params").GetProperty("diagnostics").EnumerateArray()
                    .Any(item => item.GetProperty("code").GetString() == scenario.ExpectedOpenDiagnosticCode),
                "didOpen did not publish the expected diagnostic.");
        }
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new
            {
                textDocument = new { uri = scenario.DocumentUri, version = 2 },
                contentChanges = new[] { new { text = scenario.ChangedText } }
            }
        }, cancellationToken).ConfigureAwait(false);
        using (var diagnostics = await ReceiveDiagnosticsAsync(socket, cancellationToken).ConfigureAwait(false))
        {
            Require(
                "lsp-lifecycle",
                !diagnostics.RootElement.GetProperty("params").GetProperty("diagnostics").EnumerateArray().Any(),
                "didChange did not clear diagnostics for the known-valid document.");
        }
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new
            {
                textDocument = new { uri = scenario.DocumentUri },
                position = new { line = scenario.CompletionPosition.Line, character = scenario.CompletionPosition.Character },
        ...
        using (var completion = await ReceiveUntilAsync(socket, static root => HasId(root, 2), cancellationToken).ConfigureAwait(false))
        {
            Require(
                ...
                completion.RootElement.GetProperty("result").GetProperty("items").EnumerateArray()
                    .Any(item => item.GetProperty("label").GetString() == scenario.ExpectedCompletionLabel),
                "Completion did not contain the expected item.");
        }
        ...
        using var closeResponse = await httpClient.DeleteAsync(
            $"/api/v1/language-sessions/{Uri.EscapeDataString(session.SessionId)}",
            cancellationToken).ConfigureAwait(false);
        Require("lsp-lifecycle", closeResponse.IsSuccessStatusCode, "The language session could not be closed.");
    }

    private async Task VerifyRejectedToolchainAsync(
        LanguageWorkerConformanceScenario scenario,
        CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<T>(body, JsonOptions)
                ?? throw new LanguageWorkerConformanceException(check, $"{path} returned an empty payload.");
        }
    ...
    private Uri CreateWebSocketUri(string path)
    {
        var baseAddress = httpClient.BaseAddress
            ?? throw new LanguageWorkerConformanceException("lsp-lifecycle", "HttpClient.BaseAddress is required.");
    ...
    private static Task<JsonDocument> ReceiveDiagnosticsAsync(
        WebSocket socket,
        CancellationToken cancellationToken) => ReceiveUntilAsync(
            socket,
            static root => root.TryGetProperty("method", out var method) &&
