...
public static partial class ReferenceSetAttestationReader
{
    ...
    public static ReferenceSetAttestation LoadAndVerify(string rootPath, string referenceSetId, string targetFramework, string resolvedVersion, string? expectedDigest, bool requireManifest, string? manifestPath = null)
    {
        ...
        var path = string.IsNullOrWhiteSpace(manifestPath)
            ? Path.Combine(root, ManifestFileName) : Path.GetFullPath(Environment.ExpandEnvironmentVariables(manifestPath));
        if (!File.Exists(path))
        {
            if (requireManifest)
        ...
        try
        {
            ...
            document = JsonSerializer.Deserialize(stream, ReferenceSetAttestationJsonContext.Default.ReferenceSetAttestationDocument) ?? throw new InvalidDataException("The reference-set attestation manifest is empty.");
        }
        catch (JsonException exception)
        {
            throw new InvalidDataException($"Reference set '{referenceSetId}' attestation manifest is invalid.", exception);
        }
        ...
        ValidateIdentity(document.ReferenceSet, referenceSetId, targetFramework, resolvedVersion, expectedDigest, requireManifest);
    ...
    {
        if (!string.Equals(attestation.Id, expectedId, StringComparison.Ordinal) || !string.Equals(attestation.TargetFramework, expectedTargetFramework, StringComparison.Ordinal) || string.IsNullOrWhiteSpace(attestation.Digest) || !IsSha256(attestation.ContentDigest) || attestation.Provenance is null || string.IsNullOrWhiteSpace(attestation.Provenance.Kind) || string.IsNullOrWhiteSpace(attestation.Provenance.ResolvedVersion))
        {
            throw new InvalidDataException($"Reference set '{expectedId}' attestation identity is invalid.");
        }
        if (!string.Equals(attestation.Provenance.ResolvedVersion, expectedResolvedVersion, StringComparison.Ordinal))
        {
            throw new InvalidDataException($"Reference set '{expectedId}' attestation resolved version does not match its configuration.");
        }
        if (requireExpectedDigest && string.IsNullOrWhiteSpace(expectedDigest))
        {
            throw new InvalidDataException($"Reference set '{expectedId}' configured digest is missing.");
        }
        if (!string.IsNullOrWhiteSpace(expectedDigest) && !string.Equals(attestation.Digest, expectedDigest, StringComparison.Ordinal))
        {
            throw new InvalidDataException($"Reference set '{expectedId}' attestation digest does not match its configuration.");
        }
    ...
    private static void ValidateProvenance(ReferenceSetAttestation attestation)
    {
        ...
        if (!string.Equals(provenance.Kind, NuGetPackageCompositionKind, StringComparison.Ordinal))
        {
            ...
            {
                throw new InvalidDataException($"Reference set '{attestation.Id}' has source provenance that is not valid for its kind.");
            }
            return;
        }

        if (!string.Equals(attestation.Id, NetFx30ReferenceSetId, StringComparison.Ordinal) || !string.Equals(attestation.TargetFramework, NetFx30TargetFramework, StringComparison.Ordinal) || !string.Equals(provenance.ResolvedVersion, NetFx30ResolvedVersion, StringComparison.Ordinal) || provenance.Package is not null || provenance.SourceUri is not null || provenance.Commit is not null || provenance.SourceArchiveDigest is not null)
        {
            throw new InvalidDataException($"Reference set '{attestation.Id}' composite provenance identity is invalid.");
        }

        var sources = provenance.Sources;
        if (sources is null || sources.Count != 2)
        {
            throw new InvalidDataException($"Reference set '{attestation.Id}' composite provenance must contain exactly two ordered sources.");
        }

        ValidateCompositeSource(attestation.Id, sources[0], "base", "all");
        ValidateCompositeSource(attestation.Id, sources[1], "extension", "assembly-version:3.0.0.0");
        ...
        {
            throw new InvalidDataException($"Reference set '{attestation.Id}' composite source identity digest does not match its provenance.");
        }
    }

    private static void ValidateCompositeSource(string referenceSetId, ReferenceSetProvenanceSource? source, string expectedRole, string expectedSelection)
    {
        if (source is null ||
        ...
        {
            throw new InvalidDataException($"Reference set '{referenceSetId}' composite source provenance is invalid.");
        }
    }

    private static string ComputeCompositeSourceIdentityDigest(ReferenceSetAttestation attestation, IReadOnlyList<ReferenceSetProvenanceSource> sources)
    {
        var canonical = new StringBuilder().Append("referenceSet=").Append(attestation.Id).Append('\n').Append("targetFramework=").Append(attestation.TargetFramework).Append('\n').Append("kind=").Append(attestation.Provenance.Kind).Append('\n').Append("resolvedVersion=").Append(attestation.Provenance.ResolvedVersion).Append('\n');
    ...
    private static bool HasCanonicalDelimiter(string? value) => string.IsNullOrWhiteSpace(value) || value.IndexOfAny(['\t', '\r', '\n']) >= 0;
    ...
    {
        var paths = Directory.EnumerateFiles(root, "*.dll", SearchOption.TopDirectoryOnly).OrderBy(static path => Path.GetFileName(path), StringComparer.Ordinal).ToArray();
        ...
        {
            using var stream = File.OpenRead(path);
            files.Add(new ReferenceSetAttestedFile(Path.GetFileName(path), stream.Length, $"sha256:{Convert.ToHexString(SHA256.HashData(stream)).ToLowerInvariant()}"));
        }
        return files;
    }

    private static void ValidateFiles(string referenceSetId, IReadOnlyList<ReferenceSetAttestedFile> expected, IReadOnlyList<ReferenceSetAttestedFile> actual)
    {
        ...
        for (var index = 0; index < expected.Count; index++)
        {
            ...
            if (string.IsNullOrWhiteSpace(file.Path) || file.Path != Path.GetFileName(file.Path) || file.Size < 0 || !IsSha256(file.Digest) || file != actual[index])
            {
    ...
    private sealed record ReferenceSetAttestationDocument(int SchemaVersion, ReferenceSetAttestation? ReferenceSet, IReadOnlyList<ReferenceSetAttestedFile>? Files);

    private sealed record ReferenceSetAttestedFile(string Path, long Size, string Digest);

    [JsonSourceGenerationOptions(JsonSerializerDefaults.Web, PropertyNameCaseInsensitive = false, UnmappedMemberHandling = JsonUnmappedMemberHandling.Disallow)]
