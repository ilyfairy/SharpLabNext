...
public sealed class ArtifactWorkerHandlerRegistry
{
    ...
    public ArtifactWorkerHandlerRegistry(
        ArtifactWorkerCapabilityManifest manifest,
        IEnumerable<IArtifactTransformHandler> transforms,
        IEnumerable<IArtifactRenderHandler> renders,
        IEnumerable<IArtifactVerificationHandler> verifications)
    {
    ...
    public IArtifactTransformHandler GetTransform(string transformId) =>
        _transforms.TryGetValue(transformId, out var handler)
            ? handler
            : throw Unsupported("transform", transformId);

    public IArtifactRenderHandler GetRender(string outputId) =>
        _renders.TryGetValue(outputId, out var handler)
            ? handler
            : throw Unsupported("render output", outputId);

    public IArtifactVerificationHandler GetVerification(string profileId) =>
        _verifications.TryGetValue(profileId, out var handler)
            ? handler
            : throw Unsupported("verification profile", profileId);

    private static Dictionary<string, THandler> Index<THandler>(
        IEnumerable<THandler> handlers,
        Func<THandler, string> idSelector,
        string description)
    ...
    private static void ValidateDeclaredHandlers(
        ...
        string description)
    {
        ...
        if (!declaredSet.SetEquals(registeredSet))
        {
            throw new InvalidOperationException(
                $"The registered artifact {description} handlers do not match the capability manifest.");
        }
    }

    private static ArtifactWorkerRequestException Unsupported(string description, string id) => new(
        "unsupported-capability",
        $"The requested artifact {description} '{Limit(id)}' is not supported.",
        Microsoft.AspNetCore.Http.StatusCodes.Status400BadRequest,
        SharpLabNext.Contracts.WorkerErrorCategory.UnsupportedCapability);
