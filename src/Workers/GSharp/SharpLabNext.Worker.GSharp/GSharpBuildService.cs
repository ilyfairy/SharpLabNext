...
public sealed class GSharpBuildService(GSharpReferenceSetProvider referenceSets, GSharpCompilerProcess compiler, GSharpWorkerSettings settings, LanguageWorkerCapabilityManifest manifest) : ILanguageWorkerBuildService
{
    public async Task<LanguageWorkerBuildExecution> BuildAsync(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            stopwatch.Stop();
            SharpLabNextTelemetry.Metrics.RecordBuild(GSharpToolchain.LanguageId, request.ToolchainId, stopwatch.Elapsed, telemetryOutcome, cacheHit: false);
        }
    }
    ...
    private async Task<LanguageWorkerBuildExecution> BuildCoreAsync(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        var workspace = GSharpWorkspaceValidator.Validate(request, manifest, toolchain);
        var referenceSet = referenceSets.Get(request.ReferenceSetId);
        var invocation = await compiler.CompileAsync(workspace, referenceSet, toolchain, cancellationToken).ConfigureAwait(false);
        ...
        if (!invocation.Succeeded)
        {
            ...
            var emitFailed = invocation.Diagnostics.Any(static diagnostic => diagnostic.Code is "GS9998" or "GS9999");
            return new LanguageWorkerBuildExecution(new BuildResult(emitFailed ? BuildOutcome.EmitFailed : BuildOutcome.CompilationFailed, null, invocation.Diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision));
        }
        ...
        {
            var diagnostics = invocation.Diagnostics.Append(CreateMissingEntryPointDiagnostic(workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision)).ToArray();
            if (request.Target == BuildTarget.CompileCheck)
            {
                return new LanguageWorkerBuildExecution(new CompilationCheckResult(false, diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision));
            }

            return new LanguageWorkerBuildExecution(new BuildResult(BuildOutcome.CompilationFailed, null, diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision));
        }

        var (outputKind, entryPoint) = ResolveOutputKind(workspace.Options.OutputKind, emittedEntryPoint);
        if (request.Target == BuildTarget.CompileCheck)
        {
        ...
        var definition = new LanguageArtifactDefinition(
            ...
            entryPoint,
            [
                new LanguageArtifactFile("primary-assembly", $"{GSharpToolchain.AssemblyName}.dll", invocation.PeImage),
                new LanguageArtifactFile("portable-pdb", $"{GSharpToolchain.AssemblyName}.pdb", invocation.PortablePdb)
            ],
    ...
    private static void ValidatePortablePdb(byte[] image)
    {
        try
        {
            using var provider = MetadataReaderProvider.FromPortablePdbStream(new MemoryStream(image, writable: false));
            _ = provider.GetMetadataReader().Documents.Count;
        }
        catch (BadImageFormatException exception)
        {
            throw new LanguageWorkerRequestException("compiler-invalid-output", "The G# compiler returned an invalid Portable PDB.", StatusCodes.Status503ServiceUnavailable, exception);
        }
    }

    private static (BuildOutputKind OutputKind, string? EntryPoint) ResolveOutputKind(BuildOutputKind requestedOutputKind, string? emittedEntryPoint)
    {
        if (requestedOutputKind == BuildOutputKind.Auto)
        {
            return emittedEntryPoint is null
                ? (BuildOutputKind.Library, null) : (BuildOutputKind.Console, emittedEntryPoint);
        }
    ...
    private static Diagnostic CreateMissingEntryPointDiagnostic(long workspaceRevision, long selectionRevision) => new("gsc", "GS9999", DiagnosticSeverity.Error, "A console output requires a managed entry point. Add a func Main(...) or use a top-level statement.", null, null, [], [], workspaceRevision, selectionRevision);
    ...
    private static string? InspectPe(byte[] image)
    {
        try
        {
            ...
            var corHeader = peReader.PEHeaders.CorHeader ?? throw new BadImageFormatException("The G# output has no CLR header.");
        ...
        {
            throw new LanguageWorkerRequestException("compiler-invalid-output", "The G# compiler returned an invalid managed PE.", StatusCodes.Status503ServiceUnavailable, exception);
        }
    }
}
