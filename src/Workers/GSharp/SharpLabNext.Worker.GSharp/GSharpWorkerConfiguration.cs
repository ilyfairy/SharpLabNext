...
public sealed record GSharpWorkerIdentity(string ReleaseId, string WorkerImageId);

public sealed record GSharpToolchainProfile(string ToolchainId, string CompilerVersion, string CompilerCommit, string CompilerAssemblyPath, string LanguageServerAssemblyPath)
{
    public BuildIdentity CreateBuildIdentity(GSharpWorkerIdentity workerIdentity, string referenceSetId) => new(workerIdentity.ReleaseId, GSharpToolchain.LanguageId, ToolchainId, CompilerVersion, CompilerCommit, referenceSetId, workerIdentity.WorkerImageId);
}

public sealed record GSharpProcessLimits(int MaximumProcessOutputBytes, long MaximumProcessWorkingSetBytes, TimeSpan SessionTtl);

public sealed record GSharpReferenceSetDefinition(string Id, string RootPath, string TargetFramework, string FrameworkVersion, string? Digest, string? AttestationPath);
    ...
    IReadOnlyList<GSharpReferenceSetDefinition> ReferenceSets)
{
    public static GSharpWorkerSettings FromConfiguration(IConfiguration configuration)
    {
        ...
        var toolchains = section.GetSection("Toolchains").GetChildren().Select(static item => new GSharpToolchainProfile(
                item.Key,
                GSharpCompilerIdentity.RequireVersion(item["CompilerVersion"], $"GSharp:Toolchains:{item.Key}:CompilerVersion"),
                GSharpCompilerIdentity.RequireCommit(item["CompilerCommit"], $"GSharp:Toolchains:{item.Key}:CompilerCommit"),
                ConfiguredPath(item["CompilerAssemblyPath"], $"GSharp:Toolchains:{item.Key}:CompilerAssemblyPath"),
                ConfiguredPath(item["LanguageServerAssemblyPath"], $"GSharp:Toolchains:{item.Key}:LanguageServerAssemblyPath"))).ToDictionary(static item => item.ToolchainId, StringComparer.Ordinal);
        if (toolchains.Count == 0)
            throw new InvalidOperationException("At least one G# toolchain profile must be configured.");

        var referenceSets = configuration.GetSection("ReferenceSets").GetChildren().Select(static item => new GSharpReferenceSetDefinition(item.Key, Required(item["Path"], $"ReferenceSets:{item.Key}:Path"), Required(item["TargetFramework"], $"ReferenceSets:{item.Key}:TargetFramework"), Required(item["FrameworkVersion"], $"ReferenceSets:{item.Key}:FrameworkVersion"), item["Digest"], item["AttestationPath"])).OrderBy(static item => item.Id, StringComparer.Ordinal).ToArray();
    ...
    public GSharpToolchainProfile GetToolchain(string toolchainId) =>
        Toolchains.TryGetValue(toolchainId, out var toolchain)
            ? toolchain : throw new InvalidOperationException($"G# toolchain profile '{toolchainId}' is not configured.");
...
{
    public static string RequireVersion(string? value, string configurationKey = "GSharp:CompilerVersion")
    {
        if (string.IsNullOrWhiteSpace(value) || !SemanticVersionPattern().IsMatch(value))
        {
            throw new InvalidOperationException($"Configuration value '{configurationKey}' must be a semantic version.");
        }
        return value;
    }

    public static string RequireCommit(string? value, string configurationKey = "GSharp:CompilerCommit")
    {
        if (string.IsNullOrWhiteSpace(value) || value.Length != 40 || value.Any(static character => !char.IsAsciiHexDigit(character)))
        {
            throw new InvalidOperationException($"Configuration value '{configurationKey}' must be a 40-character Git commit.");
        }
        return value;
    }
    ...
    [GeneratedRegex("^(0|[1-9]\\d*)\\.(0|[1-9]\\d*)\\.(0|[1-9]\\d*)(?:-(?:0|[1-9]\\d*|[0-9A-Za-z-]*[A-Za-z-][0-9A-Za-z-]*)(?:\\.(?:0|[1-9]\\d*|[0-9A-Za-z-]*[A-Za-z-][0-9A-Za-z-]*))*)?(?:\\+[0-9A-Za-z-]+(?:\\.[0-9A-Za-z-]+)*)?$", RegexOptions.CultureInvariant | RegexOptions.NonBacktracking)]
    private static partial Regex SemanticVersionPattern();
}

public sealed record LoadedGSharpReferenceSet(GSharpReferenceSetDefinition Definition, IReadOnlyList<string> ReferenceAssemblyPaths, ReferenceSetAttestation Attestation);
...
public sealed class GSharpReferenceSetProvider
{
    ...
    public GSharpReferenceSetProvider(IReadOnlyList<GSharpReferenceSetDefinition> definitions, bool requireAttestation)
    {
    ...
    public LoadedGSharpReferenceSet Get(string id) =>
        _referenceSets.TryGetValue(id, out var referenceSet)
            ? referenceSet : throw new SharpLabNext.LanguageWorker.Sdk.LanguageWorkerRequestException("unsupported-reference-set", $"G# reference set '{id}' is unavailable.", StatusCodes.Status503ServiceUnavailable);
}
