...
if (!manifest.ToolchainIds.Order(StringComparer.Ordinal).SequenceEqual(
        settings.Toolchains.Keys.Order(StringComparer.Ordinal),
...
builder.Services.AddSharpLabNextLanguageWorker<GSharpBuildService, GSharpLanguageSessionService>(
    identity,
    manifest,
