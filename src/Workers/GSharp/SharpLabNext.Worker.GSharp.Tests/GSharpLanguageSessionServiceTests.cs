...
public sealed class GSharpLanguageSessionServiceTests
{
    ...
    public async Task SessionBindsSelectedToolchainProfile(string toolchainId)
    {
        ...
        try
        {
            ...
            var service = new GSharpLanguageSessionService(
                settings,
                GSharpTestSettings.LoadManifest(),
                NullLoggerFactory.Instance);
            var build = GSharpTestSettings.CreateRequest(
            ...
            Assert.Equal(
                $"{expected.CompilerVersion}@{expected.CompilerCommit}",
                session.CompilerBuildIdentity);
