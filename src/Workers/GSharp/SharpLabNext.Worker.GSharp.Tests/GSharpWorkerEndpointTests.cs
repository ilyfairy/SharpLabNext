...
public sealed class GSharpWorkerEndpointTests
{
    ...
    public async Task WorkerPassesReusableLanguageWorkerConformanceWithRealGSharpTools()
    {
        ...
        try
        {
            ...
            var sessionWorkspace = GSharpTestSettings.CreateRequest(
                BuildTarget.CompileCheck,
                invalidSource).Workspace;
            var scenario = new LanguageWorkerConformanceScenario(
                ...
                compileCheck,
                artifact,
                new OpenLanguageSessionRequest(
                    "request-gsharp-lsp",
                    "pipeline-gsharp-lsp",
            ...
            var report = await runner.VerifyAsync(
                scenario,
                TestContext.Current.CancellationToken);
    ...
    public async Task WebSocketReturnsHoverAndSemanticTokensFromRealGSharpLanguageServer()
    {
        ...
        try
        {
            ...
            var workspace = GSharpTestSettings.CreateRequest(
                BuildTarget.CompileCheck,
                source).Workspace;
            var openRequest = new OpenLanguageSessionRequest(
                "request-gsharp-semantic-lsp",
                "pipeline-gsharp-semantic-lsp",
                GSharpToolchain.LanguageId,
                GSharpToolchain.ToolchainId,
            ...
            using (var initialized = await ReceiveUntilAsync(socket, static rootElement => HasId(rootElement, 1)))
            {
                ...
                Assert.True(
                    capabilities.GetProperty("semanticTokensProvider").GetProperty("full").ValueKind is
                        JsonValueKind.True or JsonValueKind.Object);
            }
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new
                {
                    ...
                    {
                        uri,
                        languageId = GSharpToolchain.LanguageId,
                        version = 1,
                        text = source
                    }
                }
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new
                {
                    ...
                    position = new { line = 2, character = 5 }
                }
            });
            ...
            do
            {
                ...
                if (result.MessageType == WebSocketMessageType.Close)
                    throw new InvalidOperationException(
                        $"The G# LSP WebSocket closed before the expected response: {result.CloseStatusDescription}");
