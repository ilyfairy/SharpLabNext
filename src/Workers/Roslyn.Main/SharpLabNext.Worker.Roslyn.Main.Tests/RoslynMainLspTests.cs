...
public sealed class RoslynMainLspTests
{
    ...
    public async Task WebSocketLspCompletesCSharpAndVisualBasicWithMainCompiler(string languageId, string fileName, string source, int line, int character, string expectedLabel, int? maxCompletionItems, string? expectedPrimaryNewText)
    {
        await using var factory = new RoslynMainWorkerFactory("Development", maxCompletionItems);
        using var http = factory.CreateClient();
        using var openResponse = await http.PostAsJsonAsync("/api/v1/language-sessions", CreateOpenRequest(languageId, fileName, source), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        ...
        await SendAsync(socket, new { jsonrpc = "2.0", method = "textDocument/didOpen", @params = new { textDocument = new { uri, languageId, version = 2, text = source } } });
        await SendAsync(socket, new { jsonrpc = "2.0", id = 2, method = "textDocument/completion", @params = new { textDocument = new { uri }, position = new { line, character }, context = new { triggerKind = 1, triggerCharacter = (string?)null } } });

        using var completion = await ReceiveUntilIdAsync(socket, 2);
        Assert.Contains(completion.RootElement.GetProperty("result").GetProperty("items").EnumerateArray(), item => item.GetProperty("label").GetString() == expectedLabel);
        if (expectedPrimaryNewText is not null)
        {
            var item = completion.RootElement.GetProperty("result").GetProperty("items").EnumerateArray().Single(item => item.GetProperty("label").GetString() == expectedLabel);
            var textEdit = item.GetProperty("textEdit");
            Assert.Equal(expectedPrimaryNewText, textEdit.GetProperty("newText").GetString());
            Assert.Equal(character - "task.await".Length, textEdit.GetProperty("range").GetProperty("start").GetProperty("character").GetInt32());
            Assert.Equal(character, textEdit.GetProperty("range").GetProperty("end").GetProperty("character").GetInt32());
            Assert.DoesNotContain(item.GetProperty("additionalTextEdits").EnumerateArray(), static edit => edit.GetProperty("newText").GetString()?.Contains("await", StringComparison.Ordinal) == true);
        }
    ...
    private static OpenLanguageSessionRequest CreateOpenRequest(string languageId, string fileName, string source)
    {
        ...
        var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, Revision: 5, SelectionRevision: 3, languageId, [new WorkspaceFile(fileName, 1, source)], fileName, [fileName], "net11-preview-ref", options);
        return new OpenLanguageSessionRequest($"main-lsp-{Guid.NewGuid():N}", "main-lsp-pipeline", languageId, "roslyn-main", "net11-preview-ref", workspace);
    }
    ...
    private static Task SendAsync(WebSocket socket, object message)
    {
        ...
        return socket.SendAsync(new ArraySegment<byte>(bytes), WebSocketMessageType.Text, endOfMessage: true, TestContext.Current.CancellationToken);
    }
