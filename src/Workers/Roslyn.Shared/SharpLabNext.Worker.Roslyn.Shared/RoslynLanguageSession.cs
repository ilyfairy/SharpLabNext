...
public sealed class RoslynLanguageSession : IDisposable, IAsyncDisposable
{
    ...
    internal RoslynLanguageSession(
        string sessionId,
        ...
        DateTimeOffset expiresAtUtc)
    {
        ...
        _utf8BytesByPath = validated.OrderedFiles.ToDictionary(
            static file => file.Path,
            static file => Encoding.UTF8.GetByteCount(file.Text),
    ...
    public async Task<LspDocumentState> DidOpenAsync(
        LspDidOpenTextDocumentParams parameters,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(parameters);
        if (!StringComparer.Ordinal.Equals(parameters.TextDocument.LanguageId, LanguageId))
            throw new LspInvalidParamsException($"This Roslyn session only accepts languageId '{LanguageId}'.");
        ...
        if (!path.EndsWith(extension, StringComparison.OrdinalIgnoreCase))
            throw new LspInvalidParamsException($"Document '{path}' must use the {extension} extension for this language session.");
        ...
        try
        {
            ...
            if (totalBytes > _compilationLimits.MaxTotalSourceUtf8Bytes)
                throw new LspLimitExceededException($"The session exceeds the {_compilationLimits.MaxTotalSourceUtf8Bytes} byte total source limit.");
            ...
            if (_documentsByPath.TryGetValue(path, out var existingDocumentId))
            {
                ...
                if (parameters.TextDocument.Version < currentVersion)
                    throw new LspContentModifiedException("didOpen document version is older than the current session version.");
            ...
            {
                if (_documentsByPath.Count >= _compilationLimits.MaxFiles)
                    throw new LspLimitExceededException($"The session exceeds the {_compilationLimits.MaxFiles} file limit.");
    ...
    public async Task<LspDocumentState> DidChangeAsync(
        LspDidChangeTextDocumentParams parameters,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(parameters);
        if (parameters.ContentChanges.Count == 0)
            throw new LspInvalidParamsException("didChange must contain at least one content change.");
        if (parameters.ContentChanges.Count > 100)
            throw new LspLimitExceededException("didChange contains too many incremental edits.");
        ...
        try
        {
            ...
            var currentVersion = _versionsByPath[path];
            if (parameters.TextDocument.Version <= currentVersion)
                throw new LspContentModifiedException("didChange document version must be greater than the current version.");
            ...
            foreach (var change in parameters.ContentChanges)
            {
                ...
                if (change.RangeLength is not null && change.RangeLength != span.Length)
                    throw new LspContentModifiedException("didChange rangeLength does not match the UTF-16 source span.");
    ...
    public async Task<LspDocumentState> DidCloseAsync(
        LspDidCloseTextDocumentParams parameters,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(parameters);
        var uri = ValidateUri(parameters.TextDocument.Uri);
    ...
    public Task<LspDiagnosticsReport?> GetDiagnosticsAsync(
        string uri,
        long expectedVersion,
        CancellationToken cancellationToken) =>
        _features.GetDiagnosticsAsync(uri, expectedVersion, cancellationToken);

    public Task<LspCompletionList> GetCompletionsAsync(
        LspCompletionParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetCompletionsAsync(parameters, cancellationToken);

    public Task<LspCompletionItem> ResolveCompletionAsync(
        LspCompletionItem item,
        CancellationToken cancellationToken) =>
        _features.ResolveCompletionAsync(item, cancellationToken);

    public Task<LspHover?> GetHoverAsync(
        LspTextDocumentPositionParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetHoverAsync(parameters, cancellationToken);

    public Task<LspSignatureHelp?> GetSignatureHelpAsync(
        LspSignatureHelpParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetSignatureHelpAsync(parameters, cancellationToken);

    public Task<LspSemanticTokens> GetSemanticTokensAsync(
        LspSemanticTokensParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetSemanticTokensAsync(parameters, cancellationToken);

    public Task<IReadOnlyList<LspDocumentSymbol>> GetDocumentSymbolsAsync(
        LspDocumentSymbolParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetDocumentSymbolsAsync(parameters, cancellationToken);

    public Task<IReadOnlyList<LspCodeAction>> GetCodeActionsAsync(
        LspCodeActionParams parameters,
        CancellationToken cancellationToken) =>
        _features.GetCodeActionsAsync(parameters, cancellationToken);

    public IDisposable AttachConnection()
    {
        ThrowIfUnavailable();
        if (Interlocked.CompareExchange(ref _connectionAttached, 1, 0) != 0)
            throw new LspSessionUnavailableException("The language session already has an active LSP connection.");
        return new ConnectionLease(this);
    }

    internal async Task<LspDocumentSnapshot> GetDocumentSnapshotAsync(
        string uri,
        CancellationToken cancellationToken)
    {
        uri = ValidateUri(uri);
    ...
    private async Task ApplyDocumentTextAsync(
        DocumentId documentId,
        SourceText text,
        CancellationToken cancellationToken)
    {
        var solution = _workspace.CurrentSolution.WithDocumentText(
            documentId,
            text,
    ...
    private void ApplySolution(Solution solution)
    {
        if (!_workspace.TryApplyChanges(solution))
            throw new InvalidOperationException("Roslyn rejected a language session workspace update.");
    }

    private async Task<Solution> WithAutomaticCSharpOutputKindAsync(
        Solution solution,
        CancellationToken cancellationToken)
    {
        if (!_automaticallySelectCSharpOutputKind)
            return solution;

        var project = solution.GetProject(_projectId)
            ?? throw new InvalidOperationException("The Roslyn language session project no longer exists.");
        var syntaxTrees = new List<SyntaxTree>(project.DocumentIds.Count);
        foreach (var document in project.Documents)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var syntaxTree = await document.GetSyntaxTreeAsync(cancellationToken).ConfigureAwait(false)
                ?? throw new InvalidOperationException("Roslyn did not produce a C# syntax tree for the language session document.");
            syntaxTrees.Add(syntaxTree);
        }

        var resolvedOutputKind = CSharpBuildService.ResolveOutputKind(
            BuildOutputKind.Auto,
            syntaxTrees,
            cancellationToken);
        var roslynOutputKind = CSharpBuildService.ToRoslynOutputKind(resolvedOutputKind);
        if (project.CompilationOptions?.OutputKind == roslynOutputKind)
            return solution;

        var compilationOptions = project.CompilationOptions as CSharpCompilationOptions
            ?? throw new InvalidOperationException("The C# language session does not have C# compilation options.");
        return project.Solution.WithProjectCompilationOptions(
            _projectId,
            compilationOptions.WithOutputKind(roslynOutputKind));
    }

    private DocumentId GetDocumentId(string path) =>
        _documentsByPath.TryGetValue(path, out var documentId)
            ? documentId
            : throw new LspInvalidParamsException($"Document '{path}' is not part of this language session.");

    private Document GetRequiredDocument(DocumentId documentId) =>
        _workspace.CurrentSolution.GetDocument(documentId)
            ?? throw new InvalidOperationException("The Roslyn language session document no longer exists.");

    private LspDocumentState CreateState(string uri, string path) =>
        new(uri, path, _versionsByPath[path], _workspaceRevision, _selectionRevision);
    ...
    private void ValidateDocumentSize(string path, SourceText text)
    {
        ...
        if (bytes > _compilationLimits.MaxFileUtf8Bytes)
            throw new LspLimitExceededException($"Document '{path}' exceeds the {_compilationLimits.MaxFileUtf8Bytes} byte source limit.");
    }

    private void ThrowIfUnavailable()
    {
        if (_disposed || IsExpired)
            throw new LspSessionUnavailableException("The language session has expired or was closed.");
    }
    ...
    internal static int ToPosition(SourceText text, LspPosition position)
    {
        if (position.Line < 0 || position.Line >= text.Lines.Count)
            throw new LspInvalidParamsException("LSP line is outside the document.");
        var line = text.Lines[position.Line];
        if (position.Character < 0 || position.Character > line.Span.Length)
            throw new LspInvalidParamsException("LSP UTF-16 character is outside the line.");
    ...
    private static TextSpan ToTextSpan(SourceText text, LspRange range) =>
        TextSpan.FromBounds(ToPosition(text, range.Start), ToPosition(text, range.End));
    ...
    private sealed class ConnectionLease(RoslynLanguageSession session) : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            if (owner is not null)
                Interlocked.Exchange(ref owner._connectionAttached, 0);
        }
    }
}

internal sealed record LspDocumentSnapshot(
    string Uri,
    string Path,
    Document Document,
    SourceText Text,
