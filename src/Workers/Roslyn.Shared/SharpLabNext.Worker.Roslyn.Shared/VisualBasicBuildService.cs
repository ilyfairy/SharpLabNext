...
public sealed class VisualBasicBuildService(
    ...
    AstLimits astLimits)
{
    ...
    internal static VisualBasicParseOptions CreateParseOptions(BuildOptions options)
    {
        ...
        if (!string.IsNullOrWhiteSpace(options.LanguageVersion) &&
            !LanguageVersionFacts.TryParse(options.LanguageVersion, ref languageVersion))
        {
            throw new BuildRequestValidationException(
                $"Visual Basic language version '{options.LanguageVersion}' is not supported by this Roslyn worker.");
        }

        var symbols = (options.PreprocessorSymbols ?? [])
            .Select(static symbol => new KeyValuePair<string, object>(symbol, true));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var syntaxTrees = workspace.OrderedFiles
            .Select(file => (SyntaxTree)VisualBasicSyntaxTree.ParseText(
        ...
        var compilation = VisualBasicCompilation.Create(
            ...
            CreateCompilationOptions(workspace.Options));
        ...
        if (!emitResult.Success)
        {
            ...
            return new WorkerBuildExecution(
                new BuildResult(
                    compilationHadErrors ? BuildOutcome.CompilationFailed : BuildOutcome.EmitFailed,
                    null,
                    diagnostics,
                    buildIdentity,
                    workspace.Snapshot.Revision,
        ...
        var artifact = CSharpBuildService.CreateArtifact(
            peImage,
            portablePdb,
            referenceSet.Definition,
            buildIdentity,
            outputKind,
            compilation.GetEntryPoint(cancellationToken)?.ToDisplayString(SymbolDisplayFormat.VisualBasicErrorMessageFormat));
        return new WorkerBuildExecution(
            new BuildResult(
...
public sealed class RoslynBuildService(
    CSharpBuildService csharp,
    VisualBasicBuildService visualBasic,
    RoslynWorkerIdentity identity)
{
    public Task<WorkerBuildExecution> ExecuteAsync(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BuildRequestValidationException(
                $"This Roslyn worker does not support languageId '{request.Workspace.LanguageId}'.");
        }
