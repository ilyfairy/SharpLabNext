...
public sealed class RoslynArtifactPublisher(IArtifactStoreClient artifactStore, ArtifactBundlePublishingOptions options)
{
    public async Task<ArtifactRef> PublishAsync(CompiledArtifact artifact, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(artifact);
        ValidateArtifact(artifact);

        var fileContents = GetFileContents(artifact).Select(static pair => new ArtifactBundleContent(pair.Key, pair.Value)).ToArray();
        var stored = await new ArtifactBundlePublisher(artifactStore).PublishAsync(artifact.Manifest, fileContents, options.TimeToLive, cancellationToken).ConfigureAwait(false);
    ...
    internal static void ValidateArtifact(CompiledArtifact artifact)
    {
        ...
        if (artifact.ArtifactRef != artifact.Manifest.ArtifactId || artifact.ArtifactRef != ArtifactIdentity.Compute(artifact.Manifest))
        {
            throw new InvalidOperationException("The compiled artifact identity does not match its canonical manifest.");
        }
        if (!string.Equals(artifact.ArtifactFormat, artifact.Manifest.ArtifactFormat, StringComparison.Ordinal) || !string.Equals(artifact.ReferenceSetId, artifact.Manifest.ReferenceSetId, StringComparison.Ordinal) || !string.Equals(artifact.TargetFramework, artifact.Manifest.TargetFramework, StringComparison.Ordinal) || !artifact.Files.SequenceEqual(artifact.Manifest.Files))
        {
            throw new InvalidOperationException("The compiled artifact metadata conflicts with its manifest.");
        }
    }
    ...
    private static Dictionary<string, byte[]> GetFileContents(CompiledArtifact artifact)
    {
        ...
        if (!string.Equals(primary.Path, artifact.Manifest.EntryAssembly, StringComparison.Ordinal) || artifact.Manifest.Files.Count != (portablePdb is null ? 1 : 2))
        {
            throw new InvalidOperationException("The compiled Roslyn artifact contains an unsupported file layout.");
        }
        if ((portablePdb is null) != (artifact.PortablePdb.Length == 0))
        {
            throw new InvalidOperationException("The compiled Roslyn artifact portable PDB does not match its manifest.");
        }
        ...
        foreach (var file in artifact.Manifest.Files)
        {
            ...
            {
                throw new InvalidOperationException($"The compiled artifact file '{file.Path}' failed size or checksum validation.");
            }
        }
    ...
    private static ArtifactFileDescriptor? SingleFile(IReadOnlyList<ArtifactFileDescriptor> files, string role, bool required)
    {
