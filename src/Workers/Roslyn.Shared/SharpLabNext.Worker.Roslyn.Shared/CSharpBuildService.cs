...
public sealed class CSharpBuildService(ReferenceSetProvider referenceSets, RoslynWorkerIdentity identity, CompilationLimits compilationLimits, AstLimits astLimits)
{
    ...
    {
        RoslynCompilerIdentity.Ensure(identity, "C# compiler", GetLoadedCompilerVersion(), GetLoadedCompilerCommit());
    ...
    private async Task<WorkerBuildExecution> ExecuteCoreAsync(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        var syntaxTrees = workspace.OrderedFiles.Select(file => (SyntaxTree)CSharpSyntaxTree.ParseText(SourceText.From(file.Text, Encoding.UTF8, SourceHashAlgorithm.Sha256), parseOptions, file.Path, cancellationToken)).ToArray();
        ...
        var outputKind = ResolveOutputKind(workspace.Options.OutputKind, syntaxTrees, cancellationToken);
        var compilation = CSharpCompilation.Create(AssemblyName, syntaxTrees, referenceSet.References, CreateCompilationOptions(workspace.Options, outputKind));
        ...
        if (!emitResult.Success)
        {
            ...
            return new WorkerBuildExecution(new BuildResult(outcome, null, diagnostics, buildIdentity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision), null);
        }

        var peImage = peStream.ToArray();
        var portablePdb = pdbStream?.ToArray() ?? [];
        var artifact = CreateArtifact(peImage, portablePdb, referenceSet.Definition, buildIdentity, outputKind, compilation.GetEntryPoint(cancellationToken)?.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat));
    ...
    internal static CSharpParseOptions CreateParseOptions(BuildOptions options)
    {
        ...
        if (!string.IsNullOrWhiteSpace(options.LanguageVersion) && !LanguageVersionFacts.TryParse(options.LanguageVersion, out languageVersion))
        {
            throw new BuildRequestValidationException($"C# language version '{options.LanguageVersion}' is not supported by this Roslyn worker.");
        }

        return new CSharpParseOptions(languageVersion, DocumentationMode.Parse, SourceCodeKind.Regular, options.PreprocessorSymbols ?? []);
    }

    internal static ContractOutputKind ResolveOutputKind(ContractOutputKind requestedOutputKind, IEnumerable<SyntaxTree> syntaxTrees, CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (syntaxTree.GetRoot(cancellationToken) is CompilationUnitSyntax compilationUnit && compilationUnit.Members.Any(static member => member is GlobalStatementSyntax))
            {
    ...
    internal static CSharpCompilationOptions CreateCompilationOptions(BuildOptions options, ContractOutputKind resolvedOutputKind) =>
        new(
    ...
    internal static RoslynOutputKind ToRoslynOutputKind(ContractOutputKind resolvedOutputKind) =>
        resolvedOutputKind switch
        {
            ...
            _ => throw new BuildRequestValidationException($"Resolved output kind '{resolvedOutputKind}' is not supported.")
        };

    private BuildIdentity CreateBuildIdentity(string referenceSetId) =>
        new(identity.ReleaseId, "csharp", identity.ToolchainId, identity.CompilerVersion, GetLoadedCompilerCommit(), referenceSetId, identity.WorkerImageId);

    internal static CompiledArtifact CreateArtifact(byte[] peImage, byte[] portablePdb, ReferenceSetDefinition referenceSet, BuildIdentity buildIdentity, ContractOutputKind outputKind, string? entryPoint)
    {
        _ = ToRoslynOutputKind(outputKind);
        var entryFileExtension = outputKind == ContractOutputKind.Library
            ? referenceSet.LibraryFileExtension : referenceSet.ExecutableFileExtension;
        var entryAssembly = $"{AssemblyName}{entryFileExtension}";
        var peDigest = ContentIdentity.Compute(peImage).Value;
        var files = new List<ArtifactFileDescriptor> { new("primary-assembly", entryAssembly, peImage.LongLength, peDigest) };
        if (portablePdb.Length > 0)
        {
            files.Add(new ArtifactFileDescriptor("portable-pdb", $"{AssemblyName}.pdb", portablePdb.LongLength, ContentIdentity.Compute(portablePdb).Value));
        }
        ...
        var manifest = ArtifactIdentity.WithComputedId(new ArtifactManifest(
            ...
            new ArtifactRuntimeRequirement(referenceSet.RuntimeFamily, [new FrameworkRequirement(referenceSet.FrameworkName, referenceSet.GetRuntimeFrameworkVersion())], referenceSet.Architecture, referenceSet.RequiredRuntimeFeatureTags),
        ...
        return new CompiledArtifact(artifactRef, referenceSet.ArtifactFormat, AssemblyName, referenceSet.Id, referenceSet.TargetFramework, peImage, portablePdb, manifest, files, buildIdentity);
    }
