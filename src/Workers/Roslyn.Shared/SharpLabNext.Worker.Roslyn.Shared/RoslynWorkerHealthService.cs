...
{
    public static WorkerProcessIdentity Create(string toolchainId) =>
        new($"{toolchainId}-{Guid.NewGuid():N}", DateTimeOffset.UtcNow);
}
...
internal static class RoslynCompilerIdentity
{
    ...
    public static string? GetCommit(Assembly assembly) => assembly
        .GetCustomAttributesData()
        .Where(static attribute => attribute.AttributeType.FullName == CommitHashAttributeName)
    ...
    public static bool Matches(
        RoslynWorkerIdentity expected,
        string loadedVersion,
        string? loadedCommit) =>
        StringComparer.Ordinal.Equals(loadedVersion, expected.CompilerVersion) &&
        (string.IsNullOrWhiteSpace(expected.CompilerCommit) ||
         StringComparer.OrdinalIgnoreCase.Equals(loadedCommit, expected.CompilerCommit));
        ...
        string? loadedCommit)
    {
        ...
        throw new CompilerIdentityMismatchException(
            $"Configured Roslyn {expected.CompilerVersion} ({expected.CompilerCommit ?? "any commit"}) does not match loaded {compilerName} {loadedVersion} ({loadedCommit ?? "commit unavailable"}).");
    }
}

public sealed class RoslynWorkerHealthService(
    ...
    WorkerProcessIdentity processIdentity)
{
    public async Task<HealthResponse> CheckAsync(CancellationToken cancellationToken)
    {
        ...
        var compilerHealthy = compilerIdentities.Count > 0 && compilerIdentities.All(loaded =>
            RoslynCompilerIdentity.Matches(identity, loaded.Version, loaded.Commit));
        var loadedDescription = string.Join(
            ", ",
            compilerIdentities.Select(static loaded =>
                $"{loaded.Language} {loaded.Version} ({loaded.Commit ?? "commit unavailable"})"));
        checks.Add(new HealthCheckResult(
            "roslyn-compiler-identity",
            compilerHealthy ? HealthStatus.Healthy : HealthStatus.Unhealthy,
            compilerHealthy
                ? $"Roslyn compiler identity is loaded: {loadedDescription}."
                : $"Configured Roslyn {identity.CompilerVersion} ({identity.CompilerCommit ?? "any commit"}) does not match loaded compiler identities: {loadedDescription}.",
        ...
        return new WorkerDescriptor(
            ...
            capabilities
                .Select(capability => new WorkerCapabilityDescriptor(
                    ...
                    Available: available,
                    profileIds,
                    available ? null : "Worker preflight failed."))
                .ToArray(),
            profileIds,
