...
{
    public static IReadOnlyList<ContractDiagnostic> Convert(
        ...
        int maxDiagnostics)
    {
        ...
        var converted = diagnostics
            .Select(diagnostic => Convert(diagnostic, workspaceRevision, selectionRevision))
            .DistinctBy(static diagnostic => new DiagnosticKey(
                diagnostic.Code,
                diagnostic.Severity,
                diagnostic.Message,
                diagnostic.FilePath,
                diagnostic.Range))
            .OrderBy(static diagnostic => diagnostic.FilePath, StringComparer.Ordinal)
            .ThenBy(static diagnostic => diagnostic.Range?.StartLine ?? -1)
            .ThenBy(static diagnostic => diagnostic.Range?.StartCharacter ?? -1)
        ...
        limited.Add(new ContractDiagnostic(
            "sharplabnext",
            "SLN1001",
            ContractDiagnosticSeverity.Warning,
            $"Diagnostic output was truncated at {maxDiagnostics} entries.",
            null,
            null,
            [],
            [],
    ...
    private static ContractDiagnostic Convert(
    ...
    private static (string? FilePath, TextRange? Range) ConvertLocation(Location location)
    {
        ...
        return (
            string.IsNullOrWhiteSpace(span.Path) ? null : span.Path.Replace('\\', '/'),
            new TextRange(
