...
public sealed record JSharpWorkerIdentity(string ReleaseId, string CompilerVersion, string? CompilerCommit, string WorkerImageId)
{
    public SharpLabNext.Contracts.BuildIdentity CreateBuildIdentity() => new(ReleaseId, JSharpToolchain.LanguageId, JSharpToolchain.ToolchainId, CompilerVersion, CompilerCommit, JSharpToolchain.ReferenceSetId, WorkerImageId);
}

public sealed record JSharpProcessLimits(int MaximumProcessOutputBytes, long MaximumProcessWorkingSetBytes, int MaximumDiagnostics, int MemoryPollIntervalMilliseconds);

public sealed record JSharpReferenceSetIdentity(string Digest, string ContentDigest, string SourceUri)
{
    public SharpLabNext.Contracts.ReferenceSetAttestation CreateAttestation() => new(JSharpToolchain.ReferenceSetId, JSharpToolchain.TargetFramework, Digest, ContentDigest, new SharpLabNext.Contracts.ReferenceSetProvenance("operator-image", JSharpToolchain.FrameworkVersion, SourceUri: SourceUri, SourceArchiveDigest: Digest));
}

public sealed record JSharpWorkerSettings(JSharpWorkerIdentity Identity, JSharpReferenceSetIdentity ReferenceSet, JSharpProcessLimits ProcessLimits, string WorkRoot, string CompilerHostPath, string CompilerPath)
{
    public static JSharpWorkerSettings FromConfiguration(IConfiguration configuration)
    {
        ...
        var compilerHostPath = RequiredAbsolutePath(section["CompilerHostPath"] ?? "/usr/bin/wine-stable", "JSharp:CompilerHostPath");
        var compilerPath = RequiredAbsolutePath(section["CompilerPath"] ?? "/opt/sharplabnext/jsharp20/vjc.exe", "JSharp:CompilerPath");

        var maximumProcessOutputBytes = BoundedInt(section["MaximumProcessOutputBytes"], 1024 * 1024, 4 * 1024, 16 * 1024 * 1024, "JSharp:MaximumProcessOutputBytes");
        var maximumProcessWorkingSetBytes = BoundedLong(section["MaximumProcessWorkingSetBytes"], 512L * 1024 * 1024, 64L * 1024 * 1024, 8L * 1024 * 1024 * 1024, "JSharp:MaximumProcessWorkingSetBytes");
        var maximumDiagnostics = BoundedInt(section["MaximumDiagnostics"], 100, 1, 1_000, "JSharp:MaximumDiagnostics");
        var memoryPollIntervalMilliseconds = BoundedInt(section["MemoryPollIntervalMilliseconds"], 25, 10, 1_000, "JSharp:MemoryPollIntervalMilliseconds");
    ...
    private static string? OptionalCommit(string? value)
    {
        ...
        {
            throw new InvalidOperationException("Configuration value 'JSharp:CompilerCommit' must be empty or a 40-character Git commit.");
        }
        return value;
    }
    ...
    private static InvalidOperationException InvalidDigest(string key) => new($"Configuration value '{key}' must be a lowercase SHA-256 digest.");
    ...
    private static string RequiredSourceUri(string? value)
    {
        ...
        {
            throw new InvalidOperationException("Configuration value 'JSharp:ReferenceSetSourceUri' must be an absolute source URI.");
        }
        return value;
    }
