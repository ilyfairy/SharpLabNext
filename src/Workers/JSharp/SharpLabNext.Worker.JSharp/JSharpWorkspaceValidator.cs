...
internal sealed record ValidatedJSharpWorkspace(
    WorkspaceSnapshot Snapshot,
    ValidatedJSharpWorkspaceFile SourceFile,
...
{
    public static ValidatedJSharpWorkspace Validate(
        ...
        string compilerVersion)
    {
        ...
        if (!StringComparer.Ordinal.Equals(request.ToolchainId, JSharpToolchain.ToolchainId) ||
            !StringComparer.Ordinal.Equals(request.Workspace.LanguageId, JSharpToolchain.LanguageId))
        {
            throw Invalid("wrong-toolchain", "The request does not target the J# worker.");
        }
        if (!StringComparer.Ordinal.Equals(request.ReferenceSetId, JSharpToolchain.ReferenceSetId) ||
            !StringComparer.Ordinal.Equals(request.Workspace.ReferenceSetId, JSharpToolchain.ReferenceSetId))
        {
        ...
        if (Encoding.UTF8.GetByteCount(source.Text) > manifest.Limits.MaximumSourceUtf8Bytes)
        {
            throw Invalid(
                "workspace-too-large",
                "The J# source exceeds the configured limit.",
        ...
        return new ValidatedJSharpWorkspace(
            workspace,
            new ValidatedJSharpWorkspaceFile(path, source.Version, source.Text),
    ...
    private static void ValidateOptions(BuildOptions options, string compilerVersion)
    {
            ...
            !StringComparer.Ordinal.Equals(options.LanguageVersion, compilerVersion))
        {
            throw Invalid(
                "unsupported-option",
                $"J# compiler version '{options.LanguageVersion}' is not supported.");
        }
