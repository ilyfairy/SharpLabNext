...
internal sealed class JsilArtifactHandler(
    IJsilArtifactMaterializer materializer,
    IJsilProcessRunner runner,
    IArtifactStoreClient storeClient,
    JsilWorkerSettings settings,
    ArtifactWorkerCapabilityManifest capabilityManifest) : IArtifactRenderHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var translation = await runner.TranslateAsync(
            artifact,
            maximumCharacters,
            request.DeadlineUtc,
            cancellationToken).ConfigureAwait(false);
        if (!translation.Succeeded || translation.JavaScript is null)
        {
            return new ArtifactWorkerJobExecution(new RenderArtifactResult(
                ...
                [],
                [Diagnostic(
                    "jsil-translation-unsupported",
                    translation.PublicMessage ?? "JSIL could not translate this managed assembly.",
                    translation.Detail)],
                ProcessorIdentity()));
        }
        ...
        if (stored.ContentRef != contentRef || stored.Size != bytes.LongLength)
            throw new ArtifactWorkerDependencyUnavailableException(
                "Artifact Store returned an unexpected JavaScript content identity.");
        ...
        return new ArtifactWorkerJobExecution(
            new RenderArtifactResult(
                ...
                [],
                ProcessorIdentity()),
            new ArtifactWorkerProducedContent(stored.ContentRef, mediaType, stored.Size));
    }

    private ArtifactProcessorIdentity ProcessorIdentity() => new(
        settings.ReleaseId,
        capabilityManifest.WorkerId,
        $"{settings.Version}+{settings.Commit[..12]}",
