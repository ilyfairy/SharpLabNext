...
{
    public static JsilWorkerSettings FromConfiguration(
        ...
        ArtifactWorkerCapabilityManifest manifest)
    {
        ...
        var referenceSets = configuration.GetSection("ReferenceSets")
            .GetChildren()
            .Select(item => new JsilReferenceSet(
                item.Key,
                Required(item["TargetFramework"], $"ReferenceSets:{item.Key}:TargetFramework"),
                Path.GetFullPath(Required(item["Path"], $"ReferenceSets:{item.Key}:Path"))))
            .ToDictionary(static item => item.Id, StringComparer.Ordinal);
        if (referenceSets.Count == 0)
            throw new InvalidOperationException("At least one JSIL reference set must be configured.");

        var maximumProcessOutputBytes = PositiveInt(
            section["MaximumProcessOutputBytes"],
            64 * 1024,
            "Jsil:MaximumProcessOutputBytes");
        if (maximumProcessOutputBytes > manifest.Limits.MaximumOutputArtifactBytes)
            throw new InvalidOperationException("JSIL process output capture exceeds the artifact output limit.");
    ...
    public JsilReferenceSet GetReferenceSet(string id, string targetFramework)
    {
        if (!ReferenceSets.TryGetValue(id, out var referenceSet) ||
            !StringComparer.Ordinal.Equals(referenceSet.TargetFramework, targetFramework))
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "JSIL does not support this artifact reference set or target framework.");
        }
        return referenceSet;
    }
