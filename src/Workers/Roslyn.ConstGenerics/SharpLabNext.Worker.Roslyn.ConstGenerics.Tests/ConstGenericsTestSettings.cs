...
internal static class ConstGenericsTestSettings
{
    ...
    public static string ExpectedCompilerCommit => RequiredSourceBuildEnvironment(
        "RoslynWorker__CompilerCommit");

    public static string ReferenceVersion => IsSourceBuild
        ? RequiredSourceBuildEnvironment("ReferenceSets__const-generics-ref__FrameworkVersion")
        : "0.0.0-test";
    ...
    public static RoslynWorkerIdentity CreateIdentity() => new(
        "development",
        "roslyn-const-generics",
        CompilerVersion,
        IsSourceBuild ? ExpectedCompilerCommit : null,
        "test-worker-image")
    {
    ...
    public static string GetReferencePath()
    {
        ...
        foreach (var root in roots.Where(static root => !string.IsNullOrWhiteSpace(root)))
        {
            ...
            var candidate = Directory.EnumerateDirectories(packs)
        ...
        throw new InvalidOperationException(
            "The ConstGenerics reference set was not found. Set SHARPLABNEXT_CONST_GENERICS_REF_PATH explicitly.");
    }
    ...
    private static string RequiredSourceBuildEnvironment(string name) =>
        Environment.GetEnvironmentVariable(name) is { Length: > 0 } value
            ? value
            : throw new InvalidOperationException(
                $"Source-build tests require the lock-derived environment value '{name}'.");
}
