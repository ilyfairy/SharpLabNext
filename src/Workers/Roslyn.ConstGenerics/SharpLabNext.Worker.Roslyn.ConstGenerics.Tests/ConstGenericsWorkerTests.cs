...
public sealed class ConstGenericsWorkerTests
{
    ...
    public async Task ArtifactBuildPublishesAtomicRuntimeAndMetadataRequirements()
    {
        var execution = await CreateCSharpService().ExecuteAsync(
            CreateRequest(BuildTarget.Artifact, FeatureSource),
    ...
    public async Task CompileCheckAndAstUseTheForkParserContract()
    {
        ...
        var check = await service.ExecuteAsync(
            CreateRequest(BuildTarget.CompileCheck, FeatureSource),
            TestContext.Current.CancellationToken);
        var checkResult = Assert.IsType<CompilationCheckResult>(check.Result);
        Assert.True(checkResult.CompilationSucceeded);
        Assert.DoesNotContain(checkResult.Diagnostics, static diagnostic =>
            diagnostic.Severity == SharpLabNext.Contracts.DiagnosticSeverity.Error);

        var ast = await service.ExecuteAsync(
            CreateRequest(BuildTarget.Ast, FeatureSource),
            TestContext.Current.CancellationToken);
        ...
        {
            Assert.Contains(
                Flatten(astResult.Document.Root),
                static node => node.Kind.Contains("LiteralTypeArgument", StringComparison.Ordinal));
        }
    }
    ...
    public void SourceBuildExposesTheLockedForkSurfaceAndIdentity()
    {
        ...
        Assert.NotNull(typeof(CSharpCompilation).Assembly.GetType(
            "Microsoft.CodeAnalysis.CSharp.Syntax.LiteralTypeArgumentSyntax"));
    ...
    {
        await using var manager = new RoslynLanguageSessionManager(
            CreateReferenceSets(),
            ConstGenericsTestSettings.CreateIdentity(),
    ...
    public async Task WorkerRejectsVisualBasicBeforeCompilation()
    {
        ...
        var router = new RoslynBuildService(
            new CSharpBuildService(references, identity, CompilationLimits.Default, AstLimits.Default),
            new VisualBasicBuildService(references, identity, CompilationLimits.Default, AstLimits.Default),
            identity);
        ...
        var workspace = new WorkspaceSnapshot(
            ...
            1,
            "visual-basic",
            [new WorkspaceFile("Program.vb", 1, "Module Program\nEnd Module")],
            "Program.vb",
            ["Program.vb"],
            "const-generics-ref",
            options);
        ...
        var exception = await Assert.ThrowsAsync<BuildRequestValidationException>(
            () => router.ExecuteAsync(request, TestContext.Current.CancellationToken));
    ...
    private static ReferenceSetProvider CreateReferenceSets() => new(
        [new ReferenceSetDefinition(
            "const-generics-ref",
            ConstGenericsTestSettings.GetReferencePath(),
            "net9.0",
            ConstGenericsTestSettings.ReferenceVersion)
        {
    ...
    private static BuildOptions CreateOptions() => new(
