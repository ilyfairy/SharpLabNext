...
public sealed class DerivedArtifactPublisherTests
{
    ...
    public async Task InstrumentationPublisherCreatesContentAddressedDerivedArtifact()
    {
        ...
        try
        {
            ...
            await using var artifact = new MaterializedArtifact(
                input,
                assemblyPath,
                pdbPath,
                sourceManifest,
            ...
            var processor = await new ArtifactProcessorProcessRunner(settings).RunAsync(
                artifact,
            ...
            var published = await new DerivedArtifactPublisher(store, settings)
                .PublishRuntimeInstrumentationAsync(
                    artifact,
                    processor,
                    new TransformArtifactOptions(RewriterProfileId: ProcessorProtocol.RuntimeInstrumentationProfileId),
                    TestContext.Current.CancellationToken);
        ...
        return ArtifactIdentity.WithComputedId(new ArtifactManifest(
            ...
            [
                new ArtifactFileDescriptor(
                    ...
                    "app.dll",
                    assembly.LongLength,
                    ContentIdentity.Compute(assembly).Value),
                new ArtifactFileDescriptor(
                    "portable-pdb",
                    "app.pdb",
                    pdb.LongLength,
                    ContentIdentity.Compute(pdb).Value)
            ]));
    }
    ...
    private sealed class RecordingArtifactStoreClient : IArtifactStoreClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return new PutArtifactResponse(
                manifest.ArtifactId,
                UploadedFiles.Values.Sum(static value => value.LongLength),
