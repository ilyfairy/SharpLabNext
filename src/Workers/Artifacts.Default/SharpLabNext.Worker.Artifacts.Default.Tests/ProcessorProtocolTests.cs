...
public sealed class ProcessorProtocolTests
{
    ...
    {
        Assert.Equal(RuntimeCapabilityProbeContract.ExecutionFlowProcessorVersion, ProcessorProtocol.RuntimeInstrumentationVersion);
    }

    [Fact]
    public void EnumValuesUseKebabCaseWireNames()
    {
        var json = JsonSerializer.Serialize(new ProcessorResponse(ProcessorProtocol.Version, ProcessorOutcome.InvalidArtifact, "processor", "1.0.0", "text/plain", 0, [], [], false, null), ProcessorProtocol.JsonOptions);
