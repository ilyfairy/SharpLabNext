...
public sealed class ArtifactWorkerSettingsTests
{
    ...
    {
        var settingsPath = Path.Combine(FindRepositoryRoot(), "src", "Workers", "Artifacts.Default", "SharpLabNext.Worker.Artifacts.Default", "appsettings.json");
        using var stream = File.OpenRead(settingsPath);
        var configuration = new ConfigurationBuilder().AddJsonStream(stream).Build();

        var settings = ArtifactWorkerSettings.FromConfiguration(configuration);
        Assert.Equal(ArtifactReferenceSetConfigurationContract.RequiredSystemModules.Keys.Order(StringComparer.Ordinal), settings.ReferenceSets.Keys.Order(StringComparer.Ordinal));
        foreach (var contract in NetFxManagedReferenceSets.ById.Values)
        {
            ...
            Assert.Equal([Path.GetFullPath($"/reference-sets/{contract.ReferenceSetId}")], referenceSet.Paths);
        }

        Assert.Equal("/reference-sets/jsharp20-ref", configuration["ArtifactWorker:ReferenceSets:jsharp20-ref:Paths:0"]);
        var jsharpReferenceSet = settings.ReferenceSets["jsharp20-ref"];
        Assert.Equal("mscorlib", jsharpReferenceSet.SystemModuleName);
        Assert.Equal([Path.GetFullPath("/reference-sets/jsharp20-ref")], jsharpReferenceSet.Paths);
    }

    [Fact]
    public void MissingOrUnknownReferenceSetIsRejected()
    {
        var values = ArtifactReferenceSetConfigurationContract.RequiredSystemModules.SelectMany(pair => new Dictionary<string, string?>
            {
        ...
        var exception = Assert.Throws<InvalidOperationException>(() => ArtifactWorkerSettings.FromConfiguration(configuration));
