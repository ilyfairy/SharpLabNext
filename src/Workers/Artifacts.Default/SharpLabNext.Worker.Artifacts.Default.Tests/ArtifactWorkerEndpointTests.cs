...
public sealed class ArtifactWorkerEndpointTests
{
    ...
    public async Task StartPollEventsAndIdempotencyUseTheOperationContract()
    {
        ...
        try
        {
            ...
            var events = await client.GetFromJsonAsync<OperationEvent[]>(
                $"/api/v1/operations/{firstHandle.OperationId}/events?FromSequence=0",
                JsonOptions,
            ...
            var typed = Assert.IsType<TypedResultOperationEventPayload>(
                Assert.Single(events, static item => item.Payload is TypedResultOperationEventPayload).Payload);
    ...
    public async Task CancelEndpointCancelsAWorkingOperation()
    {
        ...
        try
        {
            ...
            using var cancelled = await client.PostAsJsonAsync(
                ...
                new CancelOperationRequest(handle.OperationId, "test cancellation"),
                JsonOptions,
                TestContext.Current.CancellationToken);
            ...
            var events = await client.GetFromJsonAsync<OperationEvent[]>(
                $"/api/v1/operations/{handle.OperationId}/events?FromSequence=0",
                JsonOptions,
                TestContext.Current.CancellationToken);
            Assert.NotNull(events);
            OperationEventStreamContract.Validate(events);
    ...
    private static WebApplicationFactory<ArtifactWorkerHost::Program> CreateFactory(
        string root,
    ...
    private static RenderArtifactRequest Request(string requestId, string idempotencyKey) => new(
        requestId,
        idempotencyKey,
        "pipeline-test",
    ...
    private static async Task<T> ReadAsync<T>(HttpResponseMessage response) =>
        await response.Content.ReadFromJsonAsync<T>(JsonOptions, TestContext.Current.CancellationToken)
        ?? throw new InvalidOperationException("The endpoint returned an empty response.");
    ...
    private static async Task<OperationState> WaitForTerminalAsync(HttpClient client, string operationId)
    {
        for (var attempt = 0; attempt < 100; attempt++)
        {
            var state = await client.GetFromJsonAsync<OperationState>(
                ...
                JsonOptions,
                TestContext.Current.CancellationToken)
                ?? throw new InvalidOperationException("The operation endpoint returned an empty response.");
    ...
    private sealed class EndpointExecutor : IArtifactJobExecutor
    {
        ...
        public Task<ArtifactJobExecution> TransformAsync(
            TransformArtifactRequest request,
            string operationId,
            CancellationToken cancellationToken) =>
            Task.FromResult(new ArtifactJobExecution(new TransformArtifactResult(
            ...
            CancellationToken cancellationToken)
        {
            ...
            return new ArtifactJobExecution(
                new RenderArtifactResult(
                    ...
                    [],
                    []),
                new ProducedContent(ContentRef, "text/plain; charset=utf-8", 4));
        }

        public Task<ArtifactJobExecution> VerifyAsync(
            VerifyArtifactRequest request,
            string operationId,
            CancellationToken cancellationToken) =>
            Task.FromResult(new ArtifactJobExecution(new VerifyArtifactResult(
