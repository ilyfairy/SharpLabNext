...
public sealed record CppCliWorkerIdentity(
    string ReleaseId,
...
public sealed record CppCliProcessLimits(
    int MaximumProcessOutputBytes,
    long MaximumProcessWorkingSetBytes,
    int MaximumDiagnostics);

public sealed record CppCliReferenceSetIdentity(
    string Digest,
    ...
    string SourceUri)
{
    public SharpLabNext.Contracts.ReferenceSetAttestation CreateAttestation() => new(
        ...
        new SharpLabNext.Contracts.ReferenceSetProvenance(
            "operator-image",
            CppCliToolchain.FrameworkVersion,
            SourceUri: SourceUri));
}

public sealed record CppCliWorkerSettings(
    CppCliWorkerIdentity Identity,
    CppCliReferenceSetIdentity ReferenceSet,
    CppCliProcessLimits ProcessLimits,
    string WorkRoot,
    string CompilerPath)
{
    public static CppCliWorkerSettings FromConfiguration(IConfiguration configuration)
    {
        ...
        var compilerPath = Environment.ExpandEnvironmentVariables(
            section["CompilerPath"] ?? "/opt/msvc/bin/x64/cl");
        if (!Path.IsPathRooted(compilerPath))
            throw new InvalidOperationException("Configuration value 'CppCli:CompilerPath' must be an absolute path.");

        var maximumProcessOutputBytes = PositiveInt(
            section["MaximumProcessOutputBytes"],
            1024 * 1024,
            "CppCli:MaximumProcessOutputBytes");
        var maximumProcessWorkingSetBytes = PositiveLong(
            section["MaximumProcessWorkingSetBytes"],
            1024L * 1024 * 1024,
            "CppCli:MaximumProcessWorkingSetBytes");
        var maximumDiagnostics = PositiveInt(
            section["MaximumDiagnostics"],
            100,
            "CppCli:MaximumDiagnostics");
        if (maximumDiagnostics > 1_000)
            throw new InvalidOperationException("Configuration value 'CppCli:MaximumDiagnostics' cannot exceed 1000.");

        var referenceSetDigest = RequiredSha256(
            section["ReferenceSetDigest"],
            "CppCli:ReferenceSetDigest");
        return new CppCliWorkerSettings(
            new CppCliWorkerIdentity(
                section["ReleaseId"] ?? "development",
                RequiredIdentity(section["CompilerVersion"], "CppCli:CompilerVersion"),
    ...
    private static string RequiredSourceUri(string? value)
    {
        ...
        {
            throw new InvalidOperationException(
                "Configuration value 'CppCli:ReferenceSetSourceUri' must be an absolute source URI.");
        }
        return value;
    }
