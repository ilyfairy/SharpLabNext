...
internal sealed record CppCliCompilerInvocation(
    bool Succeeded,
    byte[] PeImage,
...
{
    Task<CppCliCompilerInvocation> CompileAsync(
...
public sealed partial class CppCliCompilerProcess : ICppCliCompilerProcess, IDisposable
{
    ...
    async Task<CppCliCompilerInvocation> ICppCliCompilerProcess.CompileAsync(
        ValidatedCppCliWorkspace workspace,
        CancellationToken cancellationToken) =>
        await CompileAsync(workspace, cancellationToken).ConfigureAwait(false);

    internal async Task<CppCliCompilerInvocation> CompileAsync(
        ValidatedCppCliWorkspace workspace,
        CancellationToken cancellationToken)
    {
        if (!File.Exists(_settings.CompilerPath))
        {
            throw new LanguageWorkerRequestException(
                ...
                "The fixed MSVC C++/CLI compiler is unavailable.",
        ...
        if (!await _concurrency.WaitAsync(0, cancellationToken).ConfigureAwait(false))
        {
            throw new LanguageWorkerRequestException(
                ...
                "C++/CLI compiler process capacity is exhausted.",
        ...
        try
        {
            ...
            var startInfo = CppCliCompilerCommand.Create(
                _settings,
                jobRoot,
                sourcePath,
                objectPath,
            ...
            if (execution.OutputLimitExceeded)
            {
                throw new LanguageWorkerRequestException(
                    ...
                    "The C++/CLI compiler exceeded its process output limit.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception)
        {
            throw new LanguageWorkerRequestException(
                "compiler-unavailable",
                "The fixed MSVC C++/CLI compiler could not be started.",
                StatusCodes.Status503ServiceUnavailable,
                exception);
        }
                ...
                try
                {
                    ...
                    if (process.WorkingSet64 > _settings.ProcessLimits.MaximumProcessWorkingSetBytes)
                    {
                        ...
                        throw new LanguageWorkerRequestException(
                            ...
                            "The C++/CLI compiler exceeded its memory limit.",
                            StatusCodes.Status429TooManyRequests);
                    }
                }
        ...
        int maximumDiagnostics)
    {
        ...
        if (diagnostics.Any(static diagnostic =>
                diagnostic.Severity == DiagnosticSeverity.Error && diagnostic.Code != "C1121"))
        {
            diagnostics.RemoveAll(static diagnostic =>
                diagnostic.Code == "C1121" &&
                diagnostic.Message.Contains("CryptoAPI failed", StringComparison.OrdinalIgnoreCase));
        }
        return diagnostics;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!info.Exists || info.Length <= 0)
        {
            throw new LanguageWorkerRequestException(
                ...
                "The C++/CLI compiler did not produce a mixed-mode PE.",
                StatusCodes.Status503ServiceUnavailable);
        }
        if (info.Length > maximumBytes)
        {
            throw new LanguageWorkerRequestException(
                "artifact-too-large",
                "The C++/CLI compiler output exceeds the configured artifact limit.",
                StatusCodes.Status413PayloadTooLarge);
        }
    ...
    [GeneratedRegex(
        "^(?<path>.+)\\((?<line>[0-9]+)(?:,(?<column>[0-9]+))?\\):\\s*(?<severity>fatal error|error|warning)\\s+(?<code>[A-Z]+[0-9]+):\\s*(?<message>.+)$",
        RegexOptions.CultureInvariant | RegexOptions.IgnoreCase)]
    private static partial Regex LocatedDiagnosticRegex();

    [GeneratedRegex(
        "^(?:(?:LINK|cl|[^:]+)\\s*:\\s*)?(?<severity>fatal error|error|warning)\\s+(?<code>[A-Z]+[0-9]+):\\s*(?<message>.+)$",
        RegexOptions.CultureInvariant | RegexOptions.IgnoreCase)]
    ...
    [LoggerMessage(
        ...
        Message = "Failed to remove C++/CLI compiler job directory {JobRoot}.")]
