...
public sealed record PeachPieReferenceSetDefinition(
    string Id,
    string Path,
...
public sealed record PeachPieWorkerIdentity(
    string ReleaseId,
    ...
    IReadOnlyList<PeachPieReferenceSetDefinition> ReferenceSets)
{
    public static PeachPieWorkerSettings FromConfiguration(IConfiguration configuration)
    {
        ...
        var compilerVersion = Pinned(
            worker["CompilerVersion"],
            PeachPieToolchain.CompilerVersion,
            "PeachPie:CompilerVersion");
        var compilerCommit = Pinned(
            worker["CompilerCommit"],
            PeachPieToolchain.CompilerCommit,
            "PeachPie:CompilerCommit");
        ...
        var referenceSets = configuration.GetSection("ReferenceSets")
            .GetChildren()
            .Select(section => new PeachPieReferenceSetDefinition(
                section.Key,
        ...
        var monoUnixNativeLibraryPath = Path.GetFullPath(Path.Combine(
            baseDirectory,
            "runtimes",
            PeachPieToolchain.NativeRuntimeIdentifier,
            "native",
            PeachPieToolchain.MonoUnixNativeLibraryName));
        return new PeachPieWorkerSettings(
            new PeachPieWorkerIdentity(
                worker["ReleaseId"] ?? "development",
                compilerVersion,
                compilerCommit,
                worker["WorkerImageId"] ?? $"sha256:{new string('0', 64)}"),
            buildProcess,
            Path.GetFullPath(Environment.ExpandEnvironmentVariables(
                worker["WorkRoot"] ?? Path.Combine(Path.GetTempPath(), "SharpLabNext", "peachpie-worker"))),
            runtimeAssemblyPath,
            libraryAssemblyPath,
            monoUnixNativeLibraryPath,
            referenceSets);
    }
