...
public sealed class PeachPieBuildService(
    PeachPieReferenceSetProvider referenceSets,
    PeachPieCompiler compiler,
    ...
    LanguageWorkerCapabilityManifest manifest) : ILanguageWorkerBuildService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!response.CompilationSucceeded || !response.EmitSucceeded)
            {
                return new LanguageWorkerBuildExecution(new BuildResult(
                    response.CompilationSucceeded ? BuildOutcome.EmitFailed : BuildOutcome.CompilationFailed,
                    null,
                    response.Diagnostics,
                    identity,
                    workspace.Snapshot.Revision,
            ...
            var supportFiles = await PeachPieSupportAssemblyResolver.ResolveAsync(
                settings,
                referenceSet,
            ...
            files.AddRange(supportFiles.Select(static file =>
                new LanguageArtifactFile(file.Role, file.Path, file.Content)));
            var supportAssemblies = supportFiles
                .Where(static file => file.Role == "support-assembly")
                .Select(static file => file.Path)
                .ToArray();
            var nativeLibraries = supportFiles
                .Where(static file => file.Role == "native-library")
                .Select(static file => file.Path)
                .ToArray();
        ...
        catch (CompilerProcessCapacityExceededException exception)
        {
            ...
            throw RequestException(
                ...
                "PeachPie compiler process capacity is exhausted.",
        ...
        catch (CompilerProcessMemoryLimitExceededException exception)
        {
            ...
            throw RequestException(
                ...
                "The PeachPie compiler process exceeded its memory limit.",
                StatusCodes.Status429TooManyRequests,
                exception);
        }
        ...
        catch (CompilerProcessException exception)
        {
            ...
            throw RequestException(
                ...
                "The isolated PeachPie compiler process failed.",
        ...
        catch (PeachPieCompilerFailureException exception)
        {
            throw RequestException(
                "compiler-failure",
                "The PeachPie compiler runtime support closure is unavailable.",
                StatusCodes.Status503ServiceUnavailable,
                exception);
        }
    ...
    private async Task<PeachPieCompilerResponse> CompileAsync(
        BuildRequest request,
        CancellationToken cancellationToken)
    {
        ...
        return await processRunner.RunAsync<BuildRequest, PeachPieCompilerResponse>(
    ...
    private static void ValidateResponse(BuildRequest request, PeachPieCompilerResponse response)
    {
        ...
        if (request.Target == BuildTarget.Artifact &&
            response.CompilationSucceeded &&
            response.EmitSucceeded &&
            response.PeImage.Length == 0)
        {
            throw new CompilerProcessProtocolException(
                "The PeachPie compiler process omitted a successful managed PE.");
        }
        if ((!response.CompilationSucceeded || !response.EmitSucceeded) && response.PeImage.Length != 0)
        {
            throw new CompilerProcessProtocolException(
                "The PeachPie compiler process returned a PE for a failed build.");
        }
        if (response.Diagnostics.Any(diagnostic =>
                diagnostic.WorkspaceRevision != request.Workspace.Revision ||
                diagnostic.SelectionRevision != request.Workspace.SelectionRevision))
        {
            throw new CompilerProcessProtocolException(
                "The PeachPie compiler process returned diagnostics for another workspace revision.");
        }
    }
    ...
    private static string InspectPe(byte[] image)
    {
        ...
        catch (BadImageFormatException exception)
        {
            throw RequestException(
                ...
                "The PeachPie compiler returned an invalid managed PE.",
                StatusCodes.Status503ServiceUnavailable,
                exception);
        }
    }
    ...
    private static Exception MapFailure(
        ...
        {
            CompilerChildFailureKind.InvalidRequest => RequestException(
                "invalid-workspace", exception.PublicMessage, StatusCodes.Status400BadRequest, exception),
            CompilerChildFailureKind.ReferenceSetUnavailable => RequestException(
                "unsupported-reference-set", exception.PublicMessage, StatusCodes.Status503ServiceUnavailable, exception),
            CompilerChildFailureKind.OutputLimitExceeded => RequestException(
                "artifact-too-large", exception.PublicMessage, StatusCodes.Status413PayloadTooLarge, exception),
            ...
            CompilerChildFailureKind.CompilerFailure => RequestException(
                "compiler-failure", exception.PublicMessage, StatusCodes.Status503ServiceUnavailable, exception),
            _ => RequestException(
                "compiler-process-unavailable",
                "The isolated PeachPie compiler process failed.",
                StatusCodes.Status503ServiceUnavailable,
    ...
    private static LanguageWorkerRequestException RequestException(
        string code,
        string message,
        int statusCode,
        Exception exception) => new(code, message, statusCode, exception);
}
