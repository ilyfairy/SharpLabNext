...
internal sealed record ValidatedPeachPieWorkspace(
    WorkspaceSnapshot Snapshot,
    IReadOnlyList<ValidatedPeachPieWorkspaceFile> OrderedFiles,
    string ActiveFile,
...
{
    public static ValidatedPeachPieWorkspace Validate(
        ...
        LanguageWorkerCapabilityManifest manifest)
    {
        ...
        if (workspace.Files.Count == 0 || workspace.Files.Count > manifest.Limits.MaximumFiles)
            throw new PeachPieBuildRequestValidationException(
                $"Workspace must contain between 1 and {manifest.Limits.MaximumFiles} files.");
    ...
    private static void ValidateOptions(BuildOptions options)
    {
        ...
        if (options.LanguageVersion is not null &&
            options.LanguageVersion is not ("default" or "latest" or "8.5"))
        {
            throw new PeachPieBuildRequestValidationException(
                $"PHP language version '{options.LanguageVersion}' is not supported by the pinned PeachPie compiler.");
        }
