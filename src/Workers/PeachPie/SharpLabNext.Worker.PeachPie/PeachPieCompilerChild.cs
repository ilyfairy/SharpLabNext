...
public static class PeachPieCompilerChild
{
    ...
    public static async Task RunAsync(WebApplicationBuilder builder)
    {
        ...
        try
        {
            ...
            await CompilerChildProtocol.WriteSuccessAsync(output, response, settings.BuildProcess.MaximumResponseBytes, CancellationToken.None).ConfigureAwait(false);
        }
        ...
        {
            await WriteFailureAsync(CompilerChildFailureKind.DeadlineExceeded, "The PeachPie compiler process deadline elapsed.");
        }
        catch (Exception)
        {
            await WriteFailureAsync(CompilerChildFailureKind.Internal, "The PeachPie compiler process failed.");
        }

        async Task WriteFailureAsync(CompilerChildFailureKind kind, string message) =>
            await CompilerChildProtocol.WriteFailureAsync<PeachPieCompilerResponse>(output, kind, message, settings.BuildProcess.MaximumResponseBytes, CancellationToken.None).ConfigureAwait(false);
    }
}
