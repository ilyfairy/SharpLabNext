...
public sealed class PeachPieCompiler(
    ...
    LanguageWorkerCapabilityManifest manifest)
{
    ...
    public async Task<PeachPieCompilerResponse> CompileAsync(
        SharpLabNext.Contracts.BuildRequest request,
        CancellationToken cancellationToken)
    {
        ...
        var references = referenceSet.ReferenceAssemblyPaths
            .Append(settings.RuntimeAssemblyPath)
            .Append(settings.LibraryAssemblyPath)
            .Distinct(PathComparer())
            .Select(static path => MetadataReference.CreateFromFile(path))
            .ToImmutableArray();
        var compilationOptions = new PhpCompilationOptions(
            OutputKind.ConsoleApplication,
            baseDirectory: settings.WorkRoot,
            sdkDirectory: null,
        ...
        if (!HasErrors(diagnostics))
        {
            ...
            diagnostics = diagnostics.AddRange(
                await compilation.BindAndAnalyseTask().ConfigureAwait(false));
        }
        if (HasErrors(diagnostics))
        {
            return new PeachPieCompilerResponse(
                Environment.ProcessId,
                CompilationSucceeded: false,
                EmitSucceeded: false,
                PeImage: [],
                ConvertDiagnostics(diagnostics, workspace, settings.WorkRoot));
        }
    ...
    private static ImmutableArray<PhpSyntaxTree> CreateSyntaxTrees(
        ValidatedPeachPieWorkspace workspace,
        string root,
        PhpParseOptions parseOptions)
    {
        var builder = ImmutableArray.CreateBuilder<PhpSyntaxTree>(workspace.OrderedFiles.Count + 1);
        var escapedEntryPath = workspace.ActiveFile
            .Replace("\\", "\\\\", StringComparison.Ordinal)
            .Replace("'", "\\'", StringComparison.Ordinal);
        var bootstrap = $"<?php\nrequire '{escapedEntryPath}';\nreturn 0;\n";
        var bootstrapPath = Path.Combine(root, BootstrapFileName);
        builder.Add(PhpSyntaxTree.ParseCode(
            SourceText.From(bootstrap, Encoding.UTF8),
            parseOptions,
            parseOptions,
            bootstrapPath));
    ...
    private static List<ContractDiagnostic> ConvertDiagnostics(
        ...
        string root)
    {
        ...
        var workspacePaths = workspace.OrderedFiles
        ...
        foreach (var diagnostic in diagnostics)
        {
            ...
            if (location is not null && location.IsInSource)
            {
                    ...
                    Path.IsPathRooted(relativePath))
                {
                    throw new PeachPieCompilerFailureException(
                        "The PeachPie compiler returned a diagnostic outside the managed workspace.");
                }
                if (StringComparer.OrdinalIgnoreCase.Equals(relativePath, BootstrapFileName))
                    continue;
                path = workspacePaths.FirstOrDefault(candidate =>
                    StringComparer.OrdinalIgnoreCase.Equals(candidate, relativePath));
                if (path is null)
                {
                    throw new PeachPieCompilerFailureException(
                        "The PeachPie compiler returned a diagnostic for an unknown source file.");
                }
                range = new SharpLabNext.Contracts.TextRange(
            ...
            var key = string.Create(
                ...
                $"{diagnostic.Id}|{diagnostic.Severity}|{path}|{range}|{message}");
            if (!seen.Add(key))
                continue;
    ...
    private static bool HasErrors(IEnumerable<Microsoft.CodeAnalysis.Diagnostic> diagnostics) =>
        diagnostics.Any(static diagnostic => diagnostic.Severity == Microsoft.CodeAnalysis.DiagnosticSeverity.Error);
