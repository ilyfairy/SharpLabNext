...
public sealed record LoadedPeachPieReferenceSet(
    PeachPieReferenceSetDefinition Definition,
...
public sealed class PeachPieReferenceSetProvider
{
    ...
    public PeachPieReferenceSetProvider(
        IReadOnlyList<PeachPieReferenceSetDefinition> definitions,
    ...
    public IReadOnlyList<ReferenceSetAttestation> Attestations =>
        _referenceSets.Values.Select(static referenceSet => referenceSet.Attestation).ToArray();

    public LoadedPeachPieReferenceSet Get(string id) =>
        _referenceSets.TryGetValue(id, out var referenceSet)
            ? referenceSet
            : throw new PeachPieReferenceSetUnavailableException(
                $"PeachPie reference set '{id}' is unavailable.");
}
