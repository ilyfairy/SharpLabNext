...
public sealed class PeachPieBuildServiceTests
{
    ...
    public async Task CompileCheckHonorsPhp85AndDiagnosticsUseWorkspaceRelativePaths()
    {
        ...
        try
        {
            ...
            var php85 = PeachPieTestSettings.CreateRequest(BuildTarget.CompileCheck, "<?php\n$result = 'sharplab' |> strtoupper(...);\necho $result;\n");
            ...
            var invalid = await service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.CompileCheck, "<?php\nfunction broken( {\n"), TestContext.Current.CancellationToken);
    ...
    public async Task ArtifactContainsManagedPeWithoutPdbAndRecursiveRuntimeClosure()
    {
        ...
        try
        {
            ...
            var execution = await service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, "<?php echo 42;"), TestContext.Current.CancellationToken);
            ...
            var nativeLibrary = Assert.Single(envelope.Manifest.Files, static file => file.Role == "native-library");
            ...
            var nativeContent = Convert.FromBase64String(envelope.FileContentsBase64[PeachPieToolchain.MonoUnixNativeArtifactPath]);
            Assert.Equal(PeachPieToolchain.MonoUnixNativeSha256, Convert.ToHexStringLower(SHA256.HashData(nativeContent)));
            Assert.Equal("false", envelope.Manifest.Metadata!["portablePdb"]);
            Assert.Equal("8.5", envelope.Manifest.Metadata["phpLanguageVersion"]);
            Assert.Equal($"{PeachPieToolchain.NativeRuntimeIdentifier}:{PeachPieToolchain.MonoUnixNativeArtifactPath}", envelope.Manifest.Metadata["nativeLibraryClosure"]);
            Assert.Equal(PeachPieToolchain.MonoUnixNativePackagePath, envelope.Manifest.Metadata["nativeLibrarySourcePath"]);
        }
        finally
        {
            PeachPieTestSettings.DeleteRoot(root);
    ...
    public async Task ArtifactFailsClosedWhenPinnedLinuxX64NativeAssetIsUnavailable()
    {
        ...
        try
        {
            var missingPath = Path.Combine(root, "runtimes", PeachPieToolchain.NativeRuntimeIdentifier, "native", PeachPieToolchain.MonoUnixNativeLibraryName);
            var settings = PeachPieTestSettings.CreateSettings(root, isolated: false) with { MonoUnixNativeLibraryPath = missingPath };
            var service = PeachPieTestSettings.CreateBuildService(settings);

            var exception = await Assert.ThrowsAsync<LanguageWorkerRequestException>(() => service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, "<?php echo 42;"), TestContext.Current.CancellationToken));
    ...
    public async Task ArtifactRejectsUnreviewedLinuxX64NativeAsset()
    {
        ...
        try
        {
            var nativePath = Path.Combine(root, "runtimes", PeachPieToolchain.NativeRuntimeIdentifier, "native", PeachPieToolchain.MonoUnixNativeLibraryName);
            ...
            await File.WriteAllBytesAsync(nativePath, unreviewedElf, TestContext.Current.CancellationToken);
            var settings = PeachPieTestSettings.CreateSettings(root, isolated: false) with { MonoUnixNativeLibraryPath = nativePath };
            var service = PeachPieTestSettings.CreateBuildService(settings);
    ...
    public async Task MultiFileArtifactRunsCoreFunctionsAndBootstrapPreservesExit()
    {
        ...
        try
        {
            ...
            var files = new[]
            {
                new WorkspaceFile("lib/functions.php", 1, "<?php\nfunction answer(): int { return 42; }\n"),
                new WorkspaceFile("src/index.php", 1, """
                    <?php
            ...
            var runBuild = await service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, files, ["lib/functions.php", "src/index.php"], "src/index.php"), TestContext.Current.CancellationToken);
            ...
            var exitBuild = await service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, "<?php exit(7);"), TestContext.Current.CancellationToken);
    ...
    public async Task CompilerRunnerStartsFreshOneShotChildForEveryBuild()
    {
        ...
        try
        {
            ...
            var first = await runner.RunAsync<BuildRequest, PeachPieCompilerResponse>(PeachPieCompilerChild.ChildArgument, request, TimeSpan.FromSeconds(30), TestContext.Current.CancellationToken);
            var second = await runner.RunAsync<BuildRequest, PeachPieCompilerResponse>(PeachPieCompilerChild.ChildArgument, request with { RequestId = $"request-{Guid.NewGuid():N}", IdempotencyKey = $"idempotency-{Guid.NewGuid():N}" },
    ...
    public async Task EmittedPhpFunctionAndClassMethodNamesRemainDiscoverableByName()
    {
        ...
        {
            var service = PeachPieTestSettings.CreateBuildService(root);
            var execution = await service.BuildAsync(PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, "<?php\nfunction global_name(): int { return 1; }\nclass Demo { public function method_name(): int { return 2; } }\necho global_name() + (new Demo())->method_name();\n", "src/index.php"), TestContext.Current.CancellationToken);
            var envelope = Assert.IsType<LanguageWorkerArtifactEnvelope>(execution.Artifact);
            Assert.NotNull(envelope.FileContentsBase64);
            var peImage = Convert.FromBase64String(envelope.FileContentsBase64[$"{PeachPieToolchain.AssemblyName}.dll"]);
            using var pe = new PEReader(new MemoryStream(peImage, writable: false));
            var metadata = pe.GetMetadataReader();
            var names = metadata.MethodDefinitions.Select(handle => metadata.GetString(metadata.GetMethodDefinition(handle).Name)).ToArray();
    ...
    public async Task RepeatedBuildsProduceTheSameManagedPeAndArtifactIdentity()
    {
        ...
        {
            var service = PeachPieTestSettings.CreateBuildService(root);
            var request = PeachPieTestSettings.CreateRequest(BuildTarget.Artifact, "<?php function answer(): int { return 42; } echo answer();");

            var first = await service.BuildAsync(request, TestContext.Current.CancellationToken);
            var second = await service.BuildAsync(request with { RequestId = $"request-{Guid.NewGuid():N}", IdempotencyKey = $"idempotency-{Guid.NewGuid():N}" },
                TestContext.Current.CancellationToken);
            ...
            Assert.Equal(firstEnvelope.FileContentsBase64[$"{PeachPieToolchain.AssemblyName}.dll"], secondEnvelope.FileContentsBase64[$"{PeachPieToolchain.AssemblyName}.dll"]);
        }
        finally
        {
            PeachPieTestSettings.DeleteRoot(root);
    ...
    {
        var request = PeachPieTestSettings.CreateRequest(BuildTarget.CompileCheck, "<?php echo 42;", PeachPieCompiler.BootstrapFileName);

        var exception = Assert.Throws<PeachPieBuildRequestValidationException>(() => PeachPieWorkspaceValidator.Validate(request, PeachPieTestSettings.LoadManifest()));

        Assert.Contains("reserved", exception.Message, StringComparison.OrdinalIgnoreCase);
    }

    private static async Task<ProcessResult> RunArtifactAsync(LanguageWorkerArtifactEnvelope envelope, string root, string name)
    {
        ...
        foreach (var (path, base64) in envelope.FileContentsBase64)
        {
            ...
            await File.WriteAllBytesAsync(outputPath, Convert.FromBase64String(base64), TestContext.Current.CancellationToken);
        }
        var runtimeConfig = new { runtimeOptions = new { tfm = "net10.0", framework = new { name = "Microsoft.NETCore.App", version = "10.0.9" } } };
        await File.WriteAllTextAsync(Path.Combine(directory, $"{PeachPieToolchain.AssemblyName}.runtimeconfig.json"), JsonSerializer.Serialize(runtimeConfig), TestContext.Current.CancellationToken);
...
internal sealed class PeachPieProcessEnvironment : IDisposable
{
    ...
    private PeachPieProcessEnvironment(IReadOnlyDictionary<string, string?> previous) => _previous = previous;
