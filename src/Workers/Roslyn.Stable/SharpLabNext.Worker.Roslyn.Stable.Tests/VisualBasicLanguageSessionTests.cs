...
public sealed class VisualBasicLanguageSessionTests
{
    ...
    public async Task AutomaticOutputKindInitializesVisualBasicSessionAsLibrary()
    {
        ...
        var contract = await manager.OpenAsync(
            LanguageSessionTests.CreateVisualBasicOpenRequest(
                "vb-automatic-output-kind",
                source,
                outputKind: BuildOutputKind.Auto),
            TestContext.Current.CancellationToken);
        var session = manager.GetRequired(contract.SessionId);

        var snapshot = await session.GetDocumentSnapshotAsync(
            "file:///Program.vb",
            TestContext.Current.CancellationToken);

        Assert.Equal(
            Microsoft.CodeAnalysis.OutputKind.DynamicallyLinkedLibrary,
            snapshot.Document.Project.CompilationOptions?.OutputKind);
    }
    ...
    public async Task VisualBasicCompletionHoverSignatureTokensSymbolsAndActionsWork()
    {
        ...
        await using var manager = CreateManager();
        var contract = await manager.OpenAsync(
            LanguageSessionTests.CreateVisualBasicOpenRequest("vb-features", source),
            TestContext.Current.CancellationToken);
        var session = manager.GetRequired(contract.SessionId);
        await session.DidOpenAsync(
            new LspDidOpenTextDocumentParams(new LspTextDocumentItem("file:///Program.vb", "visual-basic", 1, source)),
            TestContext.Current.CancellationToken);

        var hover = await session.GetHoverAsync(
            new LspTextDocumentPositionParams(
                new LspTextDocumentIdentifier("file:///Program.vb"),
                new LspPosition(5, 28)),
            TestContext.Current.CancellationToken);
        var tokens = await session.GetSemanticTokensAsync(
            new LspSemanticTokensParams(new LspTextDocumentIdentifier("file:///Program.vb")),
            TestContext.Current.CancellationToken);
        var symbols = await session.GetDocumentSymbolsAsync(
            new LspDocumentSymbolParams(new LspTextDocumentIdentifier("file:///Program.vb")),
            TestContext.Current.CancellationToken);
    ...
    public async Task VisualBasicCompletionPreservesAllRoslynPrefixMatches(string prefix)
    {
        ...
        await using var manager = CreateManager();
        var contract = await manager.OpenAsync(
            LanguageSessionTests.CreateVisualBasicOpenRequest($"vb-completion-prefix-{prefix}", source),
            TestContext.Current.CancellationToken);
        var session = manager.GetRequired(contract.SessionId);
        await session.DidOpenAsync(
            new LspDidOpenTextDocumentParams(
                new LspTextDocumentItem("file:///Program.vb", "visual-basic", 1, source)),
            TestContext.Current.CancellationToken);

        var completions = await session.GetCompletionsAsync(
            new LspCompletionParams(
                new LspTextDocumentIdentifier("file:///Program.vb"),
                new LspPosition(3, 8 + prefix.Length),
                new LspCompletionContext(1, null)),
            TestContext.Current.CancellationToken);
    ...
    {
        await using var manager = CreateManager();
        var csharpContract = await manager.OpenAsync(
            LanguageSessionTests.CreateOpenRequest("parallel-csharp", "int value = 1;"),
            TestContext.Current.CancellationToken);
        var visualBasicContract = await manager.OpenAsync(
            LanguageSessionTests.CreateVisualBasicOpenRequest(
                "parallel-vb",
        ...
        await csharp.DidOpenAsync(
            new LspDidOpenTextDocumentParams(new LspTextDocumentItem("file:///Program.cs", "csharp", 1, "int value = 1;")),
            TestContext.Current.CancellationToken);
        await visualBasic.DidOpenAsync(
            new LspDidOpenTextDocumentParams(new LspTextDocumentItem(
                "file:///Program.vb",
                "visual-basic",
                1,
                "Module Program\n    Dim value As Integer = \"bad\"\nEnd Module")),
