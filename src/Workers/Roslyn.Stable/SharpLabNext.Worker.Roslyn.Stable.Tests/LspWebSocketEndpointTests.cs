...
public sealed class LspWebSocketEndpointTests : IClassFixture<RoslynStableWorkerFactory>
{
    ...
    public async Task EndpointUsesStandardJsonRpcAndPublishesVersionedDiagnostics()
    {
        ...
        using var openResponse = await http.PostAsJsonAsync("/api/v1/language-sessions", LanguageSessionTests.CreateOpenRequest("websocket", "int value = 1;"), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        openResponse.EnsureSuccessStatusCode();
        var session = await openResponse.Content.ReadFromJsonAsync<LanguageSession>(ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        ...
        await SendAsync(socket, new
        {
            ...
            @params = new { textDocument = new { uri = "file:///Program.cs", languageId = "csharp", version = 2, text = "int value = \"bad\";" } }
        });
        ...
        Assert.Contains(diagnosticParams.GetProperty("diagnostics").EnumerateArray(), static diagnostic => diagnostic.GetProperty("code").GetString() == "CS0029");
        ...
        await SendAsync(socket, new { jsonrpc = "2.0", method = "textDocument/didChange", @params = new { textDocument = new { uri = "file:///Program.cs", version = 3 }, contentChanges = new[] { new { text = "System.Console." } } } });
        await SendAsync(socket, new { jsonrpc = "2.0", id = 3, method = "textDocument/completion", @params = new { textDocument = new { uri = "file:///Program.cs" }, position = new { line = 0, character = 15 }, context = new { triggerKind = 1, triggerCharacter = (string?)null } } });
    ...
    {
        using var http = _factory.CreateClient();
        using var openResponse = await http.PostAsJsonAsync("/api/v1/language-sessions", LanguageSessionTests.CreateVisualBasicOpenRequest("websocket-vb", "Module Program\n    Sub Main()\n    End Sub\nEnd Module"), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        openResponse.EnsureSuccessStatusCode();
        var session = await openResponse.Content.ReadFromJsonAsync<LanguageSession>(ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        Assert.NotNull(session);
        ...
        var webSocketClient = _factory.Server.CreateWebSocketClient();
        using var socket = await webSocketClient.ConnectAsync(new Uri($"ws://localhost/api/v1/language-sessions/{session.SessionId}/lsp"), TestContext.Current.CancellationToken);
        await SendAsync(socket, new { jsonrpc = "2.0", id = 11, method = "initialize", @params = new { processId = (int?)null, capabilities = new { }, rootUri = (string?)null } });
        ...
        await SendAsync(socket, new { jsonrpc = "2.0", method = "textDocument/didOpen", @params = new { textDocument = new { uri = "file:///Program.vb", languageId = "visual-basic", version = 2, text = "Module Program\n    Sub Main()\n        Dim value As Integer = \"bad\"\n    End Sub\nEnd Module" } } });
        using var diagnostics = await ReceiveUntilMethodAsync(socket, "textDocument/publishDiagnostics");
        var diagnosticParams = diagnostics.RootElement.GetProperty("params");
        Assert.Equal(2, diagnosticParams.GetProperty("version").GetInt64());
        Assert.Contains(diagnosticParams.GetProperty("diagnostics").EnumerateArray(), static diagnostic => diagnostic.GetProperty("code").GetString()!.StartsWith("BC", StringComparison.Ordinal));

        await SendAsync(socket, new
        {
            jsonrpc = "2.0",
            method = "textDocument/didChange",
            @params = new { textDocument = new { uri = "file:///Program.vb", version = 3 }, contentChanges = new[] { new { text = "Imports System\nPublic Class Demo\n    Public Sub Run()\n        Console.\n" } } }
        });
        await SendAsync(socket, new { jsonrpc = "2.0", id = 13, method = "textDocument/completion", @params = new { textDocument = new { uri = "file:///Program.vb" }, position = new { line = 3, character = 16 }, context = new { triggerKind = 1, triggerCharacter = (string?)null } } });
        ...
        using var closeResponse = await http.DeleteAsync($"/api/v1/language-sessions/{session.SessionId}", TestContext.Current.CancellationToken);
    ...
    private static async Task ReceiveCloseAsync(WebSocket socket)
    {
        ...
        try
        {
            ...
            if (socket.State == WebSocketState.CloseReceived)
                await socket.CloseOutputAsync(WebSocketCloseStatus.NormalClosure, "Test completed.", TestContext.Current.CancellationToken);
        }
        catch (IOException)
        {
