...
public sealed class WorkerEndpointTests : IClassFixture<RoslynStableWorkerFactory>
{
    ...
    public async Task SharedTokenProtectsWorkerApiButNotHealthEndpoints()
    {
        ...
        using var health = await anonymous.GetAsync("/health/ready", TestContext.Current.CancellationToken);
        using var unauthorized = await anonymous.GetAsync("/api/v1/worker/describe", TestContext.Current.CancellationToken);
    ...
    {
        using var client = _factory.CreateClient();
        var request = CSharpExplainServiceTests.CreateRequest([new WorkspaceFile("Program.cs", 1, "class Program { static void Main() { } }")], revision: 91, selectionRevision: 15);

        using var response = await client.PostAsJsonAsync("/api/v1/explain", request, ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadFromJsonAsync<WorkerExplainHttpResponse>(ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        Assert.NotNull(body);
        Assert.Equal(request.RequestId, body.RequestId);
        Assert.Equal(91, body.Result.Document.WorkspaceRevision);
        Assert.Contains(Assert.Single(body.Result.Document.Files).Nodes, static node => node.Kind == "MethodDeclaration");
    }
    ...
    public async Task BuildEndpointAcceptsContractRequestAndReturnsTypedCompileCheckResult()
    {
        ...
        var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, Revision: 88, SelectionRevision: 12, LanguageId: "csharp", Files: [new WorkspaceFile("Program.cs", 4, "System.Console.WriteLine(42);")], ActiveFile: "Program.cs", SourceOrder: ["Program.cs"], ReferenceSetId: "net10-ref", BuildOptions: options);
        var request = new BuildRequest("http-request", "http-idempotency", "http-pipeline", "roslyn-stable", "net10-ref", workspace, DateTimeOffset.UtcNow.AddMinutes(1), options, BuildTarget.CompileCheck);
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var buildResponse = await response.Content.ReadFromJsonAsync<WorkerBuildHttpResponse>(ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
    ...
    {
        using var client = _factory.CreateClient();
        var request = VisualBasicBuildServiceTests.CreateRequest(BuildTarget.CompileCheck, [new WorkspaceFile("Program.vb", 3, "Imports System\nModule Program\n    Sub Main()\n        Console.WriteLine(42)\n    End Sub\nEnd Module")], revision: 89, selectionRevision: 14);
    ...
    {
        using var client = _factory.CreateClient();
        var options = new BuildOptions(BuildConfiguration.Debug, Optimize: false, BuildOutputKind.Library, AllowUnsafe: false, EmitPortablePdb: true, NullableContextMode.Enable, LanguageVersion: "14.0");
        var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, Revision: 99, SelectionRevision: 13, LanguageId: "csharp", Files: [new WorkspaceFile("Library.cs", 1, "public static class Library { public static int Value => 42; }")], ActiveFile: "Library.cs", SourceOrder: ["Library.cs"], ReferenceSetId: "net10-ref", BuildOptions: options);
        var request = new BuildRequest("http-artifact-request", "http-artifact-idempotency", "http-artifact-pipeline", "roslyn-stable", "net10-ref", workspace, DateTimeOffset.UtcNow.AddMinutes(1), options, BuildTarget.Artifact);
    ...
    public async Task CompilerChildCrashDoesNotTakeDownLanguageSessions()
    {
        await using var factory = _factory.WithWebHostBuilder(builder =>
        {
            ...
            builder.ConfigureTestServices(services =>
            {
                ...
                services.AddSingleton<ICompilerProcessRunner>(new FailingCompilerProcessRunner(new CompilerProcessCrashedException(134, "Simulated compiler crash.")));
            });
        });
        using var client = factory.CreateClient();
        var request = CreateCompileCheckRequest();

        using var failed = await client.PostAsJsonAsync("/api/v1/build", request, ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
        ...
        Assert.Equal("compiler-process-unavailable", problem.RootElement.GetProperty("Code").GetString());

        using var opened = await client.PostAsJsonAsync("/api/v1/language-sessions", LanguageSessionTests.CreateOpenRequest("after-crash", "System.Console.WriteLine(42);"), ContractJson.CreateSerializerOptions(), TestContext.Current.CancellationToken);
    ...
    {
        var options = new BuildOptions(BuildConfiguration.Release, Optimize: true, BuildOutputKind.Console, AllowUnsafe: false, EmitPortablePdb: true, NullableContextMode.Enable, LanguageVersion: "14.0");
        return new BuildRequest("compiler-crash-request", "compiler-crash-key", "compiler-crash-pipeline", "roslyn-stable", "net10-ref", new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, 1, 1, "csharp", [new WorkspaceFile("Program.cs", 1, "System.Console.WriteLine(42);")], "Program.cs", ["Program.cs"], "net10-ref", options), DateTimeOffset.UtcNow.AddMinutes(1), options, BuildTarget.CompileCheck);
    }
...
public sealed class RoslynStableWorkerFactory : WebApplicationFactory<global::Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.UseSetting("ReferenceSets:net10-ref:Path", CSharpBuildServiceTests.GetNet10ReferencePathForHost());
        builder.UseSetting("ReferenceSets:net11-preview-ref:Path", CSharpBuildServiceTests.GetNet11ReferencePathForHost());
