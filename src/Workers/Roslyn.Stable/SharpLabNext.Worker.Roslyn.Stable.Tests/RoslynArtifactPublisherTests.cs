...
public sealed class RoslynArtifactPublisherTests
{
    ...
    public async Task PublishesEveryManifestFileWithTheConfiguredTtl()
    {
        ...
        var publisher = new RoslynArtifactPublisher(store, new ArtifactBundlePublishingOptions(new Uri("http://artifact-store:8080"), ttl));

        var published = await publisher.PublishAsync(artifact, TestContext.Current.CancellationToken);
    ...
    {
        var artifact = CreateArtifact();
        var store = new RecordingArtifactStore { ResponseArtifactRef = new ArtifactRef($"sha256:{new string('f', 64)}") };
         var publisher = new RoslynArtifactPublisher(store, new ArtifactBundlePublishingOptions(new Uri("http://artifact-store:8080"), TimeSpan.FromHours(1)));

        var exception = await Assert.ThrowsAsync<ArtifactBundlePublicationException>(() => publisher.PublishAsync(artifact, TestContext.Current.CancellationToken));
    ...
    {
        var artifact = CreateArtifact();
        var store = new RecordingArtifactStore { Exception = new ArtifactStoreHttpException(HttpStatusCode.RequestEntityTooLarge, "too large") };
         var publisher = new RoslynArtifactPublisher(store, new ArtifactBundlePublishingOptions(new Uri("http://artifact-store:8080"), TimeSpan.FromHours(1)));
    ...
    public async Task RejectsChildArtifactBytesThatDoNotMatchTheManifestDigest()
    {
        ...
        var tampered = artifact with { PeImage = [0x4d, 0x5a, 0xff, 0xff] };
         var publisher = new RoslynArtifactPublisher(new RecordingArtifactStore(), new ArtifactBundlePublishingOptions(new Uri("http://artifact-store:8080"), TimeSpan.FromHours(1)));

        await Assert.ThrowsAsync<InvalidOperationException>(() => publisher.PublishAsync(tampered, TestContext.Current.CancellationToken));
    }
    ...
    private static CompiledArtifact CreateArtifact()
    {
        ...
        var files = new[]
        {
            new ArtifactFileDescriptor("primary-assembly", "SharpLabNext.User.dll", peImage.LongLength, ContentIdentity.Compute(peImage).Value),
            new ArtifactFileDescriptor("portable-pdb", "SharpLabNext.User.pdb", portablePdb.LongLength, ContentIdentity.Compute(portablePdb).Value)
        };
        var identity = new BuildIdentity("test-release", "csharp", "roslyn-stable", "5.6.0", null, "net10-ref", "test-image");
        ...
        return new CompiledArtifact(manifest.ArtifactId, manifest.ArtifactFormat, "SharpLabNext.User", manifest.ReferenceSetId, manifest.TargetFramework, peImage, portablePdb, manifest, files, identity);
    }
    ...
    private sealed class RecordingArtifactStore : IArtifactStoreClient
    {
        ...
        public Task<PutContentResponse> PutContentAsync(ContentRef contentRef, Stream content, long? declaredSize = null, TimeSpan? timeToLive = null, CancellationToken cancellationToken = default) =>
        ...
        public async Task<PutArtifactResponse> PutArtifactAsync(ArtifactManifest manifest, IReadOnlyList<ArtifactFileUpload> files, TimeSpan? timeToLive = null, CancellationToken cancellationToken = default)
        {
            ...
            return new PutArtifactResponse(ResponseArtifactRef ?? manifest.ArtifactId, UploadedFiles.Values.Sum(static content => (long)content.Length), DateTimeOffset.UtcNow.Add(timeToLive ?? TimeSpan.FromHours(1)), AlreadyExisted: false);
        }
        ...
        public Task<ArtifactLeaseResponse> RenewLeaseAsync(string leaseToken, TimeSpan duration, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException();

        public Task ReleaseLeaseAsync(string leaseToken, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException();

        public Task<GarbageCollectionResponse> CollectGarbageAsync(int maxArtifacts = 1000, int maxContents = 5000, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException();
    }
}
