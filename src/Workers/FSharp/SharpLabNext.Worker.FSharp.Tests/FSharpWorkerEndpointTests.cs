...
public sealed class FSharpWorkerEndpointTests
{
    ...
    public async Task DescribeAndBuildEndpointsExposeThePinnedFSharpToolchain()
    {
        ...
        try
        {
            ...
            Assert.True(descriptor.Capabilities.Any(static item => item.Id == "completion" && item.Available), readyBody);
            ...
            var request = FSharpBuildServiceTests.CreateRequest(BuildTarget.Artifact, [new WorkspaceFile("Program.fs", 1, "module Program\nopen System\n[<EntryPoint>]\nlet main _ = Console.WriteLine(\"HTTP\"); 0\n")], ["Program.fs"]);
            ...
            var build = await response.Content.ReadFromJsonAsync<FSharpWorkerBuildHttpResponse>(JsonOptions, TestContext.Current.CancellationToken);
            ...
            Assert.Equal(["FSharp.Core.dll", "SharpLabNext.User.dll", "SharpLabNext.User.pdb"], build.DevelopmentArtifact.FileContentsBase64.Keys.Order(StringComparer.Ordinal));
            Assert.All(build.DevelopmentArtifact.FileContentsBase64.Values, static value => Assert.NotEmpty(Convert.FromBase64String(value)));
        }
        finally
        {
    ...
    public async Task CompilerChildTimeoutDoesNotTakeDownLanguageSessions()
    {
        ...
        try
        {
            await using var factory = CreateFactory(root).WithWebHostBuilder(builder =>
            {
                ...
                {
                    var settings = FSharpTestSettings.Create(root) with { BuildProcess = CompilerProcessIsolationOptions.Default with { Enabled = true } };
                    services.RemoveAll<FSharpWorkerSettings>();
                    services.RemoveAll<ICompilerProcessRunner>();
                    services.AddSingleton(settings);
                    services.AddSingleton<ICompilerProcessRunner>(new FailingCompilerProcessRunner(new CompilerProcessTimeoutException("Simulated compiler timeout.")));
                });
            });
            using var client = factory.CreateClient();
            var request = FSharpBuildServiceTests.CreateRequest(BuildTarget.CompileCheck, [new WorkspaceFile("Program.fs", 1, "module Program\nprintfn \"hello\"\n")], ["Program.fs"]);

            using var failed = await client.PostAsJsonAsync("/api/v1/build", request, JsonOptions, TestContext.Current.CancellationToken);

            Assert.Equal(HttpStatusCode.RequestTimeout, failed.StatusCode);
            using var problem = JsonDocument.Parse(await failed.Content.ReadAsStringAsync(TestContext.Current.CancellationToken));
            Assert.Equal("deadline-exceeded", problem.RootElement.GetProperty("Code").GetString());

            using var opened = await client.PostAsJsonAsync("/api/v1/language-sessions", CreateOpenRequest("module Program\nlet value = 42\n"), JsonOptions, TestContext.Current.CancellationToken);
            ...
            using var closed = await client.DeleteAsync($"/api/v1/language-sessions/{session.SessionId}", TestContext.Current.CancellationToken);
    ...
    public async Task WebSocketUsesStandardLspForDiagnosticsCompletionSemanticTokensAndCodeActions()
    {
        ...
        try
        {
            ...
            using var initialized = await ReceiveUntilAsync(socket, static rootElement => rootElement.TryGetProperty("id", out var id) && id.GetInt32() == 1);
            ...
            Assert.Equal(FSharpLanguageSession.SemanticTokenTypes, semanticProvider.GetProperty("legend").GetProperty("tokenTypes").EnumerateArray().Select(static item => item.GetString()!).ToArray());
            Assert.Empty(semanticProvider.GetProperty("legend").GetProperty("tokenModifiers").EnumerateArray());
            var actionKinds = capabilities.GetProperty("codeActionProvider").GetProperty("codeActionKinds").EnumerateArray().Select(static item => item.GetString()!).ToArray();
            ...
            await SendAsync(socket, new { jsonrpc = "2.0", method = "textDocument/didOpen", @params = new { textDocument = new { uri, languageId = "fsharp", version = 1, text = source } } });
            ...
            await SendAsync(socket, new { jsonrpc = "2.0", id = 2, method = "textDocument/completion", @params = new { textDocument = new { uri }, position = new { line = 3, character = "let write () = Console.Wri".Length }, context = new { triggerKind = 1, triggerCharacter = (string?)null } } });
            using var completion = await ReceiveUntilAsync(socket, static rootElement => rootElement.TryGetProperty("id", out var id) && id.GetInt32() == 2);
            ...
            using var semanticTokens = await ReceiveUntilAsync(socket, static rootElement => rootElement.TryGetProperty("id", out var id) && id.GetInt32() == 3);
            ...
            await SendAsync(socket, new { jsonrpc = "2.0", id = 4, method = "textDocument/codeAction", @params = new { textDocument = new { uri }, range = new { start = new { line = 2, character = 0 }, end = new { line = 2, character = "open System.Text".Length } }, context = new { diagnostics = Array.Empty<object>(), only = QuickFixOnly } } });
            using var codeActions = await ReceiveUntilAsync(socket, static rootElement => rootElement.TryGetProperty("id", out var id) && id.GetInt32() == 4);
            ...
            using var shutdown = await ReceiveUntilAsync(socket, static rootElement => rootElement.TryGetProperty("id", out var id) && id.GetInt32() == 5);
    ...
    private sealed class FailingCompilerProcessRunner(CompilerProcessException exception) : ICompilerProcessRunner
    {
    ...
    {
        var options = new BuildOptions(BuildConfiguration.Debug, false, BuildOutputKind.Library, false, true, NullableContextMode.Disable, "9.0");
        var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, 1, 1, "fsharp", [new WorkspaceFile("Program.fs", 1, source)], "Program.fs", ["Program.fs"], "net10-ref", options);
        return new OpenLanguageSessionRequest("request-lsp", "pipeline-lsp", "fsharp", "fsharp-stable", "net10-ref", workspace);
    }
    ...
    private static async Task ReceiveCloseAsync(WebSocket socket)
    {
        ...
        try
        {
            ...
            if (socket.State == WebSocketState.CloseReceived)
                await socket.CloseOutputAsync(WebSocketCloseStatus.NormalClosure, "Test completed.", timeout.Token);
        }
        catch (IOException)
        {
