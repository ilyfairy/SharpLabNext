...
public sealed class FSharpWorkerHealthService(FSharpReferenceSetProvider referenceSets, FSharpWorkerSettings settings, FSharpWorkerProcessIdentity processIdentity)
{
    ...
    public async Task<HealthResponse> CheckAsync(CancellationToken cancellationToken)
    {
        ...
        var checks = new List<HealthCheckResult> { new("fsharp-compiler-identity", compilerHealthy ? HealthStatus.Healthy : HealthStatus.Unhealthy, compilerHealthy ? $"FSharp.Compiler.Service {FSharpCompilerFacade.CompilerVersion} and FSharp.Core {FSharpCompilerFacade.FSharpCorePackageVersion} are pinned." : "The loaded F# compiler identity is not approved.", compilerTimer.Elapsed) };
...
public sealed class FSharpReferenceSetWarmupService(FSharpReferenceSetProvider referenceSets, ILogger<FSharpReferenceSetWarmupService> logger) : IHostedService
{
