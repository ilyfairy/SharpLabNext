...
public sealed class FSharpLanguageSessionManager(FSharpReferenceSetProvider referenceSets, FSharpCompilerFacade compiler, FSharpWorkerSettings settings) : IAsyncDisposable
{
    ...
    public async Task<LanguageSession> OpenAsync(OpenLanguageSessionRequest request, CancellationToken cancellationToken)
    {
        ...
        var validated = FSharpWorkspaceValidator.Validate(new BuildRequest(request.RequestId, request.RequestId, request.PipelineResolutionId, request.ToolchainId, request.ReferenceSetId, request.Workspace, DateTimeOffset.UtcNow.AddMilliseconds(settings.CompilationLimits.MaxBuildMilliseconds), request.Workspace.BuildOptions, BuildTarget.CompileCheck), settings.CompilationLimits);
        ...
        try
        {
            ...
            var temporary = await TemporaryFSharpWorkspace.CreateAsync(Path.Combine(settings.WorkRoot, "sessions"), validated.OrderedFiles, cancellationToken);
            try
            {
                var sourcePaths = validated.OrderedFiles.Select(file => temporary.Paths[file.Path]).ToArray();
                var projectInput = FSharpBuildService.CreateProjectInput(temporary.Root, sourcePaths, references, validated.Options, validated.Snapshot.Revision);
            ...
            var session = new FSharpLanguageSession(sessionId, validated, references, temporary, compiler, settings.CompilationLimits, settings.LspLimits, expiresAt);
            if (!_sessions.TryAdd(sessionId, session))
            {
                await session.DisposeAsync();
                throw new InvalidOperationException("A unique F# language session ID could not be allocated.");
            }
            return new LanguageSession(sessionId, "fsharp", settings.Identity.ToolchainId, $"{settings.Identity.ToolchainId}/{settings.Identity.CompilerVersion}+fsharp-core/{settings.Identity.FSharpCorePackageVersion}", ContractSchemaVersions.Lsp, validated.Snapshot.Revision, validated.Snapshot.SelectionRevision, expiresAt);
        }
        finally
        {
