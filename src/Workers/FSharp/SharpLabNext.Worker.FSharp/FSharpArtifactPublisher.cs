...
public sealed class FSharpArtifactPublisher(
    ...
    ArtifactBundlePublishingOptions options)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contents = new List<ArtifactBundleContent>
        {
            new($"{artifact.AssemblyName}.dll", artifact.PeImage),
            new("FSharp.Core.dll", artifact.FSharpCoreImage)
        };
    ...
    internal static void ValidateArtifact(FSharpCompiledArtifact artifact)
    {
        ...
        if (artifact.ArtifactRef != artifact.Manifest.ArtifactId ||
            ...
            !string.Equals(artifact.ArtifactFormat, artifact.Manifest.ArtifactFormat, StringComparison.Ordinal) ||
            !string.Equals(artifact.ReferenceSetId, artifact.Manifest.ReferenceSetId, StringComparison.Ordinal) ||
            !string.Equals(artifact.TargetFramework, artifact.Manifest.TargetFramework, StringComparison.Ordinal) ||
        ...
        if (artifact.Manifest.Files.Count != contents.Count ||
            !StringComparer.Ordinal.Equals(artifact.Manifest.EntryAssembly, $"{artifact.AssemblyName}.dll"))
        {
        ...
        {
            if (!contents.TryGetValue(file.Path, out var content) ||
                file.Size != content.LongLength ||
                !StringComparer.Ordinal.Equals(file.Digest, ContentIdentity.Compute(content).Value))
            {
                throw new InvalidOperationException(
                    $"The compiled F# artifact file '{file.Path}' failed size or checksum validation.");
            }
        }
    }
}
