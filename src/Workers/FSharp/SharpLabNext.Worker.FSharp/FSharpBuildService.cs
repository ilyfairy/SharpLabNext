...
public sealed class FSharpBuildService(FSharpReferenceSetProvider referenceSets, FSharpCompilerFacade compiler, FSharpWorkerSettings settings)
{
    ...
    private async Task<FSharpWorkerBuildExecution> ExecuteCoreAsync(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        await using var temporary = await TemporaryFSharpWorkspace.CreateAsync(settings.WorkRoot, workspace.OrderedFiles, cancellationToken);
        ...
        {
            var rejected = await FSharpSourceSafety.FindRejectedDirectiveAsync(compiler, projectInput, temporary.Paths[file.Path], file.Text, cancellationToken).ConfigureAwait(false);
        ...
        if (!string.IsNullOrWhiteSpace(response.TerminatingException) && !response.Diagnostics.Any(static diagnostic => diagnostic.Severity == CompilerDiagnosticSeverity.Error))
        {
        ...
        {
            return new FSharpWorkerBuildExecution(new CompilationCheckResult(succeeded, diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision), null);
        }

        if (!succeeded)
        {
            var outcome = response.Diagnostics.Any(static diagnostic => diagnostic.Severity == CompilerDiagnosticSeverity.Error)
                ? BuildOutcome.CompilationFailed : BuildOutcome.EmitFailed;
            return new FSharpWorkerBuildExecution(new BuildResult(outcome, null, diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision), null);
        }
        ...
        var fsharpCore = await ReadBoundedAsync(referenceSet.FSharpCoreAssemblyPath, settings.CompilationLimits.MaxPeBytes, cancellationToken);
        var artifact = CreateArtifact(pe, pdb, fsharpCore, referenceSet, identity, workspace.Options.OutputKind, workspace.OrderedFiles);
        return new FSharpWorkerBuildExecution(new BuildResult(BuildOutcome.Succeeded, artifact.ArtifactRef, diagnostics, identity, workspace.Snapshot.Revision, workspace.Snapshot.SelectionRevision), artifact);
    }

    internal static FSharpProjectInput CreateProjectInput(string root, string[] sourcePaths, LoadedFSharpReferenceSet referenceSet, BuildOptions options, long revision) => new(Path.Combine(root, "SharpLabNext.User.fsproj"), sourcePaths, CreateProjectOptions(referenceSet, options), DateTime.UnixEpoch.AddTicks(Math.Max(0, revision)));
    ...
    private async Task<FSharpWorkerBuildExecution> CreateAstAsync(ValidatedFSharpWorkspace workspace, FSharpProjectInput projectInput, TemporaryFSharpWorkspace temporary, BuildIdentity identity, CancellationToken cancellationToken)
    {
        ...
        foreach (var file in workspace.OrderedFiles)
        {
            ...
            var response = await compiler.ParseAstAsync(projectInput, temporary.Paths[file.Path], file.Text, Math.Max(1, remainingNodes), settings.AstLimits.MaxDepth, Math.Max(1, settings.AstLimits.MaxUtf8Bytes / workspace.OrderedFiles.Count), settings.AstLimits.MaxTextPreviewCharacters, cancellationToken).ConfigureAwait(false);
        ...
        var rootNode = new AstNode(
            ...
            children);
        return new FSharpWorkerBuildExecution(new AstResult(new AstDocument("fsharp", settings.Identity.ToolchainId, workspace.Snapshot.Revision, rootNode, truncated), identity), null);
    }

    private static string[] CreateCompilerArguments(string outputPath, string[] sourcePaths, LoadedFSharpReferenceSet referenceSet, BuildOptions options)
    {
    ...
    private Diagnostic[] ConvertDiagnostics(IReadOnlyList<FSharpCompilerDiagnostic> diagnostics, ValidatedFSharpWorkspace workspace, TemporaryFSharpWorkspace temporary)
    {
        ...
        return diagnostics.Take(settings.CompilationLimits.MaxDiagnostics).Select(diagnostic => new Diagnostic(
                "fsharp",
                diagnostic.Code,
                ...
                new TextRange(diagnostic.Range.StartLine, diagnostic.Range.StartCharacter, diagnostic.Range.EndLine, diagnostic.Range.EndCharacter),
                [],
                [],
    ...
    private BuildIdentity CreateIdentity(string referenceSetId) => new(settings.Identity.ReleaseId, "fsharp", settings.Identity.ToolchainId, settings.Identity.CompilerVersion, settings.Identity.CompilerCommit, referenceSetId, settings.Identity.WorkerImageId);

    private static FSharpCompiledArtifact CreateArtifact(byte[] pe, byte[] pdb, byte[] fsharpCore, LoadedFSharpReferenceSet referenceSet, BuildIdentity identity, BuildOutputKind outputKind, IReadOnlyList<ValidatedFSharpWorkspaceFile> sourceOrder)
    {
        var files = new List<ArtifactFileDescriptor> { new("primary-assembly", $"{AssemblyName}.dll", pe.LongLength, ContentIdentity.Compute(pe).Value) };
        if (pdb.Length > 0)
            files.Add(new ArtifactFileDescriptor("portable-pdb", $"{AssemblyName}.pdb", pdb.LongLength, ContentIdentity.Compute(pdb).Value));
        files.Add(new ArtifactFileDescriptor("support-assembly", "FSharp.Core.dll", fsharpCore.LongLength, ContentIdentity.Compute(fsharpCore).Value));
        ...
        var manifest = ArtifactIdentity.WithComputedId(new ArtifactManifest(
            ...
            new ArtifactRuntimeRequirement("coreclr", [new FrameworkRequirement("Microsoft.NETCore.App", referenceSet.Definition.FrameworkVersion)], "anycpu", []),
            [],
            outputKind,
            $"{AssemblyName}.dll",
            null,
        ...
        return new FSharpCompiledArtifact(manifest.ArtifactId, manifest.ArtifactFormat, AssemblyName, referenceSet.Definition.Id, referenceSet.Definition.TargetFramework, pe, pdb, fsharpCore, manifest, files, identity);
    }

    private static AstNode ConvertAstNode(FSharpAstNode node) => new(node.Kind, new TextRange(node.Range.StartLine, node.Range.StartCharacter, node.Range.EndLine, node.Range.EndCharacter), null, node.Properties.ToDictionary(static pair => pair.Key, static pair => (string?)pair.Value, StringComparer.Ordinal), node.Children.Select(ConvertAstNode).ToArray());
