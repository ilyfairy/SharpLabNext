...
internal sealed class ConstGenericsArtifactMaterializer(
...
{
    public async Task<MaterializedConstGenericsArtifact> MaterializeAsync(
        ArtifactRef artifactRef,
        string operationId,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            bundle = await storeClient.GetArtifactAsync(artifactRef, cancellationToken).ConfigureAwait(false)
                ?? throw new ArtifactWorkerArtifactNotFoundException("The requested artifact was not found.");
        }
        ...
        try
        {
            var lease = await storeClient.AcquireLeaseAsync(
                artifactRef,
                $"artifacts-const-generics:{operationId}",
            ...
            foreach (var entry in bundle.Entries)
            {
                ...
                if (portablePdbPath is null &&
                    (string.Equals(entry.Role, "portable-pdb", StringComparison.Ordinal) ||
                     string.Equals(Path.GetExtension(entry.Path), ".pdb", StringComparison.OrdinalIgnoreCase)))
                {
            ...
            return new MaterializedConstGenericsArtifact(
                artifactRef,
                root,
                assemblyPath,
                portablePdbPath,
    ...
    internal void ValidateBundle(ArtifactRef requestedRef, ArtifactBundleDescriptor bundle)
    {
        ...
        if (!string.Equals(manifest.Producer.ToolchainId, "roslyn-const-generics", StringComparison.Ordinal) ||
            !string.Equals(manifest.ReferenceSetId, "const-generics-ref", StringComparison.Ordinal) ||
            !string.Equals(manifest.TargetFramework, "net9.0", StringComparison.Ordinal))
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "The artifact was not produced by the approved ConstGenerics toolchain and reference set.");
        }
        if (!string.Equals(manifest.RuntimeRequirement.Family, "coreclr-const-generics", StringComparison.Ordinal) ||
            !string.Equals(manifest.RuntimeRequirement.Architecture, "anycpu", StringComparison.Ordinal) ||
            !Exact(manifest.RuntimeRequirement.RequiredRuntimeFeatureTags, ConstGenericsProcessorProtocol.RuntimeFeatureTag))
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "The artifact runtime requirement is not compatible with the ConstGenerics profile.");
        }
        if (manifest.RuntimeRequirement.Frameworks.Count != 1 ||
            !string.Equals(manifest.RuntimeRequirement.Frameworks[0].Name, "Microsoft.NETCore.App", StringComparison.Ordinal) ||
            !string.Equals(
        ...
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "The artifact framework requirement is not compatible with the ConstGenerics profile.");
        }
        if (!Exact(manifest.MetadataFeatureTags, ConstGenericsProcessorProtocol.MetadataFeatureTag) ||
            manifest.Metadata is null ||
            !manifest.Metadata.TryGetValue("compatibilityGroup", out var compatibilityGroup) ||
            !string.Equals(
                compatibilityGroup,
        ...
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "The artifact does not carry the approved ConstGenerics metadata compatibility identity.");
        }
        if (manifest.Files.Count is 0 || manifest.Files.Count > settings.MaximumArtifactFiles ||
            bundle.Entries.Count != manifest.Files.Count)
        {
        ...
        foreach (var entry in bundle.Entries)
        {
            if (!files.TryGetValue(entry.Path, out var file) ||
                file.Size != entry.Size ||
                !string.Equals(file.Digest, entry.Digest, StringComparison.Ordinal) ||
                !string.Equals(entry.Digest, entry.ContentRef.Value, StringComparison.Ordinal) ||
                !string.Equals(file.Role, entry.Role, StringComparison.Ordinal))
            {
                throw new ArtifactWorkerIncompatibleArtifactException(
                    "The artifact bundle does not match its manifest.");
            }
        ...
        var assembly = manifest.Files.FirstOrDefault(file =>
            string.Equals(file.Path, manifest.EntryAssembly, StringComparison.Ordinal));
        if (assembly is null || assembly.Size is <= 0 || assembly.Size > settings.MaximumAssemblyBytes ||
            Path.GetExtension(assembly.Path).ToLowerInvariant() is not (".dll" or ".exe"))
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
                "The artifact entry assembly is invalid or exceeds its limit.");
        }
        if (manifest.Files.Any(file =>
                string.Equals(Path.GetExtension(file.Path), ".pdb", StringComparison.OrdinalIgnoreCase) &&
                file.Size > settings.MaximumPortablePdbBytes))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (total != entry.Size || total != manifestFile.Size ||
            ...
            !string.Equals(entry.Digest, manifestFile.Digest, StringComparison.Ordinal))
        {
            throw new ArtifactWorkerIncompatibleArtifactException(
...
internal static class ConstGenericsTemporaryDirectory
{
    public static string Create(string configuredRoot, string operationId)
    {
        ...
        if (operationId.Length is 0 or > 128 ||
            !operationId.All(static value => char.IsAsciiLetterOrDigit(value) || value is '_' or '-'))
        {
