...
{
    public static Task<ConstGenericsProcessorResponse> ExecuteAsync(
    ...
    public static ConstGenericsProcessorResponse ToFailureResponse(
        Exception exception,
        ConstGenericsProcessorOperation operation)
    {
        var (processorId, processorVersion) = operation == ConstGenericsProcessorOperation.Verify
            ? ("ilverification-const-generics", ConstGenericsProcessorProtocol.VerificationProcessorVersion)
            : (operation == ConstGenericsProcessorOperation.DecompiledCSharp
                ? "ilspy-const-generics-csharp"
                : "ilspy-const-generics-il", ConstGenericsProcessorProtocol.IlSpyProcessorVersion);
        return exception switch
        {
            ...
            processorVersion,
            "text/plain",
            publicMessage: "Artifact processing exceeded its configured limit.",
            truncated: true),
            BadImageFormatException or InvalidDataException => Response(
            ConstGenericsProcessorOutcome.InvalidArtifact,
            processorId,
            processorVersion,
            "text/plain",
            publicMessage: "The managed PE or metadata is invalid."),
            ...
            truncated: true),
            _ => Response(
            ConstGenericsProcessorOutcome.Failed,
            processorId,
            processorVersion,
            "text/plain",
            publicMessage: "Artifact processing failed inside the isolated processor.")
        };
    }

    private static async Task<ConstGenericsProcessorResponse> RenderIlAsync(
        ConstGenericsProcessorRequest request,
        CancellationToken cancellationToken)
    {
    ...
    private static async Task<ConstGenericsProcessorResponse> RenderCSharpAsync(
        ConstGenericsProcessorRequest request,
        CancellationToken cancellationToken)
    {
        using var resolver = new BoundedAssemblyResolver(
        ...
        var decompiler = new CSharpDecompiler(
            ...
            resolver,
            CreateCSharpDecompilerSettings(request.IncludeCompilerGeneratedMembers))
        {
    ...
    internal static ConstGenericsProcessorResponse Response(
        ConstGenericsProcessorOutcome outcome,
        string processorId,
        string processorVersion,
        string mediaType,
        long outputCharacters = 0,
        IReadOnlyList<ConstGenericsProcessorLinkedRange>? linkedRanges = null,
        IReadOnlyList<ConstGenericsProcessorFinding>? findings = null,
        bool truncated = false,
        string? publicMessage = null) => new(
    ...
    private static void Validate(ConstGenericsProcessorRequest request)
    {
        ...
        if (request.ReferenceRoots.Count is 0 or > 4 || request.ReferenceRoots.Any(path =>
                !Path.IsPathFullyQualified(path) || !Directory.Exists(path)))
        {
            throw new InvalidDataException("A reference root is invalid.");
        }
        if (request.MaxCharacters is <= 0 or > 8_000_000 ||
            request.MaxFindings is <= 0 or > ConstGenericsProcessorProtocol.MaximumFindings)
        {
            throw new ProcessorLimitExceededException();
        }
