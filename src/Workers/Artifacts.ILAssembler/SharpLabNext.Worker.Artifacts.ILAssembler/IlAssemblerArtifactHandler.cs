...
internal sealed class IlAssemblerArtifactHandler(
    CilArtifactReader reader,
    IlCompilerProcessRunner compiler,
    ManagedPeArtifactPublisher publisher,
    IArtifactStoreClient storeClient,
    IlAssemblerWorkerSettings settings,
    ArtifactWorkerCapabilityManifest capabilityManifest)
    : IArtifactTransformHandler, IArtifactRenderHandler
{
    public string TransformId => "assemble-il";

    public string OutputId => "generated-il";

    public async Task<ArtifactWorkerJobExecution> TransformAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (request.Options.RewriterProfileId is not null)
        {
            throw new ArtifactWorkerRequestException(
                "invalid-request",
                "assemble-il does not accept a rewriter profile.");
        }

        await using var source = await reader.ReadAsync(
            request.ArtifactRef,
            operationId,
            cancellationToken).ConfigureAwait(false);
        var compilation = await compiler.AssembleAsync(
            source,
            source.Manifest.OutputKind,
            cancellationToken).ConfigureAwait(false);
            ...
            if (diagnostics.Length == 0)
            {
                ...
                [
                    Diagnostic(
                        ...
                        "The isolated IL assembler rejected the generated CIL.",
                        source.EntryPath,
                        null)
                ];
            }
            return new ArtifactWorkerJobExecution(new TransformArtifactResult(
        ...
        return new ArtifactWorkerJobExecution(
            new TransformArtifactResult(
                ...
                diagnostics,
                ProcessorIdentity()),
            Artifact: new ArtifactWorkerProducedArtifact(
                published.ArtifactRef,
                "dotnet-managed-pe-v1",
                "assembled-managed-pe"));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var maximumCharacters = Math.Min(
            ...
            capabilityManifest.Limits.MaximumInputArtifactBytes);
        if (source.SourceText.Length > maximumCharacters)
        {
            return new ArtifactWorkerJobExecution(new RenderArtifactResult(
                ...
                [],
                [Diagnostic(
                    "generated-il-limit-exceeded",
                    DiagnosticSeverity.Error,
                    "Generated IL exceeds the requested character limit.",
                    source.EntryPath,
                    null)],
                ProcessorIdentity()));
        }
        ...
        catch (Exception exception) when (exception is ArtifactStoreHttpException or HttpRequestException or TaskCanceledException)
        {
            throw new ArtifactWorkerDependencyUnavailableException(
                "Generated IL could not be published to the Artifact Store.",
                exception);
        }
        if (stored.ContentRef != contentRef || stored.Size != source.Utf8Content.LongLength)
        {
            throw new ArtifactWorkerDependencyUnavailableException(
                "Artifact Store returned an unexpected generated IL content identity.");
        }
        return new ArtifactWorkerJobExecution(
            new RenderArtifactResult(
                ...
                [],
                ProcessorIdentity()),
            new ArtifactWorkerProducedContent(
                stored.ContentRef,
                "text/plain; charset=utf-8",
                stored.Size));
    }

    private ArtifactProcessorIdentity ProcessorIdentity() => new(
        settings.ReleaseId,
        capabilityManifest.WorkerId,
        settings.CompilerVersion,
        settings.WorkerImageId);

    private static Diagnostic[] MapDiagnostics(
        IReadOnlyList<IlCompilerDiagnostic> diagnostics,
        ValidatedCilArtifact source)
    {
        var lines = source.SourceText
