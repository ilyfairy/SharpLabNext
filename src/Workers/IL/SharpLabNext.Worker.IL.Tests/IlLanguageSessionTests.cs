...
public sealed class IlLanguageSessionTests
{
    ...
    public async Task SessionTracksVersionsAndClosesDeterministically()
    {
        ...
        try
        {
            ...
            using var manager = new IlLanguageSessionManager(
                new IlReferenceSetProvider(settings.ReferenceSets),
                new IlLanguageService(),
                settings);
            ...
            var instructionLine = Array.FindIndex(
                sourceLines,
                static line => line.Contains("call void Helper::Ping()", StringComparison.Ordinal));
            var instructionCharacter = sourceLines[instructionLine].IndexOf("call", StringComparison.Ordinal);
            var completion = await session.GetCompletionsAsync(
                new IlLspCompletionParams(
                    new IlLspTextDocumentIdentifier(uri),
                    new IlLspPosition(instructionLine, instructionCharacter),
                    null),
            ...
            await session.DidChangeAsync(
                new IlLspDidChangeParams(
                    new IlLspVersionedTextDocumentIdentifier(uri, 2),
                    [new IlLspTextChange(null, null, invalid)]),
                TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            var request = new OpenLanguageSessionRequest(
                ...
                IlTestSettings.CreateRequest(
                    ...
                    referenceSetId: "net11-preview-ref").Workspace);
            var descriptor = await manager.OpenAsync(request, TestContext.Current.CancellationToken);
            Assert.True(manager.TryGet(descriptor.SessionId, out var session));
            Assert.NotNull(session);
            const string uri = "sharplabnext:///Program.il";
            var workspace = await session.GetCompletionsAsync(
                new IlLspCompletionParams(
                    new IlLspTextDocumentIdentifier(uri),
                    new IlLspPosition(0, 0),
                    null),
                TestContext.Current.CancellationToken);
            ...
            await session.DidChangeAsync(
                new IlLspDidChangeParams(
                    new IlLspVersionedTextDocumentIdentifier(uri, 2),
                    [new IlLspTextChange(null, null, assemblyPrefix)]),
                TestContext.Current.CancellationToken);
            var references = await session.GetCompletionsAsync(
                new IlLspCompletionParams(
                    new IlLspTextDocumentIdentifier(uri),
                    new IlLspPosition(6, assemblyPrefixLines[6].Length),
                    null),
                TestContext.Current.CancellationToken);
            Assert.Contains(
                references.Items,
                static item => item.Label == "System.Runtime" && item.TextEdit.NewText == "System.Runtime]");
            ...
            await session.DidChangeAsync(
                new IlLspDidChangeParams(
                    new IlLspVersionedTextDocumentIdentifier(uri, 3),
                    [new IlLspTextChange(null, null, runtimeApiPrefix)]),
                TestContext.Current.CancellationToken);
            var runtimeApi = await session.GetCompletionsAsync(
                new IlLspCompletionParams(
                    new IlLspTextDocumentIdentifier(uri),
                    new IlLspPosition(6, runtimeApiPrefixLines[6].Length),
                    null),
                TestContext.Current.CancellationToken);
            Assert.Contains(
                runtimeApi.Items,
                static item => item.Label == "SharpLab.Runtime" && item.TextEdit.NewText == "SharpLab.Runtime]");
        }
        finally
        {
            IlTestSettings.DeleteRoot(root);
    ...
    public async Task SessionCountAndDocumentSizeLimitsAreEnforced()
    {
        ...
        try
        {
            ...
            {
                CompilationLimits = original.CompilationLimits with { MaxFileUtf8Bytes = 64, MaxTotalSourceUtf8Bytes = 128 },
                LspLimits = original.LspLimits with { MaxSessions = 1 }
            };
            using var manager = new IlLanguageSessionManager(
                new IlReferenceSetProvider(settings.ReferenceSets),
                new IlLanguageService(),
            ...
            var workspace = IlTestSettings.CreateRequest(
                BuildTarget.CompileCheck,
                [new WorkspaceFile("Program.il", 1, source)],
                ["Program.il"]).Workspace;
            var request = new OpenLanguageSessionRequest(
            ...
            Assert.NotNull(session);
            await Assert.ThrowsAsync<IlLspLimitExceededException>(() =>
                session.DidChangeAsync(
                    new IlLspDidChangeParams(
                        new IlLspVersionedTextDocumentIdentifier("sharplabnext:///Program.il", 2),
                        [new IlLspTextChange(null, null, new string('x', 65))]),
                    TestContext.Current.CancellationToken));
        }
        finally
        {
