...
public sealed class IlWorkerEndpointTests
{
    ...
    {
        using var message = JsonDocument.Parse(SerializeLspMessage(new
        {
            jsonrpc = "2.0",
            id = 1,
            method = "initialize",
    ...
    public async Task WebSocketImplementsDiagnosticsCompletionHoverSignatureHelpTokensSymbolsAndFolding()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                ...
                capabilities.GetProperty("codeActionProvider").GetProperty("codeActionKinds")
                    .EnumerateArray().Select(static value => value.GetString()!).ToArray());
            string[] completionTriggerCharacters =
            [
                .. capabilities.GetProperty("completionProvider").GetProperty("triggerCharacters")
            ...
            [
                .. capabilities.GetProperty("signatureHelpProvider").GetProperty("triggerCharacters")
                    .EnumerateArray().Select(static value => value.GetString()!)
            ];
            ...
            [
                .. capabilities.GetProperty("semanticTokensProvider").GetProperty("legend").GetProperty("tokenModifiers")
                    .EnumerateArray().Select(static value => value.GetString()!)
            ];
            ...
            using var diagnostics = await ReceiveUntilAsync(socket, static rootElement =>
                rootElement.TryGetProperty("method", out var method) && method.GetString() == "textDocument/publishDiagnostics");
            Assert.Contains(
                diagnostics.RootElement.GetProperty("params").GetProperty("diagnostics").EnumerateArray(),
                static item => item.GetProperty("code").GetString() == "ILPAR202" &&
                    item.GetProperty("source").GetString() == "Parse" &&
                    item.GetProperty("data").GetProperty("diagnosticKind").GetString() == "Parse");
            ...
            var ldstrCompletion = Assert.Single(
                completion.RootElement.GetProperty("result").GetProperty("items").EnumerateArray(),
                static item => item.GetProperty("label").GetString() == "ldstr");
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new { textDocument = new { uri }, position = new { line = 6, character = opcodeCharacter } }
            });
            ...
            await SendAsync(socket, new
            {
                ...
                {
                    textDocument = new { uri },
                    position = new { line = 8, character = signatureLine.IndexOf('(') + 1 }
                }
            });
            ...
            Assert.Contains(
                signatureResult.GetProperty("signatures").EnumerateArray(),
                static item => item.GetProperty("label").GetString()!.Contains("::Pick(", StringComparison.Ordinal));
            ...
            await SendAsync(socket, new
            {
                ...
                {
                    textDocument = new { uri },
                    position = new { line = 1, character = typeLine.IndexOf("Program", StringComparison.Ordinal) + 1 }
                }
            });
            ...
            Assert.Equal(1, definition.RootElement.GetProperty("result").GetProperty("range")
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new { query = "Program" }
            });
            using var workspaceSymbols = await ReceiveUntilAsync(socket, static rootElement => HasId(rootElement, 9));
            var programSymbol = Assert.Single(
                workspaceSymbols.RootElement.GetProperty("result").EnumerateArray(),
                static item => item.GetProperty("name").GetString() == "Program");
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new
                {
                    ...
                    range = new
                    {
                        ...
                        end = new { line = sourceLines.Length - 1, character = sourceLines[^1].Length }
                    },
            ...
            var missingLabel = Assert.Single(
                codeActions.RootElement.GetProperty("result").EnumerateArray(),
                static action => action.GetProperty("data").TryGetProperty("diagnostic", out var diagnostic) &&
                    diagnostic.GetString() == "ILBIND201");
            ...
            Assert.Contains(
                "missing:",
                Assert.Single(missingLabel.GetProperty("edit").GetProperty("changes").GetProperty(uri).EnumerateArray())
    ...
    public async Task WebSocketUsesTheSessionWorkspaceForHoverAndSemanticTokensWithoutDidOpen()
    {
        ...
        try
        {
            ...
            await SendAsync(socket, new
            {
                ...
                {
                    textDocument = new { uri },
                    position = new { line = opcodeLine, character = opcodeCharacter }
                }
            });
            using var hover = await ReceiveUntilWithoutDiagnosticsAsync(
                socket,
                static rootElement => HasId(rootElement, 2));
            Assert.Contains(
                "ECMA-335 opcode 'ldstr'",
                hover.RootElement.GetProperty("result").GetProperty("contents").GetProperty("value").GetString(),
                StringComparison.Ordinal);
            ...
            using var semanticTokens = await ReceiveUntilWithoutDiagnosticsAsync(
                socket,
                static rootElement => HasId(rootElement, 3));
            Assert.NotEmpty(
                semanticTokens.RootElement.GetProperty("result").GetProperty("data").EnumerateArray());
    ...
    public async Task WebSocketReadsCancellationBehindAWorkspaceChangeWhileLanguageRequestsAreInFlight()
    {
        ...
        try
        {
            ...
            using var cancelled = await ReceiveUntilAsync(socket, static message =>
                    HasId(message, 17) &&
                    message.TryGetProperty("error", out var error) &&
                    error.GetProperty("code").GetInt32() == -32800)
    ...
    public async Task WebSocketCoalescesRapidChangesAndPublishesOnlyTheLatestDiagnosticsVersion()
    {
        ...
        try
        {
            ...
            await SendAsync(socket, new
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                @params = new { textDocument = new { uri, languageId = "il", version = 1, text = initial } }
            });
            ...
            Assert.Contains(
                finalDiagnostics.EnumerateArray(),
                static item => item.GetProperty("code").GetString() == "ILPAR202" &&
                    item.GetProperty("data").GetProperty("documentVersion").GetInt64() == 3);
        }
        finally
        {
            IlTestSettings.DeleteRoot(root);
    ...
    public async Task WebSocketRefreshesEveryOpenDocumentAndClearsClosedDiagnostics()
    {
        ...
        try
        {
            ...
            using var opened = await client.PostAsJsonAsync(
                "/api/v1/language-sessions",
                CreateOpenRequest(files, ["Program.il", "Helper.il"]),
                JsonOptions,
                TestContext.Current.CancellationToken);
            var session = await opened.Content.ReadFromJsonAsync<LanguageSession>(
                JsonOptions,
            ...
            using var initialReport = await ReceiveUntilAsync(socket, static message =>
                IsDiagnosticsFor(message, programUri, out _));
            var initialRevision = initialReport.RootElement
                .GetProperty("params").GetProperty("diagnostics")[0]
                .GetProperty("data").GetProperty("workspaceRevision").GetInt64();
            ...
            for (var attempt = 0; attempt < 20 && (!sawHelper || refreshedProgramRevision <= initialRevision); attempt++)
            {
                ...
                if (!IsDiagnosticsFor(message.RootElement, programUri, out var parameters))
                    continue;
                refreshedProgramRevision = parameters.GetProperty("diagnostics")[0]
                    .GetProperty("data").GetProperty("workspaceRevision").GetInt64();
            }
            ...
            await SendAsync(socket, new
            {
                ...
                @params = new { textDocument = new { uri = programUri } }
            });
            using var cleared = await ReceiveUntilAsync(socket, static message =>
                IsDiagnosticsFor(message, programUri, out var parameters) &&
                !parameters.GetProperty("diagnostics").EnumerateArray().Any());
    ...
    public async Task WebSocketClosesMessagesThatExceedTheConfiguredUtf8ByteLimit()
    {
        ...
        try
        {
            ...
            var receive = await socket.ReceiveAsync(
                new ArraySegment<byte>(new byte[128]),
                TestContext.Current.CancellationToken);
    ...
    private static async Task AssertNonEmptyArrayResultAsync(
        WebSocket socket,
        int id,
        string method,
        object parameters,
    ...
    private sealed class IlWorkerFactory(
        string root,
        int? maxMessageBytes = null,
        int? diagnosticsDebounceMilliseconds = null) : WebApplicationFactory<Program>
    {
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureTestServices(services =>
            {
                ...
                {
                    DevelopmentArtifactEnvelope = new IlDevelopmentArtifactEnvelopeOptions(true, 16 * 1024 * 1024),
                    LspLimits = settings.LspLimits with
                    {
                        DiagnosticsDebounceMilliseconds = diagnosticsDebounceMilliseconds ?? 1,
                        MaxMessageBytes = maxMessageBytes ?? settings.LspLimits.MaxMessageBytes
                    }
                };
    ...
    private static OpenLanguageSessionRequest CreateOpenRequest(string source) =>
        CreateOpenRequest([new WorkspaceFile("Program.il", 1, source)], ["Program.il"]);
    ...
    private static byte[] SerializeLspMessage(object payload) =>
        JsonSerializer.SerializeToUtf8Bytes(payload, LspJsonOptions);
    ...
    private static Task SendDidOpenAsync(WebSocket socket, string uri, string text) =>
        SendAsync(socket, new
        {
            jsonrpc = "2.0",
            method = "textDocument/didOpen",
            @params = new { textDocument = new { uri, languageId = "il", version = 1, text } }
        });
    ...
    private static async Task<JsonDocument> ReceiveUntilWithoutDiagnosticsAsync(
        WebSocket socket,
        Func<JsonElement, bool> predicate)
    {
        for (var attempt = 0; attempt < 20; attempt++)
        {
            ...
            Assert.False(
                ...
                method.GetString() == "textDocument/publishDiagnostics",
                "A read-only IL result session published diagnostics without didOpen.");
            if (predicate(document.RootElement))
                return document;
            document.Dispose();
        }
