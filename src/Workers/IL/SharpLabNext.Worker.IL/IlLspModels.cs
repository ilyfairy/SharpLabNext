...
public sealed record IlLspDidChangeParams(IlLspVersionedTextDocumentIdentifier TextDocument, IReadOnlyList<IlLspTextChange> ContentChanges);
public sealed record IlLspDidCloseParams(IlLspTextDocumentIdentifier TextDocument);
public sealed record IlLspTextDocumentPositionParams(IlLspTextDocumentIdentifier TextDocument, IlLspPosition Position);
public sealed record IlLspWorkspaceSymbolParams(string Query);
public sealed record IlLspCompletionParams(IlLspTextDocumentIdentifier TextDocument, IlLspPosition Position, IlLspCompletionContext? Context);
...
public sealed record IlLspSignatureInformation(string Label, IlLspMarkupContent? Documentation, IReadOnlyList<IlLspParameterInformation> Parameters);
public sealed record IlLspSignatureHelp(IReadOnlyList<IlLspSignatureInformation> Signatures, int ActiveSignature, int ActiveParameter);
...
public sealed record IlLspDocumentSymbol(string Name, string? Detail, int Kind, IlLspRange Range, IlLspRange SelectionRange, IReadOnlyList<IlLspDocumentSymbol> Children);
public sealed record IlLspCodeActionParams(IlLspTextDocumentIdentifier TextDocument, IlLspRange Range, IlLspCodeActionContext Context);
public sealed record IlLspCodeActionContext(IReadOnlyList<IlLspDiagnostic>? Diagnostics, IReadOnlyList<string>? Only, int? TriggerKind = null);
public sealed record IlLspCodeAction(string Title, string Kind, IReadOnlyList<IlLspDiagnostic>? Diagnostics, bool? IsPreferred, IlLspWorkspaceEdit Edit, IlLspCodeActionData Data);
public sealed record IlLspWorkspaceEdit(IReadOnlyDictionary<string, IReadOnlyList<IlLspTextEdit>> Changes);
public sealed record IlLspCodeActionData(string Id, long DocumentVersion, long WorkspaceRevision, string? Diagnostic);
public sealed record IlLspWorkspaceSymbol(string Name, int Kind, IlLspLocation Location, string? ContainerName, IlLspWorkspaceSymbolData Data);
public sealed record IlLspWorkspaceSymbolData(string Id, string Detail, long WorkspaceRevision);
...
public sealed record IlLspDiagnostic(IlLspRange Range, int Severity, string Code, string Source, string Message, IReadOnlyList<int> Tags, IReadOnlyList<IlLspDiagnosticRelatedInformation> RelatedInformation, IlLspDiagnosticData Data);
...
public sealed record IlLspDiagnosticData(long WorkspaceRevision, long SelectionRevision, long DocumentVersion, string DiagnosticKind, long ReferenceRevision);
public sealed record IlLspDiagnosticsReport(string Uri, long Version, long WorkspaceRevision, long SelectionRevision, IReadOnlyList<IlLspDiagnostic> Diagnostics);
public sealed record IlLspDocumentState(string Uri, string Path, long Version, long WorkspaceRevision, long SelectionRevision);
