...
const commonPromotionOutputs = Object.freeze([
  'deploy/images.json',
...
export const runtimePromotionBatchUsage = `Usage:
  node eng/runtime-promotion-batch.mjs init --batch-id ID --producer-root PATH --aggregate-root PATH
  node eng/runtime-promotion-batch.mjs escrow --batch-id ID --profile-id ID --producer-root PATH --aggregate-root PATH
  node eng/runtime-promotion-batch.mjs import-promote --batch-id ID --profile-id ID --aggregate-root PATH
  node eng/runtime-promotion-batch.mjs status --batch-id ID --aggregate-root PATH
  node eng/runtime-promotion-batch.mjs verify-complete --batch-id ID --aggregate-root PATH`
...
function fail(message, options) {
  throw new RuntimePromotionBatchError(message, options)
}
...
function batchParent(aggregateRoot) {
  return path.join(aggregateRoot, '.tmp', 'runtime-promotion-batches')
}

function batchPath(aggregateRoot, batchId) {
  return path.join(batchParent(aggregateRoot), safeId(batchId, 'batch ID'))
}
...
function escrowRoot(batchRoot, profileId) {
  return path.join(batchRoot, 'escrow', safeId(profileId, 'profile ID'))
}
...
function defaultPromotionRunner({ repositoryRoot, profileId, check }) {
  return promoteRuntimeMatrix({ repositoryRoot, profileId, check })
}
