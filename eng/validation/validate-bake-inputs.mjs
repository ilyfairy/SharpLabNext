...
function validateBake(source, candidateSource) {
  ...
  for (const input of [
    ...
    'WINE_CORECLR_LOCAL_OPERATOR_IMAGE',
    'WINE_CORECLR_LOCAL_OPERATOR_TAG',
  ]) {
  ...
  for (const [targetName, dockerfile, markers] of [
    ...
    ['runtime-wine-dotnet-matrix-candidate', 'deploy/docker/Dockerfile.runtime-wine-dotnet-matrix', [
      'SDK_IMAGE = BASE_DOTNET_SDK_IMAGE',
      'WINE_IMAGE = WINE_CORECLR_LOCAL_OPERATOR_TAG != "" ? WINE_CORECLR_LOCAL_OPERATOR_TAG : RUNTIME_MATRIX_WINE_IMAGE',
    ...
    if (targetName === 'runtime-mono-matrix-candidate' ||
      ...
      if (targetName === 'runtime-wine-framework-matrix-shared-candidate') {
        for (const requiredText of [
          ...
          'FRAMEWORK_ROW_OPERATOR_IMAGE = RUNTIME_MATRIX_FRAMEWORK_ROW_OPERATOR_IMAGE',
          'FRAMEWORK_ROW_DIGEST = RUNTIME_MATRIX_FRAMEWORK_ROW_DIGEST',
          'HISTORICAL_FRAMEWORK_INPUT = RUNTIME_MATRIX_HISTORICAL_FRAMEWORK_INPUT',
...
function validateCandidateBakeBoundary(productionSource, candidateSource, candidateTargets) {
  ...
  const expectedVariables = new Set([
    ...
    'RUNTIME_MATRIX_HISTORICAL_FRAMEWORK_INPUT',
    'WINE_CORECLR_OPERATOR_RECEIPT_SHA256',
    'WINE_CORECLR_OPERATOR_RECEIPT_KEY_ID',
    'WINE_CORECLR_OPERATOR_REFERENCE',
    'WINE_CORECLR_LOCAL_OPERATOR_IMAGE',
...
function validateDockerfiles() {
  ...
  for (const requiredText of [
    ...
    '"${OPERATOR_SOURCE_CONTEXT}:${OPERATOR_PROMOTION_ELIGIBLE}" = "committed:true"',
    '"${OPERATOR_SOURCE_CONTEXT}:${OPERATOR_PROMOTION_ELIGIBLE}" = "working-tree-content:false"',
  ...
  const candidateRuntimeChecks = new Map([
    ...
    ['Dockerfile.runtime-wine-framework-matrix-shared', [
      ...
      'ARG HISTORICAL_FRAMEWORK_INPUT',
      ...
      'test "${HISTORICAL_FRAMEWORK_INPUT}" = true || test "${HISTORICAL_FRAMEWORK_INPUT}" = false',
      'test "${FRAMEWORK_SOURCE_REVISION}" != "${SOURCE_REVISION}"',
      'test "${FRAMEWORK_SOURCE_REVISION}" = "${SOURCE_REVISION}"',
      '"${CANDIDATE_SOURCE_CONTEXT}:${CANDIDATE_PROMOTION_ELIGIBLE}:${HISTORICAL_FRAMEWORK_INPUT}" = "committed:true:false"',
      '"${CANDIDATE_SOURCE_CONTEXT}:${CANDIDATE_PROMOTION_ELIGIBLE}:${HISTORICAL_FRAMEWORK_INPUT}" = "working-tree-content:false:false"',
      '"${CANDIDATE_SOURCE_CONTEXT}:${CANDIDATE_PROMOTION_ELIGIBLE}:${HISTORICAL_FRAMEWORK_INPUT}" = "committed-historical-framework-input:false:true"',
      '"${CANDIDATE_SOURCE_CONTEXT}:${CANDIDATE_PROMOTION_ELIGIBLE}:${HISTORICAL_FRAMEWORK_INPUT}" = "working-tree-historical-framework-input:false:true"',
    ...
    if (sourceStatusDockerfiles.has(fileName)) {
      const sourceContextChecks = fileName === 'Dockerfile.runtime-wine-framework-matrix-shared'
        ...
        : [
            ...
            '"${CANDIDATE_SOURCE_CONTEXT}:${CANDIDATE_PROMOTION_ELIGIBLE}" = "working-tree-content:false"',
            'io.sharplabnext.source.context="${CANDIDATE_SOURCE_CONTEXT}"',
            'com.sharplabnext.runtime-candidate.promotion-eligible="${CANDIDATE_PROMOTION_ELIGIBLE}"',
          ]
