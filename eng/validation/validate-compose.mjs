...
const catalog = JSON.parse(fs.readFileSync(path.join(repositoryRoot, 'profiles/catalog/catalog.json'), 'utf8'))
const deploymentImages = JSON.parse(fs.readFileSync(path.join(repositoryRoot, 'deploy/images.json'), 'utf8')).images
...
for (const relativeFile of files) {
  ...
  const result = spawnSync('docker', ['compose', '-f', relativeFile, 'config', '--format', 'json'], { cwd: repositoryRoot, encoding: 'utf8', shell: false })
...
function validateDefaultGitHubOAuthSecretPlaceholder(config, fileName) {
  ...
  const placeholderPath = path.resolve(path.dirname(composePath), 'github-oauth-client-secret.disabled')
