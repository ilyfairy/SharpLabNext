...
for (let index = 2; index < process.argv.length; index += 1) {
  if (process.argv[index] !== '--release-lock' || index + 1 >= process.argv.length) {
    console.error('Usage: node eng/validation/validate-schemas.mjs [--release-lock PATH]')
    process.exit(64)
  }
...
if (fs.existsSync(promotionReceiptDirectory)) {
  for (const relativePath of collectJsonFiles(promotionReceiptDirectory, 'profiles/runtime-promotion-receipts')) {
...
if (fs.existsSync(promotionPlanDirectory)) {
  for (const relativePath of collectJsonFiles(promotionPlanDirectory, 'profiles/runtime-promotion-plans')) {
    documents.push([relativePath, relativePath.endsWith('.profile.json') ? 'runtime-profile.schema.json' : 'runtime-promotion-plan.schema.json'])
  }
}
...
if (fs.existsSync(promotionEvidenceDirectory)) {
  for (const relativePath of collectJsonFiles(promotionEvidenceDirectory, 'profiles/runtime-promotion-evidence')) {
    documents.push([relativePath, relativePath.endsWith('/performance.json') ? 'runtime-performance-evidence.schema.json' : 'runtime-capability-evidence.schema.json'])
  }
}
...
try {
  ...
  failures.push(...validateRuntimeProfileChannels(runtimeProfilePaths, catalogForChannels, relativePath => JSON.parse(fs.readFileSync(path.join(repositoryRoot, relativePath), 'utf8'))))
} catch (error) {
...
function inspectSchema(node, root, fileName, pointer) {
  ...
  if (typeof node.$ref === 'string' && node.$ref.startsWith('#/')) {
    const segments = node.$ref.slice(2).split('/').map(segment => segment.replaceAll('~1', '/').replaceAll('~0', '~'))
    let target = root
