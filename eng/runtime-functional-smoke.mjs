...
const probeProject = path.join(
  ...
  'SharpLabNext.RuntimeCapabilityProbe.csproj',
)
...
const supervisorSettingsPath = path.join(
  repositoryRoot,
  'src',
  'Supervisor',
...
export const runtimeFunctionalSmokeUsage = `Usage:
  node eng/runtime-functional-smoke.mjs --profile ID [--profile ID ...]
...
function fail(message, options) {
  throw new RuntimeFunctionalSmokeError(message, options)
}
...
function readSandbox() {
  ...
  const seccompPath = path.resolve(
    path.dirname(supervisorSettingsPath),
    sandbox.SeccompProfilePath ?? '',
  )
  ...
  return {
    ...
    openFilesSoftLimit: positiveSafeInteger(
      sandbox.OpenFilesSoftLimit,
      'Runtime Supervisor open-files soft limit',
    ),
    openFilesHardLimit: positiveSafeInteger(
      sandbox.OpenFilesHardLimit,
      'Runtime Supervisor open-files hard limit',
    ),
  }
}
...
function dockerArguments(profile, resultRow, mode, sandbox, outputPath, containerName) {
  ...
  const command = substituteCommand(
    profile.operations.run.command,
    `/artifact/${probeAssembly}`,
    [mode],
  )
  return [
...
function updateVerification(resultRow, profile, success, exception, sandbox, now) {
  ...
  const pending = Object.entries(updatedSmoke)
