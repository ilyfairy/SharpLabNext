...
{
    Console.WriteLine(
        "Usage: dotnet run eng/create-reference-set-attestation.cs -- " +
        "--root PATH --id ID --target-framework TFM --digest DIGEST " +
        "--provenance-kind KIND --resolved-version VERSION " +
        "[--package ID] [--source-uri URI] [--commit COMMIT] " +
        "[--source-archive-digest DIGEST]");
    return;
}
...
var document = new AttestationDocument(
    ...
    new ReferenceSet(
        ...
        new Provenance(
            provenanceKind,
            resolvedVersion,
            Optional(values, "package"),
            Optional(values, "source-uri"),
            Optional(values, "commit"),
            Optional(values, "source-archive-digest"))),
    files);
...
static string? Optional(IReadOnlyDictionary<string, string?> values, string key) =>
    values.TryGetValue(key, out var value) && !string.IsNullOrWhiteSpace(value) ? value : null;

sealed record AttestationDocument(
    int SchemaVersion,
