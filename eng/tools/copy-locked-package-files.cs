...
if (args.Length < 5 || (args.Length - 2) % 3 != 0)
{
    Console.Error.WriteLine(
        "Usage: dotnet run copy-locked-package-files.cs -- LOCK NUGET_ROOT " +
        "PACKAGE RELATIVE_PATH OUTPUT [PACKAGE RELATIVE_PATH OUTPUT ...]");
    return 64;
}
...
for (var index = 2; index < args.Length; index += 3)
{
    ...
    var versions = dependencies.EnumerateObject()
        ...
        .Select(static package => package.Value.GetProperty("resolved").GetString())
    ...
    if (!sourcePath.StartsWith(packageDirectory + Path.DirectorySeparatorChar, StringComparison.Ordinal) ||
        !File.Exists(sourcePath))
    {
        throw new FileNotFoundException(
            $"Locked package file '{packageId}/{versions[0]}/{relativePath}' is unavailable.",
            sourcePath);
    }
