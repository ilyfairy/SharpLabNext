...
function readParentManifest(parentImage, spawn) { return readImageJson(parentImage, '/opt/sharplabnext/framework-matrix.json', 'parent', spawn); }

function readMetadataMatrixInput(metadataImage, spawn) { return readImageJson(metadataImage, '/matrix-input.json', 'metadata', spawn); }
...
export function createFrameworkCandidateInputFromImages(options) {
  ...
  const matrixInput = normalizeCanonicalMatrixInput(readMetadataMatrixInput(metadataImage, spawn));
  const parentInputs = validateControlImages(parentImage, metadataImage, matrixInput, options.sourceRevision, spawn);
  ...
  const installerManifestSha256 = crypto.createHash('sha256').update(readRegularBytes(manifestPath, 'Framework installer manifest')).digest('hex');
  validateOperatorImages(rows, options.sourceRevision, parentInputs, installerManifestSha256, spawn);
  const value = {
    schemaVersion: 1,
...
export function createFrameworkCandidateInput(options) {
  const suppliedMatrixInput = normalizeCanonicalMatrixInput(readRegularJson(options?.matrixInput, 'matrix input'));
...
export function runCreateFrameworkCandidateInput(argv, options = {}) {
  ...
  try {
    ...
    const committedRuntimeMatrix = path.join(sourceContext.directory, 'profiles', 'runtime-matrix.json');
    ...
    const result = createFrameworkCandidateInput({
      ...
      installerManifest: path.join(sourceContext.directory, 'profiles', 'runtime-framework-installers.json'),
      spawn: options.spawn ?? spawnSync,
    });
