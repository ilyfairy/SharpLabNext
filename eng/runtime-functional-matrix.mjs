...
import { inspectDockerImage } from './runtime-promotion-image-binding.mjs'
...
export const runtimeFunctionalMatrixUsage = `Usage:
  node eng/runtime-functional-matrix.mjs [--output PATH]`
...
function fail(message, options) {
  throw new RuntimeFunctionalMatrixError(message, options)
}
...
function validateProfile(profile, profileId, candidateTarget, matrix) {
  ...
  const runtimeVersion = requiredString(
    ...
    `Candidate profile '${profileId}'.runtimeVersion`,
  )
...
function normalizeInspection(reference, inspection) {
  ...
  return {
    ...
    operatingSystem: requiredString(
      ...
      `Docker inspection for '${reference}'.operatingSystem`,
    ),
    architecture: requiredString(
      inspection.architecture,
      `Docker inspection for '${reference}'.architecture`,
    ),
...
export function refreshRuntimeFunctionalMatrix(options = {}) {
  ...
  for (const profileId of profileIds) {
    ...
    const { bytes, value: profile } = readBoundedJson(
      profilePath,
