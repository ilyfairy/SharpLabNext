...
export const runtimeJitSmokeUsage = `Usage:
  node eng/smoke/runtime-jit-smoke.mjs --profile ID [--profile ID ...] [--results PATH]`
...
function fail(message, options) { throw new RuntimeJitSmokeError(message, options); }
...
function validateJitFrames(frames, profile) {
  ...
  const assembly = Buffer.concat(frames.filter(frame => frame.kind === frameKinds.jitAssembly).map(frame => frame.payload)).toString('utf8')
