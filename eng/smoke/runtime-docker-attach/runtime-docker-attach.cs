...
var runtimeImage = args.Length > 0
    ? args[0]
    : Environment.GetEnvironmentVariable("SHARPLABNEXT_E2E_RUNTIME_IMAGE");
...
var supervisorOptions = Options.Create(new RuntimeSupervisorOptions
{
    DockerSocketPath = "/var/run/docker.sock"
});
using var docker = new DockerEngineClient(
    supervisorOptions,
    new RuntimeSandboxPolicy(supervisorOptions, new SmokeHostEnvironment(ResolveSupervisorRoot())));
...
try
{
    containerId = await docker.CreateContainerAsync(
        new RuntimeContainerSpec(
            ...
            {
                Id = "runtime-attach-smoke",
                MemoryBytes = 64 * 1024 * 1024,
                NanoCpus = 1_000_000_000,
                PidsLimit = 32,
                MaximumDurationSeconds = 30,
                MaximumArtifactBytes = 1024 * 1024,
                MaximumOutputBytes = PayloadBytes + AttachMarker.Length,
    ...
    Require(
        bytes.AsSpan(marker.Length).IndexOfAnyExcept((byte)'x') < 0,
        "Pre-start attach changed the Docker stdout payload.");
...
static string ResolveSupervisorRoot([CallerFilePath] string sourcePath = "")
{
    ...
    return Path.GetFullPath(Path.Combine(
        Path.GetDirectoryName(sourcePath)
            ?? throw new InvalidOperationException("Smoke source path is unavailable."),
        "..",
        "..",
        "..",
        "src",
