...
var baseAddress = args.Length == 0
    ...
    : new Uri(args[0], UriKind.Absolute);
...
await EnsureSuccessAsync(
    await http.GetAsync("/health/ready", overallTimeout.Token),
    "Gateway readiness");
...
using var catalog = JsonDocument.Parse(
    await catalogResponse.Content.ReadAsByteArrayAsync(overallTimeout.Token));
...
var catalogReleaseId = catalog.RootElement.GetProperty("ReleaseId").GetString()
    ?? throw new InvalidOperationException("Catalog release ID is missing.");
...
async Task VerifyRunFailureAsync(string mode, string expectedStatus)
{
    ...
    Require(
        terminal.State.GetProperty("Status").GetString() == "completed",
        $"Run operation ended as {terminal.State.GetProperty("Status").GetString()}.");
    ...
    Require(
        result.GetProperty("Status").GetString() == expectedStatus,
        $"Expected '{expectedStatus}', received '{result.GetProperty("Status").GetString()}'.");
    if (expectedStatus == "output-limit-exceeded")
    {
        ...
        Require(
            ...
            "Output-limit operation emitted no output-truncated event.");
    }
...
async Task VerifyCancellationAsync()
{
    ...
    using var cancelResponse = await http.PostAsJsonAsync(
        $"/api/v1/operations/{operationId}/cancel",
        new { operationId, reason = "runtime-failure-smoke" },
        json,
    ...
    var terminal = await WaitForTerminalAsync(operationId);
    Require(
        terminal.State.GetProperty("Status").GetString() == "cancelled",
        $"Cancelled run ended as {terminal.State.GetProperty("Status").GetString()}.");
    var result = SingleTypedResult(terminal.Events);
...
async Task VerifyReaperAsync()
{
    ...
    try
    {
        var volume = await RunDockerAsync(
            "volume", "create",
            "--label", "com.sharplabnext.runtime-job=workspace",
...
{
    var images = await RunDockerAsync(
        "image", "ls", "--no-trunc",
        "--filter", "label=com.sharplabnext.runtime-profile=dotnet-10-linux-x64",
        "--filter", $"label=org.opencontainers.image.version={releaseId}",
        "--format", "{{.ID}}");
    ...
    Require(
        imageIds.Count == 1,
        $"Expected one .NET 10 runtime image for release '{releaseId}', found {imageIds.Count}.");
...
async Task<string> BuildFailureHarnessAsync(JsonElement resolved)
{
    ...
    var referenceSetId = effective.GetProperty("ReferenceSetId").GetString()
        ?? throw new InvalidOperationException("Resolved reference set is missing.");
    ...
    var operationId = await StartOperationAsync(
        ...
        new
        {
            ...
            pipelineResolutionId = resolved.GetProperty("PipelineResolutionId").GetString(),
    ...
    Require(
        terminal.State.GetProperty("Status").GetString() == "completed",
        $"Failure harness build ended as {terminal.State.GetProperty("Status").GetString()}.");
    var result = SingleTypedResult(terminal.Events);
    Require(result.GetProperty("Outcome").GetString() == "succeeded", "Failure harness did not compile.");
    return result.GetProperty("ArtifactRef").GetString()
        ?? throw new InvalidOperationException("Failure harness returned no artifact reference.");
}
...
async Task<string> StartRunAsync(string mode)
{
    ...
    return await StartOperationAsync(
        ...
        new
        {
            identity.requestId,
            identity.idempotencyKey,
            pipelineResolutionId = resolution.GetProperty("PipelineResolutionId").GetString(),
            artifactRef,
...
async Task<TerminalOperation> WaitForTerminalAsync(string operationId)
{
    ...
    for (var attempt = 0; attempt < 900; attempt++)
    {
        ...
        if (state.GetProperty("Status").GetString() is "completed" or "failed" or "cancelled")
            break;
    ...
    Require(
        state.GetProperty("Status").GetString() is "completed" or "failed" or "cancelled",
        $"Operation {operationId} did not become terminal.");
...
void RecordCreatedContainer(IReadOnlyList<JsonElement> events)
{
    var messages = events
        ...
        .Where(static payload => payload.GetProperty("Kind").GetString() == "progress")
        .Select(static payload => payload.GetProperty("Message").GetString())
        .Where(static message => message?.StartsWith("Created isolated container ", StringComparison.Ordinal) == true)
        .ToArray();
...
async Task VerifyRuntimeContainerLabelsAsync(string containerId)
{
    ...
    Require(
        labels.GetProperty("com.sharplabnext.operation-id").GetString() == jobId,
        "Runtime container operation-id did not match job-id.");
    Require(
        !string.IsNullOrWhiteSpace(labels.GetProperty("com.sharplabnext.release-id").GetString()),
        "Runtime container omitted release-id.");
    Require(
        DateTimeOffset.TryParse(labels.GetProperty("com.sharplabnext.created-at").GetString(), out _),
        "Runtime container omitted a valid created-at timestamp.");
}
...
async Task<string> WaitForFreshRunContainerAsync(ResourceSnapshot baseline)
{
    for (var attempt = 0; attempt < 150; attempt++)
    {
                     ...
                     StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            ...
            if (parts.Length == 2 &&
                !baseline.Containers.Contains(parts[0]) &&
                parts[1].StartsWith("sln-run-", StringComparison.Ordinal))
            {
                return parts[0];
            }
        }
...
async Task WaitForResourceBaselineAsync(ResourceSnapshot baseline)
{
    for (var attempt = 0; attempt < 150; attempt++)
    {
        ...
        if (current.Containers.All(baseline.Containers.Contains) &&
            current.Volumes.All(baseline.Volumes.Contains))
        {
            return;
        }
    ...
    throw new InvalidOperationException(
        $"Runtime cleanup left containers [{string.Join(", ", remaining.Containers.Except(baseline.Containers))}] " +
        $"and volumes [{string.Join(", ", remaining.Volumes.Except(baseline.Volumes))}].");
}
...
static HashSet<string> ParseLines(string value) => value.Split(
        ['\r', '\n'],
        StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
    .ToHashSet(StringComparer.Ordinal);

static (string requestId, string idempotencyKey) Identity(string kind)
{
