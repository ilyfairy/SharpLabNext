...
if (args.Length != 3)
{
    throw new ArgumentException(
        "Usage: artifact-framework-reference-sets.cs <runtime-matrix.json> <reference-root> <processor.dll>");
}
...
var targets = matrix.RootElement.GetProperty("framework").GetProperty("targets")
    .EnumerateArray()
    .Select(static target => new FrameworkTarget(
        target.GetProperty("id").GetString()!,
        target.GetProperty("referenceSetId").GetString()!,
        target.GetProperty("targetFramework").GetString()!,
        target.GetProperty("version").GetString()!,
        target.TryGetProperty("referencePackage", out var package)
            ? package.GetProperty("packageContentHash").GetString()!
            : target.GetProperty("referenceComposition").GetProperty("sourceIdentityDigest").GetString()!))
    .ToArray();
...
var sdkVersion = JsonDocument.Parse(await File.ReadAllBytesAsync(Path.Combine(
    Path.GetDirectoryName(matrixPath)!,
    "..",
    "global.json"))).RootElement.GetProperty("sdk").GetProperty("version").GetString()
    ?? throw new InvalidDataException("global.json does not declare an SDK version.");
...
try
{
    foreach (var target in targets)
    {
        ...
        await File.WriteAllTextAsync(
            sourcePath,
            "public sealed class FrameworkArtifactSmoke { " +
            "public static System.Net.WebClient CreateClient() { return new System.Net.WebClient(); } }\n",
...
static void ValidateReferenceRoot(FrameworkTarget target, string root)
{
    if (!Directory.Exists(root) ||
        !File.Exists(Path.Combine(root, "mscorlib.dll")) ||
        !File.Exists(Path.Combine(root, "System.dll")) ||
        !File.Exists(Path.Combine(root, "reference-set.attestation.json")))
    {
        throw new InvalidDataException($"Reference set '{target.ReferenceSetId}' is incomplete.");
    }

    using var attestation = JsonDocument.Parse(
        File.ReadAllBytes(Path.Combine(root, "reference-set.attestation.json")));
    var identity = attestation.RootElement.GetProperty("referenceSet");
    if (identity.GetProperty("id").GetString() != target.ReferenceSetId ||
        identity.GetProperty("targetFramework").GetString() != target.TargetFramework ||
        identity.GetProperty("digest").GetString() != target.IdentityDigest)
    {
        throw new InvalidDataException(
            $"Reference set '{target.ReferenceSetId}' attestation identity is inconsistent.");
    }

    var files = attestation.RootElement.GetProperty("files").EnumerateArray()
        .Select(static file => new AttestedFile(
            file.GetProperty("path").GetString()!,
            file.GetProperty("size").GetInt64(),
            file.GetProperty("digest").GetString()!))
        .OrderBy(static file => file.Path, StringComparer.Ordinal)
        .ToArray();
    var actualPaths = Directory.EnumerateFiles(root, "*.dll", SearchOption.TopDirectoryOnly)
    ...
    foreach (var file in files)
    {
        ...
        {
            throw new InvalidDataException(
                $"Reference set '{target.ReferenceSetId}' file '{file.Path}' failed attestation.");
        }
    ...
    {
        throw new InvalidDataException(
            $"Reference set '{target.ReferenceSetId}' content digest is inconsistent.");
    }
}

static async Task CompileAsync(
    string dotnetHost,
    string compilerPath,
    ...
    string assemblyPath)
{
    ...
    arguments.AddRange(Directory.EnumerateFiles(referenceRoot, "*.dll", SearchOption.TopDirectoryOnly)
        .Where(IsManagedAssembly)
        .Order(StringComparer.Ordinal)
        .Select(static path => $"/reference:\"{path}\""));
    arguments.Add($"\"{sourcePath}\"");
    await File.WriteAllLinesAsync(responsePath, arguments, new UTF8Encoding(false));
    await RunAsync(
        dotnetHost,
        ["exec", compilerPath, $"@{responsePath}"],
...
static void ValidateCompiledReferences(FrameworkTarget target, string referenceRoot, string assemblyPath)
{
    ...
    var observed = metadata.AssemblyReferences
        .Select(handle => metadata.GetAssemblyReference(handle))
        .ToDictionary(
            reference => metadata.GetString(reference.Name),
            static reference => reference.Version,
    ...
    foreach (var (name, expectedVersion) in expected)
    {
        ...
        {
            throw new InvalidDataException(
                $"Compiled '{target.ReferenceSetId}' artifact resolved {name} {actualVersion}, " +
                $"but its own reference root requires {expectedVersion}.");
        }
    }
}
...
static async Task RunProcessorAsync(
    string dotnetHost,
    ...
    string operation)
{
    ...
    await RunAsync(
        dotnetHost,
        [processorPath, "--request", requestPath, "--response", responsePath],
        workRoot);
    ...
    {
        throw new InvalidDataException(
            $"Processor operation '{operation}' failed for '{target.ReferenceSetId}' with '{outcome}': " +
            response.RootElement.GetRawText());
    }
    if (operation == "il" && !File.ReadAllText(outputPath).Contains("FrameworkArtifactSmoke", StringComparison.Ordinal) ||
        operation == "decompiled-csharp" &&
        !File.ReadAllText(outputPath).Contains("class FrameworkArtifactSmoke", StringComparison.Ordinal))
    {
        throw new InvalidDataException(
            $"Processor operation '{operation}' returned incomplete output for '{target.ReferenceSetId}'.");
    }
}
...
static async Task RunAsync(string fileName, IReadOnlyList<string> arguments, string workingDirectory)
{
    ...
    if (process.ExitCode != 0)
    {
        throw new InvalidOperationException(
            $"'{fileName}' exited with {process.ExitCode}.\n{await stdout}\n{await stderr}");
    }
}
