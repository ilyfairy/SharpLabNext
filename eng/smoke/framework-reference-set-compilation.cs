...
if (args.Length != 2)
    throw new ArgumentException("Usage: framework-reference-set-compilation.cs <runtime-matrix.json> <materialized-reference-set-root>");
...
var identity = new RoslynWorkerIdentity("framework-reference-smoke", "roslyn-stable-netfx48", CSharpBuildService.GetLoadedCompilerVersion(), null, "framework-reference-smoke-image");
...
foreach (var target in targets)
{
    ...
    {
        var csharpDebug = await VerifyCSharpAsync(csharp, target, BuildOutputKind.Console, BuildConfiguration.Debug);
        var visualBasicDebug = await VerifyVisualBasicAsync(visualBasic, target, BuildOutputKind.Console, BuildConfiguration.Debug);
    ...
    Console.WriteLine($"{target.Definition.Id}: C#/VB library+console passed" + (target.Definition.TargetFramework is "net20" or "net30" or "net35" ? "; legacy Debug portable PDB passed" : string.Empty));
}
...
static IReadOnlyList<FrameworkTarget> LoadTargets(string matrixPath, string referenceRoot)
{
    ...
    {
        var referenceSetId = target.GetProperty("referenceSetId").GetString() ?? throw new InvalidDataException("Framework target has no referenceSetId.");
        var targetFramework = target.GetProperty("targetFramework").GetString() ?? throw new InvalidDataException($"Framework target '{referenceSetId}' has no targetFramework.");
        var resolvedVersion = target.TryGetProperty("referencePackage", out var package)
            ? package.GetProperty("version").GetString() : target.GetProperty("referenceComposition").GetProperty("resolvedVersion").GetString();
        ...
        var digest = attestation.RootElement.GetProperty("referenceSet").GetProperty("digest").GetString() ?? throw new InvalidDataException($"Reference set '{referenceSetId}' attestation has no digest.");

        var definition = new ReferenceSetDefinition(referenceSetId, path, targetFramework, resolvedVersion, digest, attestationPath, IncludeSharpLabRuntime: false);
        result.Add(new FrameworkTarget(definition, definition.GetRuntimeFrameworkVersion()));
    }
    return result;
}

static async Task<CompiledArtifact> VerifyCSharpAsync(CSharpBuildService service, FrameworkTarget target, BuildOutputKind outputKind, BuildConfiguration configuration)
{
    var source = outputKind == BuildOutputKind.Library
        ? "public sealed class Calculator { public int Add(int left, int right) { return left + right; } }" : "public static class Program { public static void Main() { System.Console.WriteLine(42); } }";
    var request = CreateRequest("csharp", target.Definition.Id, "Program.cs", source, outputKind, configuration, languageVersion: "14.0");
    var response = await service.ExecuteAsync(request, CancellationToken.None);
    return VerifyArtifact(response, target, outputKind, "C#");
}

static async Task<CompiledArtifact> VerifyVisualBasicAsync(VisualBasicBuildService service, FrameworkTarget target, BuildOutputKind outputKind, BuildConfiguration configuration)
{
    var source = outputKind == BuildOutputKind.Library
    ...
    var request = CreateRequest("visual-basic", target.Definition.Id, "Program.vb", source, outputKind, configuration, languageVersion: "latest");
    var response = await service.ExecuteAsync(request, CancellationToken.None);
    return VerifyArtifact(response, target, outputKind, "Visual Basic");
}

static BuildRequest CreateRequest(string languageId, string referenceSetId, string fileName, string source, BuildOutputKind outputKind, BuildConfiguration configuration, string languageVersion)
{
    var options = new BuildOptions(configuration, Optimize: configuration == BuildConfiguration.Release, outputKind, AllowUnsafe: false, EmitPortablePdb: true, NullableContextMode.Disable, languageVersion, PreprocessorSymbols: [], CheckOverflow: true);
    var workspace = new WorkspaceSnapshot(ContractSchemaVersions.WorkspaceSnapshot, 1, 1, languageId, [new WorkspaceFile(fileName, 1, source)], fileName, [fileName], referenceSetId, options);
    var requestId = $"framework-smoke-{Guid.NewGuid():N}";
    return new BuildRequest(requestId, $"idempotency-{requestId}", "framework-reference-compilation-smoke", "roslyn-stable-netfx48", referenceSetId, workspace, DateTimeOffset.UtcNow.AddMinutes(2), options, BuildTarget.Artifact);
}

static CompiledArtifact VerifyArtifact(WorkerBuildExecution response, FrameworkTarget target, BuildOutputKind outputKind, string language)
{
    var result = response.Result as BuildResult ?? throw new InvalidDataException($"{language} {target.Definition.Id} returned no BuildResult.");
    if (result.Outcome != BuildOutcome.Succeeded)
    {
        throw new InvalidDataException($"{language} {target.Definition.Id} {outputKind} failed: " + string.Join(" | ", result.Diagnostics.Select(static diagnostic => $"{diagnostic.Code}: {diagnostic.Message}")));
    }

    var artifact = response.Artifact ?? throw new InvalidDataException($"{language} {target.Definition.Id} {outputKind} returned no artifact.");
    ...
    var expectedEntryAssembly = outputKind == BuildOutputKind.Library
        ? "SharpLabNext.User.dll" : "SharpLabNext.User.exe";
...
{
    if (artifact.PortablePdb.Length == 0 || !artifact.Manifest.Files.Any(static file => file.Role == "portable-pdb"))
    {
