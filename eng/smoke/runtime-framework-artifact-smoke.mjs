/**
 * Exercise the current-source Framework Roslyn worker, Artifact Store, and
 * default artifact worker without going through Gateway or a runtime container.
 */
...
export const runtimeFrameworkArtifactSmokeUsage = `Usage:
  node eng/smoke/runtime-framework-artifact-smoke.mjs --profile ID [--profile ID ...]
    --artifact-store URL --roslyn-worker URL --artifact-worker URL
    [--token-file PATH] [--results PATH]`

export class RuntimeFrameworkArtifactSmokeError extends Error {
  constructor(message, options) {
    super(message, options)
    this.name = 'RuntimeFrameworkArtifactSmokeError'
  }
}

function fail(message, options) { throw new RuntimeFrameworkArtifactSmokeError(message, options); }
...
function requireProfileRuntimeBinding(profile, binding, row) {
  const expectedFamily = row.family === 'mono' ? 'mono' : 'netfx-clr-wine'
  if (!isObject(profile) || profile.id !== row.profileId || profile.family !== expectedFamily ||
      !Array.isArray(profile.acceptedRuntimeFamilies) || !profile.acceptedRuntimeFamilies.includes(expectedFamily) ||
      !Array.isArray(profile.acceptedArtifactFormats) ||
      !profile.acceptedArtifactFormats.includes('dotnet-framework-managed-pe-v1') ||
      !Array.isArray(profile.acceptedFrameworks)) {
    fail(`Runtime profile '${row.profileId}' does not accept the required Framework managed artifact contract.`)
  }
...
function readRuntimeBinding(matrixPath, results, row, profile) {
  ...
  if (!Array.isArray(matrix?.framework?.targets)) fail('Runtime matrix has no Framework targets.')
  const frameworkTargets = matrix.framework.targets
  let target
  if (row.family === 'mono') {
    if (!isObject(matrix.mono) || matrix.mono.id !== row.matrixTargetId ||
        matrix.mono.referenceSetId !== 'netfx48-managed-ref') {
      fail(`Runtime matrix Mono row '${row.matrixTargetId}' must bind netfx48-managed-ref.`)
    }
    target = frameworkTargets.find(value => value?.referenceSetId === matrix.mono.referenceSetId)
    if (target?.id !== 'netfx48') fail('Runtime matrix must bind Mono to the netfx48 Framework target.')
  } else if (row.family === 'netfx-clr-wine') {
    const matches = frameworkTargets.filter(value => value?.id === row.matrixTargetId)
    if (matches.length !== 1) fail(`Runtime matrix must bind Framework target '${row.matrixTargetId}' exactly once.`)
    target = matches[0]
  } else {
    fail(`Functional row '${row.profileId}' has unsupported Framework artifact family '${row.family}'.`)
  }
  const referenceSetId = requiredString(target.referenceSetId, `Runtime matrix target '${target.id}' referenceSetId`)
  const runtimeVersion = requiredString(target.version, `Runtime matrix target '${target.id}' version`)
  const referencePackage = isObject(target.referencePackage) ? target.referencePackage : undefined
  const referenceComposition = isObject(target.referenceComposition) ? target.referenceComposition : undefined
  if ((referencePackage === undefined) === (referenceComposition === undefined)) {
    fail(`Runtime matrix target '${target.id}' must define exactly one Framework reference identity.`)
  }
  const referenceIdentity = referencePackage === undefined
    ? {
        kind: requiredString(referenceComposition.kind, 'Framework reference composition kind'),
        resolvedVersion: requiredString(referenceComposition.resolvedVersion, 'Framework reference composition resolved version'),
        digest: requireDigest(referenceComposition.sourceIdentityDigest, 'Framework reference composition source identity digest'),
        sources: readCompositionSources(referenceComposition, frameworkTargets),
      }
    : {
        kind: 'nuget-package',
        resolvedVersion: requiredString(referencePackage.version, 'Framework reference package version'),
        digest: requiredString(referencePackage.packageContentHash, 'Framework reference package content hash'),
        package: requiredString(referencePackage.id, 'Framework reference package id'),
      }
  const binding = {
    matrixTargetId: target.id,
    referenceSetId,
    targetFramework: requiredString(target.targetFramework, `Runtime matrix target '${target.id}' targetFramework`),
    referenceIdentity,
    runtimeFramework: {
      name: '.NETFramework',
      minimumVersion: runtimeVersion,
    },
    runtimeFamily: 'netfx-clr-wine',
    // Managed Framework PE remains runnable on Mono as well as Wine. The
    // Wine-only tag belongs to the distinct mixed-PE netfx48-ref contract.
...
function readCompositionSources(composition, frameworkTargets) {
  if (!Array.isArray(composition.sources) || composition.sources.length === 0) {
    fail('Framework reference composition sources must be a non-empty array.')
  }
  return composition.sources.map((source, index) => {
    if (!isObject(source)) fail(`Framework reference composition source ${index} must be an object.`)
    const targetId = requiredString(source.targetId, `Framework reference composition source ${index} targetId`)
    const matches = frameworkTargets.filter(value => value?.id === targetId)
    if (matches.length !== 1 || !isObject(matches[0].referencePackage)) {
      fail(`Framework reference composition source '${targetId}' must identify one package-backed Framework target.`)
    }
    const packageInfo = matches[0].referencePackage
    return {
      role: requiredString(source.role, `Framework reference composition source ${index} role`),
      selection: requiredString(source.selection, `Framework reference composition source ${index} selection`),
      package: requiredString(packageInfo.id, `Framework reference composition source '${targetId}' package id`),
      resolvedVersion: requiredString(packageInfo.version, `Framework reference composition source '${targetId}' package version`),
      sourceUri: requiredString(packageInfo.url, `Framework reference composition source '${targetId}' package URL`),
      sourceArchiveDigest: `sha512:${requiredString(packageInfo.sha512, `Framework reference composition source '${targetId}' package SHA-512`)}`,
      packageContentHash: requiredString(packageInfo.packageContentHash, `Framework reference composition source '${targetId}' package content hash`),
    }
  })
}
...
function joinUrl(base, pathname) { return `${base}${pathname}`; }

function newId(prefix) { return `${prefix}-${crypto.randomUUID().replaceAll('-', '')}`; }

function artifactDigest(value) { return requireDigest(value, 'ContentRef').slice('sha256:'.length); }
...
async function readBoundedResponseBytes(response, label, maximumBytes) {
  ...
  try {
    ...
    if (error instanceof RuntimeFrameworkArtifactSmokeError) throw error
...
async function readBoundedResponseText(response, label, maximumBytes) {
  try {
    return new TextDecoder('utf-8', { fatal: true }).decode(await readBoundedResponseBytes(response, label, maximumBytes))
  } catch (error) {
    if (error instanceof RuntimeFrameworkArtifactSmokeError) throw error
...
async function getWorkerDescriptor(request, baseUrl, expected) {
  ...
  const descriptor = requirePascalCaseObject(await readJson(await request(joinUrl(baseUrl, '/api/v1/worker/describe')), `${label} descriptor`), `${label} descriptor`)
...
async function getArtifactStoreIdentity(request, baseUrl) {
  const identity = requirePascalCaseObject(await readJson(await request(joinUrl(baseUrl, '/api/v1/artifacts/status')), 'Artifact Store status'), 'Artifact Store status')
...
function requireReferenceSet(worker, binding, label) {
  ...
  if (attestation.TargetFramework !== binding.targetFramework || attestation.Digest !== binding.referenceIdentity.digest) {
  ...
  if (provenance.Kind !== binding.referenceIdentity.kind ||
      provenance.ResolvedVersion !== binding.referenceIdentity.resolvedVersion ||
      (binding.referenceIdentity.package !== undefined && provenance.Package !== binding.referenceIdentity.package)) {
    fail(`${label} reference-set provenance does not match the runtime matrix binding.`)
  }
  if (binding.referenceIdentity.sources !== undefined) {
    if (!Array.isArray(provenance.Sources) || provenance.Sources.length !== binding.referenceIdentity.sources.length) {
      fail(`${label} reference-set composition sources do not match the runtime matrix binding.`)
    }
    for (let index = 0; index < binding.referenceIdentity.sources.length; index++) {
      const expected = binding.referenceIdentity.sources[index]
      const actual = requirePascalCaseObject(provenance.Sources[index], `${label} reference-set composition source ${index}`)
      if (actual.Role !== expected.role || actual.Selection !== expected.selection ||
          actual.Package !== expected.package || actual.ResolvedVersion !== expected.resolvedVersion ||
          actual.SourceUri !== expected.sourceUri || actual.SourceArchiveDigest !== expected.sourceArchiveDigest ||
          actual.PackageContentHash !== expected.packageContentHash) {
        fail(`${label} reference-set composition source ${index} does not match the runtime matrix binding.`)
      }
    }
  }
  return attestation
...
function requireManifestRuntimeRequirement(runtimeRequirement, binding) {
  ...
  if (requirement.Family !== binding.runtimeFamily || !Array.isArray(requirement.Frameworks) || requirement.Frameworks.length !== 1 ||
      !Array.isArray(requirement.RequiredRuntimeFeatureTags) ||
      JSON.stringify(requirement.RequiredRuntimeFeatureTags) !== JSON.stringify(binding.requiredRuntimeFeatureTags)) {
    fail('Artifact manifest runtime requirement is not the required Framework contract.')
  }
...
async function getArtifactManifest(request, artifactStoreUrl, artifactRef, binding) {
  const descriptor = requirePascalCaseObject(await readJson(await request(joinUrl(artifactStoreUrl, `/internal/v1/artifacts/sha256/${artifactDigest(artifactRef)}`)), `Artifact Store manifest ${artifactRef}`), `Artifact Store manifest ${artifactRef}`)
  ...
  if (manifest.ArtifactId !== artifactRef || manifest.ReferenceSetId !== binding.referenceSetId ||
       manifest.TargetFramework !== binding.targetFramework || manifest.ArtifactFormat !== 'dotnet-framework-managed-pe-v1') {
...
function buildRequest(referenceSetId, now) {
  ...
  return {
    ...
    PipelineResolutionId: 'runtime-framework-artifact-smoke-v1',
...
async function buildArtifact(send, roslynUrl, descriptor, referenceSetId, now) {
  ...
  const response = requirePascalCaseObject(await readJson(await send(joinUrl(roslynUrl, '/api/v1/build'), { method: 'POST', body: JSON.stringify(build) }), 'Roslyn build'), 'Roslyn build')
  ...
  if (identity.ReleaseId !== descriptor.releaseId || identity.LanguageId !== 'csharp' ||
      identity.ToolchainId !== 'roslyn-stable-netfx48' ||
...
async function waitForOperation(request, artifactUrl, operationId, maxPolls, sleep, pollDelayMilliseconds) {
  ...
  for (let attempt = 0; attempt < maxPolls; attempt++) {
    state = requirePascalCaseObject(await readJson(await request(joinUrl(artifactUrl, `/api/v1/operations/${encodeURIComponent(operationId)}`)), `Artifact operation ${operationId}`), `Artifact operation ${operationId}`)
  ...
  const events = await readJson(await request(joinUrl(artifactUrl, `/api/v1/operations/${encodeURIComponent(operationId)}/events?FromSequence=0`)), `Artifact operation ${operationId} events`)
...
async function renderArtifact(request, artifactUrl, descriptor, artifactRef, outputId, now, maxPolls, sleep, pollDelayMilliseconds) {
  ...
  const events = await waitForOperation(request, artifactUrl, operationId, maxPolls, sleep, pollDelayMilliseconds)
...
async function readContent(request, artifactStoreUrl, contentRef) {
  const response = await request(joinUrl(artifactStoreUrl, `/internal/v1/contents/sha256/${artifactDigest(contentRef)}`), { headers: { Accept: 'text/plain' } })
...
export async function runRuntimeFrameworkArtifactSmokes(options = {}) {
  ...
  const roslyn = await getWorkerDescriptor(request, roslynWorkerUrl, {
    id: 'roslyn-stable-netfx48', serviceKind: serviceKinds.toolchainWorker,
  ...
  for (const row of rows) {
    ...
    const il = await renderArtifact(request, artifactWorkerUrl, artifactWorker, build.artifactRef, 'il', now, maxPolls, sleep, pollDelayMilliseconds)
    const decompiled = await renderArtifact(request, artifactWorkerUrl, artifactWorker, build.artifactRef, 'decompiled-csharp', now, maxPolls, sleep, pollDelayMilliseconds)
...
export async function runRuntimeFrameworkArtifactSmokeCli(argv, options = {}) {
  const output = options.output ?? console
  try {
    const parsed = parseArguments(argv)
    if (parsed.help) {
      output.log(runtimeFrameworkArtifactSmokeUsage)
      return 0
    }
    await runRuntimeFrameworkArtifactSmokes({
      ...options,
      ...parsed,
    ...
    output.error(`runtime framework artifact smoke error: ${error.message}`)
    return 1
  }
}

if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.exitCode = await runRuntimeFrameworkArtifactSmokeCli(process.argv.slice(2))
}
