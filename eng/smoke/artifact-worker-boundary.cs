...
using var start = await http.PostAsJsonAsync("/api/v1/artifact-renders", new
{
    requestId,
    idempotencyKey = $"render:{requestId}",
    pipelineResolutionId = "const-generics-boundary-smoke",
    artifactRef,
...
var operationId = handle.RootElement.GetProperty("OperationId").GetString()
    ?? throw new InvalidOperationException("The artifact worker returned no operation ID.");
...
for (var attempt = 0; attempt < 300; attempt++)
{
    ...
    if (status == "completed")
    {
        ...
        var results = events.RootElement.EnumerateArray()
            .Where(static operationEvent =>
                operationEvent.GetProperty("Payload").GetProperty("Kind").GetString() == "typed-result")
            .Select(static operationEvent => operationEvent.GetProperty("Payload").GetProperty("Result").Clone())
            .ToArray();
        if (results.Length != 1)
            throw new InvalidOperationException($"The ordinary worker returned {results.Length} typed results.");
        var result = results[0];
        if (result.GetProperty("ResultType").GetString() != "artifact-render" ||
            result.GetProperty("Outcome").GetString() != "invalid-artifact" ||
            result.TryGetProperty("ContentRef", out var contentRef) && contentRef.ValueKind != JsonValueKind.Null ||
            !result.GetProperty("Diagnostics").EnumerateArray().Any(static diagnostic =>
                diagnostic.GetProperty("Code").GetString() == "invalid-artifact"))
        {
