...
export const runtimeWineFrameworkSmokeUsage = 'Usage:\n  node eng/smoke/runtime-wine-framework-smoke.mjs --profile ID [--profile ID ...] [--representative] [--results PATH]'
...
function verifyCandidateImageTag(spawn, row, options) {
  const inspected = runProcess(spawn, 'docker', ['image', 'inspect', '--format', '{{.Id}}', row.candidateImage], options, `Wine Framework candidate image tag '${row.candidateImage}'`, [0], 30000)
...
function representativeMeasurement(run, wallElapsedMilliseconds, stderr, maximumMemoryBytes) { return { ...run, wallElapsedMilliseconds, ...representativePeakMemory(stderr, maximumMemoryBytes) }; }
