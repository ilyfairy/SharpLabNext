...
function candidateProfile(id) { return readJson(`profiles/runtimes/candidates/${id}.json`); }
...
function assertRetainedRuntime(runtime, profileId) {
  const profilePath = path.join(activeProfileDirectory, `${profileId}.json`)
  assert.ok(fs.existsSync(profilePath), `${profileId} retained runtime needs an active profile`)
  const profile = readJson(`profiles/runtimes/${profileId}.json`)
  assert.equal(profile.id, profileId)
  assert.equal(profile.runtimeVersion, runtime.resolvedVersion)
  assert.equal(profile.runtimeImageId, runtime.runtimeImageId)
  assert.equal(profile.runtimeCommit, runtime.runtimeCommit)
  assert.equal(profile.jitCommit, runtime.jitCommit)
}

function assertPromotionBranch(capability, runtime, profileId) {
  assertCapabilityState({ id: profileId, capability })
  if (capability.promotionState === 'verified') {
    assert.equal(runtime?.availability?.installed, true, `${profileId} verified runtime must be installed`)
    assert.equal(runtime?.availability?.health, 'healthy', `${profileId} verified runtime must be healthy`)
    assertRetainedRuntime(runtime, profileId)
  }
  else {
    assert.equal(capability.promotionState, 'blocked')
    if (runtime?.availability?.installed === true && runtime.availability.health === 'healthy') {
      assertRetainedRuntime(runtime, profileId)
    }
    else {
      assert.equal(runtime?.availability?.installed, false, `${profileId} blocked candidate cannot be installed`)
      assert.notEqual(runtime?.availability?.health, 'healthy', `${profileId} blocked candidate cannot be healthy`)
    }
  }
}
...
test('blocked and verified capability states are fail-closed', () => {
  for (const row of matrixRows) {
    ...
    for (const runtime of exactCatalogRows) {
      // A blocked candidate may share an ID with a retained last-known-good runtime.
      if (runtime.availability?.installed === true && runtime.availability?.health === 'healthy') {
        assertRetainedRuntime(runtime, runtime.id)
        continue
      }
...
test('Wine CoreCLR 5-11 records Run and JIT feasibility and promotion state', () => {
  for (const id of requiredWineCoreClrIds) {
    const row = coreClrById.get(id)
    ...
    const runtime = catalog.runtimes.find(candidate => candidate.id === `wine-${id}-linux-x64` && candidate.resolvedVersion === row.version)
    assertPromotionBranch(row.wineCapability, runtime, `wine-${id}-linux-x64`)
...
test('Framework 4.0-4.7.2 keep promotion state and independent receipts', () => {
  ...
  for (const version of compatibilityOnlyVersions) {
    ...
    const runtimeId = `wine-${row.id}-linux-x64`
    const runtime = catalog.runtimes.find(candidate => candidate.id === runtimeId && candidate.resolvedVersion === version)
    assertPromotionBranch(row.capability, runtime, runtimeId)
    if (row.capability.promotionState !== 'verified') continue
...
test('released .NET 10 and .NET 11 profiles agree with matrix candidates', () => {
  for (const id of ['dotnet-10-linux-x64', 'dotnet-11-preview-linux-x64']) {
    ...
    const matrixRow = coreClrById.get(matrixId)
    assert.ok(matrixRow, `${id} must retain its matrix row`)
    assertPromotionBranch(matrixRow.linuxCapability, runtime, id)
    assert.equal(matrixRow.version, runtime.resolvedVersion)
  }
})
