...
test('Wine CoreCLR prepares a bounded non-root XDG runtime directory through the shared entrypoint', () => {
  ...
  assert.match(entrypoint, /SHARPLABNEXT_WINE_CLEANUP:-0/)
  assert.match(entrypoint, /WINESERVER.*wineserver64/s)
  assert.match(entrypoint, /\"\$WINESERVER\" -k/)
  assert.match(entrypoint, /timeout 2 \"\$WINESERVER\" -w/)
  assert.match(entrypoint, /trap forward_term TERM/)
  assert.match(entrypoint, /trap forward_int INT/)
