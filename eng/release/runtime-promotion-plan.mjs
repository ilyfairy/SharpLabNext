...
export const runtimePromotionPlanUsage = `Usage:
  node eng/release/runtime-promotion-plan.mjs <candidate-target> \\
...
function readRegularFile(filename, label) {
  ...
  if (before.size <= 0 || before.size > maximumInputBytes) {
    throw new RuntimePromotionPlanError(`${label} must contain between 1 and ${maximumInputBytes} bytes.`)
  }
...
function requireEqual(actual, expected, label) {
  if (actual !== expected) {
    throw new RuntimePromotionPlanError(`${label} must equal ${JSON.stringify(expected)}; observed ${JSON.stringify(actual)}.`)
  }
}
...
function selectSecurityPolicy(profile) {
  if (!Array.isArray(profile.allowedSecurityPolicyIds) ||
    ...
    throw new RuntimePromotionPlanError('The candidate Runtime Profile must select exactly one allowed security policy.')
  }
...
function selectCapabilities(profile, binding) {
  ...
  if (JSON.stringify(candidateCapabilities) !== JSON.stringify(expectedCandidateCapabilities)) {
    throw new RuntimePromotionPlanError('The blocked candidate Runtime Profile must expose exactly the matrix operation capabilities; instrumentation is enabled only in the immutable preflight profile.')
  }
  ...
  if (profilerProvider !== undefined) {
    ...
    if (capabilities.includes('jit-asm')) {
      ...
      if (typeof profile.operations?.jit?.profilerPath !== 'string' ||
        ...
        throw new RuntimePromotionPlanError('A profiler-backed candidate JIT operation must bind its native profiler path.')
      }
    }
  }
  if (capabilities.some(capability => nonSelectableInstrumentation.has(capability)) &&
    ...
    throw new RuntimePromotionPlanError('Instrumentation preflight requires the modern Runner implementation.')
  }
  return capabilities
}
...
function createAtomicStage(repositoryRoot, relativeOutputPath, bytes) {
  ...
  const expectedDirectories = new Set([path.join(repositoryRealPath, 'profiles', 'runtime-promotion-plans'), path.join(repositoryRealPath, 'profiles', 'runtime-operator-receipts')])
  ...
  const temporaryPath = path.join(outputDirectory, `.${path.basename(outputPath)}.${process.pid}.${crypto.randomUUID()}.tmp`)
  ...
  return {
    ...
    install() {
      if (fs.existsSync(outputPath)) {
        backupPath = path.join(outputDirectory, `.${path.basename(outputPath)}.${process.pid}.${crypto.randomUUID()}.bak`)
...
function materializePreflightProfile(profile, image, capabilities) {
  if (profile.promotionReceipt !== undefined) {
    throw new RuntimePromotionPlanError('A candidate Runtime Profile cannot already contain a promotion receipt.')
  }
...
function createOrVerifyRuntimePromotionPlan(input, options, verifyExisting) {
  ...
  if (!pinnedReferencePattern.test(pinnedReference ?? '')) {
    throw new RuntimePromotionPlanError('The pinned image reference must be repository@sha256:<64 lowercase hex>.')
  }
  ...
  if (typeof performancePolicyPath !== 'string' ||
    ...
    throw new RuntimePromotionPlanError('The performance policy must use its canonical path below profiles/runtime-performance-policies.')
  }
