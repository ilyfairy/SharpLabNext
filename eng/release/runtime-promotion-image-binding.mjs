...
function requiredString(value, field, reference) {
  if (typeof value !== 'string' || value.length === 0) {
    throw new RuntimePromotionImageBindingError(`Docker inspection for '${reference}' has no valid ${field}.`)
  }
  return value
}
...
export function parseDockerImageInspection(stdout, reference) {
  ...
  if (!Array.isArray(result) || result.length !== 1 ||
    ...
    throw new RuntimePromotionImageBindingError(`Docker returned an invalid inspection result for '${reference}'.`)
  }

  const image = result[0]
  if (!Number.isSafeInteger(image.Size) || image.Size <= 0) {
    throw new RuntimePromotionImageBindingError(`Docker inspection for '${reference}' has invalid Size; expected a positive safe integer.`)
  }
  ...
  if (repoDigests === undefined) {
    throw new RuntimePromotionImageBindingError(`Docker inspection for '${reference}' has invalid RepoDigests.`)
  }
  ...
  if (labelsValue !== undefined && labelsValue !== null &&
    ...
    throw new RuntimePromotionImageBindingError(`Docker inspection for '${reference}' has invalid Config.Labels.`)
  }
  const labels = {}
  for (const [name, value] of Object.entries(labelsValue ?? {})) {
    if (typeof value !== 'string') {
      throw new RuntimePromotionImageBindingError(`Docker inspection for '${reference}' has a non-string label '${name}'.`)
    }
    labels[name] = value
  }
...
function validateHelperPath(containerPath) {
  if (typeof containerPath !== 'string' || !containerPath.startsWith(trustedHelperRoot) ||
    ...
    throw new RuntimePromotionImageBindingError(`Runtime helper path '${containerPath}' is outside ${trustedHelperRoot}.`)
  }
  const segments = containerPath.slice(1).split('/')
  if (segments.some(segment => segment.length === 0 || segment === '.' || segment === '..')) {
    throw new RuntimePromotionImageBindingError(`Runtime helper path '${containerPath}' is not canonical.`)
  }
}
...
function hashRegularFile(filename, maximumBytes) {
  ...
  if (!before.isFile() || before.isSymbolicLink()) {
    throw new RuntimePromotionImageBindingError(`Copied runtime helper '${filename}' must be a regular non-link file.`)
  }
  if (before.size === 0) {
    throw new RuntimePromotionImageBindingError(`Copied runtime helper '${filename}' is empty.`)
  }
  if (before.size > maximumBytes) {
    throw new RuntimePromotionImageBindingError(`Copied runtime helper '${filename}' exceeds the ${maximumBytes}-byte limit.`)
  }
...
export function hashDockerImageFile(imageId, containerPath, options = {}) {
  if (!imageIdPattern.test(imageId ?? '')) {
    throw new RuntimePromotionImageBindingError('Runtime helper extraction requires sha256:<64 lowercase hex> image ID.')
  }
