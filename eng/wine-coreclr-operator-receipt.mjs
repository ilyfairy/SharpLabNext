...
export const wineCoreClrOperatorReceiptPublicKeyPath = path.join(
  path.dirname(fileURLToPath(import.meta.url)), 'profiles', 'trust', 'wine-coreclr-operator-receipt-public.pem')
export const wineCoreClrOperatorCommittedFiles = Object.freeze([
...
function boundedAscii(value, maximum = 2048) {
  return typeof value === 'string' && value.length > 0 && value.length <= maximum && printableAscii.test(value)
}
