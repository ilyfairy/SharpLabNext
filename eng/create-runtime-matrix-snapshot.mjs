...
function runGit(repositoryRoot, args, options = {}) {
  ...
  if (result.error) {
    throw new RuntimeMatrixSnapshotError(
      `Could not run Git (${args.join(' ')}): ${result.error.message}`,
    )
  }
  ...
  if (!allowed.includes(result.status)) {
    const details = `${result.stderr ?? ''}\n${result.stdout ?? ''}`
      ...
      .slice(0, 4096)
    throw new RuntimeMatrixSnapshotError(
      `Git failed (${args.join(' ')})${details ? `: ${details}` : '.'}`,
    )
  }
  return {
...
function requireRegularParents(root, candidate) {
  ...
  for (const segment of relative.split(path.sep)) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
      throw new RuntimeMatrixSnapshotError(
        `Snapshot parent '${current}' must be a regular directory and cannot be a link.`,
      )
    }
  }
}
...
function requireCleanSubmodules(repositoryRoot) {
  for (const relative of listSubmodulePaths(repositoryRoot)) {
    ...
    if (status.length > 0) {
      throw new RuntimeMatrixSnapshotError(
        `Submodule '${relative}' must be clean before creating a snapshot.`,
      )
    }
  }
}
...
function requireStableCapture(repositoryRoot, indexEnvironment) {
  ...
  if (tracked.status !== 0) {
    throw new RuntimeMatrixSnapshotError(
      'Repository files changed while the runtime matrix snapshot was being captured.',
    )
  }
  ...
  if (untracked.length > 0) {
    throw new RuntimeMatrixSnapshotError(
      'New untracked files appeared while the runtime matrix snapshot was being captured.',
    )
  }
...
function resolveRepositoryRoot(value) {
  ...
  if (requested !== actual) {
    throw new RuntimeMatrixSnapshotError(
      `Repository root must be the Git top-level directory '${actual}'.`,
    )
  }
  return actual
}
...
export function createRuntimeMatrixSnapshot(options, dependencies = {}) {
  ...
  if (ignored.status !== 0) {
    throw new RuntimeMatrixSnapshotError(
      `Snapshot output '${outputRelative}' must be ignored by the repository.`,
    )
  }
  requireCleanSubmodules(repositoryRoot)
  ...
  const temporaryIndex = path.join(
    repositoryRoot,
    '.tmp',
    `.runtime-matrix-snapshot-${process.pid}-${crypto.randomBytes(8).toString('hex')}.index`,
  )
  ...
  try {
    ...
    let commit = head
    if (tree !== headTree) {
      const commitEpoch = runGit(repositoryRoot, ['show', '-s', '--format=%ct', 'HEAD'])
        .stdout.trim()
    ...
    snapshotRef = options.snapshotRef ??
      `refs/sharplabnext/runtime-matrix-snapshots/${commit}`
    if (!refPattern.test(snapshotRef)) {
      throw new RuntimeMatrixSnapshotError(
        `Snapshot ref '${snapshotRef}' must stay below refs/sharplabnext/runtime-matrix-snapshots/.`,
      )
    }
    ...
    if (existing.status === 0 && existingCommit !== commit) {
      throw new RuntimeMatrixSnapshotError(
        `Snapshot ref '${snapshotRef}' already points to a different commit.`,
      )
    }
