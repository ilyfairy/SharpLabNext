...
static class RuntimeCapabilityProbeApplication
{
    ...
    private const string ProbeProject =
        "tests/Fixtures/SharpLabNext.RuntimeCapabilityProbe/SharpLabNext.RuntimeCapabilityProbe.csproj";
    ...
    private static async Task PublishAsync(ProbeOptions options)
    {
        ...
        var preflightProfilePath = ResolvePromotionPath(
            root,
            options.PreflightProfilePath!,
            $"profiles/runtime-promotion-plans/{candidateProfile.Id}.profile.json",
            "immutable preflight Runtime Profile");
        var planPath = ResolvePromotionPath(
            root,
            options.PlanPath!,
            $"profiles/runtime-promotion-plans/{candidateProfile.Id}.json",
        ...
        var performancePolicyPath = ResolvePromotionPath(
            root,
            performancePolicyRelativePath,
            performancePolicyRelativePath,
        ...
        try
        {
            plan = RuntimePromotionPlanWorkflow.CreateContext(
                profileBytes,
        ...
        catch (BundleValidationException exception)
        {
            throw new ProbeFailureException(
                $"Runtime promotion plan binding is invalid: {exception.Message}",
                exception);
        }
        if (!StringComparer.Ordinal.Equals(plan.SourceRevision, options.SourceRevision))
            throw new ProbeFailureException("Promotion plan source revision does not match --source-revision.");
        var preflightProfile = ReadProfile(
            preflightProfileBytes,
            preflightProfilePath,
            ".profile.json");
        ...
        var requiresExecutionFlow = preflightProfile.Capabilities.Contains(
            "execution-flow",
            StringComparer.Ordinal);
        if (requiresExecutionFlow != !string.IsNullOrWhiteSpace(options.ArtifactWorkerBaseAddress))
        {
            throw new ProbeFailureException(requiresExecutionFlow
                ? "--artifact-worker-base-address is required by the immutable preflight Runtime Profile."
                : "--artifact-worker-base-address is not allowed when the immutable preflight Runtime Profile does not declare execution-flow.");
        }

        await BuildProbeAsync(root, target.TargetFramework, options.Configuration!).ConfigureAwait(false);
        VerifyInputs(inputFiles);
        RequireSameSourceState(
            source,
            InspectSource(
                root,
                options.SourceRevision!,
        ...
        {
            BaseAddress = NormalizeBaseAddress(options.ArtifactStoreBaseAddress!),
            Timeout = TimeSpan.FromSeconds(options.TimeoutSeconds)
        };
        client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", token);
        var store = new ArtifactStoreClient(client);
        var uploads = files.Select(static file => new ArtifactFileUpload(
            file.Descriptor.Path,
            new MemoryStream(file.Bytes, writable: false),
            file.Bytes.LongLength)).ToArray();
        try
        {
            var stored = await store.PutArtifactAsync(
                manifest,
                uploads,
                TimeSpan.FromSeconds(options.TimeToLiveSeconds),
                CancellationToken.None).ConfigureAwait(false);
            if (stored.ArtifactRef != manifest.ArtifactId)
                throw new ProbeFailureException("Artifact Store returned a different probe artifact identity.");
        }
        finally
        {
        ...
        {
            using var workerClient = new HttpClient(new HttpClientHandler
            {
                AllowAutoRedirect = false
            })
            {
                BaseAddress = NormalizeBaseAddress(options.ArtifactWorkerBaseAddress!),
                Timeout = TimeSpan.FromSeconds(options.TimeoutSeconds),
                MaxResponseContentBufferSize = 4 * 1024 * 1024
            };
            workerClient.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", token);
            executionFlowArtifactRef = await DeriveExecutionFlowArtifactAsync(
                workerClient,
                store,
                manifest.ArtifactId,
        ...
        var result = new JsonObject
        {
            ...
            ["Files"] = new JsonArray(files.Select(static file => (JsonNode)new JsonObject
            {
                ...
                ["Sha256"] = file.Descriptor.Digest
            }).ToArray())
        };
        if (executionFlowArtifactRef is { } flowArtifactRef)
            result["ExecutionFlowArtifactRef"] = flowArtifactRef.Value;
        if (options.OutputPath is { } outputPath)
            WriteAtomicJson(Path.GetFullPath(outputPath, root), result);
        Console.WriteLine(result.ToJsonString(OutputJson));
    }

    private static async Task<ArtifactRef> DeriveExecutionFlowArtifactAsync(
        HttpClient client,
        ArtifactStoreClient store,
        ArtifactRef sourceArtifactRef,
        int timeoutSeconds)
    {
        var nonce = Guid.NewGuid().ToString("N");
        var request = new TransformArtifactRequest(
            $"runtime-capability-flow-{nonce}",
            $"runtime-capability-flow-{nonce}",
            "runtime-capability-preflight-v1",
            sourceArtifactRef,
        ...
        using var startResponse = await client.PostAsJsonAsync(
            ...
            request,
            ContractJsonOptions).ConfigureAwait(false);
        await RequireWorkerSuccessAsync(startResponse).ConfigureAwait(false);
        var handle = await startResponse.Content.ReadFromJsonAsync<OperationHandle>(ContractJsonOptions)
            .ConfigureAwait(false)
            ?? throw new ProbeFailureException("Artifact worker returned an empty operation handle.");
        if (string.IsNullOrWhiteSpace(handle.OperationId) ||
            !StringComparer.Ordinal.Equals(handle.RequestId, request.RequestId))
        {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            using var response = await client.GetAsync(
                $"api/v1/operations/{Uri.EscapeDataString(handle.OperationId)}/events" +
                $"?FromSequence={fromSequence}").ConfigureAwait(false);
            await RequireWorkerSuccessAsync(response).ConfigureAwait(false);
            var events = await response.Content.ReadFromJsonAsync<OperationEvent[]>(ContractJsonOptions)
                .ConfigureAwait(false)
                ?? throw new ProbeFailureException("Artifact worker returned an empty event stream.");
            foreach (var operationEvent in events)
            {
                if (!StringComparer.Ordinal.Equals(operationEvent.OperationId, handle.OperationId) ||
                    operationEvent.Sequence <= fromSequence)
                {
                ...
                if (operationEvent.Payload is TypedResultOperationEventPayload { Result: TransformArtifactResult result })
                    transform = result;
                if (operationEvent.Payload is FailedOperationEventPayload failed)
                {
                    throw new ProbeFailureException(
                        $"Execution Flow transform failed with '{failed.Error.Code}'.");
                }
                if (operationEvent.Payload is CompletedOperationEventPayload completed)
                {
                    ...
                    {
                        throw new ProbeFailureException(
                            "Execution Flow transform completed without a valid derived artifact.");
                    }
                    var descriptor = await store.GetArtifactAsync(derivedRef).ConfigureAwait(false)
                        ?? throw new ProbeFailureException(
                            "Execution Flow derived artifact was not retained by Artifact Store.");
                    ...
                    if (descriptor.Manifest.ArtifactId != derivedRef ||
                        ...
                        !metadata.TryGetValue(
                            RuntimeCapabilityProbeContract.InstrumentationPointsKey,
                            out var pointsText) ||
                        !int.TryParse(
                            pointsText,
                    ...
                    {
                        throw new ProbeFailureException(
                            "Execution Flow derived artifact does not bind the required instrumentation profile.");
                    }
                    return derivedRef;
                }
            }
    ...
    private static async Task RequireWorkerSuccessAsync(HttpResponseMessage response)
    {
        ...
        if (body.Length > 4096)
            body = body[..4096];
        throw new ProbeFailureException(
            $"Artifact worker returned HTTP {(int)response.StatusCode}: {body}");
    }
        ...
        IReadOnlyList<ProbeFile> files)
    {
        ...
        return ArtifactIdentity.WithComputedId(new ArtifactManifest(
            ...
            new ArtifactRuntimeRequirement(
                profile.Family,
                [new FrameworkRequirement(profile.FrameworkName, profile.FrameworkVersion)],
                "anycpu",
                []),
            [],
    ...
    private static ProbeTarget SelectTarget(ProbeProfile profile)
    {
        if (!profile.Capabilities.Contains("run", StringComparer.Ordinal))
            throw new ProbeFailureException($"Runtime Profile '{profile.Id}' does not declare Run.");
        if (profile.AcceptedArtifactFormats.Contains("dotnet-managed-pe-v1", StringComparer.Ordinal) &&
            profile.Family is "coreclr" or "coreclr-wine")
        {
        ...
        if (profile.AcceptedArtifactFormats.Contains("dotnet-framework-managed-pe-v1", StringComparer.Ordinal) &&
            profile.Family is "mono" or "netfx-clr-wine")
        {
            return new ProbeTarget(
                "net20",
        ...
        throw new ProbeFailureException(
            $"Runtime Profile '{profile.Id}' has no supported managed probe artifact contract.");
    }

    private static string SelectMethodFilter(ProbeProfile profile) => profile.Family == "coreclr-wine"
        ? "SharpLabNext.RuntimeCapabilityProbe.Program.WindowsAbi"
        : "SharpLabNext.RuntimeCapabilityProbe.Program.MultipleSequencePoints";
    ...
    private static ProbeProfile ReadProfile(byte[] bytes, string path, string fileSuffix)
    {
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object)
                throw new InvalidDataException("Runtime Profile must be an object.");
            ...
            if (frameworks.ValueKind != JsonValueKind.Array || frameworks.GetArrayLength() != 1)
                throw new InvalidDataException("Probe Runtime Profiles must declare exactly one accepted framework.");
            ...
            var frameworkVersion = framework.TryGetProperty("exactVersion", out var exactVersion)
                ? RequiredStringValue(exactVersion, "exactVersion")
                : framework.TryGetProperty("minimumVersion", out var minimumVersion) &&
                  framework.TryGetProperty("maximumVersion", out var maximumVersion)
                    ? runtimeVersion
                    : throw new InvalidDataException(
                        "Accepted framework must declare exactVersion or a complete version range.");
            if (!IsStableId(id) || !IsStableId(family) ||
                Path.GetFileName(path) != $"{id}{fileSuffix}" ||
                acceptedRuntimeFamilies.Length == 0 || formats.Length == 0 || capabilities.Length == 0)
            {
    ...
    private static void ValidateProfileBinding(
        ProbeProfile candidate,
        ProbeProfile preflight,
        RuntimePromotionPlanContext plan)
    {
        ...
        RequireEqual(
            preflight.FrameworkName,
            candidate.FrameworkName,
            "preflight accepted framework name");
        RequireEqual(
            preflight.FrameworkVersion,
            candidate.FrameworkVersion,
            "preflight accepted framework version");
        RequireSetEqual(
            preflight.AcceptedRuntimeFamilies,
            candidate.AcceptedRuntimeFamilies,
            "preflight accepted runtime families");
        RequireSetEqual(
            preflight.AcceptedArtifactFormats,
            candidate.AcceptedArtifactFormats,
            "preflight accepted artifact formats");
        var expectedCandidateCapabilities = preflight.Capabilities
            .Where(static capability => capability is not ("inspection" or "execution-flow"))
            .ToArray();
        RequireSetEqual(
            candidate.Capabilities,
            expectedCandidateCapabilities,
            "candidate non-instrumentation capabilities");
    ...
    private static void RequireEqual(string actual, string expected, string description)
    {
        ...
        {
            throw new ProbeFailureException(
                $"The {description} does not match the candidate promotion binding.");
        }
    }
    ...
    private static string ReadPerformancePolicyPath(byte[] planBytes)
    {
        try
        {
            ...
            {
                throw new InvalidDataException(
                    "Promotion plan performance policy path is not canonical.");
            }
            return relativePath;
        }
        ...
        {
            throw new ProbeFailureException(
                $"Runtime promotion plan is invalid: {exception.Message}",
                exception);
        }
    }
    ...
    private static async Task BuildProbeAsync(string root, string framework, string configuration)
    {
        ...
        using var process = Process.Start(start)
            ?? throw new ProbeFailureException("Could not start the deterministic probe build.");
        ...
        {
            throw new ProbeFailureException(
                $"Probe build failed with exit code {process.ExitCode}.\n{await stdout}\n{await stderr}");
        }
    }
    ...
    private static SourceState InspectSource(
        string root,
        string expectedRevision,
        bool allowDirty,
        HashSet<string> allowedGeneratedPaths)
    {
        if (!IsCommit(expectedRevision))
            throw new ProbeFailureException("Source revision must be a full lowercase Git commit.");
        var revision = RunGit(root, "rev-parse", "HEAD").Trim();
        if (!StringComparer.Ordinal.Equals(revision, expectedRevision))
            throw new ProbeFailureException("Source revision does not match the repository HEAD.");
        var status = RunGit(
            root,
            "status",
            "--porcelain=v1",
            "-z",
            "--untracked-files=all");
        var unexpected = ParseGitStatus(status)
            .Where(path => !allowedGeneratedPaths.Contains(path))
            .ToArray();
        if (unexpected.Length > 0 && !allowDirty)
        {
            throw new ProbeFailureException(
                $"A promotable probe artifact requires an exact source tree; unexpected change " +
                $"'{unexpected[0]}'.");
        }
    ...
    private static List<string> ParseGitStatus(string status)
    {
        ...
        for (var index = 0; index < records.Length; index++)
        {
            ...
            {
                throw new ProbeFailureException(
                    "Git returned a non-canonical source status for the capability probe.");
            }
            ...
            {
                throw new ProbeFailureException(
                    "Git returned a non-canonical source path for the capability probe.");
            }
    ...
    private static string RunGit(string root, params string[] arguments)
    {
        ...
        using var process = Process.Start(start)
            ?? throw new ProbeFailureException("Could not inspect the Git source state.");
        var stdout = process.StandardOutput.ReadToEnd();
        var stderr = process.StandardError.ReadToEnd();
        process.WaitForExit();
        if (process.ExitCode != 0)
            throw new ProbeFailureException($"Git source inspection failed: {stderr.Trim()}");
        return stdout;
    }
    ...
    private static string ResolveProfilePath(string root, string value)
    {
        ...
        if (!path.StartsWith(expectedRoot, PathComparison) ||
            !StringComparer.OrdinalIgnoreCase.Equals(Path.GetExtension(path), ".json"))
        {
    ...
    private static string ResolvePromotionPath(
        string root,
        string value,
        string expectedRelativePath,
        string description)
    {
        if (!StringComparer.Ordinal.Equals(value, expectedRelativePath) ||
            value.Contains('\\') || Path.IsPathRooted(value))
        {
            throw new ProbeFailureException(
                $"The {description} path must be '{expectedRelativePath}'.");
        }
        var path = Path.GetFullPath(value, root);
    ...
    private static void EnsureNoReparsePoints(string root, string path)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            {
                throw new ProbeFailureException(
                    $"Capability probe input path contains a reparse point: '{current}'.");
            }
        }
    }
    ...
    private static byte[] ReadBoundedRegularFile(string path, string description, long maximumBytes)
    {
        ...
        if (!info.Exists || (info.Attributes & FileAttributes.ReparsePoint) != 0 ||
            info.Length is < 1 || info.Length > maximumBytes || info.Length > int.MaxValue)
        {
            throw new ProbeFailureException(
                $"The {description} must be a bounded, non-link regular file.");
        }
        var bytes = File.ReadAllBytes(path);
        if (bytes.LongLength != info.Length)
            throw new ProbeFailureException($"The {description} changed while it was read.");
        return bytes;
    }
    ...
    private static void VerifyInputs(IEnumerable<ProbeInput> inputs)
    {
        foreach (var input in inputs)
        {
            ...
            if (!CryptographicOperations.FixedTimeEquals(
                    ...
                    SHA256.HashData(input.Bytes)))
            {
                throw new ProbeFailureException(
                    $"The {input.Description} changed while the capability probe was produced.");
            }
        }
    }
    ...
    {
        if (expected != actual)
            throw new ProbeFailureException("Git source state changed while the capability probe was produced.");
    }
    ...
    private static Uri NormalizeBaseAddress(string value)
    {
        ...
        {
            throw new ProbeFailureException(
                "Artifact Store base address must be an absolute HTTP URL on an IP loopback address.");
        }
    ...
    private static string ReadToken(string? tokenFile)
    {
        var token = tokenFile is null
            ...
            : Encoding.UTF8.GetString(ReadBoundedRegularFile(tokenFile, "token file", 8192));
    ...
    {
        var core = new ProbeProfile(
            "dotnet-10-linux-x64",
            "coreclr",
            "10.0.10",
            "sharplabnext/runtime-dotnet-10-linux-x64:candidate",
            "sharplabnext/runtime-dotnet-10-linux-x64:candidate",
            ["coreclr"],
            ["dotnet-managed-pe-v1"],
            ["run", "jit-asm"],
            ...
            "10.0.10");
        var framework = new ProbeProfile(
            "wine-netfx48-linux-x64",
            "netfx-clr-wine",
            "4.8",
            "sharplabnext/runtime-wine-netfx48-linux-x64:candidate",
            "sharplabnext/runtime-wine-netfx48-linux-x64:candidate",
            ["netfx-clr-wine"],
        ...
        if (SelectTarget(core).TargetFramework != "netcoreapp2.0" ||
            SelectTarget(framework).TargetFramework != "net20" ||
            !SelectMethodFilter(core).EndsWith(".MultipleSequencePoints", StringComparison.Ordinal) ||
            !SelectMethodFilter(wineCore).EndsWith(".WindowsAbi", StringComparison.Ordinal))
        {
    ...
    private static string RequiredString(JsonElement parent, string name)
    {
        ...
        if (value.ValueKind != JsonValueKind.String || string.IsNullOrWhiteSpace(value.GetString()))
            throw new InvalidDataException($"Runtime Profile property '{name}' is invalid.");
    ...
    private static bool IsStableId(string value) => value.Length is > 0 and <= 128 &&
        value.All(static character => char.IsAsciiLetterLower(character) || char.IsAsciiDigit(character) ||
...
sealed class ProbeOptions
{
    public const string Usage = """
        Usage:
          dotnet run eng/runtime-capability-probe.cs -- [options]
        ...
        for (var index = 0; index < args.Length; index++)
        {
            ...
            switch (name)
            {
                ...
                case "--time-to-live-seconds":
                    if (!int.TryParse(Value(), out var ttl) || ttl is < 60 or > 86400)
                        throw new ArgumentException("--time-to-live-seconds must be between 60 and 86400.");
                    options.TimeToLiveSeconds = ttl;
                    break;
                case "--timeout-seconds":
                    if (!int.TryParse(Value(), out var timeout) || timeout is < 5 or > 300)
                        throw new ArgumentException("--timeout-seconds must be between 5 and 300.");
